...
> **⚠️ 용어 정리 (2026-07-16)**: 아래 본문은 작성 당시 명명(RAG="단기 기억", 파인튜닝="장기
> 기억")을 그대로 둔 역사적 기록입니다. 현재 제품 용어는 업계 통념에 맞춰 **대화 이력=단기
> 기억(llm.ts, 에이전트별 최근 10턴 인메모리), RAG/LanceDB=장기 기억(memory.ts), 파인튜닝=
> 학습(지식 내재화)**으로 통일했습니다. 코드 주석·UI(memory.html)·다음단계 가이드가 새 용어
> 기준입니다.
