...
const SLIDES = [
  ...
  {
    after: 7, shot: "04-운영가이드-점검거버넌스",
    title: "유지보수 — 점검 주기와 거버넌스",
    menu: "보안 업무 › 보안 운영 › 유지보수",
  ...
  {
    after: 8, shot: "07-추천LLM가이드",
    title: "LLM 가이드 — 어떤 모델을 쓸지 안내",
    menu: "AI › AI 어시스턴트 › LLM 가이드",
    lead: "보안 특화 모델과 파인튜닝 베이스 모델을 용도·용량과 함께 정리해 설치까지 연결한다.",
    points: [
      ["보안 특화 LLM", "역할에 맞는 보안 도메인 모델을 용량과 함께 제시."],
      ["베이스 모델", "자체 학습에 쓸 베이스 모델을 따로 구분해 안내."],
      ["설치 연결", "고른 모델을 그 자리에서 내려받아 모델 풀에 등록."],
    ],
  },
  {
    after: 9, shot: "16-지식모델-온톨로지",
    title: "온톨로지 — 표준 사이의 연결을 그린다",
    menu: "AI › AI 지식·모델 › 온톨로지",
  ...
  {
    after: 10, shot: "18-LLM합성-모델머지",
    title: "LLM 합성 — 보안 특화 모델 병합",
    menu: "AI › AI 어시스턴트 › LLM 합성",
    lead: "성격이 다른 보안 모델 둘을 합쳐 조직에 맞는 한 개의 모델로 만든다.",
    points: [
      ["도입 모델 현황", "설치된 보안 특화 모델과 베이스 모델을 계열별로 정리."],
      ["합성 계획", "합칠 두 모델을 고르면 병합 방식(SLERP)과 설정을 계획으로 만든다."],
      ["폐쇄망 수행", "병합·변환·양자화까지 외부 호출 없이 로컬 GPU에서 끝낸다."],
    ],
  },
  {
    after: 11, shot: "15-레드팀-가드레일",
    title: "AI 견고성 — 가드레일과 레드팀",
    menu: "AI › AI 견고성 › 레드팀·가드레일",
    lead: "실시간 입력을 감시해 막고, 별도로 공격 페이로드를 직접 던져 견고성을 실측한다.",
    points: [
      ["런타임 가드레일", "끄기·탐지·차단 세 모드. 프롬프트 인젝션·탈옥 시도를 잡아낸 누적 건수를 표시."],
      ["레드팀 실행", "점검 대상을 고른 뒤 14개 페이로드를 자동으로 던져 뚫림·방어를 가리고 견고성 점수를 매긴다."],
  ...
  {
    after: 13, shot: "19-사용량-요금",
    title: "사용량·요금 — 외부 API를 쓰지 않는다는 증거",
    menu: "시스템 › 설정 › 사용량·요금",
    lead: "외부 API 대비 절감액과 로컬 LLM 엔진 상태를 함께 보여준다.",
    points: [
      ["누적 절감액", "같은 작업을 외부 API로 돌렸을 때의 비용을 환산해 표시."],
      ["엔진 상태", "자체 호스팅 중인 모델과 포트를 그대로 노출."],
      ["내보내기", "사용량을 CSV로 뽑아 정산 자료로 쓴다."],
    ],
  },
  {
