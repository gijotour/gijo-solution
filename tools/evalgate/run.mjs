...
// ★ 2026-09-11: **6종**이 됐다(본문근거없음 표지 — B1 확장의 짝인 B2. 문서 본문 근거 없이
//   온톨로지(제품 내장 지식 그래프)나 문서 이름·등록부 목록으로 답한 자리에 붙는다). 이 문장에도
//   「없습니다」가 들어 있어, 안 떼면 2026-08-14·09-05·09-06에 세 번 막은 자동통과가 또 열린다.
//   ⚠ 이 표지는 배너표(noevidence 근거없음배너표) **밖**이다 — 화면 옅힘 규약이 읽는 값은 아니지만
//     채점 본문에서는 배너와 똑같이 떼야 한다(두 표지가 한 머리 문장을 공유해 한 갈래로 잡힌다).
const 배너_RE = /^\s*⚠\s*\*\*(근거 약함|이 PC의 사내 자료에는|지정하신 문서 범위에서는|이건 우리 회사 고유 정보|사내 자료에 없는 수치|사내 문서 본문 근거는 없습니다)[^\n]*\n+/;
