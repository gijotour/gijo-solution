...
// ★★ 2026-09-11 검토관 [하] 수리 — **같은 도착지를 쓰는 행이 둘 이상이면 하나가 남을 덮었다.**
//   예전 판은 도착·판별을 한 Map에 넣고 `사전.set`으로 덮어써, 표 뒤쪽 행이 앞 행을 이겼다.
//   실제로 `도착: "explain"`인 행이 둘이 되자(제품 설명 질문 · 취약점 우선순위 기준 물음)
//   「SecuFW가 무슨 제품이야?」의 「왜」에 우선순위 기준 설명이 붙었다. 도착지·도구는 멀쩡한데
//   **사람이 읽는 이유만 거짓**이 되는 부류라 어떤 관문도 못 잡는다(2026-08-10 「거짓 겹침
//   경보가 진짜 겹침을 묻었다」와 같은 표면).
//   → 덮지 않는다. **키마다 목록으로 모아 전부 보여준다** — 둘이면 둘 다 읽고 사람이 고른다.
//   ⚠ 도착 키와 판별 키를 **따로** 둔다(한 Map에 섞으면 한쪽의 판별 문자열이 다른 쪽의 도착
//     문자열과 같을 때 또 섞인다). 강제도구 단계는 판별이 전부 "forcedToolFor"라 도착으로만 갈린다.
function 왜사전() {
  const s = fs.readFileSync(path.join(뿌리, "server/src/engine/routes.ts"), "utf8");
  const 도착별 = new Map();
  const 판별별 = new Map();
  const 담기 = (m, k, v) => { if (!m.has(k)) m.set(k, []); m.get(k).push(v); };
  const re = /이름: "([^"]+)", 층: "[^"]+", 파일: "[^"]+", 판별: "([^"]+)",\s*\n?\s*도착: "([^"]+)",\s*\n?\s*왜: "((?:[^"\\]|\\.)*)"/g;
  for (const m of s.matchAll(re)) {
    const 행 = { 이름: m[1], 판별: m[2], 왜: m[4].replace(/\\"/g, '"') };
    담기(도착별, m[3], 행);
    담기(판별별, m[2], 행);
  }
  return { 도착별, 판별별 };
...
/** 이 단계에 해당하는 표 행들 — 하나면 한 줄, 둘 이상이면 **전부** 돌려준다(누가 대표인지 모른다). */
function 왜줄들(사전, 단계) {
  const 행들 = 사전.도착별.get(단계.도착) ?? 사전.판별별.get(단계.판별) ?? [];
  return 행들;
}
...
async function 설명(말, 화면) {
  ...
  걸린것.forEach((r, i) => {
    ...
    if (i === 0) {
      const 행들 = 왜줄들(사전, r);
      if (행들.length === 1) console.log(`      왜: ${행들[0].왜}`);
      else if (행들.length > 1) {
        console.log(`      ⚠ 이 도착지를 쓰는 표 행이 ${행들.length}개입니다 — 어느 규칙이 잡았는지는 말만 보고 못 가립니다. 전부 보입니다:`);
        for (const 행 of 행들) console.log(`      · [${행.이름}] ${행.왜}`);
      }
...
export const 승인된겹침 = [
  ...
  // ── 2026-09-14 사장님 「판단해서 처리할 것」 위임 — 복합 지시(오케스트레이션)가 리포트 한 장을 이기는 자리 ──
  {
    말: "우선순위 분석하고 리포트 작성해줘",
    이김: "복합 지시",
    밀림: "리포트 만들기",
    이유: "「분석하고 … 작성해줘」는 동사가 둘이라 **두 단계를 시킨 말**이다 — 1단계가 조치 대상 상위 8건을 근거 문장과 함께 분석하고 " +
