...
- 건너뛴 문항은 **대상에서 빼지 않는다** — 결과 줄로 남고(`건너뜀` 사유가 찍힌다), 통과율
  분모는 **잰 것만**(`■ 끝 — 200/200 불편 없음 · 건너뜀 5(법령 5)` 꼴). 씨앗문서에 걸린 문항
  (⑨ 새 문서·⑱ 지목 문서 넷 등)은 qa4100에 씨앗문서가 **있으므로** 건너뛰지 않는다 — 계속
  재고, 제품 결함(B8 등)이 있으면 계속 빨개진다. 표식은 성격을 설명할 뿐 가리지 않는다.
  ⚠ 위 「11」·「194」는 이 기능이 설계될 때(933a88de, 실측 전) 적었던 본보기 숫자였다.
  실제 관측값(`GIJO_AS_AI팀_증류학습_계획서.md` 09-12·09-13 줄)은 **회차마다 다르다** —
  회차를 뭉뚱그리지 않고 갈라 적는다:
  - **09-12 18:24~ 저녁 첫 회차**(② 프로필 첫 실행, 배포 44 직후): **197/199 · 건너뜀 6** · 8분.
    불편 2건이 있었다(「이 서버 어떤 서비스」·「이 취약점 다음 단계」가 직전 배정 탐침이 남긴
    「직전 대상」을 물고 답함 — 10분 뒤 재측 11/11·12/12). **야간 회차가 아니다.**
  - **09-13 03:00~ 야간 회차**(배포 45 기준, ② 프로필 첫 밤): **199/199 · 건너뜀 6
    (법령 5 자동 감지·업무 1)** · 5분.
  2026-09-13 SLA/라우팅 라운드 ⑤가 업무 표식 1개(「10.0.0.100 에 뭐가 있어?」 — 4100에서도
  「자산」 갈래로 통과해 데이터 의존이 아니었다)를 뺐으므로 **다음 회차부터 건너뜀은 5(법령 5)**이고
  잰 문항은 200이 된다. ⚠ 「200/200」은 **아직 안 잰 값이다** — 다음 밤 회차 결과로 이 줄을 채운다
  (2026-09-13 검토관 [하]: 정정한다며 넣은 줄이 다시 실측과 달랐다. 안 잰 값을 단정형으로 적지 않는다).
- `--no-evidence`(사장님 확정안 ⑥) — **근거 「조각」 본문만 뺀다(⚠ 반쪽이다).**
  ...
  **가드횟수만** 남기고 조각·추가원천 본문은 뺀다 — 지금은 씨앗 문서뿐이라 위험이 낮지만,
  ...
  - ⚠⚠ **이 스위치로는 기록이 깨끗해지지 않는다.** 빠지는 것은 조각·추가원천 본문뿐이고
    **질문(q)과 답 본문(out)은 그대로 `.tmp-reports/ops-sim-4100.json`에 평문으로 쌓인다**
    (답이 사내 문서를 인용하면 그 대목도 함께 남는다 — 하네스가 실행 머리에 같은 말을 찍는다).
    계획서 `GIJO_AS_AI팀_증류학습_계획서.md` §14 ③이 이것을 「**부분(⚠ 반쪽)**」으로 못박아
    두었다 — **「껐으니 되돌렸다」로 출하 점검 칸을 닫지 말 것.** 완전히 지우려면 그 기계에서
    `.tmp-reports/ops-sim*.json`을 비우고 야간 하네스를 **안 돌린다**.
