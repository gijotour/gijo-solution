...
/**
 * 3갈래 미리 가르기 — 51건을 통째로 읽게 하면 검수가 안 된다.
 * 코드가 먼저 추천/보류/제외로 갈라 두고 **왜 그렇게 갈랐는지 한 줄**을 붙인다.
 * ⚠ 결정은 여전히 사람이 한다 — 이 등급이 승인을 대신하지 않는다.
 */
function 갈래(물음, 답) {
  if (/^\|/.test(답.trim())) return ["보류", "표 위주라 문장이 적다 — 표는 배워도 문장으로 안 나온다"];
  if (답.length < 200) return ["보류", "답이 짧다(200자 미만) — 배울 내용이 적다"];
  if (/GIJO AS|제품과의 연결|활용 팁/.test(물음)) return ["제외", "제품 소개에 가깝다 — 보안 지식이 아니라 우리 화면 설명이다"];
  if (/^[A-Za-z]/.test(물음) && !/뭐야|알려줘/.test(물음)) return ["보류", "장비 이름만 있는 항목 — 물음이 어색하다"];
  if (답.length > 900) return ["보류", "답이 길다(900자 초과) — 나눠야 한다"];
  return ["추천", "현장에서 그대로 묻는 말이고, 답이 절차·기준으로 떨어진다"];
}
...
for (const { 파일, 주제 } of 원천) {
  ...
  for (const 덩이 of 조각) {
    ...
    const [등급, 판정이유] = 갈래(물음, 답);
    결과.push({ 주제, 출처: 파일, 물음, 답, 등급, 판정이유 });
  }
}
...
const 주제별 = {};
for (const r of 최종) if (r.등급 === "추천") 주제별[r.주제] = (주제별[r.주제] ?? 0) + 1;
const 갈래셈 = {};
for (const r of 최종) 갈래셈[r.등급] = (갈래셈[r.등급] ?? 0) + 1;
const 순서 = { 추천: 0, 보류: 1, 제외: 2 };
최종.sort((a, b) => 순서[a.등급] - 순서[b.등급]);
...
줄.push("> **추천만 훑어보시고 빼실 것만 알려 주시면** 나머지를 승인 문답으로 넣습니다.");
줄.push("");
줄.push("## 갈래");
줄.push("");
줄.push("| 갈래 | 건수 | 뜻 |");
줄.push("|---|---:|---|");
줄.push(`| ✅ 추천 | ${갈래셈["추천"] ?? 0} | 현장에서 그대로 묻는 말이고 답이 절차·기준으로 떨어진다 |`);
줄.push(`| ⏸ 보류 | ${갈래셈["보류"] ?? 0} | 표 위주·너무 짧음·너무 김 — 손보면 쓸 수 있다 |`);
줄.push(`| ✕ 제외 | ${갈래셈["제외"] ?? 0} | 보안 지식이 아니라 우리 화면 설명이다 |`);
줄.push("");
줄.push("## 주제별 (추천만)");
줄.push("");
...
for (const [i, r] of 최종.entries()) {
  if (r.등급 !== 현재갈래) {
    현재갈래 = r.등급;
    줄.push(`## ${{ 추천: "✅ 추천", 보류: "⏸ 보류", 제외: "✕ 제외" }[현재갈래]}`);
    줄.push("");
  }
  줄.push(`### ${i + 1}. [${r.주제}] ${r.물음}`);
  줄.push("");
  줄.push(`*${r.판정이유} · 출처: ${r.출처}*`);
  줄.push("");
