...
// ⚠ 차단 표시는 **답의 첫머리**에서만 본다(2026-08-01 실측: 「작업 내역」 오판).
//   "최근에 내가 한 작업 보여줘"의 정답에는 지난 감사기록이 실리는데, 거기 "[차단] 프롬프트
//   인젝션 차단"이 **자료로** 들어 있다 — 막힌 것이 아니라 막았던 기록을 보여준 것이다.
//   본문 아무 데나 찾으면 보안 도메인 어휘가 자기 규칙에 걸린다(바로 위 자기차단 함정과 같은 꼴).
//   진짜 차단은 답이 🛡 표지로 **시작**한다.
//   ⚠ 문구를 여기 베껴 적으면 제품이 문구를 바꿀 때 이 점검이 조용히 헛돈다(2026-08-02 검토 지적).
//   단일 출처(redteam.ts의 GUARDRAIL_BLOCK_MARK)에서 읽어 온다 — 빌드 없이 소스에서 바로.
const 차단표지 = (() => {
  try {
    const src = fs.readFileSync(new URL("../server/src/engine/redteam.ts", import.meta.url), "utf-8");
    const m = src.match(/GUARDRAIL_BLOCK_MARK\s*=\s*"([^"]+)"/);
    if (m) return m[1];
  } catch (e) { /* 아래 기본값으로 */ }
  throw new Error("가드레일 표지를 redteam.ts에서 못 읽었습니다 — 상수 이름이 바뀌었는지 확인하세요.");
})();
// ⚠ 이 줄은 한 번 깨진 적이 있다(역슬래시가 먹혀 도구가 통째로 안 돌았다).
//   Node 스크립트로 이 파일을 고칠 때는 역슬래시가 살아남는지 반드시 확인할 것.
const 차단 = new RegExp("^\\s*🛡|^\\s*" + 차단표지.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
...
function 판정(r, q) {
  ...
  // ⚠ **숫자로 답하는 질문은 짧은 것이 정답**이다(2026-08-01 실측: 「우리 자산 몇 대야?」 →
  //   "등록된 AI 자산은 총 57대로 확인됩니다."(24자)를 오판했다). 길이를 강요하면 군더더기를
  //   붙이게 만든다 — 담당자가 원한 것은 숫자 하나다. 0건 답과 같은 이유로 예외를 둔다.
  // ⚠ "답에 숫자가 있으면 통과"는 **너무 넓다**(2026-08-01 검토 지적). 날짜·버전이 섞인
  //   반쪽 답까지 전부 통과시켜 길이 검사가 사실상 꺼진다 — 검사를 무디게 만드는 일이다.
  //   **건수를 묻는 문항**에서만, 그리고 답이 숫자+단위로 끝맺을 때만 예외로 둔다.
  const 건수질문 = /몇\s*(대|건|개|명)|얼마나|개수|수는/.test(r.q || "");
  const 숫자답 = 건수질문 && /\d+\s*(대|건|개|명)/.test(o) && o.replace(/\s/g, "").length >= 12;
  if (o.replace(/\s/g, "").length < 30 && !없음답.test(o) && !숫자답) return { ok: false, why: "너무 짧음(" + o.length + "자)" };
