...
//
// ⚠ **알려진 오탐 — 아직 안 고쳤다** (2026-08-10 Mac 실측). ② 문서 보강 뒤 10회를 재니
//   전 회차가 「지체 없이 파기 · 5일 이내 · 개인정보 보호법 제21조」로 **같은 결론**인데
//   상충으로 찍혔다. 갈린 것은 결론이 아니라 **덧붙이는 맥락**이었다:
//     · 「6개월·1년 등의 보관 기간이 **정해져 있지 않다**」  ← 부정 예시
//     · 「다만 **접속기록은** 별도로 1년/2년 이상」          ← 다른 대상(계정이 아니다)
//     · 「근로계약서류는 근로기준법에 따라 3년」              ← 다른 대상
//   맥락 수치까지 세면 **정확해진 답이 상충으로 잡힌다** — 잣대가 개선을 벌준다.
//
//   ⚠ 문장 단위로 「부정문」·「다만/별도」를 빼는 손질을 **해 봤다가 되돌렸다**:
//   「개인정보 접속기록」이 원래 {1년,2년}로 10회 일관인데, 그 필터가 **정당한 조건별 구분인
//   2년을 잘라** 일부 회차를 {1년}으로 만들었다 — 상충 1/3 → 2/3으로 **더 나빠졌다.**
//   거칠게 자르면 멀쩡한 답을 깬다. 고치려면 「질문 대상의 기간인가」를 봐야 하는데
//   그건 문장 필터로는 안 된다.
//   ⚠ 별도 결함: 날짜(2023년·22일)도 기간으로 잡힌다.
function 기간값(text) {
  const s = new Set();
