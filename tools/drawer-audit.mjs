...
//
// ⚠ **「근거 약함」은 폴백이 아니다.** 그건 답을 준 뒤 「확인해 보라」고 덧붙인 것이라
//   담당자에게 쓸모가 있다. 2026-08-03에 그 문구가 처음 「찾지 못했습니다」였는데,
//   아래 목록과 글자가 겹쳐 **좋은 답에도 실패 딱지**가 붙었다.
//   지금은 제품이 폴백과 겹치지 않는 말을 쓰지만, 여기서 한 번 더 걸러 둔다 —
//   재는 쪽과 재이는 쪽이 같은 말을 쓰면 언제든 다시 부딪힌다.
const 근거약함머리 = /^[^\n]*근거 약함[^\n]*\n+/;
const FAIL_MARKS = [
...
for (const it of items) {
  ...
  // ⚠ 「근거 약함」 머리말은 **폴백이 아니다** — 답을 준 뒤 확인하라고 덧붙인 줄이다.
  //   떼고 본문으로 판정한다(2026-08-03: 이 줄 때문에 좋은 답에 실패 딱지가 붙었다).
  const out = String(r.output || "").replace(근거약함머리, "");
