...
async function runClient() {
  ...
  await scenario("QA-C05", "전체메뉴", "11그룹 — ⓪자산 + 절차 5허브 + 보안제품·공급망·AI·추가 기능·설정", {
    given: "그룹 통합(2026-08-09 승인) 뒤 ⓪자산 신설(08-18)·보안제품/공급망 분리(08-21~22)까지 반영된 상태에서",
    when: "셸의 왼쪽 메뉴를 보면",
    then: "11그룹이 절차 순서대로 서고, 앞 9개 대표 그룹은 하위 줄이 없으며, 항목은 승인된 집합 그대로다",
  }, async () => {
    await open("app.html");
    await page.waitForTimeout(1500);
    ...
    // ★ 2026-08-22 갱신 — 기대값이 **또 뒤처져 있었다**(정찰 적발). 그동안 실제 메뉴는
    //   「등록부」가 **「보안제품」**으로 갈리고 **「공급망」**(공급망 점검 — 중-7)이 신설돼
    //   11그룹이 됐는데, 여기는 10그룹·「등록부」를 기다리고 있었다.
    //   ⚠ 기대값은 **추측이 아니라 nav.js의 GROUPS에서 뽑아 맞췄다** — 이 표가 메뉴의 원천이다.
    if (m.groups.length !== 11) throw new Error(`그룹 ${m.groups.length}개: ${m.groups.join(",")}`);
    const 그룹순서 = ["자산", "발견", "우선순위", "조치", "검증", "보고", "보안제품", "공급망", "AI", "추가 기능", "설정"]; // ⓪ 자산이 맨 앞(2026-08-18 승인 시안 — 범위 축이 절차보다 먼저)
    그룹순서.forEach((이름, i) => {
    ...
    m.perGroup.forEach((pg, i) => {
      const 대표 = i < 9; // 순서 검증을 통과했으므로 앞 9개(자산~AI)가 대표 그룹이다(2026-08-22: 보안제품·공급망이 갈려 7→9)
    ...
    const 기대항목 = ["대시보드", "팀 사무실 (창)", "작업 내역", "내 문서", "보안 로그 파일 분석", "법령·판례", "업무 넘기기", "설정", "기록"];
