...
(async()=>{
  ...
  if (!Array.isArray(대장)) {
    ...
    // ── 반쪽 유령 — 이름은 맞는데 **판이 다르다** (2026-09-07) ────────────────
    // ⚠ 종료 코드는 **바꾸지 않는다.** 이 도구의 빨강은 「리포↔운영↔목록이 어긋났다」는 뜻이고
    //   배포 관문이 그 뜻으로 걸려 있다. 조각 수 어긋남은 재인입으로 푸는 다른 일이라,
    //   섞으면 배포가 엉뚱한 이유로 멈춘다. 그래서 **경고 줄로만** 말한다 — 감춘 것과 다르다.
    if (대조.조각어긋남.length) {
      console.log(`  ⚠ 조각 수 어긋남 ${대조.조각어긋남.length}건 (이름 · 대장/저장소) — **반쪽 유령**입니다(이름은 있는데 판이 다릅니다):`);
      for (const x of 대조.조각어긋남.slice(0, 20)) console.log(`     ${x.이름} · ${x.대장}/${x.저장}`);
      if (대조.조각어긋남.length > 20) console.log(`     … 그 밖 ${대조.조각어긋남.length - 20}건`);
      console.log("    ℹ 저장소가 적으면 벡터가 부분적으로 날아간 것이고, 많으면 옛 판 조각이 남아 **AI가 두 판을 섞어 읽습니다.**");
      console.log("      재인입할지는 사람이 정합니다 — 이 도구는 아무것도 지우지 않고 종료 코드도 바꾸지 않습니다.");
    } else {
      console.log("  ✓ 대장과 저장소의 조각 수가 모두 같습니다.");
    }
    if (대조.조각미기재.length) {
      console.log(`  ℹ 대장에 조각 수가 안 적힌 문서 ${대조.조각미기재.length}건 — 조각 수 대조에서 **뺐습니다**(0으로 치면 전부 어긋남이 됩니다).`);
    }
