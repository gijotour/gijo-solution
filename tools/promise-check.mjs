...
function 지목화면찾기(글) {
  const 잡힘 = [];
  // 「화면(설정)에서」처럼 위치를 괄호로 덧붙인 꼴도 지목이다(2026-08-06 검토 #3b —
  // 우리가 고쳐 넣은 문구 2건이 이 꼴이라 검사망 밖에 있었다).
  for (const m of 글.matchAll(/「?([가-힣A-Za-z·]+(?:\s[가-힣A-Za-z·]+)?)」?\s*(화면|메뉴|탭)(\([^)\n]{1,14}\))?에서/g)) {
    const 낱말 = m[1].trim().split(/\s+/);
    const 후보들 = [낱말.slice(-2).join(" "), 낱말[낱말.length - 1]]
      .map((s) => s.replace(/[\s·]/g, "")).filter((s) => s.length >= 2);
    // ⚠ **낡음을 현재보다 먼저** 본다(2026-08-06 검토 #3a). 반대로 하면 「작업 기록」의
    //   후보 「기록」이 현재 라벨 「기록 보기」에 부분일치로 걸려 통과가 되고, 옛 이름 사전까지
    //   내려가지 못한다 — 검사기가 자기가 잡았다는 결함을 영영 못 잡는 상태였다.
    //   짧은 후보의 오탐을 막으려 부분일치 방향에 길이 조건을 둔다: 옛 이름이 후보를 품는
    //   방향(l.includes(c))은 후보 4글자 이상일 때만(「기록」 2글자가 「작업기록」에 걸려
    //   멀쩡한 「감사 기록 화면」을 낡음으로 잡는 것을 막는다).
    const 낡음맞춤 = (c) => 옛이름.some((l) => l === c || c.includes(l) || (l.includes(c) && c.length >= 4));
    if (후보들.some(낡음맞춤)) {
      잡힘.push({ 이름: 낱말.join(" "), 판정: "낡음" });
    } else if (후보들.some((c) => 라벨정규.some((l) => l.includes(c) || c.includes(l)))) {
...
{
  // 자기 검증 표본에 **한 번 뚫린 꼴을 반드시 넣는다**(2026-08-06 검토 #3c — 표본이 쉬운
  // 세 개뿐이라 가림 구멍을 안 덮었고, "자기 검증 내장"이 실제 보장보다 넓게 읽혔다):
  //   · 「작업 기록」 — 후보 「기록」이 현재 라벨 「기록 보기」에 가려 통과되던 가림 사례
  ...
  if (!(검증.length === 기대.length && 검증.every((r, i) => r.판정 === 기대[i]))) {
    console.log("\n✗ ③ 검사기 자기 검증 실패 — 이 검사를 믿으면 안 됩니다: " +
      JSON.stringify(검증.map((r, i) => r.이름 + "=" + r.판정 + (r.판정 === 기대[i] ? "" : "(기대 " + 기대[i] + ")"))));
    process.exitCode = 1;
  }
