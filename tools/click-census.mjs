...
const 보고 = [
  ...
  `>`,
  `> ⚠ 실측으로 확인된 오탐 2종(2026-08-01):`,
  `> · **파일 선택창을 여는 버튼** — 네이티브 창이라 DOM에 안 나타난다(자산 목록 「탐지 결과 가져오기」)`,
  `> · **콘솔 오류의 주인** — 배경 폴링이 1.4초 창 안에 실패하면 그 클릭 탓으로 잡힌다.`,
  `>   고장 목록은 tools/click-verify.mjs로 하나씩 다시 눌러 **어떤 요청이 몇 번으로 실패했는지**까지 확인할 것`,
  ``,
  `## 안 누른 것 ${건너뜀.length}건`,
  ...건너뜀.map((x) => "- " + x),
].join("\n");
