...
export function 등급관문(rows, 창집합 = null) {
  ...
  return { ok: 사유.length === 0, 사유, 칸없음: 칸없음.length, 등급아님: 등급아님.length, 글이C: 글이C.length, 행: 목록.length };
}
...
export function 구성표(보고) {
  ...
  return [
    ...
    "",
    "⚠ **홀드아웃 제외 기준은 하나다** — 「홀드아웃 파일에 든 질문」. 회전 4까지의 두 갈래(그 회전 안에서 떼기 · 파일로 고정)를 섞지 않는다.",
  ].join("\n");
}
...
if (process.argv[1] && process.argv[1].replace(/\\/g, "/").endsWith("material-r5.mjs")) {
  ...
  // 회전 간 eval_loss를 견주려면 시험지가 같아야 하므로, 이 자리를 0으로 두는 것이 기본 선택이다.
  const 홀드아웃수 = Number(opt("--holdout-n", "100"));
  ...
  if (!dry && (관문.ok && 홀드관문.ok)) {
    ...
    if (repP) {
      ...
      fs.writeFileSync(repP, JSON.stringify({ 만든때: new Date().toISOString(), 입력: baseP, 규격: specP, 보고, 관문, 홀드관문 }, null, 2));
