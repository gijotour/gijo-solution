...
/**
 * 절이 **정말 있는가** — 「제목이 줄 머리에 있고 그 아래 본문이 한 줄 이상」이라야 한 절로 센다.
 *
 * ★ 왜 고쳤나(2026-09-04): 예전 채점은 `t.includes(절이름)`이었다. 그러면 본문 한가운데
 *   「…조치 방법을 정해야 합니다」라고 **말만 해도** 그 절을 쓴 것으로 세어졌다. 실측에서
 *   report_fix가 절 이름을 나열만 하고도 6/6을 받았다 — 서식을 안 지켰는데 만점이라, 이 채점기가
 *   막으려던 회귀(서식 붕괴)를 원리상 못 잡았다.
 * 허용하는 줄 머리 꼴: `## 제목` · `1. 제목` · `**제목**` · `- 제목` · `제목:` — 그리고 그 조합.
 *   ⚠ 제목만 있고 아래가 비면 안 센다. 서식은 「칸이 있다」가 아니라 「칸이 채워졌다」이다.
 */
export function 절세기(text, 절이름들) {
  const 줄들 = String(text ?? "").split(/\r?\n/);
  let n = 0;
  const 있는절 = [];
  for (const 이름 of 절이름들) {
    const 안전 = 이름.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s*");
    // 줄 머리 장식(#·번호·불릿·굵게)을 걷어낸 자리에서 이름이 시작해야 한다.
    const 머리 = new RegExp(`^\\s{0,3}(?:#{1,6}\\s*)?(?:[-*+]\\s*)?(?:\\d+[.)]\\s*)?(?:\\*\\*|__)?\\s*${안전}\\s*(?:\\*\\*|__)?\\s*[:：]?\\s*$|^\\s{0,3}(?:#{1,6}\\s*)?(?:[-*+]\\s*)?(?:\\d+[.)]\\s*)?(?:\\*\\*|__)?\\s*${안전}\\s*(?:\\*\\*|__)?\\s*[:：]\\s*\\S`);
    for (let i = 0; i < 줄들.length; i++) {
      const m = 머리.exec(줄들[i]);
      if (!m) continue;
      // 같은 줄에 본문이 붙어 있으면(「담당: 인프라팀」) 그것으로 채워진 것이다.
      const 같은줄본문 = /[:：]\s*\S/.test(줄들[i]);
      const 아래본문 = 줄들.slice(i + 1).find((l) => l.trim().length > 0);
      if (같은줄본문 || (아래본문 && !절이름들.some((x) => 아래본문.includes(x) && 아래본문.trim().length <= x.length + 6))) {
        n++; 있는절.push(이름); break;
      }
    }
  }
...
export const T4 = {
  ...
  score(text) {
    const t = text || "";
    const { 수: n } = 절세기(t, 필수절); // ★ 본문 한가운데의 낱말이 아니라 **줄 머리 + 아래 본문**으로 센다
