...
export function 뽑기(db, { 코퍼스조각들 = null, root = 저장소 } = {}) {
  ...
  for (const r of q("SELECT id, question, answer, cites, topic, origin, teacher, createdAt FROM chat_logs WHERE rating = 1 ORDER BY createdAt ASC")) {
    ...
    //   가리킨 파일 32개 · 등급 O만 **29개**.
    //   ★ 고친 뒤 **다시 돌려 보니 결과가 갈렸다** — C 창 6,386 → **6,388**(원천 글자 4,445,876 → 3,924,617).
    //     늘어난 2개는 「등급 C 문답만 가리킨 파일」의 글이라 전에는 공개 원천으로 오해되어 빠져 있던 창이다.
    //     방향이 **넓어지는 쪽**이라 잣대가 더 엄해진다(고친 집합이 옛 집합을 통째로 품는다 — 전에만 있던 창 0).
    //     ⚠ 처음엔 「차이 3개가 다 공개 문서라 안 갈렸다」고 적을 뻔했다. **재지 않고 적을 뻔한 것**이고,
    //     실제로 재니 틀렸다. 그 2개까지 태워 gb10 datasets 13파일을 다시 재도 「글에 C 본문」은 0이다.
    if (g === "O") {
