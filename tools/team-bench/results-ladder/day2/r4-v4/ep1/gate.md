# 증류 사다리 게이트 — r4-v4-ep1 · **불합격**

잰 때 2026-09-05T00:42:05.334Z · needle_64k 제외
...
| ❌ | 13과제 평균 (needle_64k 제외 → 12과제) | 0.654 | 기준선 0.867 초과 | needle_64k는 ctx 초과라 양쪽 0 — 빼고 잰다(넣어도 순위는 안 바뀌고 평균만 낮아진다) |
...
| ❌ | 한글 비율 평균 | 0.606 | 기준선 0.75 이상 | JSON만 뱉는 과제는 0이 정상이라 기준선도 같은 방식으로 잰다(과제 구성이 같을 때만 견줄 수 있다) |
| ✅ | 생성 속도 낙폭 | 7.1% (20.24 tok/s) | 기준선 21.78 tok/s 대비 10% 이내 | 8080 /health는 붕괴해도 200이라 속도로 본다 — 크게 느려졌으면 두뇌가 스왑에 밀린 것이다 |
| ❌ | 근거 인용(정답 조각 20자 겹침) | 3/8 (38%) · 건너뜀 4 | 베이스 4/8 (50%) 이상 | RAFT가 가르치려는 바로 그것 — 근거를 줬을 때 그 문장을 옮겨 적는가. 건너뛴 문항은 근거 ref를 회수 못 한 것이고 셈에서 뺐다(0으로 세지 않는다) |
| ❌ | 인용 창작 0건(persona+KEV) | 2건 / 대상 15 | 0건 | 모집단: **거절 아닌 답**만 — persona 12행 + KEV 3행(대조군 noprompt 제외 — 관문 ①과 같은 자리) / [persona] 윈도우 서버에서 최신 패치를 설치할 때 네트워크 연결 상태는 어떻게 해야… 「[n]에 따르면」 꼬리표(근거를 안 준 자리 — 가리킬 [n]이 없다) · [prompt] 우리 취약점 목록에 KEV 표시가 붙어 있던데, KEV 카탈로그를 발표하… 「[n]에 따르면」 꼬리표(근거를 안 준 자리 — 가리킬 [n]이 없다) / 거절률(참고) persona 0/12 · KEV 0/3 — 밝히기만 하고 답을 안 한 답은 「지어낼 인용」이 없어 셈에서 뺐다 / bare 창작 0건/12(참고값 — **제품 조건이 아니다**: 시스템 프롬프트 없이 던진 자리) |
| ✅ | 자료 없음이라 말함 | 8/8 (100%) · 건너뜀 4 | 75% 이상 | 판정 잣대는 제품의 자료없음중복가드 그 정규식이고, 제품과 같이 앞 60자만 본다. 학습 재료에 「모른다」 시연 행이 0건이라 여기가 먼저 무너진다 |
| ✅ | 서술 답 길이 낙폭 (report_draft·report_fix·glossary_cite) | -26.6% (중앙값 324 토큰) | 기준선 256 토큰 대비 40% 이내 | 점수는 그대로인데 답만 짧아지는 회귀가 실제로 났다(34~72%). 스키마 강제 과제는 길이가 스키마에 눌려 안 드러나므로 뺐다: priority_rank·priority_6·scan_messy |
| ✅ | 베낀 글자 비율(통째 복사 아님) | 평균 29% · 통째 복사 0건 / 8 | 평균 60% 이하 · 통째 복사 0건 | 베이스 평균 20% · 통째 0건 / ⑧은 20자 창이 하나만 남아도 100%가 되므로 **통째 복사로도 만점**이다 — 그래서 둘을 함께 읽는다 |
| ❌ | 근거 준 자리의 거절(베이스 대비 증가 0) | 6/8 (75%) · 건너뜀 4 | 베이스 0/8 이하 | 근거를 준 자리에서 「자료에 없다」고 시작하는 것은 RAFT가 가르치려는 것의 정반대다 / ⑧·⑫는 원리상 못 본다 — 거절 문장 **뒤에** 근거를 복사해 이으면 둘 다 초록이 된다(회전 3 실측 8/8 초록에 7/8 거절) / 판정 잣대는 ⑩과 **같은 자**(제품의 자료없음 정규식 · 앞 60자)다 |
...
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/easy/qwen3-14b+r4-v4-ep1.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/hard/qwen3-14b+r4-v4-ep1.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/samples-grounded.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/samples-distractor-only.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/samples-bare.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/samples-persona.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r4-v4/ep1/kev.json
