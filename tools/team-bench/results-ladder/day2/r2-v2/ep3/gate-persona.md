# 증류 사다리 게이트 — r2-v2-ep3 · **불합격**

잰 때 2026-09-04T15:25:11.552Z · needle_64k 제외
...
| ❌ | 13과제 평균 (needle_64k 제외 → 12과제) | 0.863 | 기준선 0.867 초과 | needle_64k는 ctx 초과라 양쪽 0 — 빼고 잰다(넣어도 순위는 안 바뀌고 평균만 낮아진다) |
| ✅ | 잘린 답(베이스 대비 증가 0) | 증가 -4건 (이번 1건 · 베이스 5건) | 베이스보다 늘지 않는다 | 자리별 이번/베이스 — easy 1/0 · hard 0/0 · grounded 0/0 · distractor-only 0/0 · bare 0/5. 절대 0건은 **베이스가 못 넘는 기준**이었다(bare 5건 — 시스템 프롬프트 없이 900토큰 상한). 그 기준은 답이 짧아질수록 초록이 되어 ⑪의 표적에 상을 준다 |
| ❌ | 한글 비율 평균 | 0.745 | 기준선 0.75 이상 | JSON만 뱉는 과제는 0이 정상이라 기준선도 같은 방식으로 잰다(과제 구성이 같을 때만 견줄 수 있다) |
| ✅ | 생성 속도 낙폭 | 7.4% (20.18 tok/s) | 기준선 21.78 tok/s 대비 10% 이내 | 8080 /health는 붕괴해도 200이라 속도로 본다 — 크게 느려졌으면 두뇌가 스왑에 밀린 것이다 |
...
| ✅ | 인용 창작 0건(persona+KEV) | 0건 / 대상 15 | 0건 | 모집단: persona 표본 12행 + KEV 3행(대조군 noprompt 제외 — 관문 ①과 같은 자리) / 근거를 안 준 자리에서 「원문:」·「[n]에 따르면」을 안 붙이고, 제 말을 원문이라 인용하지도 않는다 / bare 창작 11건/12(참고값 — **제품 조건이 아니다**: 시스템 프롬프트 없이 던진 자리) |
| ✅ | 자료 없음이라 말함 | 8/8 (100%) · 건너뜀 4 | 75% 이상 | 판정 잣대는 제품의 자료없음중복가드 그 정규식이고, 제품과 같이 앞 60자만 본다. 학습 재료에 「모른다」 시연 행이 0건이라 여기가 먼저 무너진다 |
| ✅ | 서술 답 길이 낙폭 (report_draft·report_fix·glossary_cite) | -34.4% (중앙값 344 토큰) | 기준선 256 토큰 대비 40% 이내 | 점수는 그대로인데 답만 짧아지는 회귀가 실제로 났다(34~72%). 스키마 강제 과제는 길이가 스키마에 눌려 안 드러나므로 뺐다: priority_rank·priority_6·scan_messy |
| ❌ | 베낀 글자 비율(통째 복사 아님) | 평균 87% · 통째 복사 0건 / 8 | 평균 60% 이하 · 통째 복사 0건 | 베이스 평균 20% · 통째 0건 / ⑧은 20자 창이 하나만 남아도 100%가 되므로 **통째 복사로도 만점**이다 — 그래서 둘을 함께 읽는다 |
...
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/easy/qwen3-14b+r2-v2-ep3.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/hard/qwen3-14b+r2-v2-ep3.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/samples-grounded.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/samples-distractor-only.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/samples-bare.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/samples-persona.json
- 원천: /home/gijohn_llm/gijo-as/tools/team-bench/results-ladder/day2/r2-v2/ep3/kev.json
