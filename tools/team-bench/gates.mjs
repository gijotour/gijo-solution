...
//   ⑬ grounded_no_refusal  **근거를 준** 자리에서 「자료에 없다」로 시작하는 답이 베이스보다 늘지 않는다
//                    (모집단 최소 24건 — 8건이면 한 건이 12.5%p라 흔들림이 판정을 뒤집는다)
//   ⑭ local_knowledge  **로컬 14B 단독**(원격 없음·근거 없음)에서 지식 문항 정답률이 베이스 이상
//
// ■ ⑭는 왜 생겼나 (2026-09-10 · 회전 5 설계 · 계획서 §12.12)
//   회전 5의 목적이 「원격(gb10) 없는 설치본의 14B 바닥 올리기」로 바뀌었다. 그런데 ①~⑬은 전부
//   **근거를 주거나 과제를 주는** 자리에서 잰다 — 고객 설치본에서 실제로 벌어지는 「근거도 원격도 없이
//   물어본다」를 재는 자가 **0개**였다. 목적을 재는 자가 없으면 그 회전은 무엇을 고쳤는지 말할 수 없다
//   (⑧~⑪이 뒤늦게 생긴 그 이유와 같다). 정답 판정은 ⑧과 같은 자(overlap20)를 쓴다 — 새 잣대 금지.
//
// ■ 1차 성공 정의 (회전 5 · 2026-09-10 못 박음 — 굽기 **전에** 적는다)
//   채택은 **14개 전부 초록**이 원칙이고, 그중 「1차 성공」이라 부를 최소선을 미리 적어 둔다:
//     · ① KEV 하락 0 (사실이 안 밀렸다)
//     · ④ 12과제 평균 ≥ 0.867 (needle_64k 제외 · 회전 2 ep2의 0.871에 못 미치면 재료가 얇았다는 뜻)
//     · ⑥ 한글 비율 ≥ 0.75
//     · ⑬ 근거 준 자리의 거절이 베이스 이하 (모집단 24건 이상)
//     · ⑭ 로컬 14B 단독 지식 정답률 무하락
//   ⚠ 이 다섯만 초록이고 나머지가 빨강이면 **채택이 아니라 「가능성 있음」**이다 — 채택은 14/14다.
//
// ■ 옛 회전을 이 잣대로 다시 적을 때는 **두 칸**으로 적는다 (2026-09-10)
//   r1~r4의 표를 새 관문 수로 덮어쓰지 않는다. 「그때 잣대(관문 N개·모집단 8건)」와
//   「지금 잣대(14개·모집단 24건)」를 나란히 둔다 — 한 칸으로 적으면 그때 사람이 무엇을 보고
//   판단했는지가 사라지고, 「그때도 불합격이었다」는 사후 판정이 그때의 결정을 덮어쓴다.
//   ⚠ 옛 회전은 모집단이 8건이라 ⑬·⑭를 **다시 잴 수 없다**(표본이 없다) — 그 칸은 「미측정」이지
//     「불합격」이 아니다. 없는 값을 빨강으로 적으면 그것도 거짓이다.
//
// ■ ⑬은 왜 생겼나 (2026-09-05 · 회전 3 실측)
//   r3-v3 ep1·ep2에서 **grounded 8건 중 7건이 거절문으로 시작**했다(베이스 0/8 · 회전 2 0/8). 근거를 준
//   자리에서 「자료에 없다」고 말하는 것은 RAFT가 가르치려는 것의 **정반대**인데, 그 판에서 ⑧(근거 인용)은
//   8/8 초록이었다 — 거절 문장 **뒤에** 근거를 복사해 이었기 때문이다. ⑧은 「20자 창이 남았나」만 묻고
//   ⑫는 「얼마나 베꼈나」만 물어서, 둘 다 **답의 첫머리가 거절인지**는 원리상 못 본다. 그 사각지대를 메운다.
//   기준을 「0건」이 아니라 **베이스 대비 증가 0**으로 두는 이유는 ①·⑤와 같다: 베이스가 못 넘는 기준은
//   늘 빨강이라 회귀를 못 알린다(지금 베이스는 0/8이므로 실질 기준은 0건이다).
//
...
/**
 * 관문 ⑬의 **최소 모집단** — grounded 표본이 이보다 적으면 미측정(=불합격)이다.
 *
 * ★ 왜 24인가(2026-09-10 · 회전 5 설계, 계획서 §12.12 뿌리): 회전 1~4의 ⑬·⑧·⑫는 전부 **8건**으로
 *   판정했다. 8건이면 한 건이 12.5%p라, 「4/8 → 3/8」 같은 흔들림이 합격·불합격을 뒤집는다 —
 *   실제로 회전 3(7/8)과 회전 4(4/8)를 같은 잣대로 견주기 어려웠다. 세 배로 늘리면 한 건이
 *   4.2%p가 되어 **관문이 회귀를 알리는 쪽**으로 돌아온다. 24는 「모집단이 세 배」의 자리다.
 * ⚠ **오늘 이 수를 채우는 문항 파일이 없다**(2026-09-10 검토관 적발 · 정정). 처음 이 자리에
 *   적었던 근거 —「홀드아웃 100 중 회수 가능분」— 은 **사실이 아니다**: holdout-vuln-100.json은
 *   키가 question/answer/system뿐이라 chunk도 cites도 없고, 회수 가능한 문항이 **0건**이다.
 *   지금 조각(chunk)이 든 문항은 samples-questions.json의 **8건**뿐이라, ⑬은 「판정이 나오던
 *   관문」에서 **미측정**으로 내려앉았다(⑭도 같은 파일을 쓰므로 함께 미측정이다).
 * ★ 그래서 이것은 **고칠 수 없는 빨강(=벽)이 아니라 할 일**이다 — 길이 있고, 아래 미측정 사유가
 *   그 길을 이름으로 말한다: 운영 4000 읽기 전용 창구로 cites를 더 회수해 samples-questions.json에
 *   조각 든 문항을 24건 이상으로 채운다(방법은 ask-samples.mjs 머리주석 「문항 파일의 조각은
 *   어떻게 채웠나」). 그 일은 운영 로그인이 필요해 **메인의 몫**이다.
 * ⚠ 수를 8로 되내리지 않는다 — 8건짜리 초록은 「통과」가 아니라 「못 잰 것」이라는 판단이
 *   이 회전의 전제이기 때문이다(§12.12 뿌리). 내리려면 그 전제부터 사장님이 뒤집어야 한다.
 * ⚠ 모자라면 **불합격**이지 「작은 표본으로 통과」가 아니다 — 못 잰 것을 통과로 세지 않는 규칙 그대로.
 */
export const 표본최소_grounded = 24;

/**
 * 관문 ⑭의 **최소 문항 수** — 로컬 14B 단독 지식 문항이 이보다 적으면 미측정(=불합격).
 * ⑬과 같은 수를 쓴다(같은 이유 · 두 곳에 다른 수를 적으면 「왜 다르냐」에 아무도 답 못 한다).
 */
export const 문항최소_지식 = 표본최소_grounded;
...
/**
 * 제품이 실제로 돌려주는 **거절 문장**(server/src/engine/llm.ts:759 원문 그대로).
 * ★ 여기가 도구 쪽 **단일 출처**다 — 빌더(tools/build-raft-dataset.mjs)가 이것을 import해 쓴다.
 *   .mjs에서 .ts를 못 부르므로 복제하되, 짝 시험(server/test/raftdataset.test.ts)이 llm.ts 원문과
 *   **글자 단위로 같은지** 감시한다. 두 도구에 따로 적으면 어긋나는 날 아무도 모른다.
 */
export const 제품거절문장 = "등록된 사내 자료에는 관련 내용이 없습니다. 사내 문서를 먼저 등록하시거나, 다른 에이전트에게 물어보세요.";

/**
 * 「거절 뒤에 남은 말」 — 자료 없음을 **밝히고 이어서 한 말**이 무엇인가. 거절 선언이 없으면 null.
 *
 * ★ 왜 필요한가(2026-09-05): 팀원 프롬프트(llm.ts:254)는 「'등록된 사내 자료에는 관련 내용이 없습니다'라고
 *   **먼저 밝힙니다**(그 뒤 필요하면 아주 짧은 일반 정의만 덧붙입니다)」라고 시킨다 — 즉 **밝히고 이어 답하는**
 *   것이 제품이 원하는 꼴이다. 그래서 「자료없음이라 말했다」만으로 「답을 안 했다」고 보면 정상 답까지
 *   거절로 센다(관문 ⑨가 그 답들을 모집단에서 빼면 늘 미측정이 된다).
 * 어떻게 가르나:
 *   ① 제품의 거절 문장 그대로로 시작하면 그것을 뗀다(실측 r2-v2: persona 12답이 전부 이 59자였다).
 *   ② 아니면 **선언이 든 첫 문장**까지 뗀다.
 *   그리고 이어 붙이는 말머리(「일반적으로 알려진 바로는,」)도 뗀다 — 그건 답이 아니라 이음말이다.
 */
export function 거절뒤남은말(text) {
  const s = String(text ?? "").trim();
  if (!자료없음이라말함(s)) return null;
  let 나머지;
  if (s.startsWith(제품거절문장)) 나머지 = s.slice(제품거절문장.length);
  else {
    const 문장끝들 = [...s.matchAll(/[.!?\n]/g)].map((m) => m.index + 1);
    let 자리 = s.length;
    for (const i of 문장끝들) if (자료없음중복가드.test(s.slice(0, i))) { 자리 = i; break; }
    나머지 = s.slice(자리);
  }
  return 나머지.replace(/^[\s,·]*(일반적으로 알려진 바로는[,，]?)?\s*/, "").trim();
}

/**
 * 「거절만 한 답」의 문턱 — 거절 뒤에 남은 말이 이보다 짧으면 **답을 안 한 것**으로 본다.
 * 왜 20자인가: overlap20이 볼 수 있는 최소 창이 20자다 — 그보다 짧으면 관문 ⑨가 셀 인용 토막조차
 * 담기지 않으므로, 그 답에 대고 「인용을 지어냈나」를 묻는 것 자체가 성립하지 않는다.
 */
export const 거절나머지최소 = 20;

/** 밝히기만 하고 **답은 안 한** 답인가 — 관문 ⑨의 모집단에서 뺀다(잴 답이 없기 때문이다). */
export const 거절만한답 = (text) => {
  const 남은 = 거절뒤남은말(text);
  return 남은 !== null && 남은.length < 거절나머지최소;
};
...
/**
 * 관문 ⑬의 잣대 — **근거를 준** 자리에서 「자료에 없다」 선언으로 시작한 답이 몇 건인가.
 *
 * ★ 왜 ⑩과 같은 자(자료없음이라말함)를 쓰나: ⑩이 「방해만 준 자리에서 그렇게 **말해야 한다**」를 재는
 *   자라면, ⑬은 같은 말을 **하면 안 되는 자리**에서 센다 — 한 자를 두 방향으로 쓰는 것이라 잣대가 하나다.
 *   여기서 새 정규식을 지으면 두 관문이 서로 다른 「거절」을 세게 된다(이 저장소가 반복해 겪은 그것).
 * ⚠ 제품과 같이 **앞 60자만** 본다(자료없음이라말함이 그렇게 자른다) — 전체를 보면 본문 중간의
 *   「영향이 없습니다」가 걸려 정상 답이 거절로 몰린다.
 * 모집단은 ⑧·⑫와 같다(답이 든 grounded 행). 조각 길이는 안 따진다 — 첫머리가 거절인지에 조각은 무관하다.
 */
export function 근거거절률(samples) {
...
/**
 * 관문 ⑭의 잣대 — **로컬 14B 단독**(원격 두뇌 없음 · 근거 블록 없음)에서 지식 문항을 맞혔는가.
 *
 * ★ 왜 이 관문이 회전 5에서 생겼나(계획서 §12.12): 회전 5의 목적이 「원격(gb10)이 없는 설치본의
 *   14B 바닥 올리기」로 바뀌었다. 그런데 관문 ①~⑬은 전부 **근거를 주거나 과제를 주는** 자리에서
 *   잰다 — 고객 설치본에서 실제로 벌어지는 「근거도 원격도 없이 물어본다」를 재는 자가 **0개**였다.
 *   목적을 재는 자가 없으면 그 회전은 무엇을 고쳤는지 말할 수 없다(⑧~⑪이 뒤늦게 생긴 그 이유).
 *
 * ★ 정답 판정은 **새 잣대를 안 짓는다** — ⑧이 쓰는 그 자(overlap20)를 그대로 쓴다.
 *   답에 정답 근거 조각의 20자가 그대로 남아 있으면 맞은 것으로 본다. 그래서 문항은
 *   **근거 ref를 회수할 수 있는 것**만 쓴다(회수 못 한 문항은 셈에서 빼고 「건너뜀」에 적는다 —
 *   0으로 세면 재료 사정이 점수로 둔갑한다).
 * ⚠ LLM 채점기는 안 쓴다(중-3 게이트 원칙·이 파일 머리주석과 같다).
 *
 * @param rows [{ id, question, text(모델 답), chunk(정답 근거 조각) }]
 */
export function 지식정답률(rows) {
  const 목록 = Array.isArray(rows) ? rows : [];
  const 대상 = 목록.filter((r) => typeof r?.text === "string" && r.text.trim()
    && typeof r?.chunk === "string" && r.chunk.replace(/\s/g, "").length >= 20);
  if (!대상.length) return null;
  const 성립 = 대상.filter((r) => overlap20(r.text, r.chunk) !== null).length;
  return { 대상: 대상.length, 성립, 비율: 성립 / 대상.length, 건너뜀: 목록.length - 대상.length };
}
...
export function 판정(입력 = {}, 기준선 = {}, 옵션 = {}) {
  ...
  //
  // ★★★ 그리고 모집단에서 **거절만 한 답**을 뺀다(2026-09-05 — r2-v2 재측정이 시킨 일).
  //   실측: r2-v2 ep2·ep3의 persona 12답이 **전부 같은 59자 거절 문장**이었다. 그때 ⑨는 「창작 0건」이라
  //   초록이었는데, 그 0건은 **「안 지어냈다」가 아니라 「답을 안 했다」**였다 — 아무 말도 안 한 답에는
  //   지어낼 인용도 없다. 그런 초록은 회귀를 못 알리는 초록이라 관문이 아니라 장식이다.
  //   그래서 「밝히고 이어 답한」 답만 센다(제품이 원하는 꼴 = llm.ts:254 「먼저 밝힙니다 … 그 뒤 …
  //   짧은 일반 정의만 덧붙입니다」). 판정 잣대는 제품의 자료없음 정규식이고, **거절 선언 뒤에 남은
  //   말이 있는지**까지 본다(거절만한답) — 선언만 보면 「밝히고 답한」 정상 답까지 모집단에서 빠져
  //   관문이 늘 미측정이 된다.
  //   ⚠ 셀 답이 하나도 없으면 **미측정=불합격**이다(「전부 거절 — 잴 답이 없다」). 거절률은 표에 참고값으로.
  const kev센행 = kev대상행(kev, 옵션.kev라벨 ?? null);
  const 답한것만 = (묶음) => (Array.isArray(묶음) ? 묶음.filter((행) => 창작인용대상인가(행) && !거절만한답(행.text)) : []);
  const persona대상 = Array.isArray(표본persona) ? 표본persona.filter(창작인용대상인가) : [];
  const kev대상 = kev센행.filter(창작인용대상인가);
  const persona답 = 답한것만(표본persona);
  const kev답 = 답한것만(kev센행);
  const f = 창작인용찾기(persona답, kev답);
  const bare참고 = 창작인용찾기(표본맨질문);
  const 거절률 = (답, 대상) => `${대상.length - 답.length}/${대상.length}`;
  const 모집단 = `모집단: **거절 아닌 답**만 — persona ${persona답.length}행 + KEV ${kev답.length}행(대조군 noprompt 제외 — 관문 ①과 같은 자리)`;
  const 거절줄 = `거절률(참고) persona ${거절률(persona답, persona대상)} · KEV ${거절률(kev답, kev대상)} — 밝히기만 하고 답을 안 한 답은 「지어낼 인용」이 없어 셈에서 뺐다`;
  ...
  if (!Array.isArray(표본persona)) {
    ...
    const 잰행 = persona대상.length + kev대상.length;
    검사.push(미측정(
      "no_fake_quote", "인용 창작 0건(persona+KEV)",
      잰행
        ? `전부 거절 — 잴 답이 없다(${거절줄}). 「창작 0건」이 아니라 **답을 안 한 것**이다`
        : "persona 표본에도 kev 결과에도 답이 든 행이 0개다",
    ));
  } else {
    검사.push({
  ...
  // ⑬ 근거를 줬는데 「자료에 없다」로 시작하는가
  //   ⚠ ⑧·⑫와 **같은 표본**을 또 다른 눈으로 본다 — 세 관문이 각각 「인용했나」·「베꼈나」·「거절부터 했나」다.
  //   ⚠ 기준은 **베이스 대비 증가 0**이다(①·⑤와 같은 꼴). 지금 베이스는 0/8이라 실질 0건이지만,
  //     베이스가 언젠가 1건이 되면 그 한 건 때문에 관문이 벽이 되는 것을 막는다.
  const rf = 근거거절률(표본grounded);
  const brf = 근거거절률(b표본);
  if (!rf || !brf) {
    검사.push(미측정(
      "grounded_no_refusal", "근거 준 자리의 거절(베이스 대비 증가 0)",
      !rf ? "grounded 표본이 없다(ask-samples --mode grounded)" : "베이스 grounded 표본(--baseline-samples)이 없다 — 무엇과 견줄지 모른다",
    ));
  } else if (rf.대상 < 표본최소_grounded) {
    // ★ 모집단이 8건이던 회전 1~4를 그대로 이어 가면, 한 건이 12.5%p라 흔들림이 판정을 뒤집는다.
    //   작은 표본으로 낸 초록은 「통과」가 아니라 「못 잰 것」이다 — 그래서 미측정으로 떨어뜨린다.
    검사.push(미측정(
      "grounded_no_refusal", "근거 준 자리의 거절(베이스 대비 증가 0)",
      `grounded 표본이 ${rf.대상}건뿐이다(최소 ${표본최소_grounded}건) — 한 건이 ${(100 / rf.대상).toFixed(1)}%p라 흔들림이 판정을 뒤집는다. **채우는 길**: tools/team-bench/samples-questions.json에 조각(chunk)이 든 문항을 ${표본최소_grounded}건 이상으로 늘린다(운영 4000 읽기 전용 창구로 cites 회수 — ask-samples.mjs 머리주석)`,
    ));
  } else {
    검사.push({
      키: "grounded_no_refusal", 이름: "근거 준 자리의 거절(베이스 대비 증가 0)",
      값: `${rf.성립}/${rf.대상} (${(rf.비율 * 100).toFixed(0)}%)${rf.건너뜀 ? ` · 건너뜀 ${rf.건너뜀}` : ""}`,
      // ★ 판정은 **건수**로 한다(2026-09-11 메인 결정 ① — 판정식은 안 바꾼다). 비율은 **병기**일 뿐이다:
      //   모집단이 8→36으로 늘어난 뒤 「몇 건」만 보면 사람이 체감을 못 하고, 「몇 %」만 보면
      //   베이스 0건의 뜻(한 건도 없었다)이 0%에 묻힌다. 두 수를 나란히 둔다(⑭ 행과 같은 꼴).
      기준: `베이스 ${brf.성립}/${brf.대상} (${(brf.비율 * 100).toFixed(0)}%) 이하 · 모집단 ${표본최소_grounded}건 이상`,
      통과: rf.성립 <= brf.성립,
      설명: [
        "근거를 준 자리에서 「자료에 없다」고 시작하는 것은 RAFT가 가르치려는 것의 정반대다",
        "⑧·⑫는 원리상 못 본다 — 거절 문장 **뒤에** 근거를 복사해 이으면 둘 다 초록이 된다(회전 3 실측 8/8 초록에 7/8 거절)",
        `판정 잣대는 ⑩과 **같은 자**(제품의 자료없음 정규식 · 앞 60자)다`,
      ].join(" / "),
    });
  }

  // ⑭ **로컬 14B 단독**에서 지식을 잃지 않았는가 (회전 5의 목적을 재는 자 · 계획서 §12.12)
  //   ⚠ 근거도 원격도 없는 자리다 — 고객 설치본(표준·라이트)에서 실제로 벌어지는 그 조건.
  //   ⚠ 기준은 **베이스 대비 무하락**이다(①·⑤·⑬과 같은 꼴). 절대 정답률은 참고값으로 표에 남긴다:
  //     14B 단독의 절대 점수는 문항이 정하는 것이라, 그 수로 합격선을 그으면 문항을 바꿀 때마다 흔들린다.
  const lk = 지식정답률(입력.지식);
  const blk = 지식정답률(기준선.지식);
  if (!lk || !blk) {
    검사.push(미측정(
      "local_knowledge", "로컬 14B 단독 지식 정답률(베이스 대비 무하락)",
      !lk ? "지식 문항 결과가 없다(--local-knowledge · 근거 ref를 회수한 문항이라야 한다)"
          : "베이스 지식 문항 결과(--baseline-local-knowledge)가 없다 — 무엇과 견줄지 모른다",
    ));
  } else if (lk.대상 < 문항최소_지식 || blk.대상 < 문항최소_지식) {
    검사.push(미측정(
      "local_knowledge", "로컬 14B 단독 지식 정답률(베이스 대비 무하락)",
      `문항이 모자라다(이번 ${lk.대상} · 베이스 ${blk.대상} · 최소 ${문항최소_지식}). **채우는 길**: tools/team-bench/samples-questions.json에 조각(chunk)이 든 문항을 ${문항최소_지식}건 이상으로 늘린다(운영 4000 읽기 전용 창구로 cites 회수 — ask-samples.mjs 머리주석)`,
    ));
  } else {
    검사.push({
      키: "local_knowledge", 이름: "로컬 14B 단독 지식 정답률(베이스 대비 무하락)",
      값: `${lk.성립}/${lk.대상} (${(lk.비율 * 100).toFixed(0)}%)${lk.건너뜀 ? ` · 건너뜀 ${lk.건너뜀}` : ""}`,
      기준: `베이스 ${blk.성립}/${blk.대상} (${(blk.비율 * 100).toFixed(0)}%) 이상`,
      통과: lk.비율 >= blk.비율 - 1e-9,
      설명: [
        "회전 5의 목적을 재는 유일한 자 — 원격 두뇌도 근거 블록도 없는 자리에서 아는 것을 잃지 않았나",
        "정답 판정은 ⑧과 **같은 자**(overlap20 · 정답 근거 조각과 20자 겹침)다 — 새 잣대를 짓지 않았다",
        "근거 ref를 회수 못 한 문항은 셈에서 뺐다(0으로 세지 않는다)",
      ].join(" / "),
    });
  }
...
export function 표만들기(결과, 참고 = {}) {
  ...
  return [
    ...
    "⚠ **⑧(근거 인용)은 통째 복사로도 100%가 된다 — ⑫(베낀 글자 비율)·⑬(근거 준 자리의 거절)과 함께 읽는다.**",
    "",
    "| | 관문 | 실측 | 기준 | 왜 이 잣대인가 |",
    "|---|---|---|---|---|",
    ...줄,
...
if (process.argv[1] && process.argv[1].replace(/\\/g, "/").endsWith("team-bench/gates.mjs")) {
  ...
  if (!easyP && !hardP) {
    console.error(
      ...
      "         [--local-knowledge lk.json] [--baseline-local-knowledge 베이스lk.json]\n" +
  ...
  // ⑭ 로컬 14B 단독 지식 — 회전 5의 목적을 재는 자리(계획서 §12.12). 없으면 미측정=불합격이다.
  const 지식P = opt("--local-knowledge", ""), b지식P = opt("--baseline-local-knowledge", "");
  const 입력 = {
