...
export function 판정(입력 = {}, 기준선 = {}, 옵션 = {}) {
  ...
  if (!rf || !brf) {
    ...
    검사.push(미측정(
      "grounded_no_refusal", "근거 준 자리의 거절(베이스 대비 증가 0)",
      `grounded 표본이 ${rf.대상}건뿐이다(최소 ${표본최소_grounded}건) — 한 건이 ${(100 / rf.대상).toFixed(1)}%p라 흔들림이 판정을 뒤집는다. **채우는 길**: tools/team-bench/samples-questions.json에 조각(chunk)이 든 문항을 ${표본최소_grounded}건 이상으로 늘린다(운영 4000 읽기 전용 창구로 cites 회수 — ask-samples.mjs 머리주석). 오늘 그 파일의 조각 든 문항은 8건이다`,
    ));
  } else {
    검사.push({
      키: "grounded_no_refusal", 이름: "근거 준 자리의 거절(베이스 대비 증가 0)",
      값: `${rf.성립}/${rf.대상} (${(rf.비율 * 100).toFixed(0)}%)${rf.건너뜀 ? ` · 건너뜀 ${rf.건너뜀}` : ""}`,
      기준: `베이스 ${brf.성립}/${brf.대상} 이하 · 모집단 ${표본최소_grounded}건 이상`,
      통과: rf.성립 <= brf.성립,
      설명: [
  ...
  if (!lk || !blk) {
    ...
    검사.push(미측정(
      "local_knowledge", "로컬 14B 단독 지식 정답률(베이스 대비 무하락)",
      `문항이 모자라다(이번 ${lk.대상} · 베이스 ${blk.대상} · 최소 ${문항최소_지식}). **채우는 길**: tools/team-bench/samples-questions.json에 조각(chunk)이 든 문항을 ${문항최소_지식}건 이상으로 늘린다(운영 4000 읽기 전용 창구로 cites 회수 — ask-samples.mjs 머리주석). 오늘 그 파일의 조각 든 문항은 8건이다`,
    ));
  } else {
    검사.push({
