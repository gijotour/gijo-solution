 ...
 *   4.2%p가 되어 **관문이 회귀를 알리는 쪽**으로 돌아온다. 24는 「모집단이 세 배」의 자리이자,
 *   근거 ref를 회수할 수 있는 문항을 실제로 모을 수 있는 수(홀드아웃 100 중 회수 가능분)다.
...
export function 판정(입력 = {}, 기준선 = {}, 옵션 = {}) {
  ...
  if (!rf || !brf) {
    ...
    검사.push(미측정(
      "grounded_no_refusal", "근거 준 자리의 거절(베이스 대비 증가 0)",
      `grounded 표본이 ${rf.대상}건뿐이다(최소 ${표본최소_grounded}건) — 한 건이 ${(100 / rf.대상).toFixed(1)}%p라 흔들림이 판정을 뒤집는다`,
    ));
  } else {
    검사.push({
  ...
  if (!lk || !blk) {
    ...
    검사.push(미측정(
      "local_knowledge", "로컬 14B 단독 지식 정답률(베이스 대비 무하락)",
      `문항이 모자라다(이번 ${lk.대상} · 베이스 ${blk.대상} · 최소 ${문항최소_지식})`,
    ));
  } else {
    검사.push({
