 ...
 * ⚠ 세는 것은 **보고서(.json)뿐**이다(회차번호 참조) — 로그만 남고 죽은 회차는 안 센다. 그래야
 *   실패한 회차가 다음 실행에서 **같은 번호로 다시** 돈다.
 */
...
/**
 * "취약점-03.json" → 3. **끝난 회차의 증거는 보고서(.json) 하나뿐이다** — 다른 확장자는 안 센다.
 * 주제가 안 맞거나 꼴이 다르면 null(모른다 — 0으로 적지 않는다).
 *
 * ⚠ 왜 .json만 세나(2026-09-03 1일차 실기동에서 데였다): 예전 규칙은 확장자를 아무거나 받아
 *   `사내규정-01.log`를 **회차 1**로 셌다. 그런데 로그는 증류가 **시작될 때** 생기고 보고서는
 *   **끝나야** 생긴다 — 그래서 죽은 회차가 「끝난 회차」로 세어지고, `--skip-done`이 그 회차를
 *   영영 건너뛰었다(그날은 로그를 손으로 `.crash.log`로 바꿔 피했다 — 자동화가 사람 손을 부른 것이
 *   결함의 증거다). 시작한 것과 끝난 것을 같은 자로 세면 실패가 성공으로 둔갑한다.
 */
export function 회차번호(파일명, 주제) {
  const 이름 = String(파일명 ?? "").replace(/\\/g, "/").split("/").pop() ?? "";
  const 안전 = String(주제).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const m = new RegExp(`^${안전}-(\\d+)\\.json$`).exec(이름);
