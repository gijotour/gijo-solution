...
//   · ⚠ 단, **반납 실패는 게시를 막지 않는다**(2026-09-05) — 빌드 품질과 무관한 뒷정리라
//     exit에 안 싣고 **큰 경고 + 요약 마지막 줄**로 알린다. 맨 아래 「L3-반납」 참고.
//
...
// ── ⑧″ 감독 API 다리(contextBridge) **봉인** — 고정 자료 판정이 왜 여기 없나(2026-09-07) ──
// ■ 되돌린 자리다. 같은 날 검토관 적발(반증 3회 실패) — 정직하게 적어 둔다.
//   앞선 판의 ⑧″는 실앱에서 `window.gijo.aiteamSupervision` 한 칸을 잠시 갈아끼워
//   고정 자료로 ⑧′의 「못 뗌·통째교체면 처음부터 펴져 있어야 한다」를 판정하려 했다.
//   **실앱에서는 원리상 안 된다.** contextBridge가 내준 window.gijo는 Electron이 얼려 내주고
//   (속성이 writable:false·configurable:false) 대입도 defineProperty도 먹지 않는다.
//   이 저장소가 이미 두 번 적어 둔 사실이다 — client/src/preload.ts:129
//   「contextBridge로 노출한 객체는 **렌더러에서 못 고친다**(조용히 무시된다. 실제로 그렇게
//   시도했다가 아무 일도 안 일어났다)」. 그래서 그 판의 ⑧″는 **늘 빨갰다.**
// ■ 그것이 왜 큰일인가 — 관문이 제품을 인질로 잡는다.
//   ⑧″가 늘 빨가면 exit 1 → publish-release.mjs가 「UI 관문 실패 — 게시 중단」으로 throw한다.
//   빠져나갈 길은 `--skip-ui-gate`뿐인데, 그 스위치는 ⑧″ 하나가 아니라 **실화면 관문 전부**를
//   끈다. 검사 하나를 살리려다 안전망을 통째로 잃는 거래다 — 그래서 되돌렸다.
// ■ 판정은 없앤 게 아니라 **자리를 옮겼다.**
//   고정 자료(갑·을) 두 벌의 자동 펼침·배지 두 갈래·펼침 기억 불변은
//   **server/test/publishgatecitefixed.test.ts**가 판정한다. 거기서는 갈아끼우기가 된다 —
//   가짜 DOM 위에 supervision.html의 **인라인 스크립트 그 자체**를 올려 돌리기 때문이다
//   (흉내 화면이 아니다). 픽셀과 실제 운영 자료는 종전대로 바로 위 ⑧′가 실화면에서 잰다.
// ■ 그러면 여기서는 무엇을 재나 — **그 「원리상 안 된다」가 아직 참인지**를 잰다.
//   봉인은 제품의 보안 경계이기도 하다: 렌더러 페이지가 다리를 갈아끼울 수 있으면 preload가
//   지키던 판단이 통째로 무력해진다(navigateTo의 탭 분기가 바로 그 위에 서 있다 — preload.ts:129).
//   봉인이 깨지면 여기가 빨개지고, 그때 ① 보안 경계를 확인하고 ② 위 고정 자료 판정을
//   실화면으로 되살릴 수 있다. 빨강이 **드문 진짜 사건**이라 게시를 막을 값이 있다.
// ⚠ 아무것도 안 바꾼다 — 서술자를 읽고, 쓰기 시도는 **원래 값과 똑같은 값**으로만 해 본다
//   (성공해도 제품 상태가 그대로다). 감독 자료를 지어내지 않는다.
{
  await 셸.evaluate(() => window.gijoTabs && window.gijoTabs.open("supervision.html", "감독", { dock: true }));
  const fr = await 프레임찾기("supervision.html", 10);
  const r = fr ? await fr.evaluate(() => {
    if (!window.gijo || typeof window.gijo.aiteamSupervision !== "function") return { API없음: true };
    const 원래 = window.gijo.aiteamSupervision;
    // 서술자만 믿지 않는다 — 실제로 갈아끼워지는지 **같은 값으로** 해 본다(제품 상태는 안 바뀐다).
    let 던짐 = null;
    try { window.gijo.aiteamSupervision = 원래; } catch (e) { 던짐 = String((e && e.message) || e); }
    const 뒤 = Object.getOwnPropertyDescriptor(window.gijo, "aiteamSupervision");
    // ⚠ 서술자가 **없으면**(상속받은 칸 등) 봉인 여부를 잰 것이 아니다 — 「없음 = 봉인」으로
    //   읽으면 못 쟀는데 초록을 주게 된다. 그때는 서술자없음:true로 빨갛게 끝낸다.
    return {
      서술자없음: !뒤,
      writable: !!뒤 && 뒤.writable === true,
      configurable: !!뒤 && 뒤.configurable === true,
      얼었나: Object.isFrozen(window.gijo),
      던짐,
      그대로: window.gijo.aiteamSupervision === 원래,
    };
  }).catch(() => null) : null;
  const 성립 = !!r && !r.API없음 && r.서술자없음 === false
    && r.writable === false && r.configurable === false   // 다리가 봉인돼 있다(그래서 판정은 시험 파일에)
    && r.그대로 === true;                                  // 그리고 재는 동안 제품 상태가 안 바뀌었다
...
// ★ L3-반납(2026-09-05 상위 결정) — 반납 실패는 **실패 목록에 안 싣는다.**
//
// ★ 왜: 이 관문의 실패 목록은 곧 `exit 1`이고, 그건 **게시 중단**이다. 그런데 세션 반납은
//   빌드 품질과 아무 상관이 없다 — 서버가 잠깐 흔들렸거나 렌더러가 먼저 죽었을 뿐인데
//   **멀쩡한 빌드의 게시가 막힌다.** 관문이 막아야 할 것은 「고객에게 나갈 물건의 결함」이지
//   「내 뒷정리 실패」가 아니다. 그래서 종료코드에서는 빼고, 사람이 못 지나치게 **큰 경고**로 알린다.
// ⚠ 대신 성공이든 실패든 **요약 마지막 줄에 항상 찍는다.** 조용히 넘어가면 다음 사람이
//   「이미 다른 곳에서 로그인 중」을 만나고도 이유를 모른다(K5가 고치려던 바로 그 자리).
if (!반납.끊었나) {
  console.error("");
  console.error("[ui관문] ⚠⚠⚠ claude-deploy 세션 반납 실패 — 30분 유령 세션.");
  console.error("[ui관문] ⚠⚠⚠ 라이브 QA 전에 유휴 만료를 기다리거나 헤더+refreshToken으로 직접 반납하세요.");
  console.error("[ui관문] ⚠⚠⚠ (사유: " + 반납.사유 + ") — 게시는 막지 않습니다(빌드 품질과 무관).");
  console.error("");
}
...
console.log("[ui관문] 관문이 자기 세션을 반납했다(유령 세션 0): " + (반납.끊었나
  ? "성공 — window.gijo.logout()으로 끊었다(taskkill은 before-quit을 건너뛴다)"
  : "실패 — " + 반납.사유 + " · claude-deploy 세션이 유휴 만료(30분)까지 남는다(종료코드에는 안 싣는다)"));
