...
const 가시화 = await 셸.evaluate(async () => {
  ...
  const 약자들 = 로스터.slice(0, 7).map((b) => (b.title || ""));
...
ok("레일 로스터 11개(팀7+부품4)", 가시화.로스터수 === 11, "개수=" + 가시화.로스터수);
ok("로스터 약자(등록부 단일 출처) 반영", 가시화.약자적용 === 7, "약자 " + 가시화.약자적용 + "/7");
...
ok("배지 갱신이 실제로 한 번은 돌았다(aria-label 기록)", 배지자리.갱신됨, JSON.stringify(배지자리));
{
  ...
  ok("대시보드 팀 카드 7칩", !!r && r.칩 === 7, r ? "칩=" + r.칩 : "프레임 못 찾음");
