 ...
 *
 * ★ 제목(2026-09-05 K4) — 제품이 「[n] 《문서 제목》 본문」으로 낸다. 그래서 빌더도 **제목을 싣는다.**
 *   안 실으면 학습 표본과 추론 지문이 제목 유무로 갈리는데, 그 어긋남은 **아무 오류도 안 내고**
 *   성능만 깎는다(tools/ladder/README.md가 계속 경고하던 그것 — 여기서 닫는다).
 * ⚠ 제목을 **안 주면 옛 꼴 그대로**다(ragBlock과 같은 규칙) — 제목이 없는 재료도 그대로 돈다.
 * ⚠ 정직한 한계: 빌더가 아는 제목은 코퍼스의 `documentId`다. 제품은 침해사고 사례·개인 문서를
 *   사람이 읽는 제목으로 바꿔 싣는데(server memory.ts 사람이읽는문서제목), 코퍼스에는 그 두 종류가
 *   안 들어오므로 실제로는 같은 값이다. 들어오기 시작하면 여기도 함께 고쳐야 한다.
 */
export function 참고자료블록(머리말, 조각들, 제목들) {
  return 머리말 + "\n" + 조각들.map((c, i) => {
    const t = String(제목들?.[i] ?? "").trim();
    return t ? `[${i + 1}] 《${t}》 ${c}` : `[${i + 1}] ${c}`;
  }).join("\n");
}

/** 창구 예시(`ragBlockSample`)와 대조할 때 쓰는 **자리표 한 벌** — 다섯 곳이 이 상수를 쓴다(단일 출처). */
export const 예시조각들 = ["<조각 본문>"];
export const 예시제목들 = ["<문서 제목>"];
/** 창구가 줄 예시와 **같은 글**을 빌더 규칙으로 지어 본다 — 다르면 부르는 쪽이 멈춘다(fail-closed). */
export function 예시블록(머리말) {
  return 참고자료블록(머리말, 예시조각들, 예시제목들);
}

/**
 * **제목이 생기기 전(K4 이전)의 예시 블록** — 「[1] <조각 본문>」.
 *
 * ★ 왜 남겨 두나(2026-09-05 검토관): 저장소에 보관된 회전 1~4의 규격 파일 9개가 전부 이 꼴이다.
 *   새 꼴 하나만 아는 규격읽기는 그 아홉을 「어긋납니다」로 **죽여서**, 굽는 길만이 아니라
 *   **옛 회전을 재현·재측정하는 길**까지 닫았다(ask-samples `--prompt-spec <보관 규격>`).
 *   그런데 보관 규격은 낡은 것이 아니라 **그때의 사실**이다 — 옛 회전을 오늘 꼴로 재는 것이야말로
 *   딴 틀을 재는 것이다. 그래서 fail-open으로 풀지 않고 **아는 꼴을 둘로** 늘린다:
 *   규격은 여전히 「자기 안에서 앞뒤가 맞아야」 하고, 어느 꼴이었는지가 `꼴`로 따라 나온다.
 */
export function 예시블록옛꼴(머리말) {
  return 참고자료블록(머리말, 예시조각들, null);
}
...
export function 근거조각뽑기(system, 머리말) {
  ...
  const 몸 = 뒤.slice(1);
  if (!/^\[1\] /.test(몸)) return [];
  // ⚠ 제목이 실린 꼴(「[1] 《제목》 본문」)도 되돌린다(2026-09-05 K4). **제목은 근거 본문이 아니다** —
  //   붙여 둔 채 돌려주면 인용 대조가 제목 글자만큼 어긋난다. 옛 파일(제목 없음)도 그대로 읽힌다.
  return 몸.split(/\n(?=\[\d+\] )/).map((t) => t.replace(/^\[\d+\] (?:《[^》\n]*》 )?/, ""));
}
...
export function 규격읽기(경로, root = 저장소) {
  ...
  // ★ **아는 꼴이 둘**이다(2026-09-05 검토관 — 예시블록옛꼴 주석): 오늘 꼴(제목 있음)과
  //   K4 이전 꼴(제목 없음). 둘 다 아니면 그대로 죽는다(fail-closed는 그대로다).
  //   어느 꼴이었는지는 `꼴`로 실어 돌려준다 — 「옛 회전과 견줄 때는 그 차이를 적어 두고 견준다」가
  //   말이 아니라 값이 되게. 부르는 쪽은 이 값을 결과에 남긴다.
  const 꼴 = 예시블록(j.ragHeader) === j.ragBlockSample ? "제목있음"
           : 예시블록옛꼴(j.ragHeader) === j.ragBlockSample ? "제목없음(K4 이전)"
           : null;
  if (!꼴) {
...
/**
 * 섞은 본문 배열에 **자리를 맞춘 제목 배열**을 만든다(2026-09-05 K4).
 * ⚠ 본문으로 되찾는 이유는 섞기의 정렬 키가 문자열이라서다 — 객체를 섞으면 옛 회전이 재현되지 않는다.
 *   본문이 같은 조각은 방해조각고르기가 이미 뺐다(본문들 Set) — 그래서 짝이 엉키지 않는다.
 */
export function 제목맞추기(섞인본문들, 조각객체들) {
  const 표 = new Map((조각객체들 ?? []).map((c) => [c.text, c.문서 ?? ""]));
  return 섞인본문들.map((t) => 표.get(t) ?? "");
}
  ...
  for (const l of 문답들) {
    ...
    if (!정답실림) {
      ...
      담기("B", {
        ...
        system: [팀원프롬프트, 참고자료블록(ragHeader, 조각들, 제목맞추기(조각들, 방해))].join("\n\n"),
      });
      continue;
    }
    ...
    // ★ 제목은 **섞은 뒤 본문으로 되찾는다**(2026-09-05 K4). 섞기의 정렬 키가 문자열이라 객체를 섞으면
    //   `[object Object]`가 되어 회전 1~4의 재현이 끊긴다 — 섞는 것은 그대로 두고 짝만 맞춘다.
    const 제목들 = 제목맞추기(조각들, [...정답들, ...방해]);
...
async function main() {
  ...
  if (예시블록(프롬프트.ragHeader) !== 프롬프트.ragBlockSample) {
