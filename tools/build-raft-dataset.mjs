...
export function 인용붙이기(answer, 근거텍스트들) {
  ...
  if (!후보.length) return { answer: null, 왜: "인용할 문장 없음(20자 겹침)" };
  ...
  if (!쓸만한.length) return { answer: null, 왜: `인용할 문장 없음(${인용상한}자 넘는 덩어리뿐)` };
  ...
  return { answer: `${a} 원문: "${고를것[0]}"`, 붙임: true };
}
...
export function 구성비(rows, 종류들) {
  ...
  종류들.forEach((k, i) => { if (!표[k]) return; 표[k].행 += 1; if (인용있나(rows[i].answer)) 표[k].인용 += 1; });
...
export function 사전검사(구성) {
  ...
  if (비A.인용 > 0) {
    ...
    const 범인 = Object.entries(구성.표)
      .filter(([k, v]) => k !== "A" && v.인용 > 0)
      .map(([k, v]) => `${k} ${v.인용}/${v.행}`)
      .join(" · ");
    실패.push(`비A행 ${비A.행}개 중 ${비A.인용}개에 「원문:」이 남아 있다(${범인}) — 근거가 없는 자리에서 인용하는 법을 가르치게 된다`);
  }
  if (A.행 > 0 && A.비율 < 0.95) {
