...
#
# ■ ★ **gb10 사본에는 「git이 이미 실어 나른 것」만 담는다**(2026-09-08 신설)
#   ⚠ 먼저 오해부터 막는다 — **아래 사본 만들기는 win이 아니라 gb10 안에서 돈다.**
#     146행 `<<'REMOTE'`부터 327행까지가 ssh로 넘겨 gb10이 실행하는 셸이고, 그 안의
#     SRC_ROOT는 **gb10 자신의 `~/gijo-as`**다(166행). 즉 사본 만들기는 「gb10의 저장소 →
#     gb10의 /tmp」이지 win→gb10 전송이 아니다. **이 스크립트는 파일을 원격으로 보내지 않는다**
#     (gb10을 갱신하는 것은 오로지 win의 `git push gb10 main`이다).
#   그럼 여기서 지키는 것은 무엇인가 — **사본의 재료를 git이 실어 나른 것으로 묶는 것**이다.
#     사람이 쓴 문서도 사본에 들어간다(뿌리 *.md 53개 · knowledge/ · mockups/). 그건 저장소에
#     커밋된 제품 문서라 push가 이미 gb10에 올려 둔 것이고, 사본은 그 사본일 뿐이라 새로 나가는
#     것이 없다. 반대로 **git이 안 나르는 것(.gitignore 대상)은 사본도 요구하지 않는다** —
#     사본이 그것을 요구하기 시작하면 「gb10에서도 이 시험을 돌리자」며 누군가 그 파일을 gb10에
#     **갖다 놓는** 날이 오고, 그날이 사내 문서 본문이 이 기계에 자리 잡는 첫날이 된다.
#   실전 답 기록(.tmp-reports/ops-sim.json)이 정확히 그 부류다 — **2026-09-08부터 그 파일에
#   「가드가 본 근거 조각 본문」이 실린다.** 하네스 계정 claude-deploy는 admin이라 등급 C
#   문서(운영 1,598건)의 조각까지 담긴다. 그래서 아래 사본 만들기에서 그 두 줄을 **뺐다**
#   (자세한 왜는 그 자리에). 오늘 기준 gb10의 `~/gijo-as/.tmp-reports/`에는 그 파일이 없다.
#   ★ 형제 도구 wsl-test.sh는 **그대로 옮긴다** — 거기는 같은 기계의 WSL이라 경계를 안 넘는다.
#     실전 답을 재료로 쓰는 시험을 진짜로 재려면 그쪽으로 간다.
#   ⚠ 그러므로 이 절을 「이 도구가 유출 통로였다」로 읽지 말 것. 통로였던 적이 없다. 진짜 통로가
#     생긴다면 저장소 뿌리를 통째로 복사하는 다른 관행(백업·인계) 쪽이지 여기가 아니다.
#
...
#   ■ 그래서 무엇이 문제였나 — ⚠ **이 cp가 win→gb10 전송이었던 것은 아니다.**
#     이 줄은 ssh 힙독(146~327행) **안**이라 gb10이 제 저장소(`~/gijo-as`)에서 제 /tmp로
#     하던 복사였다. 문제는 방향이 아니라 **요구**다 — 사본이 이 파일을 재료로 요구하면,
#     「gb10에서 이 시험이 자꾸 건너뛴다」를 고치려는 사람이 그 파일을 gb10에 **갖다 놓는다.**
#     그 순간 등급 C 사내 문서 조각이 별도 계정(gijohn_llm)의 기계에 자리 잡는다.
#     형제 도구 wsl-test.sh가 옮기는 곳은 같은 기계의 WSL이라 그런 유인이 없다.
...
#   ⚠ 이 자리에서 `.tmp-reports`를 **어떤 꼴로든** 다시 건드리면(cp·rsync·위 `for d in …`
#     루프에 한 낱말 추가·빈 mkdir까지) **server/test/shipscripts.test.ts가 빨강**이 된다.
#     감시 잣대가 낱말 "ops-sim"이 아니라 **폴더 이름 `.tmp-reports`**라서 그렇다
#     (2026-09-08 반증: 낱말만 보던 판은 되살림 6꼴 중 5꼴을 그냥 통과시켰다).
