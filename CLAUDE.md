...
- **UI 변경은 「추천 시안 1개」 먼저** (자체완결 HTML 목업) → 승인받고 구현. 곧바로 구현 금지. 화면별 시안은 mockups/ 아래 별도 폴더(파일명 `시안.html`).
  ...
  - 시안에 반드시 붙일 것: **왜 이 안인지**(다른 길을 왜 안 골랐는지) · 무엇을 포기했는지 · **밀도 숫자**(줄 높이·세로 총합·기존 대비 증감) · 제품 원칙 자가점검. ⚠ **px는 제품 화면에서 잰 값**으로 적는다 — 목업 안에서 잰 31px이 설치본에선 35px였다(2026-09-06 실측). 목업 값이 잣대가 되면 게시 관문이 그 숫자로 거짓 실패·거짓 통과를 낸다. 방법: 설치본(win-unpacked)을 CDP로 열어 형제 줄과 함께 getBoundingClientRect로 재고, 「지금 N px → 제안 M px」의 「지금」은 반드시 제품에서 잰다. 사용자가 다른 방향을 원하면 **그때** 대안을 만든다.
...
- **설계관을 앞에 세운다**(2026-08-20 사장님 지시 「설계관 만들고」) — 큰 기능 묶음은 **구현 착수 전에** `gijo-architect`에게 계획을 태운다. 사슬: 시안 → **설계관** → 구현 → 검토관(병렬) → 수리 → 게이트 → 배포 → 관문 → 게시.
  근거(2026-08-20 실측): 그날 검토관이 잡은 21건 중 상당수가 **착수 전에 한 문장만 물었으면** 막혔다 — 「이 API가 주는 필드가 뭔가」(필드명 오인 5번째)·「이 숫자를 이미 세는 곳이 있나」(잣대 3중화)·「이걸 지우면 뭐가 그것을 가리키나」(흡수 연쇄 6곳). 이 저장소의 결함은 **몰라서가 아니라 안 물어봐서** 난다.
...
- **⏸ Mac 인계는 당분간 멈춘다 — 제품 완성 후 한 번에 전달**(2026-08-22 사장님 지시 「MAC 전달은 한동안 하지 말자, 제품 완성 이후 한번에 전달」). 보고마다 인계 블록을 붙이지 않는다.
  - ⚠ **멈춘다 ≠ 잊는다.** 전달할 것은 `GIJO_AS_MAC_인계_대기.md`에 **그때그때 쌓는다** — 안 쌓으면 나중에 한 번에 전달할 때 **무엇이 바뀌었는지 아무도 모른다**(이 저장소가 반복해 겪은 「기록 안 하면 사라진다」의 전형).
  - 특히 **공용 파일**(nav.js·screenguide.ts·agentloop.ts·routes.ts·CLAUDE.md)과 **max가 실기 검증해야 할 것**(라이트 dmg·Metal·올인원)은 빠짐없이 적는다.
  - 재개 시점: 사장님이 「제품 완성」이라 판단하실 때. 그때 이 대기 문서로 한 번에 인계한다.
  - ~~작업을 마치면 상대 머신(Mac)에 전달할 것이 있는지 보고, 있으면 인계 블록을 붙인다(2026-08-09)~~ — 위 지시로 **보류**. 형식은 `.claude/commands/GIJOAS인계.md`에 그대로 있다(재개할 때 쓴다).
- 문서(가이드·PDF·Notion 등) 갱신은 요청받을 때만.
- **모든 작업 보고에 `gb10` 이용 내역을 적는다**(2026-09-01 사장님 지시 「모든 작업보고에 gb10 이용내역 알려줘」). 무엇에 썼나 · 무엇을 돌려받았나 · **값을 했나**(정직하게). **안 썼으면 「안 썼음 + 왜」**를 적는다 — 빈칸으로 두지 않는다.
  - 왜: 2026-08-27 「gb10으로 토큰 줄일 수 있는 거 있으면 해줘」 지시 뒤에도 계속 안 써서 사장님이 **세 번을 물어야 했다.** 만들어 놓고 안 쓰는 것이 이 저장소의 반복 실패라, 보고에 칸을 만들어 **매번 드러나게** 한다.
  - ⚠ **못 한 것도 적는다** — 실측(2026-09-01): `sbom.ts` 발췌는 답을 못 줘서 결국 grep을 따로 했다.
  - ⚠ **서브에이전트에 「쓰라」고 적는 것으로는 안 된다** — 실측: 워크플로 프롬프트에 안내를 넣었는데 **120명 중 3명만** 실행했다(2차는 0명). 안내가 아니라 **강제**여야 한다 — 워크플로 첫 단계에서 요약을 미리 만들어 프롬프트에 넣어 준다. 시켰으면 **기록으로 세어** 보고한다.
  - ★ 값어치는 「결함 찾기」가 아니라 **「큰 파일을 싸게 읽기」**다. 실측: 실결함 30건은 전부 Claude 검토관이 찾았고 gb10 1차 선별의 후보 1건은 오탐이었다. 반면 4,000줄 `handlers.ts`를 40줄로 받아 거기서 낡은 주석(실결함)을 찾았다.
...
⚠ 사람에게 안내할 때 **기계 이름을 먼저** 적는다 — "`win`에서 …", "`max`에서 …". 자세한 건 `GIJO_AS_3머신_개발환경_가이드.md` §0.

### 개발 기준은 프로 (2026-08-19 사용자 결정 — "프로가 원래 내가 생각했던 모델")
- **신규 기능·UX는 프로 셸(대화 중심·도킹/팝업·카드) 기준으로 먼저 설계·구현**하고, 표준·라이트에는 **괜찮은 것만 반영**한다. 표준 시절에 만든 화면 26종은 프로가 그대로 쓰는 자산(도킹/팝업으로 열림)이다 — 표준을 버리는 게 아니라 **셸 우선순위**의 문제.
- 실무 함의: QA·시연·첫인상 점검의 기준 화면=프로 / 「지시는 대화창」·카드·이관(B16·ⓐ87)의 우선순위 상승 / 화면 전용 조작을 새로 만들기 전에 「프로에서 이게 대화·카드로 되나」를 먼저 묻는다.
- 계획서(전중후)와 무충돌 — 전-7 「보여 주기」의 연장이며 판매 구성(3에디션)은 그대로.

### 제품 분담 — **작업은 전부 `win` 주도, `max`는 검증** (2026-08-20 사용자 결정으로 개정)
- **`win` = 전 제품 작업 주도**: 메인 제품(계획서 전·중·후 항목·공용 엔진·운영 배포·게시·QA)에 더해 **라이트 에디션 작업도 `win`이 한다**(2026-08-20 사장님 「라이트 에디션 작업도 니가 해 … 메인은 너야」 — 2026-08-13 「라이트는 max 전담」을 대체).
- **`max` = 검증이 메인**: 라이트 산출물의 mac 실기 검증(dmg 빌드·Metal·실화면), 올인원 검증. 라이트 dmg 빌드는 mac에서만 되므로 그 단계는 `max`가 수행. **수정·구현 작업은 사장님이 max에 직접 시킬 때만 한다**(2026-08-20 — max가 알아서 작업을 집어가지 않는다; 발견한 결함은 종전대로 발견·재현·패치안까지 내고 `win`에 인계).
- (참고 — 옛 분담 시절 자산: 라이트 화면 9개 · `lite-*` 파일 · `lite-screens.json`/`lite-tools.json` · 라이트 게이트(`tools/lite-gate.mjs`·`lite-routing.json`) · 라이트 빌드(dmg·`electron-builder.lite.json`). 이제 이 파일들도 `win`이 만진다.)
...
| `gijo-architect`(설계관) | 읽기 + git 조회 | **구현 착수 전** 계획 검토 — ①쓸 API의 실제 필드명 ②걸리는 계약·감시 ③이미 세는 곳 ④연쇄 목록. 검토관의 앞쪽 대칭(2026-08-20 사장님 지시) |
| `gijo-reviewer`(검토) | 읽기 + git 조회 | 커밋·변경 묶음 결함 검토 — 특히 **예약 루틴 커밋 사후 검토**, 게시 전 검토. ★**항상 쪼개서 병렬로**(아래) |
| `gijo-mockup`(시안) | mockups/ 전용 쓰기 | UI 시안(자체완결 HTML) 제작 — 추천 1개 원칙, 제품 코드 수정 금지 |
- ★ **검토관은 항상 쪼개서 병렬로 돌린다**(2026-08-20 사장님 지시 「검토관 항상 쪼개서 병렬로 돌려줘」). 한 명이 전 범위를 순차로 읽으면 13~16분이 걸리는데, 느린 원인은 모델이 아니라 **읽을 양**이다(검토관은 이미 opus다 — 모델을 내리면 「그럴듯한데 틀린 지적」이 늘어 검증 비용이 더 든다).
  - 표준 구성: **관점 5갈래 동시**(① 신규 코드 로직 ② 필드명·API 원천 대조 ③ 기존 화면·부품에 미치는 영향 ④ 감시·계약 등록 누락 ⑤ 약속(커밋·주석·시안)과 코드의 불일치) → 적발 건마다 **반증 담당**을 붙여 오탐을 거른다(「이것이 오탐임을 입증해 보라」).
  - 도구는 `Workflow`(parallel/pipeline). 각 갈래는 **좁은 범위만** 본다 — 범위를 좁혀야 깊이 판다.
  - ⚠ 병렬은 **읽기 전용 검토에만** 쓴다. 검증·배포·게시·QA는 직렬 자원(4000·9227·계정 1세션)이라 아래 금지가 그대로다.
  - ★ **Claude 검토관을 부르기 전에 gb10 로컬로 1차 선별**한다(2026-08-31 사장님 「토큰수 줄이려고 gb10 쓰기로 한 게 반영 안 된 거냐」): `node tools/local-digest.mjs review <커밋>`으로 후보를 뽑아 **범위를 좁혀** 병렬 검토관에 넘긴다. 로컬 결과는 **후보일 뿐** 확정이 아니고, **게시 전·보안 경계는 종전대로 정식 검토관**이다(diff에 안 나오는 부류는 로컬이 원리상 못 본다).
- ★ **큰 파일은 통째로 읽지 않는다 — gb10 발췌부터**(2026-08-31 확립). 800줄 넘는 파일은 Read 전에 `node tools/local-digest.mjs file <경로> "<질문>"`(gb10 Qwen3-Coder-30B-A3B · 실측 1,416줄→92줄 · 25초 · **원문 그대로**, 요약 아님). 긴 로그는 `log`, 서버가 죽어 있으면 `up`(30~60초).
  근거(2026-08-31 실측 사고): 이 도구는 2026-08-27에 이미 가동됐는데 **한 라운드를 통째로 안 썼다** — 큰 파일을 직접 읽고 검토를 전부 Claude 병렬로 돌려 **지출 한도로 검토관 18개가 죽었다**(그 설계가 막으려던 바로 그 사고). 원인은 규칙이 **메모리 산문에만** 있었던 것이라, 세션마다 읽히는 유일한 자리인 여기로 옮겨 적는다. 상세: `GIJO_AS_하이브리드LLM_비용최적화_설계서.md`.
- ★ **구현자 모델 = Sonnet 시험 중**(2026-09-11 사장님 「그렇게」). 설계관·검토관·수리는 Opus 유지, **구현자·작성자(문서·도구 스크립트)만 `model:"sonnet"`** — 단 설계관 지시서가 파일:줄까지 특정한 작업에서만. 게시 전 검토·보안 경계 코드(gateway·auth·egress)는 Opus 그대로. **판정**: 2라운드(EPSS 숫자 지어냄·승인 라우팅) 동안 검토관 적발 수·상 등급 건수를 09-10 기준(Opus 구현: 13~21건·상 0~3)과 비교, 상이 늘거나 수리가 두 바퀴를 넘으면 되돌린다. 근거: 09-10 실측에서 실결함은 설계관·검토관이 잡았고 구현자는 지시서대로 손을 놀리는 단계였다. gb10 큰 모델을 구현자로 쓰는 것은 **보류·백로그**(틀 없음·09-10 선별 후보 8건 중 실재 경로 아닌 것 3·오탐 5·낮 GPU는 원격 팀원 서빙) — 밤에 정형 작업 하나로 소규모 시험 뒤 판단. 10라운드 누계 상급 0/2/0/0/0/2/0/0/3/4 — 상급은 전부 설계관 지시서 공백(라우팅 라운드)이라 `.claude/agents/gijo-architect.md`의 라우팅 지시서 점검표를 착수 조건으로 둔다(2026-09-12).
...
## 3머신 개발환경 (Windows ↔ M1 Max ↔ GB10) — GIJO_AS_3머신_개발환경_가이드.md
