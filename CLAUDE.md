...
- **게시 전 검토 관문**(2026-08-14 사용자 동의) — 큰 기능 묶음은 「구현 → 검토관 → 반영 → **그 다음** 게시」 순서를 고정한다. **게시가 검토를 앞지르지 않는다.** 보안 경계 코드(무인증 창구·인증·egress)는 구현과 검토를 병행.
  근거(같은 날 실측): 검토 3회가 전부 실결함을 찾았다 — 「고쳤다」가 거짓 2건, 고치다 만든 새 구멍 2건. 게시 후에 3차 검토가 와서 **게시본에 결함이 실렸고 재게시가 필요해졌다.** 시험 3,400개·소스 감시로 못 잡는 부류(비동기 경합·반쪽 수정·약속-코드 불일치)는 검토관만 잡는다.
