...
3. **게시 전 실화면 검증**: 게시 명령이 **UI 실화면 관문을 자동으로 태운다**(2026-08-20 도입,
   ...
   **앱이 떠 있음**: 사람이 쓰는 앱은 관문이 절대 닫지 않으니 닫고 다시).
   ...
   - ★ **2026-09-07 검사 4개 추가**(⑧⁗ 💬 답 지적 — 두 번째 원장). 게시 전에 이 넷이 초록이어야 한다:
     ① 대화창 꼬리 — **펼친 채 마우스를 올려도** 「▶ 이 답 이상해요」가 안 되살아난다(CSS 특이도
        수리) + **무른 뒤 다시 올리기가 진짜로 눌린다**(굳은 단추 — disabled를 읽고 끝내지 않고
        실제로 클릭해 본다). ⚠ 이 절은 부품에 **우리 창구(ops)** 를 넘겨 돌린다 —
        **지적을 서버로 보내지 않는다**(관문이 만든 줄이 결재판에 쌓이면 그 숫자를 못 믿는다).
     ② 결재판 **💬 답 지적** 세그먼트 — admin 목록이 뜨거나 「0건」이라고 **말하고**(빈 상자 금지),
        갈래 4칸이 서고, 취약점 원장의 그림 띠·VEX가 내려간다.
     ③ 감독 **🔧 고칠 것** — 「🗔 결재판에서 처리」가 **누른 뒤에** 결재판(`approvals.html?fix=open`)
        탭을 연다(죽은 단추 감시). ⚠ **누르기 전·후를 견준다** — 앞의 ②가 열어 둔 결재판 탭이
        남아 있어도 통과하지 않는다(2026-09-07 검토관 [중]: 옛 판은 「approvals 탭이 있나」만 봐서
        단추가 완전히 죽어 있어도 초록이었다). 탭은 `gijoTabs.list()`·`activeScreen()`으로 읽는다 —
        옛 판이 쓰던 `#tabBar [data-page]`는 셸에 **그런 요소가 없어**(app.html의 탭은 `div.tab`)
        늘 빈 값이었고, 그 탓에 🔧 줄이 서면 **게시가 관문에 막히는** 상태였다([상] 수리).
     ④ 내 문서 🏢 회사 지식 — 「조각 없음(대장 N개)」 **칩 수 = ⚠ 배지 수**. **양방향 계약**이라
        재인입해서 0건이 되면 칩도 배지도 없어야 통과한다(관문이 「고친 것」을 결함이라 말하면 안 된다).
     - 판정식 자체는 `server/test/publishgateflag.test.ts`가 관문 코드를 **떼어** 가짜 DOM에서 돌린다
       (⑧′↔publishgatecite와 같은 짝). 실화면 호버·CSS는 관문이, 논리는 그 시험이 잰다.
     - ⚠ 그 시험은 **판정식만** 떼어 오므로, 그 식에 들어가는 값의 **생산자**(관문의 evaluate)가
       틀리면 못 본다 — 실제로 그렇게 한 번 놓쳤다. 그래서 같은 파일 ⑥이 생산자 쪽 **소스 계약**을
       따로 잰다(무엇으로 탭을 읽는가 · 셸이 그 API를 정말 내놓는가 · 전후를 견주는가).
