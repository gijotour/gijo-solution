...
최신 코드로 클라이언트를 빌드해 띄운다(개발 실행 — 설치본과 별개). **`win`·`max` 공통**(`gb10`은 화면 없는 서버 실측용이라 해당 없음).

1. 빌드: `cd <repo>/client && npm run build` (tsc + preload 번들. 실패하면 중단·보고)
2. 실행:
   - **`win`**: `Start-Process -FilePath "node_modules\electron\dist\electron.exe" -ArgumentList "--remote-debugging-port=9223","."`
     (검증 자동화를 위해 CDP 9223을 기본으로 켠다. npx 경유는 Start-Process에서 조용히 실패하는 이력 있음 — electron.exe 직접 실행)
   - **`max`**: `npx electron . --remote-debugging-port=9223 &` (또는 `node_modules/.bin/electron`)
...
4. 안내: 로그인 화면에서 서버 주소 선택 —
   - `win`: 운영 `localhost:4000` / `max` 서버 테스트는 `10.8.0.11:4000`
   - `max`: 자기 서버 `localhost:4000` / 운영 교차 테스트는 `10.8.0.1:4000`
