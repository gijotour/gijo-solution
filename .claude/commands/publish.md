# /publish — 클라이언트 빌드·게시 (Windows 전용)

Electron 클라이언트를 빌드해 운영 서버에 게시한다(자동 업데이트 배포). **Windows에서만 수행** — macOS에서 이 명령을 받으면 "게시는 Windows 담당"이라 안내하고 /handoff를 권할 것.
...
전제: 사용자가 게시를 지시했을 때만 실행("빌드 및 게시" 등). 서버 변경이 함께 있으면 /deploy를 먼저.
