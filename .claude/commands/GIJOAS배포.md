...
> ⛔ **03:00~03:05 KST에는 배포·라이브 검증을 하지 않는다.** 그 창에 야간 회귀(`tools/nightly-ops-sim.ps1`,
> 158문항)가 **claude-deploy 계정으로** 서버에 붙는다. 계정당 1세션이라 우리가 그때 로그인하면
> 회귀가 튕기고, 서버를 재시작하면 회귀가 통째로 깨진다. 실측(2026-09-06): 03:00:09 시작 →
> 03:03:42 완주(4분). **끝난 것을 확인하고 들어간다** — `.tmp-reports/ops-sim-nightly-<날짜>.log`의
> 마지막 줄에 「끝 …」이 찍혀 있으면 끝난 것이다.
