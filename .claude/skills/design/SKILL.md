---
name: design
description: GIJO AS 디자인 작업의 단일 진입점. "디자인해줘", "화면 만들어줘", "UI 개선", "이 화면 갈아엎어줘", "레이아웃", "스타일 통일", "애니메이션 넣어줘", "모션 다듬어줘", "제품 업그레이드" 같은 요청에 사용한다. 추천 시안 1개를 먼저 만들어 승인받는 프로토콜을 강제하고, 알맞은 전문 디자인 스킬로 연결하며, 이 제품의 제약(Electron + 순수 HTML/CSS/JS, gijo-ui.css 디자인 시스템, 다크 팔레트 고정, 온프렘 오프라인)을 함께 적용한다. design, UI, layout, style, animation, motion 관련 요청의 첫 관문.
---
...
1. **추천 시안 1개 프로토콜을 강제한다** (아래 1장 — 가장 중요)
2. 요청에 맞는 전문 스킬을 고른다 (2장)
3. 이 제품의 제약을 그 위에 덮어씌운다 (3장)

## 1. 절대 규칙 — UI 변경은 **추천 시안 1개** 먼저

`CLAUDE.md`의 확립 원칙이다. **곧바로 구현하지 않는다.**

```
요청 → 추천 시안 1개(자체완결 HTML) → 사용자 승인 → 그 다음에 구현
```

> **왜 1개인가**(2026-08-09 사용자 결정 — 그전에는 3종이었다):
> 3종을 만들면 고르는 데 시간이 들고, 실무에서는 결국 추천안으로 가는 일이 많았다.
> **판단은 우리가 해서 하나를 내놓고, 사용자는 승인/반려만** 하는 편이 빨랐다.
> 대신 **왜 이 안인지**를 반드시 밝힌다 — 근거 없이 하나만 내밀면 고를 여지를 뺏는 것이다.

- 시안은 `mockups/<화면명>/` 아래 별도 폴더에 만든다. 기존 125개 폴더가 선례다.
- 파일명은 `시안.html`(단일안 기본형). 여러 안이 필요하면 뜻이 드러나게: `v1-popover.html` 등.
- **자체완결 단일 HTML**이다. `<style>`을 인라인으로 넣고 외부 파일을 링크하지 않는다.
- 시안에도 아래 3장의 팔레트·토큰을 그대로 쓴다. 목업이라고 색을 새로 만들지 않는다.
- **밀도를 숫자로 적는다** — 줄 높이·세로 총합·기존 대비 증감(사용자 확립 요구).
  "깔끔해졌습니다"가 아니라 "274px → 387px(+113)"로 말한다.

시안에 반드시 붙일 것:
1. **무엇을 노렸고 무엇을 포기했는지** 한 줄
2. **왜 이 안인지** — 다른 길을 왜 안 골랐는지 한 줄
3. **밀도 숫자**
4. 제품 원칙을 지켰는지 자가 점검(§3 — 특히 "화면에 자유 입력칸 없음")

**사용자가 다른 방향을 원하면 그때 대안을 만든다.** 처음부터 3개를 만들지 않는다.
결정을 받기 전에 `client/` 코드를 건드리지 않는다.

## 2. 라우팅 표

| 사용자가 원하는 것 | 호출할 스킬 |
|---|---|
| 화면 신규·개편 (레이아웃·구조) | 1장 프로토콜(추천 1개) + `emil-design-eng` |
...
**`prototype` 스킬은 쓰지 않는다.** 1장의 추천 시안 1개 프로토콜이 같은 일을 하고,
이쪽이 이 저장소의 확립 규칙이다.
...
**배색은 에디션 이름표다** (2026-08-20 사장님 확정 — 옛 「다크 전용」 규칙을 대체):
- **표준 = 다크**(원본 — 화면 `:root`와 gijo-ui.css의 기본값은 언제나 다크다).
- **프로 = 흰 바탕** — `pro-white.css`(html.theme-light 스코프)가 덮는다. 신호는 주소(`theme=light`)로만 온다.
