...
function CTXShort() {
  return `[제품] GIJO AS — 온프렘 AI 보안관리 플랫폼. Electron 클라이언트(client/) + Node/TS 서버(server/). 저장소 ${ROOT}, 읽기 전용(파일 수정 금지, ${RUN_NOTE}). 클라 5.14.0, 오늘 2026-09-01. app.html = 탭 셸(각 화면 ?embed=1 iframe), nav.js 사이드바, titlebar.js 상단·세션·업데이트 배지, gijo-ui.css 토큰. 제품 원칙: 한글 텍스트 / 화면엔 정체성 한 줄+⚠만(설명은 챗봇 screenguide+ⓘ) / 정직한 구현 / 오프라인 / 다크 팔레트 / 지시는 대화창·변경은 결재판 / 죽은 조작·막막한 빈 화면·날것 오류 금지. 코드 주석의 「사용자 결정/지시(날짜)」는 의도된 설계다.
`
}
