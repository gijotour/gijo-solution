...
```
어느 머신이든:  /sync → 개발 → /handoff
운영 반영:     (Windows) /deploy → 클라 변경 있으면 /publish
```

| 명령 | 용도 | 핵심 동작 |
|---|---|---|
| `/sync` | 작업 시작 전 | hub 최신 받기(ff-only). 갈라졌으면 rebase 안내. 서버/클라 변경 감지 시 후속 조치 알림 |
| `/handoff` | 작업 마무리 | 커밋 → (서버 변경 시) `npm test` → hub push → **상대 머신 인계 블록 출력** |
| `/deploy` | 운영 배포 | Windows: 테스트 게이트(914개, 실패=중단)→WSL rsync·tsc→MainPID kill→health. Mac: ssh 한 줄 안내/인계 |
| `/publish` | 클라 게시 | Windows 전용: 버전 bump→`npm run dist`→**실화면 검증 필수**→claude-deploy 게시→server-dist 원복→bump 커밋 |
...
- 예방: 작업 시작 전 `/sync` 습관 + 가능하면 한 번에 한 머신 + 영역 분담(파일 안 겹치게).
...
| `.claude/commands/*.md` | /sync·/handoff·/deploy·/publish 동작 정의 |
