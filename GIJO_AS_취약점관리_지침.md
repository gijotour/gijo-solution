...
## 부록: 구현 현황 (2026-07-16 기준)

| 지침 항목 | GIJO AS 상태 |
|---|---|
| 플러그인 단위 집계 | ✅ 구현 (Plugin ID 병합, CVE/포트 수집) |
| EPSS/VPR 저장·표시 | ✅ 구현 (배지 + "악용확률 순" 정렬 + "등급↓ 악용↑" 경고) |
| 호스트 정보(OS/DNS) | ✅ 구현 (Nessus HTML 임포트 → 자산 이름·구성요소) |
| 호스트 → 취약점 드릴다운 | ✅ 구현 (심각도 필터·검색·상세) |
| SLA·담당자 붙은 조치 | 🔶 부분 (유지보수/승인 워크플로우 존재, 취약점 직결은 미완) |
| New/Fixed/Resurfaced 상태 추적 | ⬜ 미구현 (재스캔 비교 필요) |
| KEV 등재 여부 | ⬜ 미구현 (지표 추가 여지) |
| 조치 진행률/번다운 측정 | ⬜ 미구현 (KPI 연결 여지) |
