...
const dir = path.resolve("mockups", "analysis-workspace");
const b = await chromium.launch({ channel: "msedge", headless: true }).catch(() => chromium.launch({ headless: true }));
for (const [f, n] of [["design-A-feed","A"],["design-B-lanes","B"],["design-C-soc","C"]]) {
  const p = await b.newPage({ viewport: { width: 1240, height: 940 }, deviceScaleFactor: 2 });
