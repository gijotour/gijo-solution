...
(function () {
  ...
  var TOP = [
    ...
    // (「나만의 연락처」 숨김 항목은 2026-08-31 5그룹 재편으로 **📓 내 문서 그룹의 보이는
    //  항목**이 됐다 — 같은 주소·같은 이름이라 숨김을 남기면 🔍 화면찾기에 두 번 나온다.)
  ...
  var GROUPS = [
    // ══ 5대 메뉴 (2026-08-31 사장님 — 실화면 캡처 3장 「안티그래비티처럼」 + 「모든 메뉴를
    //    크게 5가지로 통합 및 그룹핑」, 시안 mockups/menu5-antigravity) ═══════════════════
    //
    // 왜 바꿨나: 절차 5단계 재편(2026-08-02) 이후 그룹이 12개까지 불었다(지식·자산·절차5·
    //   보안제품·공급망·AI·추가 기능·설정). 사장님이 업무 시나리오 축으로 다시 묶으셨다:
    //   내 문서=모든 데이터의 기준 → 내 지식에서 대화로 확인(+법령) → 취약점 업무·
    //   보안제품 관리가 그 자산으로 일하고 보고 → 설정은 나머지 전부.
    // ⚠ 전부 **있는 화면의 자리 이동**이다(새 화면 0개) — 파일·허브 구조는 그대로다.
    // ⚠ 「대표 그룹」(항목 1개=그룹 줄이 곧 메뉴, 2026-08-09 사용자 지시)은 이 재편으로
    //   **소멸**했다 — 5그룹 전부 다항목이라 아래 렌더러의 `var 대표` 분기가 더는 안 탄다.
    //   코드는 남긴다(그룹이 1항목으로 줄면 다시 사는 것이 규약이고, 죽은 날을 여기 적어
    //   두면 다음 사람이 「왜 안 타지」로 안 헤맨다). qa-auto의 대표 그룹 검사도 같이 걷었다.
    // ⚠ 절차 단계 id(assets0·s1-find…s5-report)는 그룹에서 **항목 한 줄로 내려갔다** —
    //   workflow.test(사이드바=서버 표)·assetscope.test·console.js 홈 히어로 칩이 이 id
    //   문자열을 본다. 그 항목 줄은 page·label·id를 **한 줄**에 다 적을 것(workflow.test가
    //   id가 있는 그 줄만 읽는다). 필드 순서는 page → label → id — howto.test·screenguide.test가
    //   「page 바로 뒤 label」 무늬로 이름을 대조하므로 id가 둘 사이에 끼면 안 된다.
    //   ⚠ 이 주석에조차 그 id 무늬를 글자 그대로 적지 않는다 — workflow.test의 indexOf가
    //     주석을 먼저 물면 파서가 빈손이 된다(2026-08-31 실측 — 주석의 예시가 파서를 깼다).
    // ⚠ 그룹 라벨 맨 앞의 원문자(①~⑤)는 렌더러가 숫자칩으로 바꿔 그린다(아래 렌더 주석).

    // ① 🧠 내 지식 — 학습한 데이터를 대화로 질문·확인 + 법령(2026-08-28 「내 지식 강조」 계보).
    ...
    // ② 📓 내 문서 — 「모든 데이터 업로드 확인 + 위키처럼 디렉토리 지정 상시 확인 + 업무넘기기
    //   포함」(사장님 원문). 「내 문서」 홈(mydocs.html)은 TOP 고정에 있다 — 여기 정확히 같은
    //   주소를 또 넣으면 문서 배지가 두 곳에 뜬다(makeItem의 `it.page === "mydocs.html"` 정확
    //   일치 — 그 주석이 이미 경고). 그래서 하위 갈래만 둔다.
    //   (조치요청서는 **진입 주소가 아직 없어** 항목을 안 만든다 — 가짜 UI 금지.
    ...
    { id: "mydocs", icon: "book", label: "② 📓 내 문서", items: [
      ...
      // 📂 지켜보는 폴더(2026-08-31 2단계) — 서버가 주기 확인해 자동 반입하는 폴더 판.
      //   지정·해제는 대화창(admin 결재판)만 — 판은 보기 전용(「메뉴는 보기용」).
      { page: "mydocs.html?tab=watch", label: "지켜보는 폴더" },
      // 📨 조치 요청서(2026-08-31) — 대화창에서 만든 요청서를 되찾는 자리(보기 전용).
      //   menu5 재편 때 「진입 주소가 없어」 뺐던 항목이 이 라운드에서 생겼다.
      { page: "mydocs.html?tab=req", label: "조치 요청서" },
    ]},
    // ③ 🩹 취약점 업무 — 「취약점 분석 파일·보안로그 분석 → 담당자 지정·관리·보고」(사장님
    //   원문). ⓪ 자산(범위 축)과 절차 ①~⑤ 허브 전부 + 보안 로그 파일 분석(옛 추가 기능).
    //   ⚠ ⓪의 id를 `s0-…`로 짓지 않는다(assetscope.test — 절차가 아니라 범위 축이라는 것을
    //     이름에서 드러낸다. 원 설계 주석 2026-08-18의 계보).
    { id: "vulnwork", icon: "target", label: "③ 🩹 취약점 업무", items: [
      { page: "assets.html", label: "⓪ 자산 고르기", id: "assets0" },
      { page: "discover.html", label: "① 발견·수집", id: "s1-find" },
    ...
    // ⑤ ⚙ 설정 — 「AI팀 등부터 위에 메뉴 없는 나머지 모두」(사장님 원문).
    //   설정 항목의 주소 문자열(settings.html?s=my)은 clientglobals.test의 배지 부착점이다 —
    //   바꾸면 새 버전 배지가 어디에도 못 붙는다.
    { id: "settings", icon: "slider", label: "⑤ ⚙ 설정", items: [
      { page: "settings.html?s=my", label: "설정" },
      // AI 허브 — 팀·지식·학습·안전장치 통합(2026-08-20 「AI팀 메뉴에 안전장치 통합」 유지).
      { page: "aihub.html", label: "AI" },
      // 기록 허브 — 누가 무엇을 했나(감사)와 서버가 무엇을 했나(진단)를 한 자리에서.
  ...
  // TOP(고정 4자리)도 내준다(2026-08-31) — 셸 화면이름찾기가 mydocs.html·sessions.html처럼
  // **TOP에만 정확 일치가 있는 주소**의 이름을 찾을 때 쓴다(그룹만 뒤지면 파일명 일치로
  // 🧠의 ?tab= 변형에 걸려 「GIJO AS 안내」 같은 엉뚱한 이름이 탭에 실린다). 읽기 전용.
  try { window.gijoNavTop = TOP; } catch (e) {}
  ...
  function renderBody(container) {
    ...
    GROUPS.forEach(function (g) {
      ...
      // ⚠ 2026-08-31 5그룹 재편으로 1항목 그룹이 0개가 되어 이 분기는 지금 **안 탄다** —
      //   지우지 않는 이유는 규약이 살아 있어서다(그룹이 1항목으로 줄면 다시 대표가 된다).
