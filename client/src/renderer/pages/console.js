...
(function () {
  ...
  function injectCss() {
    ...
    st.textContent = [
      ...
      ".cs-updec button{background:var(--panel,#30302e);border:1px solid var(--border-strong,rgba(255,255,255,.16));color:var(--text-strong,#dfe6ff);",
  ...
  function uploadResultMsg(r) {
    ...
    // 📦 제품 소개자료(2026-08-23) — **지식 저장소로 안 간다**(사장님 결정: 「깊은 학습은 아니다」).
    //   그래서 SBOM과 같이 `r.memory`가 없다 — 이 갈래를 안 적으면 아래 마지막 줄에서
    //   「상세 정보 없음」으로 떨어져 **대장에 등록됐다는 사실을 한 마디도 안 한다.**
    //   (SBOM 때 정확히 그 실수를 했고 그 사고가 바로 위 주석에 적혀 있다.)
    if (r.routedTo === "productintro") {
      var pi = r.productIntro;
      if (!pi) return "제품 소개자료로 등록하지 못했습니다 — " + r.reason;
      return "📦 제품 소개자료 등록 — **" + pi.name + "**" +
        (pi.category ? " · 분류 " + pi.category : "") +
        "\n원본을 보관했습니다(비교의 근거). **지식 저장소에는 넣지 않습니다** — 벤더 홍보 문구가 답변 근거로 섞이지 않게 합니다.";
    }
  ...
  function 보관안내(r, keep) {
    ...
    //   파일은 저장됐어도 임베딩이 죽어 수집을 못 하면 문서 목록에 안 뜨고 열면 404다 —
    //   문서 메타는 수집 맨 끝에 기록되기 때문이다. 파일 저장만 보고 안내하면 없는 화면을 가리킨다.
    if (보관됨 && !r.ingested) {
      return "\n⚠ 파일은 보관했지만 **검색에는 아직 안 들어갔습니다**(임베딩 미기동)." +
        "\n   지금은 「내 문서」 목록에 안 보이고 물어봐도 못 찾습니다 — 임베딩이 올라온 뒤 다시 올리면 들어갑니다.";
    }
    ...
    // ⚠ 여기서 원인을 **지어내지 않는다**(검토관 확정). 예전엔 「취약점으로 바로 반영되는
    //   갈래라」라고 단정했는데, 같은 자리에 추출 실패·빈 문서·보관 실패도 함께 떨어진다 —
    //   틀린 사유를 확신에 차서 말하는 것이 아무 말 안 하는 것보다 나쁘다. 아는 것만 말한다.
    if (keep) {
      return r.routedTo === "vulnscan"
        ? "\n⚠ 이 파일은 원본 보관 대상이 아닙니다 — 취약점으로 바로 반영되는 갈래라 문서로는 저장하지 않습니다."
        : "\n⚠ 원본·추출본이 남지 않았습니다 — 문서에서 글자를 뽑지 못했거나 보관에 실패했습니다. 서버 기록을 확인하세요.";
    }
    return "";
  }
