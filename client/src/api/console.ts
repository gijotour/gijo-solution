...
export interface WorkSessionTurn {
  ...
  at: number;
  /**
   * 「근거 없음」 표식 — 이 답에 ⚠ 배너가 붙어 있으면 그 종류(2026-09-05).
   * 이어보기로 되살린 답도 **방금 받았을 때와 똑같이** 숫자가 옅게 보이도록 서버가 읽어 준다.
   * ⚠ 저장된 값이 아니라 **읽을 때 판정한** 값이다(판정기: server/src/engine/noevidence.ts).
   */
  // ⚠ **서버 근거없음종류(noevidence.ts)와 값이 같아야 한다** — 「숫자무근거」가 빠져 있던 동안
  //   그 갈래로 실어 보낸 noev는 타입상 존재하지 않아 「고칠 것」 자료 부족 칸에서 조용히 샜다
  //   (2026-09-07 검토관). 서버의 Record 컴파일 가드는 클라까지 못 미치므로 짝 시험이 지킨다:
  //   server/test/answerflagui.test.ts ④.
  근거없음?: "자료없음" | "자료요청" | "지정범위" | "근거약함" | "숫자무근거";
}
