...
// ⚠ `붙은적있나`가 핵심이다(2026-09-03 게시 전 검토). 이 모듈은 **프레임마다 따로** 뜨는데
//   최상위 셸(app.html)은 소켓을 여는 곳이 아니다 — 실시간을 구독하는 화면들이 자기 프레임에서 연다.
//   그래서 셸에서는 「연결 안 됨」이 **정상**이다. 그것을 끊김으로 알리면 서버가 멀쩡한데도
//   빨간 경보가 상시로 떠서, 정작 진짜 끊겼을 때 아무도 그 표시를 안 믿는다(가짜 UI).
type 연결상태 = { 연결됨: boolean; 끊긴시각: number | null; 붙은적있나: boolean };
let 상태: 연결상태 = { 연결됨: false, 끊긴시각: null, 붙은적있나: false };
...
function 상태알림(연결됨: boolean): void {
  ...
  if (연결됨) 상태 = { 연결됨: true, 끊긴시각: null, 붙은적있나: true };
  // ⚠ 끊긴 시각은 **한 번이라도 붙은 뒤**에만 찍는다 — 안 그러면 「아직 안 붙음」이 끊김이 된다.
  else 상태 = { 연결됨: false, 끊긴시각: 상태.붙은적있나 ? (상태.끊긴시각 ?? Date.now()) : null, 붙은적있나: 상태.붙은적있나 };
