...
# 최신 버전을 확인하고, 있으면 설치파일을 내려받아 그대로 실행한다(현재 NSIS 빌드는
# oneClick이라 조용히 설치되고 앱이 자동 재기동된다).
#
# 사용:
...
$check = Invoke-RestMethod -Uri "$ServerUrl/api/client/latest-release?current=$currentVersion" -Headers $headers
...
Write-Host "설치가 진행됩니다 — oneClick 빌드라 자동으로 조용히 설치되고 앱이 재기동됩니다."
