# GIJO AS — 제품 소개 자료 (v2.0.0)

> **한 명의 보안담당자가 여러 명 몫을 해내도록.**
> 쏟아지는 보안 정보를 AI 에이전트가 **자동으로 엮어** 처리하는 온프레미스 보안 운영 플랫폼.

*본 자료의 모든 수치·예시·**스크린샷**은 v2.0.0을 실제 구동해 뽑은 **실측 백데이터**입니다(서버 자동화 테스트 278개 통과).*
...
| **품질** | 서버 자동화 테스트 **278개 통과**, 41개 엔진 모듈, 실기동 엔드투엔드 검증 |
...
모든 화면의 사이드바는 v2.0.0에서 **업무 흐름 기준 6개 섹션**(모니터링·자산/취약점·보안운영·보고/컴플라이언스·AI팀/학습·시스템)으로 재편됐습니다.
...
> 재생성: 서버를 `GIJO_SERVER_PORT=4068`로 기동 후 `node tools/gen-screenshots.mjs` (Windows Edge 사용).
> 실제 운영 환경 캡처가 필요하면 GIJO AS 데스크톱 앱을 로그인해 각 메뉴에서 촬영하면 됩니다.

---

*문서 버전: v2.0.0 · 데이터 기준일: 2026-07-17 · 수치·스크린샷 모두 실제 서버 구동 결과.*
