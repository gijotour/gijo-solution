...
### ③ 쓸수록 똑똑해진다 — 어떻게, 그리고 무엇은 아닌가 *(2026-09 가이드라인)*
쓸수록 똑똑해지는 것은 **모델이 아니라 사내 기억과 잣대**입니다. 담당자가 쓰는 만큼 다섯 갈래로 늡니다.
1. **문서를 올리면 근거가 됩니다.** 반입한 문서가 답의 근거로 쓰이고, 조각이 사라진 문서는 문서함이 「조각 없음」으로 알려 다시 넣게 합니다.
2. **답을 승인하면 사내 기억이 됩니다.** 승인 문답이 검색 대상에 쌓입니다(수천 건). 다만 사내 문답이 법·기준 문서를 밀어내지 않도록 검색 자리의 몫을 나눕니다.
3. **틀린 답은 지적으로 줄어듭니다.** 답 꼬리의 「이 답 이상해요」가 결재판의 답 지적 원장으로 흐르고, 담당자가 자료 부족·사내 규정·제품 갈래로 닫습니다. 고친 답은 회귀 검사 문항 후보가 되어 같은 실수를 다시 잽니다. 내부 LLM은 누를 때만 수정 초안을 쓰고 갈래를 정하지 않습니다.
4. **오탐·위험 수용 판단은 기록으로 남습니다.** 지금은 사람이 보는 기록이며, AI가 이 기록을 읽어 답에 쓰는 자리는 아직 없습니다.
5. **모델 가중치(어댑터) 학습은 평가 게이트를 통과할 때만** 합니다. 2026년 9월 4회전 시험에서 전부 불채택이라 지금은 보류 상태이고, 그 전까지 「똑똑해진다」는 위 1~4를 뜻합니다.

**무엇은 안 똑똑해지나** — 모델의 일반 지식과 말투는 바뀌지 않습니다. 행동을 고칠 때 프롬프트에 규칙을 더하지 않고 코드로 막습니다(지어낸 인용·수치·자산·의무를 출구에서 걷어냄).
**담당자가 할 일 셋** — 문서 올리기 · 답 승인 또는 지적 · 오탐·위험 수용 표시. **확인하는 곳** — AI 지식 화면, 학습 루프 화면, 감독 화면의 「고칠 것」, 야간 회귀 결과.
...
| GIJO | 용어 해설 · 침해사고 히스토리(등록된 사고 사례를 쉬운 말로 부연 — 사내 지식 근거) | 기반 모델 |
...
> **예시(시연용 표본) — 실제 현황이 아닙니다.** 지시 예시: *"CTI 영향 자산 스캔하고 리포트까지"*
...
**예시(시연용 표본) — 실제 현황이 아닙니다.** 수집 위협 5건 중 4건이 자산에 매칭, 영향 자산 3곳인 화면 예시입니다:
...
**예시(시연용 표본) — 실제 현황이 아닙니다.** 서비스 3곳 모두 CTI 위협으로 "영향도 높음"인 화면 예시입니다:
...
> **예시(시연용 표본) — 실제 현황이 아닙니다.** 샘플 취약 호스트 1대: **Log4Shell**(Critical · CISA KEV 실제 악용) · OpenSSH(High) · Oracle 패치누락(Medium · 악용확률↑) · SSL(**Fixed**로 검증) — 심각도·KEV·상태를 한눈에 보여 주는 화면 예시입니다.

실 Nessus 리포트에서 **1,171행을 282개 플러그인으로 중복 제거**하는 것은 위 표찰과 달리 **시연 표본이 아니라 실데이터로 확인한 결과**입니다.
...
> **예시(시연용 표본) — 실제 현황이 아닙니다.** 4종 제품(방화벽·EDR·DLP·WAF) + 제품/로그 매뉴얼을 종류별 그룹으로 관리하는 화면 예시입니다.
...
**예시(시연용 표본) — 실제 현황이 아닙니다.** 상태 6가지를 보여 주는 화면 예시입니다:
...
- **전문가 어댑터(LoRA):** 「베이스 1개 + 어댑터 N개」 구조. 주제(취약점·장비운영·사내규정·위협대응)별로
  승인된 문답이 **300건**을 넘으면 그 주제 어댑터를 학습합니다. 학습해도 평가 게이트를 통과해야 채택되며, 2026년 9월 4회전 시험은 전부 불채택이라 지금은 보류입니다(3-③ 가이드라인).
...
**예시(시연용 표본) — 실제 현황이 아닙니다.** 통합 스냅샷 화면 예시입니다:
...
| 외부 연결 | 인터넷으로 나갈 수 있는 통로는 **전부 목록으로 관리**합니다 — 위협 인텔(CTI) 피드 · CISA KEV 갱신 · 법령 조회 · 모델 검색·다운로드 · 외부 저장소 스캔 · URL 지식화(웹·유튜브 자막) · 메일(SMTP) · SIEM 전달 · 원격 장비 점검(SSH) · 외부 AI 대상 점검 · 학습 도구(python·모델 배포처 CLI) · **선택적 클라우드 LLM**(기본 OFF, 관리자만 켤 수 있고 사내 식별자가 섞이면 자동 차단). 필요 없는 것은 끄고, 지금 무엇이 열려 있는지는 화면에서 그대로 확인합니다 |
| GPU 조율 | 학습 시 추론 엔진 자동 정지→재기동(GPU 1대를 나눠 쓰다 생기는 메모리 충돌 방지) |
...
- **전문가 어댑터(LoRA):** 기반 모델 하나에 주제별 어댑터를 갈아 끼워 역할을 나눔 — 평가 게이트·A/B 통과 후에만 채택 (두 모델을 섞던 「LLM 합성」은 2026-09-07에 제품에서 없앴습니다 — 전문화는 어댑터가 맡습니다)
...
**예시(시연용 표본) — 실제 현황이 아닙니다.** 화면 구성 확인용이며, 표에 적힌 수치는 촬영 당시 그 서버에 들어 있던 **시연용 표본 데이터**입니다 — 설치본을 처음 켰을 때 보이는 값이 아닙니다.
...
> 이 화면들은 **시연용 표본 데이터가 들어 있는 서버**에서 찍은 것입니다. **우리 환경의 실제 화면**이 필요하면
> GIJO AS 앱에 로그인해 각 메뉴에서 직접 촬영하시면 됩니다.

---

*예시(시연용 표본) — 실제 현황이 아닙니다. 데이터 기준일: 2026-08-10 · 촬영 당시 그 서버에 들어 있던 시연용 표본 데이터에서 뽑은 값입니다.*
