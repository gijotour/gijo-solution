...
- 2026-09-12 **공용 파일 screenguide.ts·dispatcher.ts·routes.ts**(B12 화면 안내 「X 뭐야?」 도달 — 설계관 지시서·Sonnet 구현) — 「이 답 이상해요 뭐야?」는 대화 홈에서도 닿는데 「근거 지정 뭐야?」는 그 화면(mydocs.html) 위에서만 닿던 비대칭을 닫았다.
  · `screenguide.ts`: `resolvePanelHit`에 **셋째 훑기**를 열었다 — 명시 allowlist `대화창구역들()`(초기 5줄, 전부 mydocs.html: 「근거 지정과 첨부(📎)」·「근거 지정」·「지난 작업 첨부」·「AI 포함과 공유의 차이」·「격리 원리」)만 대화 홈에서도 닿는다. 전역 순회(모든 화면 구역을 다 여는 것)는 실측으로 189문장 코퍼스 중 7건을 뒤집어(incident_sources·aibom_status·[33] 침해사고초동절차 등을 빼앗는다) **하지 않았다** — allowlist는 같은 코퍼스에서 변동 0건. 전역 히트는 강제 도구가 있으면 **정체물음이어도 무조건 물러선다**(N11「근거 지정 규정 뭐야?」→explain을 지키는 자리). `isHelpIntent`·`formatScreenGuide`에 `lite` 4번째 인자를 받아 라이트에서는 이 훑기를 끈다(라이트엔 mydocs.html이 없다).
  · **오늘 살아 있던 별개 결함**을 같은 층에서 닫았다: 화면 안내(dispatcher [22])가 **쓰기 흐름**(「…알려주고 승인/배정/할당/맡겨」)을 삼켜, 「현황판 알려주고 배정해줘」류가 안내 한 수로 끝나고 승인·배정이 영영 안 갔다. `writeflow.쓰기흐름인가()`를 `isHelpIntent` **맨 앞**에 걸었다(설계관 지시서는 hit 분기 안쪽을 지시했으나, 그러면 HELP_RE의 「가이드\s*(줘|알려|보여)」 가지가 먼저 채 가는 문장 — 「시작 가이드 알려주고 승인해줘」·「활용 가이드 알려주고 배정해줘」 — 을 못 막는다는 것이 전수 시험으로 드러나 맨 앞으로 옮겼다. 편차는 실측 근거와 함께 보고에 적었다).
  · `dispatcher.ts`: 화면 안내 실호출(1782 근처)·route-explain 진단 호출(체인훑기)에 넷째 인자(에디션)를 함께 넘긴다 — 실경로와 route-explain이 같은 눈으로 재게 하려는 것. `routes.ts` 차례 22의 「왜」·「감시」 문구를 그에 맞춰 갱신(자리 번호는 안 밀림 — 새 특수경로를 안 만들었다).
  · 새 감시: `panelname.test`·`aliaspair.test`에 전역표 전수(어느 화면·대화 홈에서도 같은 답 · 쓰기 흐름 안 삼킴 · 알려줘↔보여줘 대칭 · 값이 실재 구역을 가리킴) · `writeflow-guard-source.test`에 screenguide 소스 감시(네 번째 이관) · `screenguidefix.test`에 라이트 격리·본문 검증 · `compare-concept-routing.test`에 B7이 「닫았다」고 적었지만 실은 안 닿았던 「AI 포함과 공유의 차이」 도달 확인.
  · **결정 대기로 남긴 것**(design not_doing) — 강제 도구가 있는 구역 이름(AI-BOM·견고성 점수 등) 16개, 홑낱말 「첨부」·「문서 지정」, 「2차 인증」, supervision/approvals 계열(「인용 제거」·「고칠 것」·「답 지적」·「수정 초안」)은 이번 allowlist에 **안 넣었다** — 넣으면 panelname 대장이 16×38로 폭발하거나(강제 도구 충돌) 데이터 물음을 삼킨다(실측: 「첨부」 10문장 중 6 오삼킴). 다음 라운드 후보.
  · max 실기 확인 사항: **없음**(서버 라우팅 로직 — 렌더러 화면 변경 없음).
- 2026-09-12 **B12 수리(검토관 12건 전수 실결함)** — 바로 위 B12 항목의 **세 문장을 정정한다**(공용 파일 `screenguide.ts`·`writeflow.ts`·`dispatcher.ts`).
  · ❌ 「allowlist는 같은 코퍼스에서 변동 0건」 → **실제로는 9문장이 새고 있었다.** 착수 전 코퍼스 189문장에 「격리」의 보안 일반 의미가 **한 문장도 없어서** 안 보였을 뿐이다. 전역표의 「격리 원리」가 「망 격리 원리 뭐야?」·「네트워크 격리 원리가 뭐야?」·「망분리 격리 원리 설명해줘」·「샌드박스/컨테이너/VM 격리 원리」·「감염 PC 격리 원리 뭐야?」·「랜섬웨어 감염 장비 격리 원리 알려줘」 8문장을, 「근거 지정」이 「개인정보 처리 **법적 근거 지정** 어떻게 해?」 1문장을 [22] 화면 안내로 채 가서 「격리는 검색 쿼리 단계의 하드 필터」라는 **근거 0건짜리 확신 있는 오답**을 냈다. ⇒ **「격리 원리」를 전역표에서 뺐고**(일반 보안 용어라 앞말 배제어로는 못 막는다 — mydocs.html 화면 위에서는 그대로 닿는다), 「근거 지정」에는 법 낱말 앞말 배제(적법·처리·수집·이용·제공·동의·위탁)를 달았다.
  · ❌ 「`writeflow.쓰기흐름인가()`를 `isHelpIntent` 맨 앞에 걸었다」 → 그 함수의 앞가지(isAssign)는 **문장 끝 고정이 아니라** 「배정」 낱말만 있으면 참이라, 시키는 말이 아닌 **사용법 물음**까지 안내를 잃었다(실측 대조쌍: 「배정 도움말」⑨ ↔ 「승인 도움말」[22] · 「배정 사용법 알려줘」⑨ ↔ 「일괄 처리 사용법 알려줘」[22] · 「기한 설정 화면 사용법 알려줘」⑨ ↔ 「알림 설정 화면 사용법 알려줘」[22] · 「담당자 지정 사용법 알려줘」⑨). ⇒ writeflow에 **뒷가지만 내보내는 `쓰기명령꼴인가()`**(문장 끝 명령꼴)를 추가하고 안내 층은 그것만 부른다. 낱말은 여전히 한 곳에만 적혀 있다(`쓰기흐름인가`가 이 함수를 부른다).
  · ❌ 「라이트엔 mydocs.html이 없다」 → **있다.** `client/src/renderer/pages/lite-mydocs.html`은 화면이 아니라 **이름표**라서 `location.replace`로 프로 mydocs.html로 넘긴다(lite-screens.json no.8 「프로의 「내 문서」를 **그대로** 쓴다 … 포크 금지」). 라이트에서 전역 훑기를 끄는 **진짜 근거**는 「라이트 대화창(lite-chat.html)에 ☑ 근거 지정·📎 첨부가 없다」(실측 grep 0건)이다 — 동작은 그대로 두고 근거만 바로잡았다(거짓 근거는 다음 사람이 쉽게 뒤집는다).
  · 함께 고친 것 — ① **값요구 꼬리도 전역에서는 진다**: 「근거 지정 목록/현황 알려줘」·「지난 작업 첨부 목록 알려줘」가 목록 대신 기능 설명문으로 갔다(전역표 넷은 강제 도구가 0이라 「값을 달라는 말은 도구로 간다」는 이 파일의 계약이 전역에서만 꺼져 있었다). 전역 히트는 이제 **사유(정체물음·안내낱말)가 있을 때만** 이긴다 — 덤으로 「알려줘 ↔ 보여줘」 대칭이 전역에서 성립한다. ② `panelname.test`의 새 대칭 시험이 `if (!fa || !fb) continue`로 **0회 돌던 빈 초록**이었다 — 강제 도구 조건을 빼고 실제로 재게 고치고, 「반복 횟수가 0이 아닌가」를 재는 줄을 함께 뒀다. ③ 전역표 죽은 줄 판정을 제품 함수 `죽은전역구역()`으로 내렸다(시험이 `getScreenGuide`로 재고 있었는데 그 함수는 모르는 열쇠에 OVERVIEW로 **폴백**해서, 제품이 영영 안 걸리는 줄을 시험은 초록으로 넘겼다).
  · 커밋 de7e84f9 메시지의 **「WSL 게이트: 시험 8097/실패 0」은 거짓**이다 — 같은 실행의 실제 값은 **실패 1**(examquestions.test.ts · 본 변경과 무관한 구운 시험지 드리프트, 아직 미해소)이다. 기준선을 그대로 물려받는 저장소라 「무관하다」와 「0이다」는 다르다.
  · max 실기 확인 사항: **없음**(서버 라우팅 로직 — 렌더러 화면 변경 없음).
