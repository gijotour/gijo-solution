...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  },
  {
    ...
    re: /^(?![\s\S]*(현황|보여\s*줘|알려\s*줘|목록|얼마나|어때|뭐야|뭔가|무엇|뭘|뭐를|어느|어떻게|어떤\s*걸|하나요|해야\s*(해|하나|되나)))[\s\S]*((AI-?BOM|에이아이\s*봄)[\s\S]{0,30}(기재|기입|적어|채워)|(기초\s*모델|아키텍처|파인튜닝\s*이력|데이터셋\s*출처|벡터\s*DB|시스템\s*프롬프트|가드레일|API\s*목록|MCP\s*서버|서빙\s*환경|호스팅\s*공급업체)[\s\S]{0,24}(기재|기입|적어|채워))/,
  ...
  },
  {
    ...
    //   ⚠⚠ **개념 물음을 먼저 뺀다**(2026-09-01 검토관 [중]). 첫 대안 `VEX`에 조건이 없어
    //     「VEX가 뭐야?」·「VEX 표준 설명해줘」까지 삼켜 **설명 대신 건수표**를 답했다.
    //     같은 파일의 sbom_review_status 규칙은 이미 이 관례를 지키고 있었는데 여기만 빠뜨렸다.
    //     화면에는 screenguide의 「VEX 내보내기」 설명 패널이 있는데 거기 못 닿았다.
    //   ⚠⚠ **배제를 문장 앞쪽으로 좁혔다**(2026-09-01 재검토 [중]). 처음엔 [\s\S]*로 문장
    //     **어디에 있든** 설명·어떻게·왜·방법·절차를 잡았는데, 그러면 「VEX 현황 설명해줘」·
    //     「VEX 대상이 왜 0건이야?」 같은 **평범한 조회까지 규칙이 죽어** 아무 도구에도 안
    //     닿았다 — 이 창구를 만든 이유가 바로 「받아 줄 곳이 없어 모델 판단으로 새던 자리」다.
    //     고치려다 그 자리를 되살린 셈이었다. 개념 물음은 **VEX 바로 뒤**에 물음말이 붙는
    //     꼴(「VEX가 뭐야」·「VEX 설명해줘」)이라 거기만 본다.
    re: /^(?!\s*(VEX|vex)[^가-힣a-zA-Z0-9]{0,4}(가|는|은|란|이란)?\s*(뭐|뭔|무엇|어떤\s*(것|거)|설명|알려\s*주|가르)|^\s*(VEX|vex)\s*(표준|규격|문서)?\s*(설명|소개))[\s\S]*(VEX|vex\s*(문서|파일|내보내|현황))/,
    tool: "vex_status",
    args: {},
  },
  {
