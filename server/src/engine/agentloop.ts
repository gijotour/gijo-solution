...
export function 선택을박는다(instruction: string, 선택: string): string {
  ...
  const 지시어 =
    /(^|\s)(?:(?:이|그|저|해당|요)\s*(?:자산|서버|장비|호스트|시스템|머신|항목|취약점|할\s*일|거)|(?:지금\s*)?(?:선택한?|고른)\s*(?:자산|서버|장비|호스트|시스템|머신|항목|취약점|할\s*일|거|것)|이거|이걸|이것|얘)/;
  const m = 지시어.exec(t);
  if (!m) return t;
...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  // ⚠ "자산 목록"·"자산 몇 개"(조회)와 갈라야 한다 — **등록 의사**를 밝힌 말만 잡는다.
  {
    ...
    re: /(자산|장비|서버|모델)\s*(을|를|으?로)?\s*(새로\s*)?(등록|추가)(할게|하고\s*싶|해\s*줘|해줘|하려|할래|하자)|(새|신규)\s*(자산|장비|서버|모델)\s*(등록|추가)|(새|신규)?\s*(자산|장비|서버|호스트|모델)[^\n]{0,40}?(자산|장비)\s*(으?로)\s*(등록|추가)/,
    tool: "register_asset",
    args: {},
  },
