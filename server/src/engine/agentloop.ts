...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  {
    //   ⚠⚠ **배제로 쫓지 말고 조회 낱말을 양성으로 요구한다**(2026-09-01 3차 검토 [상]).
    //     한때 「뭐야·설명·어떻게·왜·방법·절차」를 문장 전체에서 배제했더니 **평범한 조회까지
    //     죽었고**(「VEX 현황 설명해줘」), 배제를 좁혔더니 이번엔 「VEX 어떻게 쓰는 거야?」·
    //     「VEX 표준이 뭐야?」가 **건수표로 되돌아갔다.** 배제 목록을 늘렸다 줄였다 하는 것은
    //     끝이 없다 — 낱말 두더지잡기다.
    //     → **조회임을 드러내는 낱말**(현황·내보·파일·상태·건수·대상·목록·나가·추출)이
    //       있을 때만 잡고, 그 위에 **개념 물음꼴**(VEX가 뭐야 / VEX 설명 / VEX 어떻게 쓰는)만
    //       좁게 뺀다. 양성 요구가 있어 배제가 새도 무관한 문장을 삼키지 않는다.
    re: /^(?![\s\S]{0,30}(VEX|vex)[^가-힣a-zA-Z0-9]{0,6}(가|는|은|란|이란|이)?\s*(뭐|뭔|무엇|어떤\s*(것|거))|[\s\S]*(VEX|vex)\s*(표준|규격|형식|이란)?\s*(설명|소개)\s*(해|좀|을|를)?|[\s\S]*(VEX|vex)[\s\S]{0,10}(어떻게\s*(쓰|하|만드)|만드는\s*방법|쓰는\s*(법|거|방법)|왜\s*(필요|쓰|써)))[\s\S]*(?:(VEX|vex)[\s\S]{0,16}(현황|내보|파일|상태|건수|대상|목록|나가|추출|export)|(현황|내보|파일|상태|목록)[\s\S]{0,16}(VEX|vex))/i,
    tool: "vex_status",
    args: {},
  },
  {
