...
// ⚠ **순환이다**(screenguide → agentloop → screenguide). 이름만 가져오고 **부르는 것은 함수
//   안에서만** 한다(아래 구역이름물음) — 최상위에서 부르면 초기화에 걸린다(screenguide 머리글).
//   실측으로 양쪽 적재 순서를 다 확인했다(어느 쪽을 먼저 불러도 답이 같다).
import { 구역이름들 } from "./screenguide";
...
async function 사람에게내보낸다(
  ...
  // ⚠ 온톨로지근거임을밝힌다는 지식없음을밝힌다 바깥쪽이다(2026-09-11 B2) — 둘은 배타적
  //   조건이라 순서 자체가 정답을 가르지 않지만, 지식없음을밝힌다가 먼저 억제/배너를 정한
  //   뒤 그 결과에 온톨로지 표지를 덧대는 편이 「도구가 아예 근거 없음」과 「도구가 온톨로지만
  //   가져옴」을 같은 자리에서 겹쳐 읽기 쉽다.
  return 다음단계붙이기(
    온톨로지근거임을밝힌다(지식없음을밝힌다(법령한계를밝힌다(조문근거를보탠다(guardAgainstDenial(composed, calls), calls), calls), calls), calls),
    calls,
  );
}
...
export const 도구단계: Record<string, { 다음: string; 말: string }> = {
  ...
  // 2026-09-11 B4 후속 — ops-sim ⑧ 「경영진 보고용으로 짧게 요약해줘」가 갈 곳 없음으로 잡힘
  exec_brief: {
    다음: "⑤ 보고",
    말: '보고 문서로 뽑으려면 "이번 달 보안 KPI 리포트 만들어줘" · 숫자의 출처는 "보안 KPI 현황 알려줘"',
  },
};
...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //   "취약점 몇 건이야"처럼 **세는 질문**은 목록이 맞으므로 비켜 준다(아래 적용부에서 거른다).
  {
    ...
    // ★ 2026-09-13 B-2 ⑧ — 앞말 토큰을 최대 3개까지 연다(공백 있는 제품/자산 이름 대응).
    //   handlers.ts·되물음()이 「GIJO AS 서버 자산 취약점 알려줘」·「샘플 웹서버 자산 취약점
    //   알려줘」를 안내로 적는데 옛 앵커는 공백 없는 한 낱말만 받아 못 지켰다(실측 P05·P06).
    //   ⚠ 따옴표는 이번엔 허용하지 않는다(사장님 결정 — 인용 물음 과포착 위험, 설계관 risk).
    //   토큰 상한 3으로 좁혀 [14] 취약점 목록·today 영토를 안 뺏는지 N21·N37·N38로 확인했다.
    // ⚠ 2026-09-13 B-2 검증 중 발견 — 토큰 상한을 3으로 열자 「스캔 실패랑 취약점이 뭐가
    //   달라?」·「작년 취약점이랑 올해 취약점 뭐가 달라」·「sample-web01이랑 db-01 취약점 뭐가
    //   달라?」가 **비교개념질문(explain)의 영토를 뺏었다**(compare-concept-routing.test.ts
    //   실측 회귀) — 「랑/이랑」으로 이어진 두 대상이 다음 토큰으로 흡수됐다. 「뭐」 트리거가
    //   「뭐가 달라」(비교 어미)까지 삼키는 것이 원인이라, 그 꼴만 배제한다(정상적인 「…뭐야?」·
    //   「…뭐 있어?」는 그대로 살아 있다).
    // ⚠ 두 대상을 「이랑/랑/와/과」로 이은 꼴(비교 물음)은 별도로도 배제한다 — 위 「뭐가 달라」
    //   배제만으로는 「sample-web01이랑 db-01 취약점 알려줘」(비교 물음의 **대조군**, 「달라」가
    //   없다)까지 search가 삼켜 compare-concept-routing.test.ts의 대조군 전제가 깨졌다(실측).
    // ⚠ WSL 전체 게이트 실측(2026-09-13) — 토큰 다중화가 **동사구까지 이름으로 오인**했다:
    //   「승인 기다리는 취약점 있어?」(approval_status 영토, 「기다리는」이 관형사형 어미) ·
    //   「간밤에 새로 올라온/밤새 새로 등록된 취약점」(briefing 영토, 「올라온·등록된」도 관형사형
    //   어미) — 앵커가 이 동사구를 자산 이름처럼 삼켰다. 그 두 영토의 주제어(승인·결재·간밤·밤새)를
    //   배제해 앞자리 규칙(approval_status·briefing)이 원래대로 받게 한다.
    // ⚠⚠ 2026-09-13 B-2 검토관 수리(적발 ①·⑨·⑪ — 실결함 3건이 이 한 줄에 겹쳐 있었다)
    //   ① **여러 토막을 여는 순간 「이름」이 아닌 것까지 이름으로 읽었다.** 실측(수리 전):
    //      「고위험 미조치 취약점 알려줘」·「담당자 없는 취약점 알려줘」·「KEV 등재된 취약점
    //      알려줘」·「어제 발견된 취약점 알려줘」·「김보안이 맡은 취약점 알려줘」가 전부 search로
    //      **확정**됐다. 앞의 상태어취약점(finding_status)은 상태어가 **하나일 때만** 걸리므로
    //      상태어가 둘이 되는 순간 이 규칙이 먼저 받아, 2864줄 주석이 막으려던 바로 그 사고
    //      (「상태·속성은 글자 검색이 아니라 상태 칸의 몫」, 2026-08-08 리허설 3/3)가 되살아났다.
    //      적용부(아래 `f.tool === "search"`)가 「취약점」 앞을 통째로 query로 넘기므로 검색어가
    //      「고위험 미조치」·「어제 발견된」이 되어 「찾지 못했습니다」로 끝난다(routes.test.ts
    //      107-109가 못 박아 둔 2026-08-03 「잃은 취약점」 사고와 같은 표면).
    //      → **고친 방법**: 토막이 둘 이상일 때는 자산 종류 명사(서버|자산|호스트|시스템|장비)를
    //        **반드시** 요구한다. 이 규칙을 넓힌 목적 자체가 「공백 있는 자산·제품 이름」이었고
    //        (P05 「GIJO AS 서버 자산 취약점」·P06 「샘플 웹서버 자산 취약점」) 그 꼴은 전부
    //        종류 명사로 끝난다. 한 토막 이름(P04·P07)은 종전 그대로 종류 명사 없이도 받는다.
    //   ⑪ **비교 배제가 「…과/와」로 끝나는 부서 이름을 통째로 떨궜다.** 실측(수리 전):
    //      「총무과 서버 취약점 알려줘」·「정보보안과 서버 취약점 알려줘」·「보안과 웹서버 취약점
    //      알려줘」가 전부 ⑨ — 「총무+과」가 비교 연결어로 읽혔다(「총무팀 …」은 search로 갔다).
    //      국내 조직에서 가장 흔한 이름 꼴에서 이 규칙의 목적과 정반대로 깨진 것이다.
    //      → 「이랑|랑」은 그대로 붙여 받고, 「와|과」는 **앞뒤 공백이 있을 때만** 비교로 읽는다.
    re: /^(?!.*(기한|SLA|승인|결재|간밤|밤새))(?!.*[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}(?:\s*(?:이랑|랑)|\s+(?:와|과))\s*[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}[^\n]*취약점)(?:[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}\s*(?:웹\s*)?(?:서버|자산|호스트|시스템|장비)?|[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}(?:\s+[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}){1,2}\s*(?:웹\s*)?(?:서버|자산|호스트|시스템|장비))\s*(의|에)?\s*취약점.{0,6}(알려|보여|뭐(?!가?\s*달라)|무엇|있어|있나|현황|정리)/,
    tool: "search",
    args: {},
  },
  {
  ...
  // ⚠ 2026-09-11 수리 — **배제 목록이 아니라 「실행 지시를 요구」하는 허용 조건**이다.
  //   앞선 판(2026-09-10)은 조회 낱말(결과·목록·어느·몇…)을 금지어로 깔았는데, 그러면
  //   **금지어를 곁들인 실행 문장이 통째로 샌다**: 「하드닝 점검 결과가 필요하니 지금 돌려줘」·
  //   「어느 장비부터 하드닝 점검해줘」·「취약점 진단해줘 결과 정리해서」가 결정적 갈래 어디에도
  //   안 걸려 ⑨ 모델 재량으로 떨어졌다(route-explain 실측). 금지어는 셀 수 없고 허용어는 셀 수 있다.
  //   ⇒ 앞 보기는 **주제**(하드닝·보안설정 점검 / CCE / 기준 점검 / 취약점 진단 / 정기점검 /
  //     장비·서버·시스템 점검), 뒤 보기는 **실행 지시**(점검·진단·스캔·체크 + 해·하자·하라·시켜 ·
  //     돌려 · 수행 · 가동 · 실행). 둘 다 있어야 잡는다 — 조회 꼴엔 실행 어미가 없어 저절로 비켜 간다.
  //   ⚠ 「실행」 뒤에 이력·기록·결과·내역·현황이 붙으면 조회다(「하드닝 점검 실행 이력 보여줘」).
  //   ⚠ 「돌아」·「돌게」는 실행어가 아니다 — 「정기점검 언제 돌아?」는 스케줄 조회다(evalgate).
  //   ⚠ 정규식은 **글자 그대로의 리터럴**로 둔다 — test/helpers/routing.ts가 소스에서 떼어
  //     eval로 읽는다(new RegExp로 조립하면 그 진단이 통째로 죽는다).
  //   시험: server/test/hardening-unchecked-routing.test.ts
  {
    re: /^(?=[\s\S]*((하드닝|보안\s*설정)\s*(점검|진단|스캔|체크)|CCE\s*(점검|진단|기준)|(장비|서버|시스템)\s*(보안\s*)?(점검|진단)|기준.{0,3}(점검|진단)|취약점\s*진단|정기\s*점검))(?=[\s\S]*((점검|진단|스캔|체크)\s*(해|하자|하라|시켜)|돌려|수행|가동|실행(?!\s*(이력|기록|결과|내역|현황))))/,
  ...
  {
    // ★ 2026-09-13 B-2 ⑧ — 「점검 일정」 갈래를 OR로 더한다. session-explorer.js·nextguide.ts가
    //   「GIJO AS 서버 점검 일정 알려줘」·「유지보수 점검 일정 알려줘」를 안내로 적어 두는데
    //   앵커가 유지보수/정비를 필수로 요구해 제품 이름이 앞에 붙은 꼴(앞이 유지보수가 아님)을
    //   못 받았다(실측 P24·P25). ⚠ 하드닝·원격·보안설정을 제외구에 추가한다 — 하드닝 쪽은
    //   배열에서 이 규칙보다 **앞**이라 이미 방어되지만(N25·N35 실측), 「보안설정 점검 일정」처럼
    //   하드닝 규칙 자체가 못 잡는 동의어까지 이 규칙이 삼키면 안 된다(N32).
    // ⚠⚠ 2026-09-13 B-2 검토관 수리(적발 ②·③⑫·⑮ — 실결함 3건이 이 한 줄에 겹쳐 있었다)
    //   ③⑫ **하드닝·원격·보안설정 배제를 공용 `^` 머리에 달아, 넓히려던 규칙의 옛 갈래가 죽었다.**
    //      그 배제는 **새로 붙인 「점검 일정」 갈래**의 사정(N32 「보안설정 점검 일정」)인데 두
    //      갈래 공통 머리에 붙어 유지보수 갈래까지 좁혔다. 실측(수리 전, 전부 커밋 전에는
    //      maintenance_status): 「원격 유지보수 점검 일정 알려줘」·「원격 유지보수 점검 현황
    //      알려줘」·「원격지 서버 유지보수 점검 현황 알려줘」·「원격지 장비 유지보수 현황 알려줘」·
    //      「원격 근무자 장비 정비 현황 알려줘」·「보안 설정 유지보수 점검 현황 알려줘」가 전부 ⑨로
    //      샜다. 「원격 유지보수」는 장비 운영의 표준 표현이고 하드닝 영토는 **앞자리가 이미**
    //      막는다(N25·N35 실측) — 새로 얻은 방어는 겹치고 잃은 영토만 남은 조용한 회귀였다.
    //      → 배제를 **새 갈래 안쪽으로만** 옮긴다(유지보수 갈래는 종전 배제어 그대로).
    //   ② **「점검 일정」 앵커가 레드팀·스캔 점검 일정까지 삼켰다.** 실측(수리 전, 착수 전 ⑨):
    //      「레드팀 점검 일정 알려줘」·「스캔 점검 일정 알려줘」→maintenance_status. 이 제품에는
    //      레드팀 점검(run_redteam·redteam_status)·스캔(scan_status)이라는 **다른 점검**이 실재해
    //      유지보수 점검 일정 데이터로 확신 있게 답하면 거짓이 된다. 앞 판 주석은 하드닝 세 낱말만
    //      막아 두고 「동의어까지 삼키면 안 된다」고 스스로 적었는데 나머지 점검 종류가 열려 있었다.
    //   ⑮ **문서·양식 물음까지 삼켰다** — 「점검 일정표 문서 있어?」→maintenance_status(트리거에
    //      「있」이 있고 앞이 `.*`라 열려 있었다). 업무 데이터 0인 자리에서는 「등록된 점검 일정이
    //      없습니다」가 나가 **문서가 없는 것으로** 읽힌다(같은 규칙 주석이 경계한 그 사고의 재발).
    re: /^(?!.*(절차|방법|항목\s*설명|순서|체크\s*리스트|어떻게\s*하))(?:.*(유지\s*보수|유지보수|정비)\s*(점검|일정)?\s*[^.\n]{0,8}(뭐|무엇|남았|현황|어때|보여|알려|확인|밀린|지난)|(?!.*(하드닝|원격|보안\s*설정|레드팀|red\s*team|스캔|견고성|인젝션|문서|양식|서식)).*점검\s*일정[^.\n]{0,8}(뭐|무엇|알려|보여|확인|있|없))/,
  ...
  //   ⚠ 취약점 관련 세는 말만 잡는다 — "문서 몇 건"·"점검 몇 건"은 각자 도구가 따로 있다.
  {
    ...
    // ★★ 2026-09-11 배제구 추가(고객 QA 예행 2026-09-10 실측) — 「배정 승인 대기 취약점
    //   몇 건이야?」같이 **승인/결재 대기**가 붙은 세는 물음은 취약점 개수가 아니라 결재
    //   현황을 묻는 말이다. 여기서 채 가면 취약점 결재판(배열 끝 [88] approval_status)으로
    //   못 가고 반쯤 맞는 숫자만 나간다. ⚠⚠ 배제구는 `^(?!…)` 뒤에 `[\s\S]*?`를 반드시 둔다
    //   — 그게 없으면 본문 시작(0번째 글자)에 못박혀 「미조치 취약점 몇 건이야?」 같은 옛
    //   문장까지 안 걸린다(원래 이 규칙엔 `^` 앵커가 없었다 — 실측으로 확인하고 넣었다).
    //   시험: server/test/approval-routing.test.ts(옛 영토 회귀).
    // ★★★ 2026-09-11 재수리(검토관 [하] 둘 — 실측으로 확인) — 배제구를 **좁힌다.**
    //   처음 판은 「승인/결재 대기」만 보고 무조건 비켜서 **어느 규칙에도 안 걸리는 구멍**을 냈다:
    //     · 「점검에서 나온 취약점 결재 대기 몇 건이야?」 → 실측 ∅(전에는 여기가 받던 말이다).
    //     · 「승인 대기 중인 critical 몇 건이야?」 → [88] approval_status로 갔는데 그 도구는
    //       params가 없어 **critical이 조용히 버려진다**(전체 숫자가 나간다).
    //   ∅는 곧 모델 재량이고, 그것이 이번에 고친 결함 ②(제품소개 시연 표를 사실로 인용)의
    //   발생 조건이다. → **[88]이 표현 못 하는 좁힘말**(점검 출처·심각도·KEV)이 있으면 여기서
    //   그대로 받는다. 아래 finding_status 인자 뽑기가 그 좁힘을 실제로 넘긴다(같은 파일 「조건」).
    //   좁힘말이 없는 순수 결재 물음(「배정 승인 대기 취약점 몇 건이야?」)만 [88]로 보낸다.
    re: /^(?!(?![\s\S]*(점검|정비|유지보수|critical|긴급|high|고위험|medium|중간|low|낮음|kev))[\s\S]*(승인|결재)\s*(대기|기다리))[\s\S]*?(?:(미배정|배정\s*안\s*[된함]|담당자?\s*없)[^\n]{0,10}(취약점|건|것)?[^\n]{0,6}(몇|얼마)|(취약점|고위험|critical|kev)[^\n]{0,10}몇\s*(건|개)|(기한\s*(초과|지난)|지연)[^\n]{0,10}몇\s*(건|개)|(검토|확인)\s*(안\s*[한된]|못\s*한|미)[^\n]{0,10}몇\s*(건|개)|kev[^\n]{0,6}(걸린|잡힌|뜬)[^\n]{0,6}(있|없))/i,
    tool: "finding_status",
    args: {},
  },
  ...
  //   순서로 한 번, 시킴꼴 조건으로 또 한 번 막는다.
  {
    ...
    // ★ 2026-09-13 B-2 ⑧ — 「영향」과 「위협」 사이를 연다. nextguide.ts의 안내 문구가
    //   「우리 자산에 걸리는 것만 보려면 "내부 자산에 영향 주는 위협 알려줘"」라고 적었는데
    //   기존 정규식은 「영향 위협」(사이 낱말 없음)만 알아 약속-코드가 어긋나 있었다(실측 P08).
    re: /(?!.*위협\s*(도|모델링))(최근|요즘|새로|오늘)\s*(뜬|들어온|올라온|새로)?\s*(탐지|위협|위협\s*정보)\s*(내역|목록)?\s*(알려|보여|뭐|있어|있나|없어)|위협\s*인텔|(우리|내부)\s*자산에?\s*(걸리는|영향\s*(?:주는|받는|있는)?)\s*위협/,
    tool: "threats",
    args: {},
  },
  {
    ...
    //   ★ 2026-09-13 B-2 검토관 수리(적발 ⑭) — `critical`을 더한다. 목록 갈래를 넓힌 뒤
    //     「critical 자산 목록 보여줘」가 뒤의 목록 규칙(args `{}`)에 걸려 **조건을 잃고 전체**를
    //     답했다. 자산에는 critical 등급이 따로 없고 자산위험등급()이 critical을 high로 세므로,
    //     여기서 받아 args{risk:"high"}로 넘기는 것이 제품의 잣대와 같다.
    re: /(위험도?\s*(높은|높음|큰)|고위험|위험한|위험\s*큰|critical|크리티컬)\s*(ai\s*)?자산/i,
    tool: "list_assets",
    args: { risk: "high" },
  },
  {
  ...
  {
    // ★ 2026-09-13 B-2 ⑧ — 목록·리스트 갈래를 OR로 덧댄다. handlers.ts·되물음()이 「자산 목록
    //   보여줘」를 안내 문구로 적어 두는데 이 규칙은 「몇 대」류(세는 물음)만 알아 그 약속을
    //   못 지키고 있었다(실측 P01~P03). ⚠ 「이 뭐야?」(N14, 지식 물음)·「좀」(N27, 홑말)은
    //   보여/알려가 없어 그대로 ⑨ — [18] 자산 현황(isAssetStatusAsk)이 목록|리스트를 배제해
    //   가로채지 않으므로 N33과도 안 부딪힌다.
    // ⚠ 2026-09-13 B-2 검토관 수리(적발 ⑥) — 새 목록 갈래에 **방법·화면 배제가 없어 사용법
    //   물음을 자산 목록으로 답했다**. 실측(수리 전, 착수 전 ⑨): 「자산 목록 보여주는 방법
    //   알려줘」·「자산 목록 화면 알려줘」→list_assets. 형제 규칙(maintenance_status)이
    //   `절차|방법|순서|어떻게 하`를 배제하는 것과 잣대가 어긋나 있었다 — 같은 잣대로 맞춘다.
    //   (「자산 목록 어디서 봐?」는 [20] 자리 안내가 먼저 받는다 — 그 갈래는 그대로다.)
    // ⚠ 2026-09-13 B-2 검토관 수리(적발 ⑭) — 이 갈래의 args는 `{}`라 **조건어가 조용히 사라진다**
    //   (handlers.ts runListAssets는 risk가 없으면 전체를 답한다). 실측(수리 전): 「critical 자산
    //   목록 보여줘」가 여기 걸려 **전체 AI 자산**을 답했다 — 담당자는 조건을 걸었는데 머리줄은
    //   전체 건수를 말한다(이 저장소가 반복해 겪은 「조건이 조용히 사라진다」 부류).
    //   ⓐ 등급 조건(고위험·critical)은 **앞자리 규칙**이 args{risk:"high"}로 받게 했다(아래
    //     위험도 규칙에 critical을 더했다 — 자산위험등급()이 critical을 high로 세는 잣대 그대로).
    //   ⓑ 방법·화면·사용법 물음은 배제한다 — 형제 규칙(maintenance_status)과 잣대를 맞춘다.
    //   ⚠ **문장 머리에 못 박지는 않는다.** titlescope-routing.test.ts가 「제목 토큰만 뺀 말과
    //     같은 도착지」를 계약으로 두고 있어(「제로트러스트 관련 자산 목록 보여줘」) 머리에 고정하면
    //     그 계약이 깨진다(실측으로 확인). 그래서 이름·종류 조건(「방화벽 장비 목록 알려줘」)은
    //     여전히 전체를 답한다 — 이 도구에 **종류 필터가 아예 없어** 넘길 인자가 없다(별건).
    re: /(자산|장비|서버|호스트)[^\n]{0,8}(몇\s*(대|개|건)|개수|총\s*몇)|자산\s*(총계|전체)\s*(몇|얼마)|(?!.*(방법|어떻게|사용법|화면|메뉴))(자산|장비|서버|호스트)\s*(목록|리스트)[^\n]{0,10}(보여|알려)/,
    tool: "list_assets",
    args: {},
  },
  ...
  // 「승인 기다리는 점검 있어?」 — 실측 12.4초·도구 미선택(그날그날 다른 답).
  //   drawer의 정답 표본이 유지보수 점검의 승인 대기/반려 목록이라 maintenance_status로 못 박는다.
  //   ⚠ 「승인해줘」(쓰기 시킴꼴)·「승인 기준 알려줘」(지식)는 다른 말이다 — 기다림/대기 앵커 필수.
  //   ★★ 2026-09-11 좁혔다(고객 QA 예행 2026-09-10 실측) — **점검 낱말 없이** 「승인 기다리는
  //     것 있어?」・「승인 기다리는 취약점 있어?」까지 여기로 삼켜, 담당자가 물은 취약점 결재판을
  //     유지보수 점검으로 잘못 안내했다. 주석은 전부터 「정답 표본이 유지보수 점검」이라 적어
  //     두었는데 정규식엔 그 조건이 없었다(약속-코드 불일치, 형제 규칙 [71]은 이미 걸어 뒀다).
  //     종류를 안 밝힌 「승인 기다리는 것 있어?」는 아래 배열 끝의 결재·승인 대기 조회
  //     규칙([88])이 취약점 결재와 점검 승인을 **둘 다** 센다. 시험: server/test/approval-routing.test.ts.
  {
    re: /^(?=[\s\S]*(점검|정비|유지보수))[\s\S]*?승인[^.\n]{0,6}(기다리|대기)[^.\n]{0,8}(있|뭐|건|목록|보여|알려)/,
    tool: "maintenance_status",
    args: {},
  },
  ...
  },
  {
    ...
    //   ⚠ 조회는 비켜 준다 — 「승인 대기 뭐 있어?」는 **아래 [88] approval_status**의 몫이다
    //     (2026-09-11까지는 「위쪽 규칙」이라 적혀 있었는데, 그날 [45]가 점검 낱말을 필수로
    //      좁히면서 자리가 배열 끝으로 옮겨 갔다 — 주석만 낡아 있었다. 검토관 [하] 적발).
  ...
  // ── 🗂️ 「결재·승인 대기 있어?」 — 종류를 안 밝힌 물음은 둘 다 센다 (2026-09-11 고객 QA 예행 수리) ──
  //
  // ★★ 뿌리 — 고객 QA 예행(2026-09-10 밤, 업무 데이터 0·승인 대기 0)에서 실측한 결함 둘.
  //   ① 「승인 기다리는 것 있어?」·「승인 기다리는 취약점 있어?」가 위 [45]로 가서
  //     **점검 일정(유지보수)** 도구만 답했다 — 담당자가 물은 것은 취약점 배정·판정
  //     **결재판**(finding approvals)인데 「승인」 낱말이 유지보수 승인 규칙에 먼저 잡혔다.
  //   ② 「결재 대기 있어?」는 **forcedToolFor 실측 null**이라 모델로 샜고, 업무 데이터가
  //     0인 서버에서 **GIJO_AS_제품소개.md의 시연 표**(점검 6건·「프롬프트 가드레일 점검 |
  //     승인 대기」)를 RAG로 읽어 사실처럼 답했다(「프롬프트 가드레일 점검이 승인 대기
  //     상태입니다」) — 리셋 뒤 다시 물어도 같았다.
  //   88개 규칙 어디에도 「결재」 낱말이 없었다(dispatcher의 결재승인요청_RE는 **쓰기**
  //   「승인해줘」만 잡고 조회 「결재 대기 있어?」는 안 잡는다 — approval-routing.test.ts가
  //   그 경계를 이미 못 박아 두었다).
  //
  // ★ 제품 사실 — 결재판은 두 종류다: 취약점 결재(approvals.ts approvalSummary().pending,
  //   그중 담당자 미배정은 isUnassignedReview)와 점검 승인(report.ts maintenanceSummary().reported).
  //   「승인 기다리는 것 있어?」처럼 종류를 안 밝힌 물음은 **둘 다 세어 결정적으로** 답하는
  //   것이 정직하다 — 취약점 결재는 approval_status(여기), 점검 승인은 종전 그대로
  //   maintenance_status(위 [45]·[49])다.
  //
  // ⚠ **자리는 배열 끝이다.** routes.ts 규칙표가 FORCED_INTENTS[n] 자리 번호로 가리키므로
  //   중간에 끼우면 뒤 번호가 통째로 밀린다(2026-08-10 실사고: 18줄). 끝에 붙이면 안 밀린다.
  //   붙인 뒤 반드시 `node tools/routes-renumber.mjs --write` — 손으로 세지 않는다.
  // ★ **앞자리가 곧 방어다** — 위 [45](점검 낱말 필수로 좁혔다)·[49](기간별 점검 일정)·
  //   [14](세는 질문, 승인/결재 대기 배제구 추가)·[56]·[71]·[84]가 전부 먼저 훑는다.
  //   그중 하나가 이겨야 할 자리(점검 승인)는 그쪽으로 그대로 간다 — 여기는 **아무 데도
  //   안 걸린 나머지**만 받는다.
  // ⚠ 배제어 3겹: ⒜ 점검 계열(점검/정비/유지보수 — 있으면 위 [45]/[49]/[71]의 몫이다)
  //   ⒝ 쓰기 시킴꼴(승인해줘·반려 처리해·눌러·올려 — **진짜 방어는 dispatcher의
  //   결재승인요청_RE**(FORCED_INTENTS보다 앞 층)다. 여기 배제는 이중 방어일 뿐이다)
  //   ⒞ 지식 물음(기준·절차·방법·뜻·이란·「어떻게 하」 — 「결재 대기 승인 절차 알려줘」는
  //   안내·RAG의 몫이다).
  //   ★★ 2026-09-11 재수리(검토관 [하]) — ⒞에서 **명사 「규정·정책」과 홑낱말 「왜」를 뺐다.**
  //     실측: 「정책 승인 대기 있어?」·「규정 승인 대기 뭐 있어?」·「결재 대기 왜 이렇게
  //     많아?」가 전부 ∅(어느 규칙에도 안 걸림)였다 — 셋 다 **현황 물음**인데 지식 배제어에
  //     걸려 모델·RAG로 떨어졌고, 그것이 이번에 고친 결함 ②가 나던 바로 그 경로다.
  //     「어떻게」도 「어떻게 하/해」로 좁힌다(isHowtoNotCommand와 같은 잣대) — 「결재 대기가
  //     어떻게 돼 있어?」는 현황 물음이다. 배제어 목록은 routes.ts 규칙표와 **글자까지 같게** 적는다.
  //     ⚠ 「왜」를 배제어에서 빼는 것만으로는 「결재 대기 왜 이렇게 많아?」가 안 걸렸다(실측 ∅) —
  //     조회 앵커에 「많」이 없었기 때문이다. 배제를 푸는 것과 앵커를 여는 것은 **다른 일**이라
  //     둘 다 해야 말이 도구에 닿는다. 답은 숫자와 갈 곳이다(「왜」의 이유까지 대답하지는
  //     않지만, 시연 표를 사실로 인용하던 옛 답보다 정직하다).
  // ⚠ 이름을 approval_status로 둔 것도 방어다 — dispatcher의 집계조회도구_RE(sourcebadge.test)에
  //   `_status` 조각이 이미 있어 근거 둔갑(RAG 재검색)이 자동으로 막힌다. 그 정규식을
  //   따로 건드리지 않는다(부분일치 함정 — cti가 action에 걸렸던 전례).
  // 시험: server/test/approval-routing.test.ts · server/test/overdue-routing.test.ts.
  {
    re: /^(?![\s\S]*(점검|정비|유지보수))(?![\s\S]*(승인|반려|결재)[^.\n]{0,8}(해\s*줘|해\s*주세요|해라|처리해|눌러|올려))(?![\s\S]*(기준|절차|방법|뜻|이란|어떻게\s*[하해]))(?=[\s\S]*(결재|승인))(?=[\s\S]*(대기|기다리))(?=[\s\S]*(있|없|뭐|무엇|몇|건수|목록|보여|알려|현황|확인|많))/,
    tool: "approval_status",
    args: {},
  },
  // ── [89] 임원 보고용 세 줄 요약 — B4 수리(2026-09-11, 평가 게이트 kr-report-tone) ──────────
  //
  // ★ 왜 생겼나: 「임원 보고용으로 이번 주 보안 현황을 세 줄로 요약해줘」가 route-explain 실측
  //   「걸리는 규칙 없음 → ⑨ 모델 선택」이었다 — 회차마다 도구가 갈려(09-10은 모델 작문 16.0/53.9초,
  //   09-11은 briefing 1,380ms) 게이트가 답이 아니라 **운**을 쟀다. briefing은 directAnswer라
  //   그 도구를 고른 날은 영어 취약점 이름·IP 목록 원문이 그대로 담당자에게 나갔다(한글 10%).
  //
  // ⚠ **자리는 배열 끝이다.** 중간에 끼우면 뒤 번호가 통째로 밀린다(2026-08-10 실사고 18줄).
  //   붙인 뒤 반드시 `node tools/routes-renumber.mjs --write` — 손으로 세지 않는다.
  // ★ **앞자리가 곧 방어다** — FORCED 88개가 이 규칙보다 먼저 돈다(배열은 위에서부터 첫 매치로
  //   끝난다). 실측(2026-09-11): 「취약점 현황 요약해줘」→findingListAnswer(특수경로 routes.ts:173)
  //   · 「자산 현황 요약해줘」→assetStatusAnswer(특수경로 routes.ts:197) · 「최근 위협 인텔
  //   요약해줘」→threats · 「컴플라이언스 현황 요약해줘」→compliance_status · 「보안 현황 보고서
  //   만들어줘」·「임원 보고용 리포트 만들어줘」→generateReport(특수경로 routes.ts:305) ·
  //   「오늘 브리핑 보여줘」→briefing[13] — 전부 이 규칙 앞에서 먼저 잡혀 안 뺏긴다.
  //   ★ 2026-09-11 검토관 [중] 정정 — 첫 판은 이 셋을 [14]·[18]·[36]이라 적었는데 그 자리들은
  //     각각 finding_status·time_saved·report_activity다(routes.ts 표 대조). 셋은 애초에 FORCED
  //     자리번호가 없는 **특수경로 행**이고, 「오늘 브리핑」은 [13](=아침 브리핑, [87]은 간밤).
  //     자리번호가 어긋나면 route-explain이 엉뚱한 규칙을 읽는다(2026-08-10 거짓 겹침 경보).
  // ⚠ **제목지목질문(explain)은 이 규칙보다 뒤에 온다** — 첫 판 주석은 「먼저 돈다」고 적었는데
  //   코드는 정반대다(forcedToolFor 안의 순서: FORCED_INTENTS 루프 → 제목지목질문, `return null`
  //   바로 앞). ★ 첫 판은 이 자리를 **줄 번호**(2626행·2813행)로 가리켰는데, 배열 끝에 [90]이
  //   붙으면서 46줄이 밀려 그 번호가 딴 주석을 가리키게 됐다(2026-09-11 검토관 [중]). 이 저장소가
  //   「번호로 가리키면 밀린다」로 이미 실사고를 겪은 자리다 — **판별자 이름으로 가리킨다.** 그 자리는
  //   2026-09-08에 **일부러** 꼬리에 둔 것이다(앞머리면 FORCED를 통째로 뺏는다). 그러니
  //   **배제어가 유일한 방어다** — 없으면 「○○ 문서 세 줄로 요약해줘」가 explain의 몫을 이 규칙이
  //   가로챈다. 배제어를 줄이려는 사람은 그 순간 이 방어를 여는 것임을 알고 줄여야 한다.
  //   ⚠ 남는 구멍(2026-09-11 검토관 [중], 알고 두는 것): 제목 토큰에 **유형어가 없는** 문서
  //     (예: 「전사_보안현황_요약.pdf」)를 지목하면 배제어가 안 걸려 이 규칙이 먼저 잡는다.
  //     다른 88개 규칙도 똑같은 성질이라(꼬리 배치의 대가) 여기서만 예외를 두지 않았다 —
  //     대신 배제어에 「자료」를 더해 이 제품이 문서를 부르는 또 하나의 이름을 막았다
  //     (citeguard.ts 자료없음안내 「등록된 **사내 자료**에는…」 — 제품 자신이 쓰는 말이다).
  {
    re: /^(?![\s\S]*(문서|자료|파일|보고서|리포트|회의|메일|기사|뉴스|논문|계약서|매뉴얼|가이드|지침|법령|조문|판례|로그|CVE-|점검|정비|유지보수|이력|절차|방법|뜻|이란))(?=[\s\S]*((임원|경영진|상급자|윗분|대표)[^\n]{0,8}보고|보고용|(보안|전사|전체)\s*(현황|상황)))(?=[\s\S]*(요약|정리|간단|간략|짧게|(세|3)\s*줄|한\s*줄))/,
    tool: "exec_brief",
    args: {},
  },
  // ── [90] 취약점 우선순위 **기준** 물음 — B5 수리(2026-09-11, 평가 게이트 kr-priority 23/24) ──
  //
  // ① 왜 생겼나: 게이트 문항 「취약점 조치 우선순위를 정할 때 무엇을 먼저 봐?」가
  //   route-explain 실측 「걸리는 규칙 없음 → ⑨ 모델 선택」이었다 — 도구 카탈로그의 today
  //   설명 줄(「지금 조치할 취약점 **우선순위**를 … KEV→EPSS→VPR 순 …」)이 이 문장과 낱말이
  //   거의 그대로 겹치는 유일한 줄이라 14B가 today로 기울었고, today는 directAnswer라
  //   그 회차는 「오늘 조치 우선순위 상위 5건 (KEV→EPSS→VPR 순): 1. 🔴 … @ oracle.local
  //   (192.168.219.98) — KEV(실제악용) · EPSS 100% · VPR 9.7 | 담당 미지정 …」을 그대로
  //   냈다(한글 비율 25~44%). 이 물음은 **판단 기준(지식)**을 묻는 말이지 오늘의 목록을
  //   달라는 말이 아니다 — 지식 답(KEV → EPSS/VPR → CVSS → 자산 중요도)은
  //   ontology-seed.ts:100 `t("취약점 우선순위","판단지표", …)`과 rag-seed의
  //   epss_vs_vpr.md 「우선순위 반영 순서(권장)」·kev_bod·취약점관리_지침에 이미 있다.
  //   explain이 그 근거를 모으는 유일한 도구라 새 도구를 만들지 않고 그리로 못 박는다.
  //   args의 topic은 **고정 문자열**이어야 한다 — expandOntology는 부분문자열 포함으로
  //   걸리는데, 지시문 전체("취약점 **조치** 우선순위를…")를 넘기면 온톨로지 주어
  //   「취약점 우선순위」가 그 안에 포함되지 않아(정규화해도 "취약점조치우선순위"≠
  //   부분에 "취약점우선순위"를 안 담음) 판단지표 5줄이 원리상 안 걸린다.
  //   ⚠⚠ **여기서 못 박는 것은 「도구」까지다 — 「순서」는 아니다**(2026-09-11 검토관 [중], 알고 둔다).
  //     위에 적은 KEV → EPSS/VPR → CVSS → 자산 중요도라는 **차례**를 만드는 원천은
  //     rag-seed/security-references/epss_vs_vpr.md 「우선순위 반영 순서(권장)」 한 곳뿐이고,
  //     온톨로지 판단지표 5줄(ontology-seed.ts)에는 순서 정보가 없다(적재 순서는 CVSS가 먼저다).
  //     runExplain은 RAG 조각이 0건이어도 온톨로지 줄만으로 답을 내므로, 임베딩이 흔들려 그 문서가
  //     안 걸린 회차에는 모델이 CVSS-먼저로 말할 수 있다 — 그런데 tools/regress의 expect는
  //     KEV·EPSS|VPR·CVSS **낱말의 존재만** 보고 evalgate kr-priority는 그중 하나만 보므로
  //     둘 다 초록이다. 「순서까지 코드가 붙인다」로 한 층 더 내리는 것은 **이 라운드가 하지 않았다**:
  //     (ㄱ) regress에 순서 패턴을 넣으면 14B 어순 다양성에 게이트가 흔들리고(게이트를 못 믿게 된다),
  //     (ㄴ) 씨앗 배열을 KEV-먼저로 재정렬하는 쪽은 **이미 적재된 운영 DB에는 안 먹히고**(재시드가
  //     필요하다) expandOntology의 시드별 공평 담기를 지나 순서가 보존되는지도 실측해야 한다.
  //     둘 다 라이브 실측이 있어야 정할 수 있어 **메인 판단으로 넘긴다**(백로그).
  //
  // ② **자리는 배열 끝이다.** 중간에 끼우면 뒤 번호가 통째로 밀린다(2026-08-10 실사고
  //   18줄). 붙인 뒤 반드시 `node tools/routes-renumber.mjs --write` — 손으로 세지 않는다.
  //
  // ③ **앞자리가 곧 방어다** — 실측으로 확인된 앞 규칙들이 먼저 걸려 이 규칙에 안 온다:
  //   workflow_status[28](「우선순위 정하려면 뭘 봐야 해?」·「조치 우선순위는 어떻게
  //   정해?」) · today[2](「제일 급한 취약점」·「오늘 뭐부터」) · exec_brief[89] ·
  //   그리고 **자리 번호가 없는 dispatcher 특수경로 셋** — isFindingListAsk(「취약점 현황
  //   요약해줘」→findingListAnswer) · isAssetStatusAsk(「자산 현황 요약해줘」) ·
  //   방법질문화면찾기(「취약점 우선순위 정하려면 뭘 봐야 해?」→formatScreenGuide).
  //   ★ 2026-09-11 검토관 [중] 정정 — 첫 판은 이 셋을 [14]·[18]·[21]이라 적었는데 그 자리들은
  //     각각 finding_status(세는 질문)·time_saved(AI가 아낀 시간)·threats(위협 인텔)다. 게다가
  //     화면 안내는 FORCED_INTENTS 배열에 **아예 없다**(dispatcher 특수경로). 「안 뺏긴다」의
  //     유일한 서면 근거가 딴 규칙을 가리키면 다음 사람이 열어 보고 헛짚는다 — 바로 위 [89] 행이
  //     같은 실수로 이미 한 번 정정받은 자리다. **자리 번호 대신 판별자 이름으로 가리킨다.**
  //
  // ④ **배제어는 무엇을 막나** — 낱말마다 성질이 다르다. 뭉뚱그려 「배제어가 유일한 방어다」라고
  //   적으면 다음 사람이 틀린 자리를 고친다(첫 판이 그렇게 적었다 · 2026-09-11 검토관 [중]).
  //   · 문서·규정 계열(문서|자료|파일|매뉴얼|가이드|지침|규정|법령|조문|판례) — 제품설정질문·
  //     문서지목질문·사내규정질문이 **FORCED 루프보다 먼저** 돈다(코드 순서 그대로). 여기 배제어는
  //     **이중 방어**다(실측: 「우선순위 기준 규정 알려줘」→forcedToolFor explain·topic
  //     「우선순위 기준 규정」 = 사내규정질문이 잡은 것이지 이 규칙이 아니다).
  //   · **뒤 규칙이 없는 낱말**(점검|정비|유지보수|보고서|리포트|어디) — 여기 배제어가 **유일한
  //     방어**다. 실측: 「취약점 점검 우선순위 기준이 뭐야?」는 지금 ∅인데, 「점검」을 지우면
  //     그 자리에서 곧바로 이 규칙이 잡는다.
  //   · 배열 **뒤**에 오는 explain 갈래는 제목지목질문 **하나뿐**이다(유형어 없는 제목 —
  //     「취약점_우선순위_기준_2026」). 그 자리는 배제어로 못 막는다(제목에 유형어가 없으니까) —
  //     알고 둔 구멍이다.
  //   · 장비·제품 낱말(방화벽|WAF|IPS|IDS|VPN|EDR|백신|프록시|스위치|라우터|SIEM|DLP|백업)과
  //     `CVE-` — **topic이 고정 문자열이라** 이 규칙이 잡으면 다른 영토의 물음에 취약점 일반
  //     기준을 확신 있게 답한다(2026-09-11 검토관 [중] 실측: 「방화벽 룰 우선순위 기준이 뭐야?」
  //     ·「IPS 차단 우선순위 기준 뭐야?」·「백업 우선순위 기준이 뭐야?」·「CVE-2021-44228
  //     우선순위 어떻게 판단해?」가 전부 topic「취약점 우선순위」로 갔다 — ∅이던 자리를 **코드가
  //     틀린 주제로 확정**한 것이라 전보다 나쁘다). 그래서 빼 둔다.
  //     ⚠ 낱말 목록을 아래 `제품낱말` 상수와 공유하지 않는 이유: 이 배열의 `re:`는 **정규식
  //       리터럴이어야** 한다(조립식으로 바꾸면 guidance-check의 강제규칙 추출기가 이 규칙을
  //       통째로 못 읽고 조용히 빠진다 — [83] 머리글이 적어 둔 함정).
  //   · 「기준/지표 + 으로/대로 + 정리·정렬·매겨…」 — **시킴꼴 목록 지시**다(「EPSS 기준으로
  //     우선순위 매겨줘」·「취약점 우선순위 기준으로 정리해줘」). 지식 강의로 못 박으면
  //     「EPSS 높은 순으로 보여줘」에 개념 강의를 39초 했던 [19] 사고와 같은 모양이 된다.
  //     ⚠ 「으로/대로」가 없는 「우선순위 기준 정리해줘」는 **기준을 요약해 달라는 말**이라
  //       그대로 explain이다 — 가르는 것은 조사 한 글자다.
  //   · 「오늘|지금|현재|상위|목록|N건」 — today·목록형 물음(「오늘 조치 우선순위 알려줘」·
  //     「우선순위 상위 5건 보여줘」)이 ∅(모델 재량)로 남게 두는 배제다. 그 갈래를 여기서
  //     가로채면 today의 실시간 데이터 답을 지식 답이 밀어낸다.
  //   · 「정하려면」은 **일부러 안 쓴다** — workflow_status[28]의 영토라 앞자리가 이미 지킨다
  //     (같은 신호를 두 곳에 적으면 어긋난다).
  //
  // 시험: server/test/priority-criteria-routing.test.ts.
  {
    re: /^(?![\s\S]*(문서|자료|파일|매뉴얼|가이드|지침|규정|법령|조문|판례|보고서|리포트|점검|정비|유지보수|어디))(?![\s\S]*(오늘|지금|현재|상위|목록|\d+\s*건))(?![\s\S]*(CVE-|방화벽|WAF|IPS|IDS|VPN|EDR|백신|안티바이러스|프록시|스위치|라우터|SIEM|DLP|백업))(?![\s\S]*(기준|지표)\s*(으로|대로)[^\n]{0,24}(정리|정렬|매겨|매기|뽑아|뽑|추려|골라|나열|줄\s*세))(?=[\s\S]*우선\s*순위)(?=[\s\S]*(기준|판단\s*지표|평가\s*지표|(무엇|뭐|뭘)(을|를)?\s*먼저|먼저\s*(봐|보|본|볼)|어떻게\s*(정|매기|판단|반영|적용)|(어떤|무슨)\s*순서|순서(로|대로)\s*(봐|보)|매길\s*때|정할\s*때))/i,
    tool: "explain",
    args: { topic: "취약점 우선순위" },
  },
  // ★★ 2026-09-13 B-2 ⑨ — 배열 **끝**에 새 규칙 2건. 자리 번호는 손으로 세지 않는다
  //   (node tools/routes-renumber.mjs --write). 둘 다 registry.ts 도구 설명이 스스로 든
  //   예시 문구인데 강제 규칙이 하나도 없어 ⑨(모델 재량)로 새고 있었다(실측 P20·P21·P22·P23).
  //
  //   [suggest_command] terminal.html data-prompts `"점검 명령 추천해줘"` · registry.ts:951
  //   예시 `"포트 스캔 명령 알려줘"`. write:false·directAnswer:true — 인자(request)는 모델이
  //   뽑는다(argsByModel, 위 register_incident_case와 같은 방식). ⚠ 「명령어 설명해줘」·
  //   「이 명령 뭐 하는 거야?」(지식 물음)를 안 삼키도록 트리거를 추천/알려/만들어로 좁힌다.
  //   ⚠ 이 도구는 조회로못박지않을것 Set에 **있다**(위 ⑦) — 「…추천해주고 맡겨줘」는 거기서 막힌다.
  {
  // ⚠ 2026-09-13 B-2 검토관 수리(적발 ⑤⑬) — **트리거를 좁힌 것만으로는 안 됐다.** 실측
  //   (수리 전, 전부 착수 전 ⑨): 「명령 승인 절차 알려줘」·「명령 실행 이력 알려줘」·「스크립트
  //   실행 결과 알려줘」·「이 스크립트 뭐 하는지 알려줘」→suggest_command. 이 도구는
  //   directAnswer:true(registry.ts)라 결재판도 없이 **PowerShell 한 줄이 바로 나간다** —
  //   절차·이력을 물은 담당자가 실행하라는 명령문을 받는다. 형제 규칙(maintenance_status·
  //   아래 run_redteam)은 절차·방법·이력·결과를 배제구로 갖고 있는데 이쪽만 빠져 짝이 안 맞았다.
  //   ⚠ 「방화벽 차단 명령 알려줘」·「포트 스캔 명령 알려줘」(P21)는 그대로 살아 있다.
    re: /^(?!.*(절차|방법|규정|이력|기록|감사|승인|권한|사용법|실행\s*결과|뭐\s*하는|무슨\s*일))[^\n]*?(명령|command|cli|powershell|스크립트)[^\n]{0,8}(추천|알려|만들어)/i,
    tool: "suggest_command",
    args: {},
    argsByModel: true,
  },
  // [run_redteam] session-explorer.js:57 `(a.name||a.id) + " 레드팀 점검해줘"` · screenguide.ts
  ...
  //   말인데 승인 창이 떠, 무심코 승인하면 살아 있는 모델에 **실제 공격이 발사**된다
  //   (registry.ts run_redteam 주석). 「해야|할까|할지|언제」를 배제해 묻는 말을 걸러낸다.
    re: /^(?!.*(결과|현황|이력|해야|할까|할지|언제))(?:.*)?(레드팀|red\s*team|견고성|프롬프트\s*인젝션)[^\n]{0,10}(점검|테스트|실행)\s*(해|해줘|해\s*주세요|시켜|돌려)/i,
    tool: "run_redteam",
    args: {},
    argsByModel: true,
  },
  ...
  // [96] 「이거 조치완료로 바꿔줘」·「이 취약점 조치 완료 처리해줘」 — 상태 변경은 결재판을 거치는 쓰기 도구로 못 박는다
  //   (2026-09-15 고객 QA 예행 ⑳). 규칙이 없어 모델이 답했고 세 번 연속 말을 바꿔 가며 완료를 지어냈다(「조치하였습니다」·
  //   「처리되었습니다」·「취약점 조치 완료.」) — 거짓 완료 관문은 뒤에서 잡는 그물이지 앞에서 길을 정하는 규칙이 아니다.
  //   대상은 update_finding_status의 autoFill이 화면에서 고른 항목(⌗이름::키)에서 채우고, 없으면 결재판이 빈 칸을 보여 사람이 채운다.
  //   ⚠ 「어떻게」·「절차」·「방법」·「뭐야」는 배제(안내·지식 물음) · 「오탐」·「위험수용」은 이 규칙의 영토가 아니다(다른 상태 말).
  {
    re: /^(?![\s\S]*(어떻게|절차|방법|뭐야|무엇|설명|오탐|위험\s*수용|알려줘|보여줘))[\s\S]*(조치\s*완료|완료)\s*(로|으로|처리|상태로)?\s*(바꿔|바꾸어|변경해|처리해|표시해|해)\s*(줘|주세요|달라|라)?\s*[.!]?\s*$/,
...
// ★ 2026-09-13 B-2 갈림 결정(가) — 「어떻게 해」 꼴을 더한다. screenguide.ts maintenance.html
//   can[]·handlers.ts runWorkSteps 되묻기 예시가 「방화벽 점검 어떻게 해?」를 안내로 적어
//   두는데 옛 규칙은 절차/방법/순서 같은 명사만 알아 못 지켰다(실측 P12). ⚠ dispatcher의
//   내할일절차질문([9])이 이 규칙([37])보다 **먼저** 돈다(chain 순서) — 열린 할 일 이름과
//   겹치면 여전히 work_steps가 이긴다. 겹치지 않을 때만 여기가 받는다.
// ⚠ 2026-09-13 B-2 검증 중 발견 — 「(점검|유지보수)[^\n]{0,8}어떻게」로 8자까지 열자 「점검서
//   올리려면 어떻게 해?」(점검**서** = 문서, 점검 절차 물음이 아니다)까지 삼켜 [12] 설정 켜는
//   법과 새 겹침을 냈다(--겹침 실측). 갭을 \s*(공백만)로 좁혀 「점검 어떻게 해?」류만 받는다.
const MAINT_PROCEDURE_RE =
  /((정기|월간|주간|분기|연간)\s*점검|유지보수(\s*점검)?)\s*.{0,8}(절차|방법|순서|단계|항목|체크\s*리스트)|(점검|유지보수)\s+(절차|방법|순서|단계|체크\s*리스트)|(점검|유지보수)\s*어떻게\s*(해|하나|하지)/;
...
/** 두 개념을 견주어 묻는 말인가(B7 ① 수리, 2026-09-11 · 검토관 수리 2026-09-12) — 「A랑 B 뭐가 달라?」 꼴.
 *  ⚠ 여기서 잡는 게 아니라 **forcedToolFor 맨 끝(FORCED 배열 뒤·제목지목질문 앞)**에서
 *  explain(topic=원문)으로 못 박는다 — 이 함수는 판정만 한다(시험이 이 함수를 그대로 부른다).
 *
 *  ★★ **쓰기 흐름 방어는 여기 낱말이 아니라 잣대 한 곳이 한다**(2026-09-12 검토관 [상] 수리).
 *    첫 판은 쓰기 동사를 손으로 베껴 적었고, 베끼다 빠진 말이 그대로 샜다 — 실측:
 *    「EPSS랑 VPR 차이 알려주고 **승인해줘**」·「취약점이랑 위협 차이 알려주고 **조치해줘**」·
 *    「…차이 알려주고 김보안한테 **할당해줘**」·「…**맡겨줘**」가 전부 explain 한 수로 끝나
 *    승인·조치·배정이 **영영 안 갔다**(2026-08-09 파일럿에서 today가 배정 명령을 흡수한 것과
 *    같은 사고). → 분기 조건에 **쓰기흐름인가()**(모듈 최상위·단일 출처의 잣대 하나)를
 *    태웠다. 「할당」은 그 잣대에도 없던 말이라 **잣대 쪽에 더했다**(두 곳에 적지
 *    않는다 — 아래 쓰기흐름인가() 머리글. 2026-09-12 B10-①로 지역 const에서 모듈 함수로
 *    뽑혔다 — 이 분기와 다른 넷이 같은 함수를 함께 부른다).
 *    ⚠ 아래 배제에 **남은** 쓰기 동사(배정·담아·추가해·등록해·삭제·지워·올려·보내·메일·
 *      만들어·생성해·작성해)는 지우지 않는다. 성질이 다르기 때문이다: 잣대는 **문장 끝
 *      명령꼴**만 보고, 배제는 **문장 아무 데나** 걸린다(「배정된 취약점이랑 미배정 취약점
 *      뭐가 달라?」는 명령이 아니라 우리 데이터 물음인데 잣대로는 안 걸린다). 잣대가 아는
 *      말도 배제에 남아 있는 것은 belt일 뿐 — **권위는 잣대**다.
 *
 *  ★★ **데이터 물음을 지식 답으로 못 박지 않는다**(2026-09-12 검토관 [상] 수리 · 두 갈래).
 *    explain은 topic으로 온톨로지·사내 문서만 뒤진다 — 우리 자산·스캔·건수·로그는 원천 자체가
 *    없다. 실측으로 샌 것들:
 *      ① 「sample-web01이랑 db-01 취약점 뭐가 달라?」·「KEV랑 EPSS 취약점 **건수** 차이 알려줘」·
 *        「지난 **스캔 결과** 차이 알려줘」·「방화벽이랑 IPS **로그** 건수 차이 알려줘」
 *        → 데이터 낱말·자산 이름 꼴을 배제로 막는다. 자산 이름 꼴은 **하이픈 뒤 숫자**로만
 *        본다(sample-web01·db-01) — 「ISO27001」·「CVSS 3.1」 같은 표준 이름을 같이 막지
 *        않으려고(실측: 양성 「ISMS-P랑 ISO27001 차이가 뭐야?」가 그 넓힘에 죽는다).
 *      ② 「스캔하고 나서 뭐가 **달라졌어**?」 — 두 개념 비교가 아니라 **시간에 따른 변화**다
 *        (연결어 「하고」가 동사 어미와 겹쳐 「스캔」+「나서」를 두 주제로 오인했다).
 *        차이어에서 「달라졌·달라진·달라지」를 뺀다 — 이 한 수로 「…하고 나서 뭐가 달라졌어」
 *        부류가 통째로 끊긴다.
 *
 *  ⚠ 배제어의 성질은 **낱말마다 다르다** — 「어차피 앞 규칙이 잡는다」로 읽지 말 것
 *    (2026-09-12 검토관 [하] 정정. 첫 판은 문서 계열을 통째로 「이중 방어」라 적었다).
 *    문서·규정·법령 계열이라도 앞 규칙이 **안 잡는** 말이 있다 — 실측: 「가이드와 매뉴얼 차이
 *    알려줘」는 제품설정질문·문서지목질문·사내규정질문 어디에도 안 걸려 ∅이고, 「가이드」·
 *    「매뉴얼」을 배제에서 빼면 곧바로 여기가 잡는다(그 문장은 배제어가 **유일한 방어**다).
 *    시점어(오늘|어제|작년…)·데이터 낱말도 마찬가지로 유일한 방어다. */
export function 비교개념질문(instruction: string): boolean {
  const 배제 = /(문서|자료|파일|매뉴얼|가이드|지침|규정|법령|법률|법적|판례|시행령|시행규칙|보호법|망법|제\s*\d+\s*조|보고서|리포트|오늘|어제|이번\s*주|지난\s*주|지난주|이번\s*달|지난\s*달|지난달|지난번|저번|작년|올해|전년|지금|현재|건수|몇\s*건|개수|로그|스캔\s*결과|(?:지난|이번|직전|이전)\s*스캔|우리\s*(자산|환경|장비|서버)|사내\s*자산|배정|담아|추가해|등록해|삭제|지워|올려|보내|메일|만들어|생성해|작성해)/;
  // 자산 이름 꼴 — 하이픈 뒤에 숫자가 오는 영숫자 토큰(sample-web01 · db-01 · CVE-2021-44228).
  // ⚠ 하이픈을 **요구**한다. /[A-Za-z][A-Za-z0-9]*\d{2,}/로 넓히면 「ISO27001」·「ISMS-P랑
  //   ISO27001 차이가 뭐야?」 같은 **표준 이름 비교**(양성)까지 막는다(실측으로 확인).
  const 자산이름꼴 = /[A-Za-z][A-Za-z0-9]*-[A-Za-z0-9]*\d/;
  const 연결 = /[가-힣A-Za-z0-9._-]{2,}\s*(?:이랑|랑|과|와|하고|vs\.?)\s*[가-힣A-Za-z0-9._-]{2,}/i;
  const 차이어 = /(달라(?!졌|진|지)|달랐|다릅|다른\s*(?:점|지|가|데|거|건)|차이|비교)/;
  return !배제.test(instruction) && !자산이름꼴.test(instruction) && 연결.test(instruction) && 차이어.test(instruction);
}

/** 화면 **구역 이름**을 통째로 부른 말인가 — 그 뜻풀이는 screenguide가 답한다(2026-09-12 수리).
 *  ■ 왜 있나(검토관 [중] 실측): mydocs 구역 이름이 **「AI 포함과 공유의 차이」**라서, 위
 *    비교개념질문이 그 이름을 통째로 삼켰다. 「AI 포함과 공유의 차이 알려줘」가 착수 전엔
 *    화면 안내(그 구역 본문에 뜻풀이가 **글자 그대로** 있다)였는데 explain(RAG — 원천이 없다)
 *    으로 갔다. 2026-07-25 「진행내역 리포트가 뭐야?」가 RAG로 새어 벤더 매뉴얼로 답한 사고의
 *    계보이고, 제품 원칙(「화면 설명·용어 풀이는 screenguide」)도 같은 쪽이다.
 *  ■ **screenguide 쪽에서 풀지 않는다.** 그 파일의 전수 감시가 「알려줘 ↔ 보여줘는 갈리지
 *    않는다」를 재는데, 안내가 이기는 게이트(홑물음_RE)에 「보여」가 없어 사유를 늘리면 같은
 *    뜻의 두 말이 갈린다(실측으로 빨간불을 보고 되돌렸다). **뿌리에서 안 잡는 것**이 답이다 —
 *    여기서 null을 내면 두 말 다 착수 전 그대로다(fa·fb 둘 다 null이라 감시도 안 센다).
 *  ⚠ 구역 이름은 짧은 것이 많아(최소 2글자: 「지도」·「주의」) **6글자 이상**만 본다 —
 *    짧은 이름까지 보면 흔한 낱말이 양성을 죽인다. 실측: 이 잣대에 걸리는 구역 이름은 165개
 *    중 **하나**(AI 포함과 공유의 차이)뿐이고, 양성 18문장은 **0개**가 걸린다.
 *  ⚠ **부르는 것은 함수 안에서만** 한다(위 import는 이름만 가져온다) — screenguide가 이 파일을
 *    이미 부르고 있어 순환이다. 최상위에서 **부르면** 초기화에 걸리지만(screenguide 머리글의
 *    경고가 그것이다), 함수 안에서 부르면 그때는 양쪽이 다 올라와 있다. `require`는 못 쓴다 —
 *    시험은 .ts를 그대로 도는데 확장자 없는 require가 안 풀린다(실측: Cannot find module). */
function 구역이름물음(instruction: string): boolean {
  const q = instruction.replace(/\s/g, "");
  return 구역이름들().some((a) => {
    const n = a.name.replace(/\s/g, "");
    return n.length >= 6 && q.includes(n);
  });
}
...
//   여기 더한 것과 같은 수리다).
// ⚠ exec_brief를 2026-09-11에 더했다(B4 수리와 같은 라운드). 「임원 보고용으로 세 줄 요약해서
...
// ★ 2026-09-13 B-2 ⑦ — list_assets·threats·suggest_command·knowledge_status·scan_status를 더한다.
//   순서가 중요하다(설계관 지시 — **⑧(FORCED 규칙 넓힘)보다 먼저** 한다): list_assets·threats·
...
export function forcedToolFor(instruction: string, scope?: ToolScope): 강제결과 | null {
  ...
  // ★★ 2026-09-12 B10-① — **쓰기 흐름이면 비켜선다.** 이 분기는 FORCED_INTENTS 배열 **밖**이라
  //   조회로못박지않을것 Set(위 `쓰기흐름인가()` 사용처)을 원리상 안 지난다 — 그래서 잣대를
  //   손으로 건다. 실측(착수 전): 「방화벽 룰셋 어떻게 확인해? 김보안한테 배정해줘」가 이 분기
  //   하나로 끝나 배정이 영영 안 갔다(P11).
  if (available.has("explain") && !쓰기흐름인가(instruction) && 제품설정질문(instruction)) {
  ...
  // ★★ 2026-09-12 B10-① — **쓰기 흐름이면 비켜선다**(배열 밖이라 Set을 안 지난다 — 위
  //   제품설정질문과 같은 이유). 실측(착수 전): 「이 매뉴얼에서 암호화 대상 알려주고 담당자
  //   배정해줘」가 이 분기 하나로 끝나 배정이 영영 안 갔다(P06).
  if (available.has("explain") && !쓰기흐름인가(instruction) && 문서지목질문(instruction)) {
    return { tool: "explain", args: { topic: instruction } };
  }

  // ★★ 2026-09-12 B10-① — **쓰기 흐름이면 비켜선다**(배열 밖이라 Set을 안 지난다). 실측
  //   (착수 전): 「접속기록 보관 규정 알려주고 김보안한테 배정해줘」가 이 분기 하나로 끝나
  //   배정이 영영 안 갔다(P01 — 이 라운드 착수 계기가 된 바로 그 문장).
  if (available.has("explain") && !쓰기흐름인가(instruction) && 사내규정질문(instruction)) {
  ...
  for (const f of FORCED_INTENTS) {
    if (f.re.test(instruction) && available.has(f.tool)) {
      ...
      // ⚠ 2026-09-08: **배정 외의 쓰기 명령**도 같은 자리에서 막는다(모듈 위쪽 쓰기흐름인가() 머리글).
      if (조회로못박지않을것.has(f.tool) && 쓰기흐름인가(instruction)) continue;
      if (f.tool === "search") {
      ...
      // ★ 2026-09-13 B-2 검토관 수리(적발 ⑭) — 「○○ 점검 일정 알려줘」의 ○○를 filter로 넘긴다.
      //   session-explorer.js:89가 `pd.name + " 점검 일정 알려줘"`로 **보안제품 이름을 넣어** 보내는
      //   말인데, 이 규칙을 넓히고도 args가 `{}`라 handlers.ts runMaintenanceStatus가 **전체 일정**을
      ...
      if (f.tool === "maintenance_status") {
        ...
        const 갈래말만 = /^(보안\s*제품|제품|장비|자산|설비|시설|점검)$/;
        if (앞말 && !뜻없는수식어.test(앞말) && !갈래말만.test(앞말)) return { tool: f.tool, args: { filter: 앞말 } };
      }
  ...
  // ★★ 비교 개념 물음 — **배열 뒤, 제목지목질문 바로 앞**(B7 ①, 2026-09-11).
  //   ⚠ **자리가 곧 방어다** — 배열(FORCED_INTENTS 91개)이 먼저 돌므로 그 안에 이미 도착지가
  //     정해진 물음은 안 뺏긴다(착수 전 실측: 「SBOM이랑 AI-BOM 차이가 뭐야?」→aibom_status·
  //     「취약점이랑 위협 차이를 문서로 정리해줘」→[24] kbhygiene·「EPSS와 VPR 차이 정리해서
  //     보고서로 만들어줘」→[36] generateReport, 전부 그대로 이긴다). 양성 18/18 걸림,
  //     음성 36문장 중 도착지 바뀐 것 0 + 새로 걸린 것 1건(「우리 방화벽이랑 IPS 뭐가 달라?」 —
  //     장비 비교도 explain이 사내 문서·온톨로지·보유 제품을 모아 답하므로 타당한 도착이다).
  //   ⚠ 배열을 안 건드리므로 routes-renumber는 필요 없다(routes.test는 FORCED_INTENTS[n] 행만 센다).
  //   왜 필요한가(B7, 2026-09-11 22:33 야간 회귀 예행 + 4100 라이브 재현): 「스캔 실패랑
  //     취약점이 뭐가 달라?」·「EPSS랑 VPR 뭐가 달라?」류가 route-explain 실측 전부 「걸리는
  //     규칙 없음 → ⑨ 모델 선택」이었다. 그날그날 도구 카탈로그의 설명 줄과 낱말이 가까운
  //     도구가 골라졌다 — 4100 라이브에서 「스캔 실패랑 취약점이 뭐가 달라?」에 fp_patterns
  //     (오탐 패턴 도구)가 골라져 「오탐 자주 나는 패턴 — …」이라는 엉뚱한 답이 나갔다.
  //   ⚠ **topic=지시문 원문**이다(「A vs B」로 합성하지 않는다) — runExplain(handlers.ts)은
  //     topic 하나를 제목지목문서·ontologyLinesFor·queryMemoryGraded·문서 필터에 그대로
  //     넘기고, expandOntology(ontology.ts)는 정규화 후 **부분문자열 포함**이라 원문에
  //     EPSS·VPR이 그대로 있으면 두 씨앗이 함께 걸린다(ontology-seed.ts에 EPSS·VPR 실재).
  //     합성하면 RAG 질의가 담당자 말과 달라지고 제목지목매치 토큰도 준다 — 얻을 것이 없다.
  //   ⚠ `데이터의존`을 안 붙인다 — 이 갈래는 순수 글자 판정이라 빈 DB에서도 「간다」고
  //     단정해도 되는 갈래다(제목지목질문과 다른 점 — 그건 문서 목록 조회에 달렸다).
  //   ★★ **쓰기 흐름은 잣대 한 곳으로 비켜 준다**(2026-09-12 검토관 [상] 수리 · B10-①로
  //     지역 const에서 모듈 함수 `쓰기흐름인가()`로 뽑힘) — 이 분기는 배열 밖이라
  //     조회로못박지않을것 Set을 안 지나므로(그 Set은 루프 안에서만 돈다), **같은 잣대를
  //     손으로 걸어 준다.** 낱말을 비교개념질문 안에 또 적지 않는다 — 베낀 목록은 반드시
  //     어긋난다(그 실측이 이 수리의 뿌리다: 승인해줘·조치해줘·할당해줘·맡겨줘가 전부
  //     explain 한 수로 끝났다).
  //   ★★ **화면 구역 이름은 안 뺏는다**(2026-09-12 검토관 [중] 수리) — 구역이름물음()이 참이면
  //     비켜선다. 구역 이름 자체가 「AI 포함과 공유의 차이」인 자리가 있어 이 분기가 그 화면의
  //     뜻풀이를 통째로 가져갔다(그 답은 screenguide 본문에 글자 그대로 있고 RAG엔 없다).
  if (available.has("explain") && !쓰기흐름인가(instruction) && 비교개념질문(instruction) && !구역이름물음(instruction)) {
    return { tool: "explain", args: { topic: instruction } };
  }
  ...
  // ★★ 2026-09-12 B10-① — **쓰기 흐름이면 비켜선다**(배열 밖이라 Set을 안 지난다). 제목지목질문
  //   (memory.ts)은 이미 행동의도RE로 「삭제·배정·올려…」류를 걸러내지만, 그 목록에 「맡겨·정해·
  //   조치해」가 없어 **행동의도RE만으로는 못 닫는 구멍이 남는다** — 서로 포함 관계가 아니다
  //   (memory.ts 행동의도RE 머리글 참고). 실측(운영 DB 사본, 착수 전): 「취약점 식별체계 기준
  //   알려주고 김보안한테 맡겨줘」가 제목지목질문=true·행동의도RE=false로 이 분기에 걸려
  //   explain 한 수로 끝났다(T1). 쓰기흐름인가()가 「맡겨」를 알아 이 구멍을 닫는다.
  if (available.has("explain") && !쓰기흐름인가(instruction) && 제목지목질문(instruction)) {
