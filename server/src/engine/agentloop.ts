...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  {
    re: /^(?!.*(해도\s*(돼|되나|될까|괜찮)|위반이야))(?=.*(법령|법률|시행령|시행규칙|고시|훈령|예규|행정규칙|판례|판결|보호법|기본법|거래법|촉진법|특별법|진흥법|방지법|관리법|보안법|망법|신용정보법|보관\s*기간|몇\s*년\s*(동안\s*)?보관|얼마나\s*(동안\s*)?보관))(?=.*(찾아|알려|보여|검색|뭐라고|어떻게\s*(돼|되어)|있어|있나|있는지|해야\s*(해|돼|되나|하나)))/,
    tool: "law_lookup",
    args: {},
  },
