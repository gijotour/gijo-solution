...
/** 두 개념을 견주어 묻는 말인가(B7 ① 수리, 2026-09-11) — 「A랑 B 뭐가 달라?」 꼴.
 *  ⚠ 여기서 잡는 게 아니라 **forcedToolFor 맨 끝(FORCED 배열 뒤·제목지목질문 앞)**에서
 *  explain(topic=원문)으로 못 박는다 — 이 함수는 판정만 한다(시험이 이 함수를 그대로 부른다).
 *  배제어의 성질이 낱말마다 다르다: 문서·규정·법령 계열(문서|자료|파일|매뉴얼|가이드|지침|
 *  규정|법령…)은 제품설정질문·문서지목질문·사내규정질문이 forcedToolFor **앞쪽**에서 이미
 *  잡으므로 여기 배제는 이중 방어이고, 시점어(오늘|어제|작년…)·쓰기 동사(배정|만들어…)는
 *  **유일한 방어**다 — 이 분기는 FORCED_INTENTS 배열 밖이라 조회로못박지않을것 Set을 안
 *  지난다(그 Set은 배열 루프 안에서만 돈다). 배제어에서 쓰기 동사를 빼면 「VPN이랑
 *  제로트러스트 차이 알려주고 김보안한테 배정해줘」가 explain 한 수로 끝나 배정이 안 간다. */
export function 비교개념질문(instruction: string): boolean {
  const 배제 = /(문서|자료|파일|매뉴얼|가이드|지침|규정|법령|법률|법적|판례|시행령|시행규칙|보호법|망법|제\s*\d+\s*조|보고서|리포트|오늘|어제|이번\s*주|지난\s*주|지난주|이번\s*달|지난\s*달|지난달|작년|올해|전년|지금|현재|우리\s*(자산|환경|장비|서버)|사내\s*자산|배정|담아|추가해|등록해|삭제|지워|올려|보내|메일|만들어|생성해|작성해)/;
  const 연결 = /[가-힣A-Za-z0-9._-]{2,}\s*(?:이랑|랑|과|와|하고|vs\.?)\s*[가-힣A-Za-z0-9._-]{2,}/i;
  const 차이어 = /(달라|달랐|다릅|다른\s*(?:점|지|가|데|거|건)|차이|비교)/;
  return !배제.test(instruction) && 연결.test(instruction) && 차이어.test(instruction);
}
...
//   그래서 Set에 explain을 넣어도 **배열 밖의 손수 explain 분기 넷은 이 가드를 안 지난다** —
//   제품설정질문 · 문서지목질문 · 사내규정질문(배열 **앞**) · 제목지목질문(배열 **뒤**).
...
export function forcedToolFor(instruction: string, scope?: ToolScope): 강제결과 | null {
  ...
  const isWriteOrder =
    /(승인|반려|조치|처리|마감|종결|차단|삭제|제거|회수|배정)\s*해\s*(줘|주세요|라|다오)?\s*[.!?？~]*\s*$|(지워|없애)\s*(줘|주세요)?\s*[.!?？~]*\s*$/.test(instruction);
  ...
  if (available.has("explain") && 비교개념질문(instruction)) {
    return { tool: "explain", args: { topic: instruction } };
  }
