...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  {
    re: /(모델|얘|이거).{0,10}(생각|thinking|추론)\s*(모드)?\s*(를|은|는)?\s*(꺼|끄|켜|켤|지정)/i,
  ...
  {
    re: /(모델|엔진)\s*(자동\s*)?(적응|맞춤|스모크)\s*(상태|결과|내용|어때|보여|알려)?|올린\s*모델.{0,10}(괜찮|잘\s*도|맞춰|상태|점검|검증|어때)|모델\s*(검증|점검)\s*(해|좀|해줘|결과)/,
