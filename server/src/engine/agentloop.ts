...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //     · briefing → briefing.ts:66 newFindings = 지난 스냅샷 대비 **차집합**. 여기 하나뿐이다.
  ...
  //   ⚠ **한계를 알고 쓴다**: newFindings는 「밤사이」가 아니라 「지난 브리핑 이후」다. 야간
  //     하네스가 매일 03:04에 브리핑을 부르며 스냅샷을 먹으므로 사람의 아침 브리핑은 사실상
  //     「03:04 이후」만 센다. 이 규칙이 만든 결함이 아니라 briefing이 원래 그렇다.
  //
  ...
  // ⚠ 매체 낱말(뉴스·유튜브·채널·블로그·사이트·소식)을 뺀다 — 「간밤에 터진 뉴스 알려줘」는
  //   incident_sources의 영토다(지금은 사건어가 없어 안 걸리지만 「터진」이 붙으면 걸린다).
  // ⚠ 쓰기 흐름은 삼키지 않는다 — 「담아·추가해·등록해」는 add_task, 「다시 넣어」는 reingest다.
  //   **배정 지시**는 여기 정규식이 아니라 아래 `조회로못박지않을것`이 막는다(잣대 한 곳).
  // ⚠ 「보고서·리포트」는 적지 않는다 — 적용부의 briefing 공통 분기가 이미 비켜 준다(같은 이유).
  {
    re: /^(?![\s\S]*(사례|히스토리|뉴스|유튜브|채널|블로그|소식|사이트|절차|방법|어떻게\s*[하해]|담아|추가해|등록해|등록\s*:|다시\s*넣|재인입))(?=[\s\S]*(간밤|밤새|어젯밤|지난밤|밤사이))(?=[\s\S]*(터진|터졌|터질|생긴|생겼|있었|일어난|일어났|(?<![지끝])났))(?=[\s\S]*(있|없|뭐|무엇|무슨|알려|보여|확인|현황|목록|어때|어떤|정리|요약|[?？]))/,
    tool: "briefing",
    args: {},
  },
];
