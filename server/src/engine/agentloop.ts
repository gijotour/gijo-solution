...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  {
    re: /^(?!.*(해도\s*(돼|되나|될까|괜찮)|위반이야))(?=.*(법령|법률|시행령|시행규칙|고시|훈령|예규|행정규칙|판례|판결|보호법|기본법|거래법|촉진법|특별법|진흥법|방지법|관리법|보안법|망법|신용정보법))(?=.*(찾아|알려|보여|검색|뭐라고|어떻게\s*(돼|되어)))/,
    tool: "law_lookup",
    args: {},
  },
  ...
  {
    re: /((SBOM|부품표)[\s\S]{0,10}(검수|점검)[\s\S]{0,8}(결과|현황|알려|보여)|공급망[\s\S]{0,4}점검|소스[\s\S]{0,2}공개[\s\S]{0,6}(요구|의무)[\s\S]{0,10}(부품|뭐|무엇|있))/i,
