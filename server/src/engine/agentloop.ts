...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  // ⚠ 2026-09-10 고객 QA 예행 ㉔ 배제구 추가 — 조회 꼴(「점검 안 한 장비 있어?」류)까지
  //   실행으로 채여 우리 호스트 하드닝 결과가 그대로 나갔다(datacard.isHardeningStatusAsk가
  //   앞에서 먼저 받는 것이 1차 방어, 이건 2차 방어). ⚠ `[\s\S]*`를 안쪽에 넣지 않으면
  //   `^`가 본문까지 앵커해 실행 명령형까지 통째로 죽는다(evalgate 2문항 재현). 배제어는
  //   조회·부정형 낱말: 안 한/된/했/하고·미점검·현황·상태·어때·결과·목록·어디·어느·누가·몇건개.
  //   시험: server/test/hardening-unchecked-routing.test.ts
  {
    re: /^(?![\s\S]*(안\s*(한|된|했|하고)|미점검|현황|상태|어때|결과|목록|어디|어느|누가|몇\s*[건개]))(?:[\s\S]*(?:(하드닝|보안\s*설정)\s*(점검|진단|스캔|체크)|CCE\s*(점검|진단|기준)|(장비|서버|시스템)\s*(보안\s*)?(점검|진단)해|기준.{0,3}(점검|진단)|취약점\s*진단해|정기\s*점검\s*(돌려|실행|해)))/,
