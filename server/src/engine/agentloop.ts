...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  //   MD5 해시까지 늘어놓았다. 목록 질문은 목록 도구가 답한다.
  {
    ...
    re: /(최근|요즘|새로)\s*(에\s*)?(올린|등록한|추가한)\s*(문서|자료|파일)|문서\s*(목록|리스트)|올린\s*문서\s*(뭐|무엇|보여|알려|잘|제대로)|(문서|자료).{0,8}(반영|들어갔|수집)(됐|되었|된)/,
  ...
  // ⚠ 2026-08-13 수리 — **어순을 한쪽만 받고 있었다.** 라이트 게이트 lt-dup-1
  //   「내 문서함에 중복된 거 있어?」가 안 걸려 모델 재량으로 넘어갔다(max 실측).
  //   한국어는 두 어순이 다 자연스럽다: 「중복된 문서」와 「문서함에 중복된 거」.
  //   앞 낱말만 보고 규칙을 쓰면 **담당자가 실제로 치는 말**을 놓친다 — 「구어체로 물으면
  //   안 닿는다」와 같은 계열이다. 뒤 어순 가지는 사이를 {0,6}으로 좁게 잡아
  //   「문서 등록 절차에 중복이…」 같은 먼 동거는 안 삼킨다.
  {
