...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //   코드는 정반대다(FORCED 루프 2626행 → 제목지목질문 2813행, `return null` 바로 앞). 그 자리는
  ...
  //   findingListAnswer(체인 [14], 「취약점 현황 요약해줘」) · assetStatusAnswer([18]) ·
  //   screenguide([21], 「취약점 우선순위 정하려면 뭘 봐야 해?」).
  //
  // ④ **배제어가 유일한 방어다** — 문서/지침/규정 계열은 이 규칙보다 **뒤에 오는**
  //   문서지목질문(2559)·사내규정질문(2563)·제목지목질문(2855)의 영토라, 배제어를
  //   지우는 순간 그 셋을 가로챈다(실측: 「우선순위 기준 규정 알려줘」는 지금
  //   explain(사내규정질문)이고 배제어 덕에 그대로다). 「오늘|지금|현재|상위|목록|N건」은
  //   today·목록형 물음(「오늘 조치 우선순위 알려줘」·「우선순위 상위 5건 보여줘」)이
  //   ∅(모델 재량)로 남게 두는 배제다 — 그 갈래를 여기서 가로채면 today의 실시간 데이터
  //   답을 지식 답이 밀어내는 새 오라우팅이 생긴다. 「정하려면」은 **일부러 안 쓴다** —
  //   workflow_status[28]의 영토라 앞자리가 이미 지킨다(같은 신호를 두 곳에 적으면 어긋난다).
  //
  // 시험: server/test/priority-criteria-routing.test.ts.
  {
    re: /^(?![\s\S]*(문서|자료|파일|매뉴얼|가이드|지침|규정|법령|조문|판례|보고서|리포트|점검|정비|유지보수|어디))(?![\s\S]*(오늘|지금|현재|상위|목록|\d+\s*건))(?=[\s\S]*우선\s*순위)(?=[\s\S]*(기준|판단\s*지표|평가\s*지표|(무엇|뭐|뭘)(을|를)?\s*먼저|먼저\s*(봐|보|본|볼)|어떻게\s*(정|매기|판단|반영|적용)|(어떤|무슨)\s*순서|순서(로|대로)\s*(봐|보)|매길\s*때|정할\s*때))/,
    tool: "explain",
    args: { topic: "취약점 우선순위" },
  },
];
