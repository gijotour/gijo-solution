...
//
// ⚠ 배제어는 물려받는다 — 위 리터럴의 `^(?!…)` 선행부정은 정규식 앞머리에 있어 **더한 갈래에도
//   그대로 걸린다**(절차·방법·기준·법령·규정·지침·뜻·정의·설명·업계·타사·국내·평균적·낮추·올리…).
// ⚠ 탐지·차단·검출은 **주체어 갈래 안에서 따로 막는다**(위 배제어에 넣으면 꺼진 기본형까지
//   글자가 바뀐다). 이 파일이 이미 정한 계약이라 게이트가 뒤집으면 안 된다:
//   「IPS 탐지율」·「WAF 차단율」은 사내 실적이 아니라 **제품 스펙**을 묻는 말이다.
//   주어만 보면 「우리 IPS 탐지율」이 걸리므로, 주체어와 「율」 사이에 그 낱말이 오면 비켜선다.
const KPI_주체어_갈래 =
  String.raw`(?:우리|사내|자사|당사|우리\s*회사)\s*(?:(?!탐지|차단|검출)[^\n]){0,20}?(?:율|률|비율|평균|건수)`;

/** 게이트가 켜졌을 때 [83]에 주체어 갈래를 **OR로 더한** 정규식. 시험이 직접 문다. */
export function kpi주체어갈래붙이기(base: RegExp): RegExp {
  const s = base.source;
  // 마지막 `)`는 `[\s\S]*(?:…)`의 닫는 괄호다 — 그 안에 갈래를 하나 더 넣는다.
  // ⚠ 모양이 달라졌으면 **아무것도 안 한다**(기본 규칙 그대로). 틀린 정규식을 조립해서
  //   조용히 다른 것을 잡는 쪽이, 게이트가 안 켜지는 것보다 훨씬 비싸다.
  if (!s.startsWith("^(?!") || !s.endsWith(")")) return base;
  return new RegExp(`${s.slice(0, -1)}|${KPI_주체어_갈래})`, base.flags);
}

if (process.env.GIJO_KPI_SUBJECT_RULE === "1") {
  const 자리 = FORCED_INTENTS.length - 1; // [83] 사내 지표율 — 배열 **맨 끝**이다
  const 원본 = FORCED_INTENTS[자리];
  if (원본?.tool !== "kpi_status") {
    // 자리가 밀렸다 — 엉뚱한 규칙을 넓히느니 아무것도 안 한다(자리 번호 사고의 계보).
    console.warn("[agentloop] GIJO_KPI_SUBJECT_RULE — 배열 끝이 kpi_status가 아니라 주체어 규칙을 안 켰습니다.");
  } else {
