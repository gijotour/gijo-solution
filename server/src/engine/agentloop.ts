...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  },
  {
    ...
    re: /^(?![\s\S]*(?:절차|방법|기준|법령|규정|지침|뜻|정의|소개|설명|뭐야|뭐예요|뭐에요|뭔데|뭔가|무엇|이란|어떻게\s*(?:계산|산출|구하|구해|재|측정|올리|올려|올릴|높이|높여|높일|줄이|줄여|개선)))[\s\S]*(?:(?:조치|완료|준수|대응|이행|패치|적용|이수|등록)\s*(?:완료\s*)?(?:율|률)|SLA\s*준수|(?:작년|전년|올해|전년도)\s*(?:대비|보다))/i,
    tool: "kpi_status",
    args: {},
  },
];
