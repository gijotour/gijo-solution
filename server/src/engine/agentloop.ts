...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //   ⒞ 지식 물음(기준·절차·방법·어떻게·왜·규정·정책·뜻·이란 — 「결재 대기 승인 절차
  //   알려줘」는 안내·RAG의 몫이다).
  ...
  {
    re: /^(?![\s\S]*(점검|정비|유지보수))(?![\s\S]*(승인|반려|결재)[^.\n]{0,8}(해\s*줘|해\s*주세요|해라|처리해|눌러|올려))(?![\s\S]*(기준|절차|방법|어떻게|왜|규정|정책|뜻|이란))(?=[\s\S]*(결재|승인))(?=[\s\S]*(대기|기다리))(?=[\s\S]*(있|없|뭐|무엇|몇|건수|목록|보여|알려|현황|확인))/,
