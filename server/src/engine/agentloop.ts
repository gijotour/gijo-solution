 ...
 *    같은 사고). → 분기 조건에 **isAssign·isWriteOrder**(같은 함수 위쪽에서 이미 계산된
 *    단일 잣대)를 태웠다. 「할당」은 그 잣대에도 없던 말이라 **잣대 쪽에 더했다**(두 곳에 적지
 *    않는다 — 아래 isWriteOrder 머리글).
...
//   김보안한테 배정해줘」를 조회로 끝내면 배정이 영영 안 된다 — 위 briefing과 같은 구멍이다.
...
//   그래서 Set에 explain을 넣어도 **배열 밖의 손수 explain 분기 다섯은 이 가드를 안 지난다** —
//   제품설정질문 · 문서지목질문 · 사내규정질문(배열 **앞**) · 비교개념질문 · 제목지목질문(배열 **뒤**).
//   ⚠ 2026-09-11 B7이 다섯째(비교개념질문)를 더했고, **그 하나만** 2026-09-12 수리로 이 잣대를
//     손수 걸어 닫았다(`!isAssign && !isWriteOrder` — 아래 그 분기 주석). 나머지 넷은 그대로
//     열려 있다 — 이 숫자는 「다 막았다」가 아니라 **남은 구멍의 수**를 세는 자리다.
//   실측(2026-09-11 검토관 [하]): 「접속기록 보관 규정 알려주고 김보안한테 배정해줘」는 지금도
//   forcedToolFor → explain(topic 「접속기록 보관 규정」)으로 한 수에 끝나 배정이 안 간다 —
//   사내규정질문이 루프 앞에서 잡기 때문이다. **B5 이전부터 있던 구멍이고 이 라운드가 닫지 않았다**
//   (닫으려면 네 분기에도 같은 잣대를 걸어야 하는데, 그건 이 라운드 범위 밖이라 백로그로 남긴다).
//   → 「explain으로 가는 길은 다 막았다」로 읽지 말 것. 막힌 것은 **배열 경로**뿐이다.
...
export function forcedToolFor(instruction: string, scope?: ToolScope): 강제결과 | null {
  ...
  //   ★★ **쓰기 흐름은 잣대 한 곳으로 비켜 준다**(2026-09-12 검토관 [상] 수리) —
  //     `!isAssign && !isWriteOrder`. 이 분기는 배열 밖이라 조회로못박지않을것 Set을 안
  //     지나므로(그 Set은 루프 안에서만 돈다), **같은 잣대를 손으로 걸어 준다.** 낱말을
  //     비교개념질문 안에 또 적지 않는다 — 베낀 목록은 반드시 어긋난다(그 실측이 이 수리의
  //     뿌리다: 승인해줘·조치해줘·할당해줘·맡겨줘가 전부 explain 한 수로 끝났다).
  ...
  if (available.has("explain") && !isAssign && !isWriteOrder && 비교개념질문(instruction) && !구역이름물음(instruction)) {
    return { tool: "explain", args: { topic: instruction } };
  }
