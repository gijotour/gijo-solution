...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //   "취약점 몇 건이야"처럼 **세는 질문**은 목록이 맞으므로 비켜 준다(아래 적용부에서 거른다).
  {
    ...
    re: /^(?!.*(기한|SLA|승인|결재|간밤|밤새))(?!.*[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}\s*(?:이랑|랑|와|과)\s*[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}[^\n]*취약점)[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}(?:\s+[가-힣ㄱ-ㅎㅏ-ㅣA-Za-z0-9._-]{2,}){0,2}\s*(웹\s*)?(서버|자산|호스트|시스템|장비)?\s*(의|에)?\s*취약점.{0,6}(알려|보여|뭐(?!가?\s*달라)|무엇|있어|있나|현황|정리)/,
    tool: "search",
    args: {},
  },
  {
  ...
  //   `유지보수·정비` 낱말을 못 박고, 절차·방법을 묻는 지식 질문은 위와 같은 방식으로 비켜 준다.
  {
    ...
    re: /^(?!.*(절차|방법|항목\s*설명|순서|체크\s*리스트|어떻게\s*하|하드닝|원격|보안\s*설정))(?:.*(유지\s*보수|유지보수|정비)\s*(점검|일정)?\s*[^.\n]{0,8}(뭐|무엇|남았|현황|어때|보여|알려|확인|밀린|지난)|.*점검\s*일정[^.\n]{0,8}(뭐|무엇|알려|보여|확인|있|없))/,
  ...
  //   그 규칙이 배열에서 **앞에 있어** 먼저 잡는다(순서가 곧 우선순위).
  {
    ...
    re: /(자산|장비|서버|호스트)[^\n]{0,8}(몇\s*(대|개|건)|개수|총\s*몇)|자산\s*(총계|전체)\s*(몇|얼마)|(자산|장비|서버|호스트)\s*(목록|리스트)[^\n]{0,10}(보여|알려)/,
    tool: "list_assets",
    args: {},
  },
  ...
  {
    re: /(명령|command|cli|powershell|스크립트)[^\n]{0,8}(추천|알려|만들어)/i,
  ...
  //   자기 도구에서 풀리면 안 된다.
  {
    re: /^(?!.*(결과|현황|이력))(?:.*)?(레드팀|red\s*team|견고성|프롬프트\s*인젝션)[^\n]{0,10}(점검|테스트|실행)\s*(해|해줘|해\s*주세요|시켜|돌려)/i,
    tool: "run_redteam",
    args: {},
    argsByModel: true,
  },
