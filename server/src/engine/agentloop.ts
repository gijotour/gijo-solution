...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string>; argsByModel?: true }[] = [
  ...
  //   붙인 뒤 반드시 `node tools/routes-renumber.mjs --write` — 손으로 세지 않는다.
  // ★ **앞자리가 곧 방어다** — FORCED 88개와 아래 제목지목질문 갈래가 이 규칙보다 먼저 돈다
  //   (배열은 위에서부터 첫 매치로 끝나고, 제목지목질문은 이 배열 자체를 **다 본 뒤**에 온다).
  //   실측(2026-09-11): 「취약점 현황 요약해줘」→findingListAnswer[14] · 「자산 현황 요약해줘」
  //   →assetStatusAnswer[18] · 「최근 위협 인텔 요약해줘」→threats · 「컴플라이언스 현황
  //   요약해줘」→compliance_status · 「보안 현황 보고서 만들어줘」·「임원 보고용 리포트
  //   만들어줘」→generateReport[36] — 전부 이 규칙 앞에서 먼저 잡혀 안 뺏긴다.
  // ⚠ 배제어의 「문서·파일·매뉴얼·가이드·지침」은 장식이 아니라 계약이다 — 없으면 「○○ 문서
  //   세 줄로 요약해줘」가 제목지목질문(explain)의 몫을 이 규칙이 가로챈다(실측: 지금은 explain).
  {
    re: /^(?![\s\S]*(문서|파일|보고서|리포트|회의|메일|기사|뉴스|논문|계약서|매뉴얼|가이드|지침|법령|조문|판례|로그|CVE-|점검|정비|유지보수|이력|절차|방법|뜻|이란))(?=[\s\S]*((임원|경영진|상급자|윗분|대표)[^\n]{0,8}보고|보고용|(보안|전사|전체)\s*(현황|상황)))(?=[\s\S]*(요약|정리|간단|간략|짧게|(세|3)\s*줄|한\s*줄))/,
    tool: "exec_brief",
    args: {},
  },
];
