...
const FORCED_INTENTS: { re: RegExp; tool: string; args: Record<string, string> }[] = [
  ...
  },
  {
    ...
    //       여기(조회)가 맞다.
    re: /^(?![\s\S]*(기재|기입|적어|채워)[\s해]*(줘|주세요|주라|둬|넣어))[\s\S]*ai[-\s_]?bom/i,
    tool: "aibom_status",
    args: {},
  },
  {
  ...
  },
  {
    ...
    re: /^(?![\s\S]*(뭐야|뭔가|무엇|이란|란[\s?]|설명|어떻게|왜\s|방법|절차))[\s\S]*(VEX|vex\s*(문서|파일|내보내|현황))/,
    tool: "vex_status",
    args: {},
  },
  {
    ...
    re: /^(?![\s\S]*(현황|보여\s*줘|알려\s*줘|목록|얼마나|어때|뭐야|무엇|어떻게))[\s\S]*((AI-?BOM|에이아이\s*봄)[\s\S]{0,30}(기재|기입|적어|채워)|(기초\s*모델|아키텍처|파인튜닝\s*이력|데이터셋\s*출처|벡터\s*DB|시스템\s*프롬프트|가드레일|API\s*목록|MCP\s*서버|서빙\s*환경|호스팅\s*공급업체)[\s\S]{0,24}(기재|기입|적어|채워))/,
