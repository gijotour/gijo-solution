...
const TOOLS: AgentTool[] = [
  ...
  },
  {
    ...
    write: false,
    description:
      '통합 보안 KPI 스냅샷을 조회한다 — 자산 위험도, 취약점 활성/조치율, 조치 SLA 준수율, 점검(유지보수) 지연/승인대기, 컴플라이언스 이행률. "보안 KPI 현황", "KPI 어때", "보안 지표 보여줘", "조치 완료율 어때", "SLA 준수율 몇 %야"에 쓴다. 예: {}',
    directAnswer: true,
    params: [],
    // ⚠ q는 **모델이 채우는 인자가 아니다**(그래서 params에 없다). 강제 라우팅(forcedToolFor)이
    //   물음을 그대로 넘겨, 제품이 안 세는 지표를 물었을 때 「아직 집계하지 않습니다」라고 말하게 한다
    //   (2026-09-06). 모델 선택 경로에서는 비어 오고, 그때는 종전과 똑같은 답이 나간다.
    run: async (args: Record<string, string>) => runKpiStatus(args.q),
  },
  // ── 내 업무(할 일) 3종 ────────────────────────────────────────────────
  ...
  },
  {
    ...
    params: [],
    // ⚠ **즉답이다**(2026-09-08 검토관 [중] 수리). 바로 위 scan_status와 **같은 함정**이었다 —
    //   본문(briefing.ts)은 「지난 브리핑 이후 신규 N건」·「기한 초과」·「기한 임박」·「오늘 추천」을
    //   또박또박 내는데, LLM 재작성이 그 줄들을 **통째로 먹었다.**
    //   실측(2026-09-08 03:04 회차 .tmp-reports/ops-sim.json): q=「오늘 브리핑 해줘」·도구=briefing·
    //   944자 답에 그 네 줄이 **하나도 없었다**(조치 상위·위협·용어 풀이만 남았다).
    //   밤사이 새로 생긴 것을 물었는데 「새로 생긴 것」 줄이 사라지면 이 도구의 값이 반쯤 죽는다.
    //   이미 우리말 요약이라 다시 쓸 이유도 없다(scan_status 2026-08-03 주석과 같은 근거).
    directAnswer: true,
  ...
  },
  {
    ...
    domain: "knowledge",
    write: false,
    // ⚠ **두 제품을 나란히 놓을 때만** 쓴다. 한 제품의 항목 하나를 묻는 말
    //   (「SafeBreach 도입 형태가 뭐야?」)까지 이 도구가 가져가면 b가 비어 「비교 대상이
    //   누락됐다」는 엉뚱한 답이 나간다 — 2026-08-29 라이브 실측에서 실제로 그랬다.
    //   한 제품 조회는 지식 검색(온톨로지 트리플)이 답한다.
    description:
      '등록된 제품 소개자료 **두 개**를 항목별 표로 나란히 비교한다(도입 형태·라이선스·주요 기능·차별점·대상 규모·연동·경쟁 제품). ' +
      '두 제품 이름이 모두 나온 물음에만 쓴다("A와 B 비교", "A랑 B 뭐가 달라"). ' +
      '제품 하나에 대한 물음("SafeBreach 도입 형태가 뭐야")에는 쓰지 마라 — 그건 지식 검색이 답한다. ' +
      '예: {"a":"SafeBreach","b":"Cymulate"}',
    params: [
  ...
  },
  {
    ...
    domain: "sbom",
    write: false,
    // 이 글은 답이 아니라 모델이 읽는 도구 설명이지만, 같은 김에 사전 상수로 맞춘다(2026-09-12).
    description:
      `VEX(취약점 대응 상태 문서)로 내보내면 어떤 상태가 몇 건 나가는지 본다. "VEX 파일 내보내줘"·"VEX로 나가면 어떤 상태야?"에 쓴다. 파일 자체는 ③ 조치 › ${표식.좋음} 조치·승인 화면에서 받는다.`,
    params: [
  ...
  {
    // ↩ 문서 다시 넣기 — 조각이 사라졌거나 옛 추출본으로 들어간 문서를 되살린다(쓰기 · 결재판 경유).
    //   ⚠ 새 파이프라인이 아니다 — memory.reingestFromExtracted가 markdown/save와 같은 자리를 쓴다.
    //   ★ 2026-09-10부터 **원본이 보관돼 있으면 오늘의 추출기로 다시 뽑는다**(갈래 D) — 표·슬라이드
    //     수리(2026-09-08~10)가 이미 반입된 문서에도 닿게 하는 유일한 길이다. 원본이 없으면 종전대로 추출본.
    ...
    write: true,
    description:
      '지식 문서를 다시 넣는다 — 원본이 서버에 보관돼 있으면 지금 추출기로 **다시 뽑아** 넣고(표·슬라이드 같은 구조가 살아난다), 원본이 없으면 예전에 뽑아 둔 추출본으로 넣는다. "<문서이름> 다시 넣어줘"·"그 문서 재인입해줘"·"표가 깨져서 다시 읽어줘"에 쓴다. 둘 다 없으면 다시 넣지 못하고 ＋로 다시 올리라고 안내한다.',
    params: [
      { name: "document", label: "문서", description: "다시 넣을 문서 이름(파일 이름)", required: true },
    ],
    // ⚠ 문구에서 「업무영역 유지」를 뺐다(2026-09-10) — **코드가 그렇게 안 한다.** 업무영역(category)은
    //   재인입이면 규칙이 다시 매기는 것이 기존 계약이고(memory.ts 문서표찰 주석), 원본에서 다시
    //   뽑으면 글이 달라져 실제로 값이 바뀐다(짝 시험 실측). 지켜지는 것은 사람이 매긴 표찰(등급·올린이)이다.
    // ⚠ 「올린이 유지」는 **적어 두는 것으로는 안 지켜졌다**(같은 날 검토관 [높음]): 첫 판이 승인자를
    //   ingestText의 uploadedBy로 넘겨 감사 귀속이 승인자에게 옮겨 갔다. 코드에서 그 인자를 뺐고
    //   (memory.ts) 짝 시험이 **실행으로** 지킨다(reingestoriginal.test ⑤) — 문구만 믿지 않는다.
    effect: (args) =>
      `문서 「${args.document ?? ""}」의 옛 조각을 지우고 **다시 인입** — 원본이 보관돼 있으면 지금 추출기로 다시 뽑고(추출본 .md도 새 글로 갱신), 없으면 보관된 추출본으로 넣습니다`
      + " · 등급·올린이·검색 범위는 그대로 유지(업무영역은 새 글로 다시 매겨집니다) · 넣을 글이 없으면 아무것도 바꾸지 않고 실패로 끝납니다",
    undo: "되돌릴 필요가 없습니다 — 넣는 글자는 그 문서에서 뽑은 글자입니다(원본이 있으면 그 보관본에서 다시 뽑습니다). 그래도 빼고 싶으면 「<문서이름> 지워줘」로 삭제할 수 있습니다.",
