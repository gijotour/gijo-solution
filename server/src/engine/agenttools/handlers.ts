...
import { 온톨로지전용알림, 목록전용알림, 본문근거없음표지 } from "../noevidence"; // 「본문 근거 없음」 표지의 주인(잎 모듈)
...
// ★★ 2026-09-11 설계관 지시서(B1 ④ / B2) — **본문 근거 없이 답한 자리의 정직 표지.**
//   문서 발췌가 0건인데 온톨로지 관계만 실리면(예: 「VPR이 뭐야?」가 explain의 topic만으로
//   검색해 문서를 못 찾고 온톨로지 덤프만 답이 될 때) 근거 없이 답한 것인데 지식근거없음표지도
//   자료없음배너도 안 걸려 아무 표시가 없었다(옛 B2 백로그가 그 자리다).
//   ⚠ 2026-09-11 검토관 수리 — **문장의 주인을 noevidence.ts로 옮겼다.** 여기 두었더니
//     「근거 없다고 밝힌 답인가」를 묻는 소비자(learncandidates·sessionpatterns·회귀 하네스)가
//     이 사실을 못 봐, 근거 없다고 스스로 밝힌 답이 학습 인용 +3점을 받았다. 이유·규약은
//     noevidence.ts의 표지 절에 적혀 있다(여기서 또 적지 않는다 — 같은 사실은 한 곳에).
export { 온톨로지전용알림, 목록전용알림, 본문근거없음표지 };

/**
 * 발췌(사내 문서 **본문**)가 안 실린 답에 붙일 정직 표지를 고른다 — 붙일 것이 없으면 빈 문자열.
 *
 * ⚠ **검색실패는 표지를 안 건다**(2026-09-11 검토관 [중]): 임베딩 미기동·조회 예외로 못 물어본
 *   것을 「사내 문서 본문 근거는 없습니다」라고 단정하면, 문서가 실재하는데 담당자는 「우리
 *   회사에 그 문서가 없구나」로 읽는다. llm.ts가 같은 규율을 이미 명문화해 뒀다
 *   (chunks:[]=0건 / chunks:null=근거를 모른다 — 두 값을 뭉개면 「고장」을 「지어냄」으로 단정한다).
 * ⚠ 지정 범위가 걸린 0건도 안 건다 — 그 사실은 별도 문장(「지정하신 문서 범위 …」)이 말한다.
 * ⚠ 살균(sanitizeRagChunks)이 전부 걸러 낸 자리는 표지를 **건다** — 조각은 찾았지만 이 답이
 *   본문을 근거로 쓰지 못한 것은 사실이고, 그 사실을 숨기면 정직 표지가 있으나 마나다.
 */
export function 본문근거없음표지고르기(상태: {
  발췌실림: boolean; 검색실패: boolean; 지정범위: boolean; 온톨로지있음: boolean; 목록있음: boolean;
}): string {
  if (상태.발췌실림 || 상태.검색실패 || 상태.지정범위) return "";
  if (상태.온톨로지있음) return 온톨로지전용알림;
  if (상태.목록있음) return 목록전용알림;
  return "";
}
...
export async function runExplain(args: Record<string, string>): Promise<string> {
  ...
  const out: string[] = [];
  // B2 — 표지가 볼 신호. 발췌(사내 문서 본문)가 실제로 실렸는지만 본다.
  let 발췌실림 = false;
  // ⚠ 「0건」과 「못 물어봤다」를 가르는 칸(2026-09-11 검토관 [중]) — 아래 본문근거없음표지고르기 참고.
  let 검색실패 = false;

  // ⚠ 문서를 콕 집은 질문(「이 영향평가 안내서에서 대상 기준…」)은 **그 문서가 권위**다 — 문서 발췌를
  //   온톨로지보다 **앞에** 싣는다. 안 그러면 7B가 앞줄(온톨로지)을 답으로 삼아, 도메인 낱말(대상=스캔
  //   타깃)의 온톨로지 관계 덤프가 문서 내용을 덮는다(2026-08-21 코퍼스 QA 실측).
  //   ⚠ 발동 범위는 **둘**이다(2026-09-08 검토관 [낮음] — 옛 주석은 「강제경로에만」이라 거짓이 됐다):
  //     ① 유형어로 집은 말(문서지목질문) ② **제목으로 집힌 문서가 있을 때**(지목.size > 0).
  //     ②는 LLM이 짧게 뽑은 일반 explain에서도 선다 — 「제로트러스트」 한 낱말이 topic이면
  //     제목지목문서가 그 문서를 집으므로 온톨로지가 뒤로 간다. **의도한 것이다**: 이름이 그 문서를
  //     정확히 가리키는데 온톨로지 덤프를 앞줄에 두면 7B가 그것을 답으로 삼는다.
  ...
  try {
    ...
    // 임베딩 미기동 등 — 본문 근거 없이 계속하되, **못 물어봤다는 사실**을 남긴다.
    검색실패 = true;
  }
  ...
  if (out.length === 0 && 꼬리.length === 0) {
    ...
    if (currentDocIds().length) {
      // 화면은 네이티브 체크박스라 글자 ☑가 없다(mydocs.html:1038) — 서버가 지어낸
      // 기호였으므로 2026-09-12 B9 후속에서 걷어냈다(설계관 실측: 화면에 없는 글자로
      // 화면을 가리키는 안내는 말투 위반 이전에 틀린 안내다).
      return `"${topic}"에 대해 지정하신 문서 범위(문서 ${currentDocIds().length}개)에서 찾은 근거가 없습니다. 문서 지정을 풀면 전체 사내 자료에서 다시 찾습니다.`;
    }
    return `"${topic}"에 대해 사내 온톨로지·문서·보안제품 등록부에서 찾은 근거가 없습니다. 일반 지식으로만 답하거나, 관련 문서를 업로드하면 근거가 쌓입니다.`;
  }
  // ★★ 2026-09-11 검토관 [중] 수리 — 표지는 결과의 **맨 끝**에 둔다(옛 판은 out.unshift로 맨 앞).
  //   왜: 이 파일 아래 orderForSmallModel 주석이 실사고로 못 박아 둔 그대로 **7B는 도구 결과의
  //   앞부분을 답으로 삼는다**. 맨 앞이 「사내 문서 본문 근거는 없습니다」면, B1이 겨냥한 바로 그
  //   물음(「VEX가 뭐야?」처럼 문서 0·온톨로지만)에서 모델이 그 한 줄만 복창한 **정의 없는 회피
  //   답**이 나온다. 게다가 그런 답은 머리 60자가 「없습니다」라 agentloop의 자료없음중복가드에
  //   걸려 **코드 배너가 억제**되고, 정직의 주인이 다시 모델 재작성 운으로 돌아간다.
  //   → 읽을 것(온톨로지 관계·문서 이름)을 앞에 두고, 표지는 끝줄로 보낸다. 표지를 보는 쪽은
  //     agentloop 온톨로지근거임을밝힌다이고 그쪽은 **줄 위치를 안 본다**(본문근거없음표지 정규식).
  // ⚠ 표지 자리를 **가장 먼저** 뗀다 — 꼬리보다 앞서 자리를 받으므로 총량 초과로 잘릴 수 없다.
  const 표지 = 본문근거없음표지고르기({
    발췌실림, 검색실패, 지정범위: 지정범위.length > 0,
    온톨로지있음: 온톨로지.length > 0, 목록있음: 꼬리.length > 0,
  });
  ...
  const 표지자리 = 표지 ? 표지.length + 1 : 0;
  const 꼬리글 = 꼬리.join("\n").slice(0, Math.max(0, EXPLAIN_MAX_CHARS - 표지자리));
  const 앞여유 = EXPLAIN_MAX_CHARS - 표지자리 - (꼬리글 ? 꼬리글.length + 1 : 0);
  const 앞글 = out.join("\n").slice(0, Math.max(0, 앞여유));
  const 본문 = !꼬리글 ? 앞글 : 앞글 ? `${앞글}\n${꼬리글}` : 꼬리글;
  if (!표지) return 본문;
  return 본문 ? `${본문}\n${표지}` : 표지;
}
...
// ⚠ EPSS 글자는 여기서 **만들지 않는다** — tone.epss표기 한 곳에서 온다(2026-09-11 검토관 적발).
//   여기 있던 `Math.round(epss*100)%`는 두 가지를 틀리고 있었다: ① EPSS 0.004(실제 스캐너에서
//   가장 흔한 크기)를 「EPSS 0%」로 만들어 **값이 있는데 없다고 단정**했고, ② approvals·analysishub가
//   저마다 따로 적어 같은 건이 「0.97」과 「97%」로 갈렸다. 자릿수를 맞추겠다는 주석은 약속일 뿐이라
//   아무것도 막지 못했다 — 같은 함수를 부르는 것만이 어긋남을 막는다.
...
export function runToday(args: Record<string, string>): string {
  ...
  const lines = top.map((r, i) => {
    const f = r.finding;
    // ⚠ 위협 지표 글자는 우선순위태그 한 곳에서만 만든다(2026-09-10, 자세히는 그 함수 주석) —
    //   이 답과 search 답의 문구가 어긋나지 않도록. EPSS 표기 자체는 다시 tone.epss표기가 낸다.
    const tags = 우선순위태그(f);
...
export function runProductStatus(args: Record<string, string>): string {
  ...
  // ⚠ 2026-09-13까지 이 도구는 숫자(제품 N건·문서 미등록 M건)를 말하면서도 예시데이터머리말이
  //   안 붙어 있었다 — 시드 제품 4건이 그대로 섞여도 고지가 없었다(계획서 §13.5.2).
  return `${예시데이터머리말()}${head}\n${lines.join("\n")}${more}${warn}`;
}
 ...
 */
/**
 * 첫 기동 시드(자산·점검·조치·제품·위협 인텔리전스 여섯 곳)가 하나라도 원본 그대로 남아 있을 때
 * 답 머리에 붙일 한 줄. 없으면 빈 문자열. 판정은 예시데이터뿐인가()(assets.ts) 한 곳뿐이다.
 *
...
function 예시데이터머리말(): string {
  return 예시데이터뿐인가()
    ? "⚠ 아래는 **예시 데이터**가 섞여 있을 수 있습니다 — 자산을 실제로 등록해도 점검·조치·제품·위협 인텔리전스 예시가 남아 있으면 계속 섞이며, 완전히 빼려면 관리자가 실사용 전환(데이터 정리)을 해야 합니다.\n"
    : "";
}
...
/**
 * 우리 지표가 **아닌 주어** — 「완료율」이 붙어도 우리 것이 아니다(2026-09-06 검토 실측).
 *
 * ★ 왜 필요한가: 위 표는 「완료율/준수율」이라는 **낱말**만 보고 주어를 안 봤다. 그래서 제품이
 *   세지도 않는 「교육 완료율」·「점검 완료율」·「훈련 이수 완료율」이 **세는 지표로 분류되어**
 *   정직 문장이 빠졌다 — 사고를 막으려 만든 「아직 집계하지 않습니다」가 같은 지표의 다른 말투에서만
 *   나온 것이다(「보안 교육 이수율」은 붙고 「교육 완료율」은 안 붙었다). 게다가 답 끝의 상시 꼬리
 *   「조치율=조치 완료율」이 함께 나가, 담당자가 조치율 %를 자기가 물은 교육 완료율의 답으로 읽는다.
 * ⚠ 여기서 이름을 빼려면 **계산하는 코드가 먼저** 생겨야 한다(위 표와 같은 규율).
 */
const 남의주어_RE = /교육|훈련|점검|인식|참여|출석|이수|응답|가동|만족|활용|출근|퇴사|채용/;
...
export function 안세는지표찾기(질문: string | undefined): string[] {
  ...
  const out: string[] = [];
  for (const c of 후보) {
    // 주어가 우리 것이 아니면 낱말이 겹쳐도 우리 지표가 아니다 — 「교육 완료율」 ≠ 「조치 완료율」.
    if (!남의주어_RE.test(c) && 집계하는지표.some((k) => k.re.test(c))) continue;
...
export async function runProductIntroAdd(args: Record<string, string>): Promise<string> {
  ...
  try {
    ...
    return (
      ...
      //   승인하면 「추가 기능 > 제품 소개자료로 가세요」라고 답해 담당자가 없는 자리를 찾아 헤맨다.
      //   ⚠ 2026-09-14 검토관 [상] 수리 — **같은 자리를 또 밟았다.** 이름 통일(mydocs.html:383)이
      //   화면만 바꾸고 이 답변·결재판 문구(registry.ts:2244-2245)를 안 고쳐, 담당자는 화면에
      //   없는 이름으로 가라는 안내를 받았다. 바로 위 2026-08-22 주석이 경고한 그 부류다.
      `「내 문서 > 📦 보안제품 비교·소개」에서 목록·비교로 볼 수 있습니다.` +
...
export async function runDocChunkGaps(): Promise<string> {
  ...
  out.push(
    "",
    ...(에디션제한중()
      ? ["되돌리는 법 — 대화창 ＋로 그 파일을 다시 올려 주세요(등급은 그대로 이어지고, 업무영역은 새 글로 다시 매겨집니다)."]
      : [
          "되돌리는 법 — 원본이 보관된 문서는 「<문서이름> 다시 넣어줘」라고 말씀하시면 결재판을 거쳐 다시 넣습니다.",
          "원본이 없으면 대화창 ＋로 그 파일을 다시 올려 주세요(등급은 그대로 이어지고, 업무영역은 새 글로 다시 매겨집니다).",
        ]),
...
export async function runReingestDocument(args: Record<string, string>): Promise<string> {
  ...
  if (!r) {
    ...
    // ⚠ 안내에 「업무영역도 이어집니다」를 적지 않는다(2026-09-10 검토관 적발 — 반쪽 수리).
    //   ＋ 재업로드도 업무영역(category)은 **규칙이 다시 매긴다**(memory.ts 문서표찰 주석의 기존 계약).
    //   결재판 문구에서는 뺐는데 여기 남아 있었다 — 같은 거짓을 두 곳에 적으면 한 곳만 고치게 된다.
    throw new Error(`「${doc.documentId}」는 서버에 원본도 AI가 읽었던 글(추출본)도 남아 있지 않아 다시 넣을 수 없습니다 — 대화창 ＋로 그 파일을 다시 올려 주세요(등급은 그대로 이어지고, 업무영역은 새 글로 다시 매겨집니다). 조각 수는 그대로 ${doc.chunks}개입니다.`);
  }
