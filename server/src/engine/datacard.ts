...
// ⚠ 아래 둘은 **잎 모듈**(import 0개)이다 — dispatcher.ts·agentloop.ts가 datacard를 부르므로
//   그쪽을 직접 import하면 순환이 된다. 배제 잣대를 베끼지 않으려고 잎으로 내려 공유한다.
import { 리포트만들기지시 } from "./reportintent";
import { 쓰기지시인가 } from "./falseclaim";
 ...
 *  ⚠ 2026-09-11(B6-①) — 「어떻게」에 **부정 앞보기**를 달았다(설계관 지시서에는 없던 최소
 *    수리 · deviations 기록). 그대로 두면 「하드닝 점검 **결과가 어떻게 돼**?」(상태를 묻는
 *    말)까지 방법 물음으로 막혀, 이 라운드가 열려는 결과꼴이 못 들어온다. 「어떻게 돼/되나/
 *    됐/되어」는 **상태**를 묻는 꼴이라 여기서 뺀다 — 「어떻게 해/하는지/하나(절차)」는 그대로
 *    막힌다(datacard.test.ts 「자산 현황 어떻게 봐?」·「통합 관제 어떻게 보나요」·hardening-
 *    unchecked-routing.test.ts 「보안설정 점검 어떻게 하는지 알려줘」로 회귀 확인). */
const 방법절차물음 = /절차|방법|어떻게(?!(돼|되나|됐|되어))|순서|매뉴얼|가이드|하려면/;

/** ★ 결과 조회꼴의 재료 셋(2026-09-11 B6-① 설계관 지시서 · 같은 날 검토관 수리) —
 *  isHardeningStatusAsk가 「하드닝 점검 결과는?」류(현황말 없는 결과 조회)까지 받도록 문을
 *  열되, 실행 지시·보고서 만들기·이웃 영토는 배제해 다른 도착지를 안 뺏는다.
 *
 *  ⚠⚠ **실행지시_RE는 배제용이라 FORCED_INTENTS[3](run_hardening_scan)의 뒤 보기보다
 *    일부러 넓다**(2026-09-11 검토관 [상]① 수리). 첫 판은 그 리터럴을 **글자 그대로 베껴** 두 곳을 같게
 *    맞췄는데, 그 목록이 「하고·해서(사이에 낱말이 낀 꼴)·실시·진행·착수」를 몰라서
 *    실측 네 문장이 통째로 **조회로 못 박혔다**:
 *      · 「하드닝 점검 다시 해서 결과 내줘」 → [17] 현황 카드(점검이 안 돈다)
 *      · 「방화벽 점검하고 결과 알려줘」·「CCE 점검하고 결과 보여줘」·
 *        「스위치 KISA 기준 점검 진행하고 결과 줘」 → ⑨에서 모델이 골라도 2차 방어가 차단
 *    커밋 전에는 넷 다 실제로 점검이 돌던 말이다. **두 규칙은 일이 다르다** —
 *    FORCED_INTENTS[3](run_hardening_scan)은 **허용**(즉시 실행이라 좁아야 한다),
 *    이쪽은 **배제**(넓어야 안전하다).
 *    지켜야 할 계약은 「글자가 같다」가 아니라 **「강제 실행으로 가는 말은 여기서 조회로 안
 *    읽힌다」**이고, 그건 시험이 제품 함수로 잰다(hardening-unchecked-routing.test.ts ⑥).
 *  ⚠ 뒤보기로 조회를 지킨다 — 「점검하고 있어?」·「점검 진행 상황」·「점검 실시했어?」는
 *    실행 지시가 아니다(그대로 두면 시킨 적 없는 점검을 돌릴 뻔한 말이 배제로 샌다).
 *  ⚠ 이 리터럴은 agentloop.ts 결과조회꼴()과 **같은 글자**다(둘 다 배제용) — 짝 감시가 잰다. */
const 결과조회_RE = /결과/;
const 실행지시_RE = /(점검|진단|스캔|체크)\s*(을|를)?\s*(다시|한\s*번|지금|좀|바로|새로)?\s*(해|하자|하라|하고(?!\s*(있|계|싶))|시켜|실시(?!\s*(했|한|된|됐|여부|주기|일|중|결과|내역|이력|현황|상황))|진행(?!\s*(했|한|된|됐|중|상황|률|율|도|현황|사항|경과|결과|내역|이력))|착수)|돌려|돌리|수행|가동|실행(?!\s*(이력|기록|결과|내역|현황))/;

/** 「결과」 한 낱말이 뺏기 쉬운 **이웃 갈래**(2026-09-11 검토관 [상]②·[중] 수리).
 *  실측: 「검증 결과 보고서 **양식** 어디 있어?」·「정기점검 결과 보고는 **누구에게** 하나요?」가
 *  [17] 현황 카드로 갔다 — 앞쪽은 [20] 자리 안내, 뒤쪽은 절차 안내(explain)의 영토다.
 *  ⚠ 현황꼴·미점검조회에는 안 건다 — 「미점검 장비 **어디**야?」는 예행 ㉔의 양성이다. */
const 결과꼴배제 = /양식|서식|템플릿|어디|누구|누가/;

/** 「안 했다」류 미점검 조회 — 「점검 안 한 장비 있어?」(2026-09-10 고객 QA 예행 ㉔).
 *  ⚠ 「대상」은 넣지 않는다 — 넣으면 「보안설정 점검 안 한 **대상** 있어?」가 이웃영토에 잡혀
 *    빠진다(실측). 「자산」은 asset_coverage 영토라 반드시 넣는다 — 안 넣으면 「점검 안 한
 *    자산 있어?」가 여기로 새서 그 도구가 죽는다.
 *  ⚠ 「조치」도 이웃영토다(2026-09-11 검토관 적발·실측). ④ 검증 단계는 **조치 검증**
 *    (verify_finding 계열)과 보안설정 점검을 함께 담는데, 이 갈래가 켜진 뒤 「검증 안 된
 *    조치 있어?」가 영토어 「검증」에 먼저 걸려 하드닝 대상 표로 갔다(수리 전 route-explain
 *    실측: [17] 검증(하드닝) 현황). 조치를 물었는데 장비 점검 표를 주면 오답이다.
 *  ⚠ 조회말에 「몇」이 있다 — 세는 말투(「점검 안 한 장비 몇 대야?」)도 같은 물음이다.
 *    없을 때 실측하면 그 말은 강제도구 list_assets로 가서 자산 목록을 답했다(2026-09-11).
 *  ⚠ 2026-09-11 검토관 [상]②·[중] — 이 가드를 **결과꼴도 같이 쓴다**(그래서 이름이 「미점검」이
 *    아니라 「이웃영토」다). 앞 판은 미점검조회 안에서만 검사해, 같은 뜻을 결과 낱말로 물으면
 *    그대로 샜다(실측: 「조치 검증 결과는?」·「SBOM 검증 결과는?」·「유지보수 점검 결과 서버별로
 *    정리해줘」 전부 [17] 카드). **같은 것을 두 갈래에 따로 적어 한쪽만 고친 반쪽 수리**였다.
 *  ⚠ 레드팀·가드레일을 더했다 — 「레드팀 검증 결과는?」이 강제도구 redteam_status를,
 *    「가드레일 검증 결과는?」이 [27] 조치 플레이북을 앞질러 죽였다(실측). */
const 이웃영토 = /자산|취약점|스캔|유지보수|정비|부품|조치|SBOM|레드팀|가드레일/i;
const 미점검말 = /(안했|안한|안된|안돼|못한|안하고|미점검)/;
const 조회말 = /(있어|있나|있는|없어|없나|어디|어느|뭐|무엇|어떤|몇|목록|알려줘|보여줘)/;

/** 「검증/하드닝/보안설정 점검 현황」류 물음인가 — 결정적 트리거(LLM 이전). */
export function isHardeningStatusAsk(text: string): boolean {
  const t = String(text || "").replace(/\s+/g, "");
  const 현황꼴 = /(현황|상태|어때|보여줘|알려줘)/.test(t);
  // ★ 결과 조회꼴(2026-09-11 B6-①) — 「하드닝 점검 결과는?」류는 현황꼴 다섯 낱말 중
  //   어느 것도 아니라 여기로 안 왔다(4100 실측: ⑨ 모델 선택 → run_hardening_scan 실행,
  //   4000에서는 hostRunner가 실 셸 명령을 돌린다).
  //   ⚠ 「결과」는 흔한 낱말이라 **배제가 다섯**이다(2026-09-11 검토관 [상]①②·[중]×2 수리):
  //     ① 실행 지시 — 시킨 점검이 조회로 못 박히면 안 된다(안 돌고 말도 안 해 준다)
  //     ② 리포트 만들기 — 배제 잣대는 [36]과 **같은 상수**를 본다(reportintent.ts).
  //        앞 판은 자기 정규식을 베껴 「출력·뽑」이 빠져 「…결과 리포트 출력해줘」를 뺏었다.
  //     ③ 이웃 영토 — 미점검조회와 **같은 가드를 공유**한다(조치·SBOM·유지보수·레드팀…)
  //     ④ 양식·자리·보고 대상 물음 — [20] 자리 안내·explain 절차의 영토
  //     ⑤ **쓰기 지시**도 배제한다(2026-09-11 검토관 [중]) — 실측 「정기점검 **결과** 김보안한테
  //     배정해줘」가 카드로 못 박혔다(「결과」를 빼면 ⑨로 가 배정이 된다). 조회로 못 박으면
  //     목록만 나가고 배정은 **영영 안 된다** — agentloop.ts 조회로못박지않을것이 막는 그 사고다.
  //     ⚠ 현황꼴(「…현황 보여주고 배정해줘」)은 2026-08-19부터의 오래된 계약이라 손대지 않았다.
  ...
  const 결과꼴 =
    ...
    !이웃영토.test(t) &&
    !결과꼴배제.test(t);
  // ★ 미점검 조회(2026-09-10 예행 ㉔) — 현황말이 없어도 「안 했다/조회」 꼴이면 받는다.
  //   screenguide.ts verify.html can[]이 이미 「점검 안 한 장비 있어?」를 제품이 답할 말로
  //   적어 두고 있었는데, isHardeningStatusAsk가 「현황·상태·어때·보여줘·알려줘」만 요구해
  //   그 약속을 못 지키고 있었다(대장 문서에 기록돼 있던 공백).
  const 미점검조회 = !이웃영토.test(t) && 미점검말.test(t) && 조회말.test(t);
  if (!현황꼴 && !결과꼴 && !미점검조회) return false;
  ...
  if (/취약점|스캔결과|스케줄|일정/.test(t)) return false;
  if (/(검증|하드닝|보안설정점검|정기점검|설정점검)/.test(t)) return true;
  // 영토어가 없으면 「점검」 + 장비류 낱말일 때만 미점검·결과 조회로 받는다
  //   (「미점검 장비 어디야?」·「장비 점검 결과는?」 — 검증·하드닝 낱말이 없어도 명백하다).
  return (미점검조회 || 결과꼴) && /점검/.test(t) && /(장비|서버|시스템|기기|호스트|노드)/.test(t);
}
...
export function hardeningStatusAnswer(): { output: string; dataCard: DataCard } {
  ...
  // ★ 미점검(점검 이력이 아예 없는 대상) 수 — 2026-09-11 검토관 적발.
  //   이 카드는 「점검 안 한 장비 있어?」(예행 ㉔)에도 답하는데, KPI 넷·글 답 어디에도
  //   **미점검 수가 없었다.** 대상 0인 QA 인스턴스에서만 우연히 기대 답과 같아졌을 뿐,
  //   대상이 다 점검된 곳에서 물으면 「없습니다」 대신 준수율 좋은 표가 나가 담당자가
  //   그 줄을 「안 한 장비」로 읽는다. 새로 세지 않는다 — 위 _급함이 이미 가른 값을 쓴다
  //   (0=점검 실패 · 1=미점검 · 2=정상/수동만).
  const 미점검수 = rows.filter((r) => r._급함 === 1).length;
  // 표는 급한 순 10줄로 잘린다 — 실패가 10곳이면 미점검이 표 밖으로 밀린다. 그때도 수는 말한다.
  const 표밖미점검 = Math.max(0, 미점검수 - shown.filter((r) => r.상태 === "미점검").length);

  const dataCard: DataCard = {
    title: "검증 — 보안설정 점검 현황",
    kpis: [
      { label: "등록 장비", value: String(targets.length), color: targets.length ? undefined : "muted" },
      // 「점검 안 한 장비 있어?」에 **숫자로** 답하는 칸 — 0이면 초록(안 한 것이 없다).
      { label: "미점검", value: String(미점검수), color: 미점검수 ? "warn" : "ok" },
  ...
  const output = [
    `검증(보안설정 점검) 현황 — 등록 장비 ${targets.length} · 미점검 ${미점검수}곳 · 활성 스케줄 ${활성.length}` +
      ` · 평균 준수율 ${평균 == null ? "측정 전" : 평균 + "%"} · 점검 실패 ${실패.length}건`,
    // ★ 물음에 **직접** 답하는 줄 — 「점검 안 한 장비 있어?」에 예/아니오가 먼저 나온다.
    rows.length
      ? (미점검수
          ? `점검 안 한 장비가 ${미점검수}곳 있습니다 — 표의 「미점검」 줄입니다.` +
            (표밖미점검 ? ` (그중 ${표밖미점검}곳은 급한 순으로 잘려 표에 안 보입니다 — 전체는 검증 화면에서.)` : "")
          : `점검 안 한 장비는 없습니다 — 등록 대상 ${rows.length}곳 모두 점검 이력이 있습니다.`)
      : "등록된 점검 대상이 없습니다 — 검증 화면에서 「+ 대상 등록」으로 시작하세요.",
    rows.length ? `대상 ${rows.length}곳 중 급한 순 ${Math.min(표상한, rows.length)}곳을 카드로 보였습니다 — 전체는 검증 화면에서.` : "",
    // 빈 줄은 떨군다 — 0-대상일 때 꼬리 개행이 남으면 하네스 대조가 흔들린다.
  ].filter(Boolean).join("\n");
