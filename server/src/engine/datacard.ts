...
/** ★ 결과 조회꼴의 재료 셋(2026-09-11 B6-① 설계관 지시서) — isHardeningStatusAsk가
 *  「하드닝 점검 결과는?」류(현황말 없는 결과 조회)까지 받도록 문을 열되, 실행 지시·보고서
 *  만들기는 배제해 이웃 도착지([37] run_hardening_scan·[36] generateReport)를 안 뺏는다.
 *  ⚠ 실행지시_RE는 agentloop.ts FORCED_INTENTS[3](1104줄)의 **뒤 보기 리터럴을 그대로
 *    베낀 것**이다 — import로 합칠 수 없다(그 배열은 test/helpers/routing.ts가 소스에서
 *    eval로 떼어 읽는다). 한쪽만 고치면 어긋나므로 짝 감시 시험(server/test/hardening-
 *    unchecked-routing.test.ts의 「④ 소스 감시」)이 두 파일에 같은 글자가 있는지 잰다 —
 *    agentloop.ts의 결과조회꼴()도 같은 리터럴을 쓴다. */
const 결과조회_RE = /결과/;
const 실행지시_RE = /(점검|진단|스캔|체크)\s*(해|하자|하라|시켜)|돌려|수행|가동|실행(?!\s*(이력|기록|결과|내역|현황))/;
const 보고서만들기_RE = /(보고서|리포트).{0,4}(만들|작성|생성|뽑아|써)/;
...
const 미점검이웃영토 = /자산|취약점|스캔|유지보수|정비|부품|조치|SBOM/i;
...
export function isHardeningStatusAsk(text: string): boolean {
  ...
  //   4000에서는 hostRunner가 실 셸 명령을 돌린다). 실행 지시·보고서 만들기는 배제한다.
  const 결과꼴 = 결과조회_RE.test(t) && !실행지시_RE.test(t) && !보고서만들기_RE.test(t);
  ...
  const 미점검조회 = !미점검이웃영토.test(t) && 미점검말.test(t) && 조회말.test(t);
