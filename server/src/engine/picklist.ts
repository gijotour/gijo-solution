...
import { 쓰기흐름인가 } from "./writeflow"; // 「조회 + 쓰기 지시」 잣대 — agentloop와 **같은 함수**(잎 모듈 단일 출처)
...
export function 미패치물음(text: string): boolean {
  ...
  // ★ 2026-09-12 검토관 [중] 수리 — 배제 대상에 **설정·정책·룰·규칙**을 더했다. 앞 판은
  //   자산·장비·문서만 막아 「**설정** 수정 안 한 거 알려줘」에 취약점 목록이 나갔다(실측) —
  //   담당자는 그 목록을 「설정 미수정 목록」으로 읽는다(머리글 ⓑ가 막겠다고 적은 바로 그 결함).
  const 배제대상 = /(자산|장비|서버|문서|자료|파일|계정|사용자|단말|PC|설정|정책|룰셋?|규칙)/i;
  if (배제대상.test(text)) return false;
  if (/왜|이유/.test(text)) return false;
  // ★ 2026-09-12 검토관 [중] 수리 — **세는 물음·추이 물음은 목록이 아니다.** 「패치 안 한
  //   취약점 **통계**/추이 알려줘」에 목록+체크칸이 나갔다(실측). 집계를 물었는데 고르라는 판이
  //   나오는 것이라, 이 갈래에서 비켜서 제 갈래(finding_status·kpi)가 받게 둔다.
  //   ⚠ 「몇 건·건수」는 **안 뺀다** — findingListAnswer의 머리줄이 건수를 그대로 답하고,
  //     이웃 「미조치 취약점 몇 건?」도 같은 목록 답을 받고 있다(영토를 갈라 놓지 않는다).
  if (/통계|추이|추세|비율|퍼센트|그래프|차트/.test(text)) return false;
  ...
  const 대상 = /(취약점|결함)/;
  // ★ 2026-09-12 검토관 [중] 수리 — **대상 없이 성립시키는 신호는 「패치」뿐이다.** 앞 판은
  //   미패치신호 전부(수정·고침 포함)에 「거/것/건」을 붙여 주어, 대상이 아예 없는 「수정 안 한
  //   거 알려줘」·「아직 수정 안 된 거 알려줘」만으로 취약점 목록이 나갔다(실측 3문장 — 착수
  //   전에는 ∅였다). 「패치」는 취약점 말고 가리킬 것이 사실상 없지만 「수정·고침」은 무엇이든
  //   가리킨다 — 그 둘은 대상(취약점·결함)을 **말한 경우에만** 받는다.
  //   ⚠ 「안 고친·안 고쳐」는 **여기 남긴다**(실측으로 좁히다 S05 「아직 안 고친 거 알려줘」를
  //     죽여 보고 되돌렸다) — 취약점을 「고친다」고 말하는 것이 이 제품의 말투이고, 문제였던
  //     세 문장은 전부 명사 「수정」 쪽이었다. 실측이 잣대다.
  const 패치신호 = /패치\s*(안|못)\s*(한|된|했)|미패치|안\s*고친|안\s*고쳐/;
  const 대상바로뒤 = new RegExp(`(?:${패치신호.source})\\s*(거|것|건)`);
...
export function isFindingListAsk(text: string): boolean {
  ...
  // ★★ 2026-09-12 B10 수리(검토관 [상] 적발) — **쓰기 흐름이면 목록으로 못 박지 않는다.**
  //   위 LIST_BLOCK_RE는 낱말 목록이라 「배정·기한·처리해·조치해」만 알고 **승인해·할당해·
  //   맡겨·담당자 정해**를 모른다. 그래서 「패치 안 한 취약점 알려주고 **승인해줘**」가 [14]
  //   목록 한 수로 끝나 승인이 영영 안 갔다(실측 4꼴). 같은 뜻을 두 곳에 적어 어긋난 것이라
  //   낱말을 더 적지 않고 **agentloop와 같은 잣대 하나**(writeflow.쓰기흐름인가)를 태운다.
  //   ⚠ 이 한 줄이 미패치물음·오래미조치물음·기본 경로 **셋 다**를 덮는다(그래서 각 함수가
  //     아니라 여기 입구에 건다 — 새 갈래를 더해도 같은 가드를 저절로 지난다).
