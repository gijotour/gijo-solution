...
export function 오래미조치물음(text: string): boolean {
  ...
  const 못거르는조건 = /(고위험|위험\s*높|kev|실제\s*악용)/i;
  ...
  // ⚠ **남은 구멍(알고 둔다)**: 시간어가 없는 같은 뜻의 말 「패치 안 한 취약점 알려줘」는
  //   여기 신호에 안 걸리고 LIST_VERB_RE에도 「알려」가 없어 지금도 ∅(⑨ 모델 선택)이다
  //   — B7 ② 이웃 문장인데 **이 라운드가 닫지 않았다**(2026-09-12 검토관 [하]로 드러남).
  //   닫으려면 대상+「알려」 꼴을 LIST_VERB_RE 쪽에서 함께 봐야 하는데, 그러면 agentloop의
  //   상태어취약점(finding_status) 영토와 겹치므로 **실측 먼저**다. 다음 라운드 백로그.
  return !배제.test(text) && !못거르는조건.test(text) && 신호.test(text) && 대상.test(text) && 조회.test(text);
}
