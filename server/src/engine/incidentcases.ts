...
const 대화창최대 = 5;
/**
 * 대화창용 — 숫자만 주고 끝내지 않는다. 다만 **얼마나 실을지는 몇 건이 걸렸느냐로 갈린다**:
 *   · 1~2건(좁혀 물었다) = 전문 — 한 줄 + 쉬운 설명 + 교훈 + CVE/제품/기법 + 출처 링크
 *   · 3건 이상(훑어 본다) = **사례당 한 줄만** — 연도·지역·업종·제목·한 줄 요지
 * 사례 글은 사람이 쓴 정본(표 칸)이라 🤖(AI가 쓴 글) 표식을 붙이지 않는다.
 *
 * ⚠ 왜 3건부터 한 줄인가(2026-09-10 ⑲ 첫 실측) — 「침해사고 히스토리 보여줘」 답이 **2,983자**로 나갔다.
 *   야간 하네스의 산문 상한은 2,000자이고(tools/ops-sim.mjs), 그 길이는 담당자가 **안 읽는다.**
 *   5건 전문(교훈·CVE·출처 URL 네 줄씩)을 덜어 한 줄로 줄이면 같은 물음이 1,000자 아래로 내려온다.
 *   덜어 낸 것은 사라지지 않는다 — 제품·업종·CVE로 좁혀 물으면 1~2건 전문으로 돌아오고,
 *   📚 침해사고 히스토리 판에서는 전부 볼 수 있다(갈 곳 한 줄은 agentloop.ts의 「이어서」가 붙인다 —
 *   문구를 여기에 또 적으면 두 벌이 되어 어긋난다).
 *
...
export function formatIncidentCases(rows: IncidentCaseRow[], ctx: { q?: string; cve?: string } = {}): string {
  ...
  /** 좁혀 물었나 — 1~2건이면 전문, 3건 이상이면 사례당 한 줄(위 주석 「왜 3건부터 한 줄인가」). */
  const 전문 = 실을것.length <= 2;
  const lines = [
    `${표식.사례} 침해사고 히스토리 — ${rows.length}건${조건 ? ` (${조건})` : ""}${rows.length < total ? ` · 전체 ${total}건` : ""}`,
    전문
      ? `${표식.알아두기} 출처 링크에서 원문을 확인하세요(외부 사이트). 새 사례는 대화창에서 「사례 등록: …」.`
      // ⚠ 약속은 **실제 조건**으로 적는다(2026-09-10 검토관 [중]). 「제품·업종·CVE로 좁혀 물으면」은
      //   거짓이었다 — 전문 여부는 좁혔느냐가 아니라 **걸린 건수**로 갈리는데(위 전문), 씨앗 20건에서
      //   업종 「통신」은 7건·낱말 「랜섬웨어」는 6건이 걸린다. 시킨 대로 좁혀 묻고도 같은 한 줄 목록과
      //   같은 약속을 되받으면 안내가 제자리를 돈다. 조건을 바꾸지 않고 **말을 사실에 맞춘** 까닭:
      //   좁힘 낱말만으로 전문을 주면 「랜섬웨어」 6건 전문이 다시 2,900자가 되어 이 수리를 되돌린다.
      : `${표식.알아두기} 사례당 한 줄만 싣습니다 — 걸린 사례가 1~2건으로 좁혀지면(제품 이름·CVE로 물으면 대개 그렇습니다) 쉬운 설명·교훈·출처 링크까지 함께 드립니다.`,
    "",
  ];
  실을것.forEach((r, i) => {
    lines.push(`${i + 1}. ${한줄(r)}`);
    if (!전문) return;
    lines.push(`   ${컷(r.plainExplain, 400)}`);
  ...
  if (rows.length > 대화창최대) lines.push("", `… 외 ${rows.length - 대화창최대}건 — 제품·업종·CVE를 함께 물으면 좁혀 보여 드립니다.`);
  return lines.join("\n").slice(0, 3500);
}
