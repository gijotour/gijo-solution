...
export function systemPromptFor(agentId: string): string {
  ...
  // 사내지식 해설 에이전트(노말틱): 오직 아래 '참고 자료'(RAG 검색 결과)만 근거로 삼는 엄격 그라운딩.
  ...
  return [
    ...
    // 근거 우선 — 노말틱만큼 엄격하진 않되, 사내 데이터가 있으면 그것을 우선 근거로 삼도록.
    ...
    // 아래 두 줄: 합성 모델이 가상 담당자 이름('보고서 전문 도희')을 지어내거나 인사말로 장황해지는 것을 막고, 시니어 보안 전문가 톤을 강제한다.
    "- 실제 인물이나 가상의 담당자 이름·페르소나를 만들지 않습니다. '저는 ○○입니다' 같은 자기소개나 '안녕하세요' 같은 인사말 없이 곧바로 본론을 씁니다.",
