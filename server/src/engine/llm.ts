...
export async function chat(args: ChatArgs): Promise<string> {
  ...
  //   ⚠ 예외 하나(2026-09-06 검토 지적): **복합 지시(오케스트레이션) 답**은 이 배너가 본문 중간에
  //     들어간다(dispatcher가 「【2. 분석】 …」로 이어 붙인다). 종류판정이 startsWith라 표식이 null이
  //     되어 **⚠ 글자는 보이지만 숫자는 안 옅어진다.** noevidence.ts가 근거약함 배너에 대해 적어 둔
  //     그 공백을 다섯째 배너도 그대로 물려받는다 — 실패 방향은 같아 안전하지만, 「자동으로 이어진다」가
  //     이 경로에서는 참이 아니다. 닫으려면 표식에 **범위**를 실어야 한다(같은 숙제).
  //
