...
// 절차를 **하려는** 의도(대응하려면/어떻게 대응/초동)는 침해·장애 플레이북 몫이라 문서를 집어도 비켜
//   준다. ⚠ 「대응 방법/절차 **있어?**」(존재)는 여기 안 넣는다 — 존재 질문은 문서 몫이다(검토관 [하]).
const 절차의도RE = /(대응하려면|어떻게\s*(대응|해야|조치|처리)|뭐부터\s*(해|하나)|첫\s*조치|초동\s*(절차|대응|조치)|복구하려면|대로\s*(하려면|대응|처리|조치))/;
// 법령 질문은 law_lookup 몫 — 명시적으로 뺀다(사내규정질문과 같은 계약). ⚠ 법 이름 꼬리말까지 —
//   업로드 문서가 「개인정보」 같은 법령 도메인 토큰을 품어도 순수 법령 질문을 안 삼키게(검토관 [높음]).
const 법령질문RE = /(법령|법적|법제처|판례|법률|보호법|기본법|시행령|시행규칙|고시\b|조례|제\s*\d+\s*조)/;
// 문서를 **콕 집는 언어 신호** — 문서유형어 + (에서/에/의/을/를). Q1 "○○ 안내서에서"엔 지시어가
//   없으므로 지시어는 필수로 안 한다. ⚠ docScopeMatch는 **안 쓴다** — 그건 RAG 랭킹의 소프트 +0.02
//   부스트용 느슨한 토큰 부분일치라, 하드 라우팅 게이트로 재사용하면 업로드 문서명 토큰(개인정보·
//   랜섬웨어…)이 무관한 법령·침해 질문을 가로챈다(검토관 [높음] 실측). 「규정/지침」은 사내규정질문의
//   영토라 여기 안 넣어 겹치지 않게 한다.
//   조사 뒤 경계(공백·문장부호·끝)를 요구해 「문서화·문서함」 같은 중간합성어 오발동을 막는다.
const 문서지목RE = /(안내서|가이드|매뉴얼|설명서|보고서|리포트|백서|점검표|체크리스트|문서)(에서|에는|엔|에|의|을|를)(?=[\s.,?!)"']|$)/;
const 내용동사RE = /(뭐야|뭔가|무엇|있어|있나|있는지|나와\s*있|담겨|알려|설명|대상|기준|얼마나|어떤|규정하)/;
/**
 * 질문이 **문서를 콕 집어 그 내용·존재**를 묻는가(「이 안내서에서 암호화 대상 뭐야」·「이 가이드에 X
 * 있어?」) — 그러면 문서 RAG(explain)가 답해야 한다(2026-08-21 라이브 실측: "이 안내서에 랜섬웨어 대응
 * 절차 있어?"가 침해 초동으로, "안내서에서 암호화 대상"이 법령 검색으로 샜다). 신호는 **문서유형어 +
 * 조사**(명시 지목)이며, 법령·절차 의도·소재는 각자 주인(law_lookup·플레이북·문서소재 search)이라 뺀다.
 */
export function 문서지목질문(instruction: string): boolean {
  const t = String(instruction ?? "");
  if (절차의도RE.test(t)) return false;                    // 절차 요청 → 침해·장애 플레이북
  if (법령질문RE.test(t)) return false;                    // 법령 질문 → law_lookup
  if (/어디\s*(있|에|서|냐|야|지|더라|가|를)/.test(t)) return false; // 소재 → 문서소재 search
  if (!내용동사RE.test(t)) return false;                   // 내용·존재를 묻는 말이어야
  return 문서지목RE.test(t);                               // 문서유형어를 콕 집었는가
}
