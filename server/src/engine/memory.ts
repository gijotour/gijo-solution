...
import { 표줄, 구분선, 열수, 표머리글들, 표본문행자리 } from "./tabletext"; // 표 술어 단일 출처(잎 · import 0)
...
// 문서유형어 — 「문서」는 화/함/철/번호/작성이 뒤따르면 합성어(문서화·문서함)라 제외한다.
const 문서유형 = "안내서|가이드|매뉴얼|설명서|보고서|리포트|백서|점검표|체크리스트|회의록|문서(?!화|함|철|번호|작성)";
// 절차를 **하려는** ACTION 의도만 침해·장애 플레이북에 양보한다. ⚠ 블랭킷 「초동/첫 조치」는 뺐다 —
//   「이 매뉴얼에 초동 조치가 뭐라고 나와?」(내용질문)를 훔치던 자리(검토관 ①-6). 「대응 방법/절차 있어?」
//   (존재)도 여기 없다 — 존재 질문은 문서 몫. 「제대로」는 (?<!제) 되보기로 -대로 액션과 가른다.
const 절차의도RE = /(대응하려면|어떻게\s*(대응|해야|조치|처리)|뭐부터\s*(해|하나)|복구하려면|(?<!제)대로\s*(대응|처리|조치|하려|해야))/;
// 법령 질문은 law_lookup 몫 — 명시적으로 뺀다(사내규정질문과 같은 계약). ⚠ 「고시」는 \b를 안 붙인다
//   — JS \b는 ASCII 경계라 한글 뒤 「고시에서/고시를」에서 죽는다(검토관 ①-5·②). 법 이름 꼬리말까지
//   포함해 업로드 문서가 「개인정보」 같은 법령 도메인 토큰을 품어도 순수 법령 질문을 안 삼킨다(검토관 [높음]).
//   ⚠ 약칭 「망법(정보통신망법)·신용정보법」도 넣는다 — FORCED law_lookup·침해아님과 **같은 세 정규식**이라
//   여기만 빠지면 「이 안내서에서 정보통신망법 신고 의무」가 「~보호법」과 다르게 라우팅된다(검토관 ② 비대칭).
const 법령질문RE = /(법령|법적|법제처|판례|법률|보호법|기본법|시행령|시행규칙|고시|조례|제\s*\d+\s*조|망법|신용정보법)/;
// 추천·소재 질문 — 「어떤/무슨/어느 [문서류]…봐야/참고/찾/확인」은 **어느 문서인지**(소재)를 묻는 것이지
//   내용이 아니다(검토관 ①-4). 소재(어디)와 함께 문서 RAG로 새지 않게 앞에서 뺀다.
const 추천질문RE = /(어떤|무슨|어느)\s*(문서|매뉴얼|가이드|안내서|보고서|지침|규정|자료|리포트).{0,10}?(봐야|참고|찾|보면|읽|확인)/;
const 소재RE = /어디\s*(있|에|서|냐|야|지|더라|가|를)|어디\?/;
// 문서를 **콕 집는 언어 신호** 두 갈래 — (A) 문서유형어+조사 직접부착(「안내서에서/보고서는」),
//   (B) 지시어(이/그/해당/위/첨부)+문서유형어(「이 매뉴얼 …」 — 조사 생략·하위참조 「3페이지에」 포착).
//   (A)는 조사 뒤 경계(공백·문장부호·끝)를 요구해 「문서화·문서함」 오발을 막고, (B)는 지시어를 반드시
//   요구해 「이 시스템 랜섬웨어…」처럼 문서유형어 없는 침해 질문은 안 잡는다. 「규정/지침」은 사내규정질문 영토.
//   ⚠ **이 함수는 제목 지목(제목지목매치)을 안 쓴다** — 여기는 *유형어*로 집는 말만 본다.
//   제목 토큰 부분일치는 느슨해서, 그것만으로 하드 게이트를 삼으면 문서명 토큰(개인정보·랜섬웨어…)이
//   무관한 법령·침해 질문을 가로챈다(검토관 [높음] 실측). ★ 아래 제목지목질문은 같은 잣대를 쓰되
//   **법령·절차·소재·행동의도를 빼고 내용동사를 요구해** 그 위험을 어미로 막는다(그 머리글 참고).
const 문서조사RE = new RegExp(`(${문서유형})(에서|에는|엔|에|은|는|도|만|의|을|를)(?=[\\s.,?!)"']|$)`);
const 지시문서RE = new RegExp(`(^|\\s)(이|그|해당|위|첨부(?:한|된)?)\\s+(?:[^\\s?!.]+\\s+){0,2}?(${문서유형})`);
const 내용동사RE = /(뭐야|뭔가|뭐였|뭐라|무엇|무슨|몇|있어|있나|있는지|나와|담겨|들어\s*있|적혀|알려|말해|설명|요약|정리|내용|대상|기준|얼마나|어떤|규정하)/;
// ★★ 2026-09-08 검토관 [높음] 수리 — **시키는 말을 문답으로 바꾸지 않는다.**
//   제목지목질문이 explain(읽기 도구)으로 못 박으면 agentloop의 `if (tool && !tool.write)`가
//   그 자리에서 실행하고 return한다 — **결재판(2674행)에도, ⑨ 모델 선택에도 못 간다.**
//   실측(운영 DB 사본·48편): 「EDR 임직원단말 로그분석 매뉴얼 삭제해줘」·「제로트러스트 가이드라인
//   담당자 배정해줘」·「보안담당자 실무매뉴얼 새 버전으로 올려줘」 등 **10/10**이 explain으로
//   못 박혔고, 제목 토큰만 뺀 같은 말(「문서 삭제해줘」)은 전부 null이었다. 지시가 조용히 문답이 된 것이다.
//   ⚠ 어미까지 함께 본다 — 낱말만 보면 「삭제 방법 알려줘」·「승인 절차 뭐야」 같은 **내용 질문**을
//     같이 죽인다(그 물음의 주인은 문서다). 「동사 + (해)줘/자/라/…」 또는 문장 끝일 때만 행동으로 읽는다.
const 행동동사 = "삭제|제거|지워|지우|등록|추가|생성|만들|배정|배포|지정|할당|변경|바꿔|바꾸|수정|고쳐|올려|올리|업로드|내보내|내려받|다운로드|실행|돌려|돌리|중지|중단|승인|반려|채택|기입|담아|담기|넣어|넣기|반입|전송|취소|해제|초기화";
const 행동의도RE = new RegExp(`(?:${행동동사})\\s*(?:해|하|어|아)?\\s*(?:줘|주세요|주실래|줄래|주시|라|자|봐|둬|둡|$)`);
// ★★ 2026-09-12 실측(B10-①) — 이 정규식과 writeflow.쓰기흐름인가는 **서로를 포함하지 않는다.**
//   행동의도RE만 잡는 말: 「…실무매뉴얼 새 버전으로 **올려줘**」(등록·업로드·내보내 계열 —
//   위 행동동사 목록엔 있지만 쓰기흐름인가는 「올려」를 모른다). 쓰기흐름인가만 잡는 말: 「취약점
//   식별체계 기준 알려주고 김보안한테 **맡겨줘**」·「…담당자 **정해줘**」(둘 다 제목지목=O로
//   agentloop 제목지목 분기의 explain에 못 박히던 것 — 위 행동동사 목록에 「맡겨·정해」가 없다).
//   그래서 둘 다 남긴다 — 권위는 잣대(writeflow.쓰기흐름인가 · 잎 모듈 단일 출처), 여기는
//   **문서 자체를 조작하라는 명령**을 막는 belt다(비교개념질문의 배제어 vs 잣대와 같은 관계 —
//   agentloop.ts 비교개념질문 머리글 참고).
//   ⚠ 2026-09-12 검토관 [중] 정정 — 첫 판은 예문에 「침해사고 대응절차 항목 알려주고 **조치해줘**」를
//     들면서 「제목지목=O로 이 파일의 explain 분기에 못 박히던 것」이라 적었는데 **실측은 다르다**:
//     그 말은 dispatcher 특수경로 [33] 침해사고 의심이 훨씬 앞에서 채 가 침해사고초동절차로 간다
//     (제목지목질문까지 오지도 않는다). 재현하는 사람이 [33]을 보고 「수리가 안 됐다」고 오판하거나
//     침해사고 갈래를 건드려 초동 절차를 깨뜨릴 자리라 예문에서 뺐다 — 주석은 근거 기록이다.
// 제목을 댄 말에서만 더 인정하는 물음 꼴 — 내용동사RE에 없는 순수 물음(「차이를 비교해줘」).
// ⚠ 여기에 낱말을 더할 때 **행동동사가 아닌지** 먼저 본다(위 배제가 먼저 돌긴 하지만, 두 목록이
//   겹치면 어느 쪽이 이겼는지 사람이 못 읽는다).
const 제목물음RE = /(비교|차이|뭔지|궁금|알고\s*싶)/;
/**
 * 질문이 **문서를 콕 집어 그 내용·존재**를 묻는가(「이 안내서에서 암호화 대상 뭐야」·「이 매뉴얼 랜섬웨어
 * 대응 절차 있어?」) — 그러면 문서 RAG(explain)가 답해야 한다(2026-08-21 라이브 실측: 문서를 집었는데
 * 「랜섬웨어 대응 절차」가 침해 초동으로, 「암호화 대상」이 법령 검색으로 샜다). 지목 신호는 **(A) 문서유형어
 * +조사** 또는 **(B) 지시어+문서유형어**(조사 생략·하위참조도 포착)이며, 추천·소재·법령·ACTION 절차 의도는
 * 각자 주인(문서소재 search·law_lookup·침해/장애 플레이북)이라 앞에서 뺀다.
 */
export function 문서지목질문(instruction: string): boolean {
 ...
 * ★★ 2026-09-08 검토관 [높음] 수리 — **묻는 말에만 선다.**
 *   처음 판은 「제목을 통째로 댄 것 자체가 강한 신호」라며 내용동사를 일부러 뺐다. 그런데 이 갈래는
 *   explain(읽기 도구)으로 **못 박는** 자리라, 제목 토큰을 품은 **지시문**까지 문답으로 바꿔 버렸다 —
 *   결재판도 ⑨ 모델 선택도 못 거친다(위 행동의도RE 머리글의 실측 10/10). 그래서 두 겹을 더 둔다:
 *     · 행동의도(삭제·배정·올려·등록…+요청 어미) → 비켜선다. 시키는 말의 주인은 쓰기 도구다.
 *     · 내용·존재를 묻는 말이어야 한다(문서지목질문과 **같은 내용동사RE** + 제목물음RE).
 *   ⚠ 증상 넷은 그대로 산다 — 「금융 취약점 평가기준 항목 알려줘」·「…에서 안전성 요건 알려줘」·
 *     「…접속기록 점검 항목 뭐야」는 전부 내용동사를 이미 품고 있다(첫 판의 걱정은 사실이 아니었다).
 *   대가: 「VPN과 제로트러스트의 차이를 비교해줘」류를 살리려 제목물음RE(비교·차이·뭔지…)를 더 뒀다.
 *     그래도 안 걸리는 말(예: 제목만 덜렁 적은 한 줄)은 ⑨ 모델 선택으로 간다 — 이 커밋 전과 같은 길이다.
 */
export function 제목지목질문(instruction: string): boolean {
  ...
  if (법령질문RE.test(t)) return false;   // 법령 질문 → law_lookup
  if (소재RE.test(t)) return false;       // 문서 소재 → 문서소재 search
  if (행동의도RE.test(t)) return false;   // 시키는 말 → 쓰기 도구·결재판(읽기 도구로 못 박지 않는다)
  if (!내용동사RE.test(t) && !제목물음RE.test(t)) return false; // 내용·존재를 묻는 말이어야
...
// ── 표(마크다운 테이블) 인지 — **정의는 engine/tabletext.ts 한 곳이다** (2026-09-08 ⓐ3 · 2026-09-10 이사) ──
//   표줄·구분선·열수·칸가르기·표머리글들·표본문행자리는 그 파일에서 온다(맨 위 import).
//   ⚠ 여기로 도로 베끼지 말 것 — 옮긴 이유가 그것이다. 이 모듈이 DB·LanceDB를 물고 있어서
//     파서(webreport)·내보내기(inspectionreport)가 import를 못 하고 **사본을 만들었다**(실측 4벌).
//     server/test/tabletext.test.ts가 server/src 전부를 훑어 사본이 다시 생기면 빨개진다.
//   ⓘ 아래 머리글만남나()는 **청킹 전용**이라 여기 남긴다 — 옮기면 tabletext가 청커 개념을 알게 된다.
 ...
 *  ⚠ **판을 올려도 코퍼스 조각이 안 바뀔 수 있다 — 그래도 올린다**(2026-09-08 검토관 적발, 실측).
 *    table-2→table-3(표 본문 행 예외)과 table-3→table-4(열 수로 좁힘) 둘 다 매니페스트 35편의
 *    조각 지문을 **한 편도 안 바꿨다**(59경로/35문서 · 조각 2,719 · 지문 전부 동일). 즉 이 두 번의
 *    올림은 값어치 0에 비용(다음 기동 1회 재임베딩)만 실재한다 — 결함은 **아직 안 들어온 문서**
 *    (워드·pptx 표가 든 업로드본)에서 나는 것이고, 그 길은 매니페스트 밖이라 이 판을 안 본다.
 *    그런데도 올리는 이유: 「출력이 같으니 안 올려도 된다」를 사람이 그때그때 판정하기 시작하면
 *    언젠가 한 번 틀리고, 그 한 번이 **조용한** 옛 조각(2026-07-31 사고)이다. 판정이 필요 없는
 *    규칙이 판정이 정확한 규칙보다 싸다. 비용은 첫 기동이 길어지는 것뿐이고 눈에 보인다. */
export const CHUNKER_VERSION = "2026-09-08-table-4";
 ...
 *
 * ■ 반대 방향은 어디까지 감수하나 — **정직하게 적어 둔다**(2026-09-07 검토관 적발)
 *   사내 문답이 **정답인 물음**(승인된 그 문답을 되묻는 자리)에서는 topK=4일 때 몫이 1이라,
 *   관련 문답 여러 건이 top-4를 채우던 것이 **1건 + 비가족 3건**이 된다. 감수하는 이유:
 *   승인 문답 문서는 **문답 한 쌍이 통째로 한 조각**(문서당 1.006조각)이라 **한 자리에 답이
 *   다 들어온다** — 3칸을 주는 PDF와 달리 이어지는 문맥이 필요 없다. 여러 건이 함께 필요한
 *   구성은 바로 그 사고의 「다수결」 모양이다.
 *   ⚠ 그래도 **버려질 조각이 그 자리를 먹는 것**은 순수한 손해라, 아래에서 그것만 막는다.
...
export function 문서를섞어자르기<T extends { documentId?: string; distance?: number; lexicalHit?: boolean; acronymHit?: boolean }>(
  ...
  // ★★ **버려질 조각으로는 가족을 못 밀어낸다**(2026-09-07 검토관 적발 · 반대 방향 사고).
  //   몫은 「사내 문답이 법을 이긴다」를 막지만, 그 자리를 **무엇이** 메우는지는 안 봤다.
  //   후보를 16→64로 넓힌 뒤에는 깊은 순위의 무관한 조각까지 후보에 늘 있어, 사내 문답이
  //   **정답인 물음**(사람이 승인한 그 문답을 되묻는 자리)에서 관련 근거가 무관 조각으로
  //   바뀔 수 있다. 관련성 게이트(0.95)가 **뒤에서 어차피 버릴** 조각이라, 그런 조각이
  //   가족의 자리를 뺏으면 근거가 **순수하게 줄기만** 한다 — 고치려다 근거를 줄인 꼴이다.
  //   → 게이트를 통과 못 할 비가족 조각은 **가족이 밀려 있을 때에 한해** 맨 뒤로 미룬다.
  //   ⚠ 잣대를 새로 적지 않는다 — 관련성 판정은 hybridsearch.isRelevant 한 곳을 그대로 부른다.
  //   ⚠ 거리를 모르는 호출자(시험·다른 목록)는 **오늘과 똑같이** 돈다(관련 있음으로 본다).
  //   ⚠ **가족이 밀려 있을 때만** 건다 — 안 그러면 모든 물음의 자리 배분이 바뀐다(실측 없이
  //     전 질의를 흔들지 않는다). 가족 경쟁이 없으면 이 줄은 원리상 발화하지 않는다.
  ...
  const 미룸무관: T[] = [];  // 비가족인데 관련성 게이트에서 버려질 것 — 가족보다도 뒤
  for (const c of 목록) {
    if (담김.length >= topK) break;
    const d = c.documentId ?? "";
    const 가족 = 가족인가(d);
    if (가족 && 가족셈 >= 가족몫) { 미룸가족.push(c); continue; }
    if (!가족 && 미룸가족.length > 0 && !관련있음(c)) { 미룸무관.push(c); continue; }
    const n = 셈.get(d) ?? 0;
  ...
  // ⚠ 무관 조각은 **가족보다도 뒤**다 — 버려질 것이 살아 있을 근거를 앞지르지 않는다.
  for (const c of [...미룸, ...미룸가족, ...미룸무관]) {
    if (담김.length >= topK) break;
    담김.push(c);
  }
  return 담김;
...
async function hybridSearch(question: string, topK: number, agentId?: string, screen?: string, viewer?: Viewer): Promise<FusedChunk[]> {
  ...
  //   후보가 좁으면 그 칸을 1조각 문서 3,778건이 먹어, 가족 몫을 걸어도 **대체할 비가족 조각이
  //   후보에 아예 없다**. topK=16이 좋았던 그 폭(64)을 topK=4에도 준다.
  //   ⚠⚠ **「기존 상위의 순서는 안 바뀐다」는 거짓이었다**(2026-09-07 검토관 실측으로 철회).
  //     fuseResults는 RRF를 **더한다**(hybridsearch.ts:401·419 `prev.rrf += rrf`). 어휘 목록이
  //     깊어지면 벡터 상위 조각이 **없던 어휘 점수를 새로 받아** 1위가 뒤집힐 수 있다
  //     (반증 실행: 후보 16 → t0 1위, 후보 64 → t1 1위). 방향은 의도한 쪽이지만
  //     **순서가 바뀐다는 사실 자체**는 근거세기 배지·sourceTitles·인수인계 cited 판정에
  //     함께 걸리므로, 라이브 실측 전에는 「그대로다」라고 적지 않는다.
  //   ⚠ 아래 fuseVariantVectors(...).slice(0, candidates)가 **합집합을 candidates로 자른다** —
  //     옛 후보는 「3변형 합집합 48」이 아니라 그냥 16이었다(그 셈은 틀렸다. 결론의 방향은 같다).
  //   ⚠ .slice(0, topK)는 융합·부스트 **뒤**라 프롬프트 길이는 그대로다. **지연은 다시 안 쟀다** —
  //     위 22→19ms는 10→16 때 값이고 여기는 4배(벡터 3×64 + 어휘 64)라 그대로 끌어 쓸 수 없다.
 ...
 *
 * ★ 2026-09-06 되좁힘(검토관 실측) — 위 「꼬리는 ASCII만」의 안전 성질은 **꼬리에 한글이 있을 때만**
 *   성립한다. 접두를 한글까지 넓히면서 「보안점검:2024.pdf」·「월간보고:202409.xlsx」·
 *   「NIST:SP800-53.pdf」·「CVE-2021-44228:PoC.md」가 전부 ID로 판정돼 **제목이 사라졌다**
 *   (짝 시험 ⑦은 셋 다 꼬리에 한글이 있어 이 자리를 원리상 못 덮었다).
 *   → **꼬리가 파일 이름꼴(`.확장자`로 끝남)이면 ID가 아니다.** 사람이 붙인 제목은 확장자로 끝나고,
 *     우리 내부 ID는 그렇지 않다(승인문답:dtmtl40khqg8uehk · incident-case:ic-… · store:abc#1).
 *   ⚠ 확장자는 **글자만**(`.pdf`)으로 본다 — 「store:v1.2」처럼 숫자로 끝나는 토막은 여전히 ID다.
 *   ⚠ 실피해는 낮았다(결과가 「제목 생략」이라 누출이 아니다). 그래도 고치는 이유: 운영은 ext4라
 *     콜론 파일명이 legal이고, 제목을 지우면 담당자가 근거를 못 찾는다.
 */
const ID꼴_RE = /^[A-Za-z가-힣][A-Za-z0-9가-힣_-]*:(?![\s\S]*\.[A-Za-z]{1,5}$)[A-Za-z0-9_.#-]+$/;
...
export async function queryMemoryGraded(
  ...
  //   배지가 강함이 되면 근거세기가 거짓말을 한다.
  // ⚠ 이 한 줄이 **정확히 무엇을 보증하나**(2026-09-11 검토관 [중] — 옛 주석이 넓게 읽혔다):
  //   ⓐ 세기는 **집합 단위**다(some). 쓸것이 **전부** 약어로만 통과한 조각이면 약한근거만=true다.
  //      가까운 조각이 하나라도 섞이면 그 답은 강함이고, 약어로 통과한 조각도 그 답의 인용에
  //      함께 실린다 — 조각별 배지는 원래 없다(0.94짜리가 강함 답에 실리던 것과 같은 자리).
  //   ⓑ 약한근거만을 **읽는 쪽까지가 보증 범위**다: chat 경로는 llm.ts가 근거약함배너를 붙이고,
  //      도구 경로(explain)는 도구근거보고가 근거세기 「약함」 배지를 올린다. 도구 경로에는
  //      근거약함**배너**가 원리상 없다(그 경로는 RAG를 꺼서 ragResult 자체가 없다) — 옛 주석의
  //      「근거약함배너가 코드로 붙는다」는 chat 경로에서만 참이었다. 약속을 코드에 맞춰 적는다.
  //   ⓒ 「해도 되나」 판정(actioncheck)은 약어로만 통과한 조각을 **판정 근거에서 뺀다**(그 파일
  //      약어로만통과 참고) — 거기엔 약함을 말할 칸이 없어서다.
