...
// 절차 의도(대응하려면/초동/복구하려면)는 침해·장애 플레이북 몫이라 문서 지목이라도 비켜 준다
//   — 「있어?/뭐야」(내용·존재)는 문서로, 「대응하려면?」(절차)은 플레이북으로 갈린다.
const 절차의도RE = /(대응하려면|대응\s*방법|어떻게\s*(대응|해야|조치|처리)|뭐부터\s*(해|하나)|첫\s*조치|초동\s*(절차|대응)|복구하려면)/;
/**
 * 질문이 **등록 업로드 문서를 콕 집어 그 내용·존재**를 묻는가 — 그러면 문서 RAG(explain)가 답해야
 * 한다(2026-08-21 라이브 실측: "이 안내서에 랜섬웨어 대응 절차 있어?"가 침해 초동으로, "안내서에서
 * 암호화 대상"이 법령 검색으로 샜다 — 낱말 트리거·LLM 라우터가 문서 검색을 가로챘다).
 * 신호 = docScopeMatch(등록 업로드 문서명) — **RAG 랭킹과 같은 잣대**. ⚠ 법령·시행령·고시는
 * 업로드 문서가 아니라 안 잡혀 **법령 질문을 안 삼킨다**(설계관 「법-안전」). 내장 문서는 docScope
 * 대상이 아니라 종전대로 흐른다(이 결함은 업로드 문서에서 났다).
 */
export function 문서지목질문(instruction: string): boolean {
  const t = String(instruction ?? "");
  if (절차의도RE.test(t)) return false;
  return docScopeMatch(t, uploadedDocIds()).size > 0;
}
