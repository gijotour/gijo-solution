...
// ★★ 2026-09-12 실측(B10-①) — 이 정규식과 agentloop.쓰기흐름인가는 **서로를 포함하지 않는다.**
//   행동의도RE만 잡는 말: 「…실무매뉴얼 새 버전으로 **올려줘**」(등록·업로드·내보내 계열 —
//   위 행동동사 목록엔 있지만 쓰기흐름인가는 「올려」를 모른다). 쓰기흐름인가만 잡는 말: 「취약점
//   식별체계 기준 알려주고 김보안한테 **맡겨줘**」·「…담당자 **정해줘**」·「침해사고 대응절차
//   항목 알려주고 **조치해줘**」(전부 제목지목=O로 이 파일의 explain 분기에 못 박히던 것 —
//   위 행동동사 목록에 「맡겨·정해·조치해」가 없다). 그래서 둘 다 남긴다 — 권위는 잣대
//   (agentloop.쓰기흐름인가), 여기는 **문서 자체를 조작하라는 명령**을 막는 belt다(비교개념질문의
//   배제어 vs 잣대와 같은 관계 — agentloop.ts 비교개념질문 머리글 참고).
