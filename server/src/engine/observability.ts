...
import { 임베딩응답확인, 임베딩나눠쓰기 } from "./embedding";
...
async function checkModel(): Promise<HealthCheck> {
  try {
    ...
    if (상주.length === 0) {
      ...
      // ⚠ 「응답한다」를 넘겨짚지 않는다(2026-09-10 검토관 적발 2건):
      //   · 서버가 스스로 「모델 올리는 중」이라 말하면(적재중) 아직 답할 수 없다 — 초록이 아니라 노랑이다.
      //   · 「이 기계가 아니다」라고 단정하지 않는다. 같은 기계에 손으로 띄운(또는 고아로 남은) 서버일 수도 있다.
      //     우리가 실제로 아는 것은 **이 앱이 띄운 것이 아니라는 것**뿐이다.
      if (외부.alive && 외부.적재중) {
        return {
          id: "model", label: "AI 모델", level: "warn",
          detail: "AI 서버가 모델을 올리는 중입니다 — 잠시 뒤에 답할 수 있게 됩니다(이 앱이 띄운 서버가 아닙니다)",
        };
      }
      if (외부.alive) {
        return {
          id: "model", label: "AI 모델", level: "ok",
          detail: `AI 서버가 응답합니다${외부.modelId ? ` · ${외부.modelId}` : ""} — 이 앱이 띄운 것이 아니라 나눠 쓰는 두뇌입니다(실제로 답을 시켜 보지는 않았습니다)`,
        };
      }
      return {
        id: "model", label: "AI 모델", level: "warn",
    ...
    if (!e || !e.running) {
      ...
      if (await 임베딩응답확인()) {
        return {
          ...
          detail: "임베딩 서버가 응답합니다 — 이 앱이 띄운 것이 아니라 나눠 쓰는 두뇌입니다",
        };
      }
      // ⚠ 안내를 갈라 놓는다(2026-09-10 검토관 적발): 이 갈래는 이제 「프로세스가 없다」뿐 아니라
      //   **「나눠 쓰는 주소가 지금 안 답한다」**일 때도 온다. 나눠 쓰는 설치는 제 모델 폴더를
      //   **일부러 비워 두는** 것이 자물쇠라, 거기에 대고 「models/ 아래 파일을 확인하세요」라고
      //   하면 따라 할수록 나빠진다. 「나눠 쓰나」는 주인 파일(embedding.ts)이 답한다.
      if (임베딩나눠쓰기()) {
        return {
          id: "embedding", label: "문서 검색 엔진(임베딩)", level: "fail",
          detail: "나눠 쓰는 임베딩 서버가 지금 응답하지 않습니다 — 문서 올리기와 지식 검색이 동작하지 않습니다",
          action: "임베딩을 띄워 둔 기계가 켜져 있는지, 그 서버가 살아 있는지 확인하세요. 이 기계의 모델 폴더는 비워 두는 것이 정상입니다.",
        };
      }
      return {
        id: "embedding", label: "문서 검색 엔진(임베딩)", level: "fail",
