...
const 표: Record<string, string[]> = {
  ...
  collect_packages: ["AI-BOM 현황 알려줘", "SBOM 없는 자산 알려줘"],
  restore_lost_findings: ["미조치 취약점 뭐 있어?", "오늘 뭐부터 할까?"],
  alert_schedule_add: ["다음 정기 리포트 언제야?", "오늘 브리핑"],
  schedule_maintenance: ["점검 일정 현황 알려줘", "승인 기다리는 것 있어?"],
  assign_owner: ["자산 현황 어때?", "미조치 취약점 뭐 있어?"],
  generate_sbom: ["AI-BOM 현황 알려줘", "SBOM 없는 자산 알려줘"],
  ...
  register_product_intro: ["보안제품 현황 알려줘", "점검 일정 현황 알려줘"],
