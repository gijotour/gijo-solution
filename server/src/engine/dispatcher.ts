...
import { 새두뇌표식수거, 두뇌표식을수거하며, 두뇌표식실어보내기, 두뇌기록실어보내기, type 두뇌기록항목 } from "./brainmark";
...
import { 근거없음종류판정, 배너억제_근거약함, 근거범위, type 근거없음종류 } from "./noevidence";
...
export interface DispatchResult {
   ...
   *   · 근거없음 = 담당자가 답에서 **실제로 읽는 배너 문장**(noevidence.ts 판정, 살균 후 0건).
  ...
  근거없음?: 근거없음종류;
  /**
   * 옅게 할 **범위** — 「어디를」만 담는다(2026-09-06 · 시안 mockups/dim-range 승인).
   * `{단계:[2]}` = 2단계 절 전부 · `{수치:["12.5%"]}` = 그 표면형만 · 둘 다면 **OR** · **없으면 오늘 동작**.
   *   (OR인 이유와 그것이 치르는 값은 noevidence.근거범위값 머리말에 있다 — 2026-09-06 수리.)
   *
   * ⚠ **근거없음 옆 칸이다 — 그 칸을 객체로 바꾸지 않는다.** 근거없음은 지금 문자열로 세 곳이
   *   읽고(console.js 2 · chatwidget.js 1) answersamples가 DB에 그대로 적는다. 객체로 갈아치우면
   *   화면은 조용히 아무것도 안 옅게 하고 표본 분포는 「[object Object]」가 된다.
   * ⚠ 값은 **답 글자에서 다시 읽는다**(noevidence.근거범위) — 저장하지 않는다. 그래야 실시간과
   *   이어보기(worksessions GET)가 같은 값을 낸다.
   */
  근거범위?: { 단계?: number[]; 수치?: string[] };
   ...
   * ⚠ `조각`은 **언제나 배열이다** — 가드가 판정을 보류한 호출(RAG 미실행·검색 실패 → null)은
   *   애초에 기록을 안 만든다(citesource.ts 머리말 ★★★). 그래서 **이 칸이 있으면 「잰 것」이다.**
   *   그런 답은 칸이 통째로 없고, 재생은 그 행을 종전 갈래(근사)로 돌린다.
   *   ⚠ `[]`(0건)은 다르다 — 가드가 실제로 돌았고 대조할 조각이 없었다는 판정이라 그대로 실린다.
   * ⚠ `보고횟수 > 1`이면 한 턴에 **재는** chat이 여러 번 돈 답이라 **이 기록은 답 전체의 근거가 아니다.**
   *
   * ★★ **무엇이 등급 누출이 아니고, 무엇은 안 걸러지는가**(2026-09-08 검토관 [낮음] — 나눠 적는다)
   *   · `조각`은 검색 단계에서 열람권(hiddenDocIds)으로 걸리고 살균(ragsanitize)을 지난 글이며,
   *     이 답을 받은 **같은 사람**이 이미 근거로 본 것이다(quotes는 비-qa에도 나간다).
   *   · `추가원천`은 **그 두 관문을 안 지난다.** 온톨로지 블록(ontology.ts:259 ontologyContextFor —
   *     viewer 인자가 아예 없다) · 용어 그라운딩 · 📎 첨부글(worksessions.ts:563 attachSessionText —
   *     소유자 검사가 없다)이 그대로 실린다. 살균은 조각에만 건다.
   *     ↳ 그래도 **권한 상승은 아니다**: 같은 로그인 사용자가 `GET /api/ontology/triples`
   *       (ontology.ts:303 — authMiddleware만)와 `GET /api/work-sessions/:id`
   *       (worksessions.ts:762 — authMiddleware만)로 **이미 그대로 받는 값**이다. 창구가 하나 는 것뿐이다.
   *     ↳ 그 두 창구에 열람권을 걸게 되면 **이 칸도 함께 걸어야 한다** — 여기 적어 두는 이유다.
   *   · `qa`는 **역할 검사가 아니라 요청 본문 플래그**다(:2244 `req.body?.qa === true`). 로그인한
   *     사람이면 누구나 켤 수 있다 — 「사람 응답에는 없다」는 **기본값**이지 권한 경계가 아니다.
   ...
   *   담당자에게 필요한 것은 「지금 바깥으로 나갔나」이고, 모델 id는 하네스가 회차를 견주는
   *   재료다 — **기본값을 좁힌 것**이지 권한 경계가 아니다. 위 근거원천 주석이 적어 둔 그대로
   *   `qa`는 `req.body?.qa === true`뿐이라 로그인한 사람이면 누구나 켤 수 있다(2026-09-10
   *   검토관 — 처음엔 여기에 「설정 권한의 몫」이라 적었는데 코드가 그 말을 안 지켰다).
   ...
   *   ⚠ **내부 호출(의도 분류기·도구 결정)도 표식을 안 남긴다** — 담당자가 읽는 답(explain)만
   *     담는다(brainmark ★★★). 이 잣대가 없던 동안 `brain`은 사실상 언제나 `{local,false}`였다.
  ...
  /**
   * 🧠 **팀원마다 어느 두뇌가 돌았나** — `qa: true` 요청에만 실린다(사람 응답에는 칸조차 없다).
   *
   * ★ 왜 칸을 뒀나(2026-09-10 · ⑲ 첫 실측 23/27) — 위 `brain`은 **답 하나에 하나**다(첫 chat ·
   *   담당자가 읽는 답). 그런데 야간 회귀 마당 ⑲가 재려는 것은 「팀원마다 다른 두뇌로 실제
   *   도는가」(정의 A)인데, 원격 배정 팀원 셋(report·normaltic·ti)의 chat은 explain 없이 돌아
   *   표식이 **원리상** 안 생긴다 — ⑲는 그 셋을 「돎(표식 없음)」으로만 적었고, 「원격에
   *   배정했다」는 말의 증거가 기록 어디에도 없었다. 이 칸이 그 자리를 메운다.
   * ⚠ 담기는 것은 **LLM에 실제로 요청이 나간 chat 전부**다 — 분류기·도구 결정도 담는다
   *   (그것을 빼면 정작 원격 팀원이 통째로 안 보인다). 반대로 인사말·자료 없음처럼 LLM을
   *   아예 안 부른 답은 여기까지 오지 않아 칸이 안 생긴다.
   * ⚠ `brain`과 **어긋나지 않는다**: 배열에서 「사람이읽는답 && !결정호출」인 첫 항목이 곧
   *   `brain`이다(brainmark 한 곳이 같은 보고로 둘을 만든다 — 두 벌 잣대를 두지 않는다).
   * ⚠ `qa`는 권한 경계가 아니다(`req.body?.qa === true`) — 위 두 칸의 주석이 적어 둔 그대로,
   *   **기본값을 좁힌 것**이다. 새는 값은 팀원 id·모델 id·위치뿐이고 주소·토큰은 없다.
   * ⚠ 마지막 항목의 `생략`이 0이 아니면 상한(brainmark 두뇌기록상한)에 걸려 **잘린 기록**이다.
   */
  두뇌기록?: 두뇌기록항목[];
}
...
// ⚠ 2026-09-14 검토관 [하] 수리 — 「뒤·다음·후」가 서로 다르게 굴렀다. 「스캔 후 리포트」는
//   `후에?\s`에 걸려 2단계인데, 뜻이 같은 「스캔 뒤 리포트」·「스캔 다음 리포트」는 앞에 「한」이
//   없어 1단계로 줄었다 — **시킨 스캔이 조용히 빠지는** 쪽이라 대칭을 맞춘다. 전수 재측
//   (말뭉치 437문장)에서 새로 2단계가 되는 문장은 0건이다.
export const STEP_JOIN_RE = /하고|해서|한\s*(?:뒤|다음|후)|후에?\s|\s(?:뒤|다음)\s|그리고|,|·|및|→|다음에|끝나면|되면/;
...
// ★★ 2026-09-11 재수리(검토관 [중] 적발 · 실측) — 「결재 **승인 대기** 있어?」는 **조회**다.
//   이 갈래는 특수경로라 강제 도구(FORCED_INTENTS)보다 **앞**에서 즉시 반환한다. 그래서
//   첫 갈래가 「결재 뒤 20자 안에 승인」만 보고 채 가면, 숫자 없는 결재판 안내만 나가고
//   결재·승인 대기 현황 도구(approval_status)에 **영영 안 닿는다** — 그날 새로 만든
//   「종류를 안 밝힌 물음에 숫자로 답한다」가 이 말투에서만 성립하지 않았다.
//   실측(결정적도착지): 「결재 승인 대기 있어?」의 첫 걸림이 이 갈래였다.
//   → 첫 갈래에 「결재 … 승인/반려 + 대기/기다리」 배제를 건다. **쓰기는 그대로 잡힌다** —
//     「결재 승인 대기 처리해줘」·「승인 대기 있으면 승인해줘」는 둘째 갈래(…처리해/승인해)가 받는다.
//   시험: server/test/approval-routing.test.ts(조회는 approval_status까지 닿는다).
export const 결재승인요청_RE = /(?!결재[^.\n]{0,20}(?:승인|반려)\s*(?:대기|기다리))결재[^.\n]{0,20}(승인|반려|처리|올려|처리해)|(승인|반려)\s*(대기|요청|올린|낸)[^.\n]{0,10}(승인|반려|처리)해/;
...
async function learnloopConfirmResult(instructionText: string, qa?: boolean): Promise<DispatchResult> {
  ...
  // 화면 자리는 screenguide 흡수자리 표 **한 곳**에서 온다. 표에서 항목이 빠지면 빈 문자열이 오므로
  //   (2026-09-10 검토관 [중]) 자리 말을 통째로 뺀다 — 「▸ 이어서 —  거기 …」로 자리 없는 문장이
  //   나가느니 단추만 말하는 편이 정직하다. 없는 자리를 지어내지도 않는다.
  ...
  const output = [
    ...
    //   ⚠ 화면 자리는 **베끼지 않는다** — screenguide의 흡수자리 표가 단일 출처다(위 `자리`).
    //   ⚠ 「학습 시작」 칩은 **개시선에 닿은 주제에만** 그려진다(learnloop.html 주제 띠 — 미달이면
    //     진행 막대뿐이다). 조건 없이 가리키면 미달 주제만 가진 담당자가 없는 칩을 찾는다
    //     (2026-09-10 검토관 [중]). 그래서 「✓학습 가능이 붙은 주제」라는 조건을 함께 적는다.
    //   ⚠ 관리자만 누를 수 있다(learnloop.html:574·594, 서버 재검증 learnloop.ts) — 안 적으면
    //     담당자가 눌러 보고 403을 만난다.
    `▸ 이어서 — ${자리 ? `${자리} 거기 ` : ""}「🔄 루프 실행 (원클릭)」 단추로 시작합니다(관리자만 누를 수 있습니다 — 위 진척에 ✓학습 가능이 붙은 주제는 주제별 띠의 「학습 시작」 칩으로도 시작합니다).`,
  ].join("\n");
...
export async function dispatchInstruction(instructionText: string, sessionId?: string, screen?: string, actor?: string, qa?: boolean, noLearn?: boolean, viewer?: Viewer, 선택?: string): Promise<DispatchResult> {
  ...
  // 📐 옅힘 **범위** — 표식이 붙을 때만 읽는다. 배너가 붙은 단계 번호 + 배너 꼬리의 수치 표면형이
  //   전부이고, 둘 다 **답에 이미 실려 나간 글자**라 이어보기에서도 같은 값이 나온다(noevidence).
  //   ⚠ 배너억제 갈래(배너가 아예 없는 답)는 범위가 undefined다 — 그 답은 종전대로 전체가 범위다.
  const 근거범위값 = 근거없음 ? 근거범위(거른것.output) : undefined;
  const 걸러진 = 근거없음
    ? (근거범위값 ? { ...거른것, 근거없음, 근거범위: 근거범위값 } : { ...거른것, 근거없음 })
    : 거른것;
...
export function registerDispatcherRoutes(app: Express): void {
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      if (first !== null) {
        ...
        // 🧠🧠 **팀원마다 어느 두뇌였나**(2026-09-10 둘째 판) — qa에만 실린다.
        //   왜 여기인가: 위 `brain`은 답 하나뿐이라 explain 없이 도는 원격 팀원 셋을 못 본다
        //   (⑲ 첫 실측 23/27 — 그 셋이 「돎(표식 없음)」으로만 남았다).
        //   ⚠ **가르는 잣대는 라우트가 아니라 brainmark 한 곳**이다 — 여기 `if (qa)`를 적으면
        //     입구가 늘 때 한쪽만 고쳐져 사람 응답에 팀원 id·모델 이름이 새는 날이 온다.
        const 두뇌기록 = 두뇌기록실어보내기(두뇌그릇, qa);
        if (두뇌기록) first.두뇌기록 = 두뇌기록;
        res.json(first);
        return;
      }
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      //   ⚠ **`두뇌기록`(qa 전용 배열)은 여기 없다**(2026-09-10 둘째 판에서 갈린 자리) — 표식과
      //     달리 그것은 사람이 볼 신호가 아니라 하네스가 회차를 견주는 재료고, 하네스는
      //     /api/dispatch만 쓴다. 근거원천이 여기 없는 것과 **같은 까닭**이라 같은 쪽으로 정했다.
      //     스트림에서도 재야 할 날이 오면 `두뇌기록실어보내기(두뇌그릇, qa)` 한 줄을 아래 done에
      //     더한다(잣대는 그 함수 안에 있으니 여기서 qa를 다시 가르지 말 것). 「샜다」가 아니다.
      const 두뇌그릇 = 새두뇌표식수거();
      try {
