...
//     「지금 바깥으로 나갔나」이고, 어느 모델인지는 설정 권한의 몫이다 —
//     remotellm.ts의 /where가 주소를 안 알려주는 것과 같은 자세다.
...
export interface 두뇌표식 {
  ...
  model: string | null;
  /**
   * 이 요청에서 **답을 만든** chat이 돈 횟수(결정 호출은 안 센다 — 아래 두뇌표식보고).
 ...
 *
 * ★★ **결정 호출(responseSchema)은 답한 두뇌가 아니다** — citesource가 겪은 함정의 대칭이다.
 *   한 요청의 **첫** chat()은 답이 아니라 의도 분류기다(dispatcher routeIntent → intent.ts:106).
 *   총괄은 언제나 이 PC라(llm.ts resolveRemoteTarget ⓪) 그 호출을 담으면 표식이 **언제나
 *   local**이 되어, 원격으로 답한 날에도 「이 PC가 답했다」는 거짓 기록이 남는다. 분류기는
 *   도구를 고르는 내부 결정문이지 담당자가 읽는 답이 아니므로 여기서 **값으로** 건너뛴다.
...
export function 두뇌표식보고(보고: {
  ...
  if (!그릇) return;
  if (보고.결정호출) return; // ★★ 분류기는 「답한 두뇌」가 아니다(위 머리말)
