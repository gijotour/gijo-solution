...
import { db } from "../db";
import { 본문근거없음표지 } from "./noevidence"; // 「본문 근거 없음」 표지의 주인(문장을 여기 베끼지 않는다)
...
export const NO_ANSWER = [
  ...
  // 「사내 자료에 없는 수치」(숫자무근거 배너, 2026-09-06) — 답은 나왔는데 그 숫자를 뒷받침할
  //   사내 자료가 없다는 뜻이라 **지식 구멍 신호**다. 학습 후보로도 삼지 않는다(추정치를 가르치게 된다).
  { re: /찾을 수 없|찾지 못|정보(가|는|이) 없|해당 내용(이|은) 없|자료(가|는|도) 없|근거 약함|사내 자료에 없는 수치/, 이유: "사내 자료에서 못 찾음" },
  // 「사내 문서 본문 근거는 없습니다 …」(B2 표지, 2026-09-11) — 온톨로지·등록부 목록으로만 답한
  //   자리라 **채워야 할 문서가 있다는 신호**다. 위 정규식에는 안 걸린다(조사가 「근거는」이다).
  //   ⚠ 문장을 여기 다시 적지 않는다 — 주인은 noevidence.ts다(같은 사실은 한 곳에).
  { re: 본문근거없음표지, 이유: "사내 자료에서 못 찾음" },
