...
//
// ★ 2026-09-06 정정 — **어느 층에서 잰 숫자인가**(검토관 적발).
//   이 표를 넣은 커밋(33608422)이 「전 1/5 → 후 4/5 · 돌아선 것은 클릭률·전환율」이라 적었는데,
//   그 A/B는 **LLM 층**(llama-server에 뜻 블록만 넣고 뺀 대조)에서 잰 값이다. 제품 층에는
//   그 앞뒤로 scopeguard·라우팅·RAG·근거 배너가 더 있어 **같은 숫자가 아니다** — 실제로
//   전환율은 위 함정 때문에 제품에서는 모델에 닿지도 못했다(그 수리가 이 커밋이다).
//   → 제품 층 숫자는 **하네스(ops-sim ⑯)를 실서버에 돌려** 다시 적는다. 층이 다른 두 숫자를
//     한 문장에 전/후로 쓰지 않는다(이 저장소가 반복해 겪은 「딴 환경을 재고 좋아졌다고 말하기」).
...
// ── **남을 가리키는 물음**에는 뜻을 안 깐다 (2026-09-06 검토관 적발) ──────────
//
// 무엇이 어긋났나: 라우팅 층 FORCED_INTENTS[83]은 「업계 평균 클릭률」·「타사 피싱 클릭률 통계」를
//   일부러 비켜 준다 — 그건 우리 실적이 아니라 **남의 통계**라 kpi_status의 답이 아니기 때문이다
//   (routes.ts [83]: 「업계 평균 클릭률·국내 백신 설치율 통계까지 채 가서 되돌린 전례」).
//   그런데 뜻 주입에는 그 배제가 없어서, 같은 물음에 「이 제품에서의 뜻 … 마케팅 CTR이 아닙니다」가
//   깔렸다(탐침 실측 4문장 전부). **라우팅이 좁힌 것을 그라운딩이 도로 넓히는 꼴**이다.
//   업계 평균을 물었는데 「우리 피싱 훈련 지표로 답하라」고 시키면 답이 틀리는 쪽으로 밀린다.
//
// ⚠ [83]의 배제어를 **통째로 베끼지 않는다** — 저쪽은 뜻풀이 갈래(뭐야·정의)와 개선 동사
//   (올리·낮추)까지 함께 뺀다. 그 둘은 여기서 **뜻이 꼭 필요한** 물음이다(「전환율이 뭐야?」가
//   ⑯ 마당의 문항이다). 여기 필요한 건 「남을 가리키는 낱말」 갈래 하나뿐이라 그것만 적고,
//   두 목록이 어긋나지 않게 domainsense.test가 **소스에서 떼어 대조**한다.
export const 남의지표_RE = /업계|타사|동종|국내|해외|글로벌|일반적|평균적|다른\s*(?:회사|기업|기관|조직)/;
...
export function glossaryGroundingFor(question: string): string | null {
  ...
  //   남을 가리키는 물음(업계 평균·타사 통계)에는 안 깐다 — 위 남의지표_RE 머리글 참고.
  const 도메인 = 남의지표_RE.test(question)
    ? []
    : Object.keys(DOMAIN_SENSE)
