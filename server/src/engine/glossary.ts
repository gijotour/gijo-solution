...
//   이 제품에서 클릭률·열람률·전환율·신고율·참여율·이수율·응답률은 전부 **피싱 모의훈련·보안 교육**의
...
//
// ★ 2026-09-07 신고율 추가 — 이 표에서 **방향이 반대인 첫 낱말**이다.
//   지식 문서 4절(knowledge/GIJO_지식_보안인식교육.md)이 신고율을 클릭률의 짝으로 못 박는다:
//   「클릭률과 신고율은 반대 방향인데 함께 봐야 한다 — 둘 다 낮으면 좋아진 게 아니라 아무도
//   반응하지 않은 것이다.」 그런데 표에는 클릭률만 있었다.
//   왜 다른 여섯보다 급한가: 나머지는 뜻을 놓치면 **딴 분야의 답**이 나가는 데서 그치지만,
//   신고율은 모델이 학습한 세상의 주인이 범죄·민원·세금 신고라 **방향까지 뒤집힌다** —
//   「신고율을 낮추려면」이라 조언하는 순간, 침해 대응에 가장 필요한 습관(즉시 신고)을
//   없애라는 말이 된다. 그래서 뜻 한 줄 안에 **방향을 함께** 적었다(줄이 잘려도 안 떨어지게).
//   ⚠ **배타 주장(「유일하게」)은 적지 않는다** — 2026-09-07 검토관 적발. 첫 판이 「모의훈련
//     지표 중 유일하게 높을수록 좋다」였는데, 문서는 「**피싱** 모의훈련 지표 중에서는 유일하게
//     … 교육 쪽 지표(응답률·이수율·참여율)도 높을수록 좋으니 「높을수록 좋은 지표는 신고율뿐」이
//     아니다」라 못 박는다(4절 표의 ↑는 신고율·응답률·이수율·참여율 **넷**이다).
//     더 나쁜 자리: 이 표는 한 번에 **두 줄**이 함께 깔려(DOMAIN_SENSE_MAX=2) 바로 옆에
//     「참여율: 보안 교육·**모의훈련** 대상자 중 …」이 붙는다 — 한 프롬프트 안에서 두 줄이
//     서로를 부정한다. 그래서 방향은 **자기 것만** 적는다.
//   ⚠ 늘릴 때도 같다: 방향 낱말은 문서 4절 표가 정본이고, domainsense.test가 그 표를 **떼어 읽어**
//     ⓐ방향 일치 ⓑ배타 주장 금지 ⓒ남의 지표 방향 규정 금지를 문다(여기 베껴 적지 말 것).
export const DOMAIN_SENSE: Record<string, string> = {
  ...
  "신고율": "피싱 모의훈련에서 훈련 메일을 신고 창구로 알린 대상자 비율(높을수록 좋다 — 클릭률과 반대 방향이라 함께 본다). 범죄·민원·세금 신고율이 아닙니다.",
