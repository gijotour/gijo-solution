...
//   · 쪽을 넘어가는 표 이어붙이기 — 유일한 실측 사례(사용자_매뉴얼 p10→p11)는 **원본이 머리글을
//     다시 그린다.** 이으면 뒤 조각이 머리글을 잃는다. 그래서 쪽마다 따로 낸다.
...
type 쪽 = { rotate: number; getOperatorList(): Promise<{ fnArray: number[]; argsArray: unknown[] }>; getTextContent(): Promise<{ items: unknown[] }> };
...
export async function pdf표복원(doc: 문서): Promise<string | null> {
  ...
  for (let p = 1; p <= doc.numPages; p += 1) {
    ...
    if (page.rotate) { 쪽별격자.push(null); continue; }
  ...
  const 쪽글들: string[] = [];
  for (let p = 1; p <= doc.numPages; p += 1) {
    const items = (await (await doc.getPage(p)).getTextContent()).items as 글자항목[];
