...
//
// ⚠ 파이프 표를 **여기서 새로 짜지 않는다** — tabletext.ts의 파이프표()·칸글()을 그대로 쓴다.
//   두 벌이 되면 열 수 맞춤·파이프 이스케이프·구분선 규격이 갈리고, 그 순간 memory.ts의 표 술어가
//   한쪽을 표로 안 읽는다(같은 것을 여러 곳에 적으면 어긋난다).
// ⚠ 「무엇이 표인가」의 판정도 tabletext.ts 한 곳이다(2026-09-10 이사 전에는 memory.ts였다).
import { 파이프표, 칸글 } from "./tabletext"; // 잎(import 0) — dataset을 물지 않아 되먹임 화살이 끊긴다
