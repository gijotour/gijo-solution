 ...
 *    종전에는 60자를 넘겨 원리상 안 걸리던 갈래라, 이 파일의 변경이 새로 연 노출이었다.
 *    고친 자리는 memory.ts 한 곳(표 술어의 단일 출처) — 커밋 3d092899(본문 행 예외를 **자리로**)
 *    + 그 뒤 열 수로 좁힘. CHUNKER_VERSION도 함께 올라갔다(2026-09-08-table-4).
 *    ★ 여기서 지켜야 할 계약: **한 표의 모든 행을 같은 열 수로 낸다.** 저쪽 예외가 「구분선과
 *      열 수가 같은 줄」까지라, 행마다 열 수가 흔들리면 그 행은 다시 지워진다(파이프표()가
 *      이미 열수로 맞춰 낸다 — 이 규칙을 깨는 손질을 하지 말 것). 짝 시험은
