...
export async function askCloud(question: string, user?: GijoUser): Promise<CloudAskResult> {
  ...
  try {
    ...
    // ★★ 그런데 **chunks=[]만으로는 반쪽이 아니라 반대로 위험하다**(2026-09-05 검토관).
    //   원천이 통째로 비면 citeguard의 「원천에있나·제목그대로있나·맨몸출처판정」이 **원리상
    //   언제나 미확인**이라, 가드가 「지어냈나」를 재는 것이 아니라 **출처 표기를 무조건 지우는
    //   필터**로 돈다. 실측: 「출처: Verizon DBIR 2023」이 매번 사라지고, 짧은 정의형 답은
    //   통째교체까지 가서 「등록된 사내 자료에는 관련 내용이 없습니다」가 붙었다 — 사내 자료를
    //   **쓴 적도 없는** 답에 확인 안 한 단정을 얹고, 사용자가 **일부러 고른** 클라우드 대신
    //   「다른 에이전트에게 물어보세요」로 보내는 문장이다(화면의 제공자 배지와도 어긋난다).
    // → 그래서 `원천없는출구`로 부른다. 이 자리에서 **확정된 거짓은 번호 인용뿐**이다:
    //   [n]은 있지도 않은 근거 블록을 가리킨다. 따옴표 인용·출처 이름은 대조 자체가 불가능하니
