...
export function 검수요약문(id?: string): string {
  ...
  // ★ 저장본 요약이 아니라 **지금 규칙으로 다시 판정한 것**을 센다(검수상세와 같은 이유 — 규칙이 바뀌면 저장본은
  //   낡는다). 그리고 판정불가를 「진짜 모름」과 「알아본 조건부(NC·ND·개별 라이선스)」로 가른다(2026-09-03 검토관):
  //   전에는 둘을 합쳐 「라이선스를 알 수 없는 부품 N개 — 공급사에 확인」이라 말해, 알아본 비영리 부품까지
  //   「모른다」로 세는 거짓이 요약 줄에 남아 있었다.
  const 판정들 = (부품읽기.all(대상.id) as { license?: unknown }[]).map((c) => 등급판정(String(c.license ?? "")));
  const 요약 = 판정들.length ? 등급요약(판정들) : null;
  const s = 요약 ? 요약.등급별 : 대상.summary;
  const 알아본 = 요약?.알아본판정불가 ?? 0;
  const 진짜모름 = Math.max(0, (s.판정불가 ?? 0) - 알아본);
