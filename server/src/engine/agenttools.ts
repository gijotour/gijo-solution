...
const TOOLS: AgentTool[] = [
  ...
  },
  {
    ...
    write: false,
    description:
      '지금 조치할 취약점 우선순위를 전 자산을 가로질러 **보여준다(조회)**(KEV→EPSS→VPR 순, 담당자·기한·지연 포함). "오늘 뭐부터?", "제일 급한 취약점", "우선순위 높은 거", "지금 급한 거", "뭐부터 조치해"에 쓴다. **단 담당자·기한을 배정/지정하라는 지시("배정해줘","담당자 지정")는 assign_finding으로 간다.** 예: {"limit":"5"}',
