 ...
 * 아래 ⓐ-1이 조건 없이 바이너리로 잡아야 하는 신호다. 여기서 걷으면 그 증거가 사라진다.
 *
 ...
 *   그래서 이 함수가 도는 자리는 **추출기 꼬리 한 쌍**뿐이다(dataset.ts 파이썬꼬리정규화 ·
 *   extract_doc.py main() 꼬리) — 진짜 파서가 글자를 뽑아낸 뒤다. 날것 바이트가 그대로
 *   들어오는 텍스트 계열 경로(.txt·.md를 바로 읽는 자리)는 **일부러 안 지난다.**
 */
...
export function isBinaryLikeChunk(text: string): boolean {
  ...
  //     실측 글자율 — BEL PDF 94.4% · 정상 한국어 93.8% · 정상 표 97.4% · 실측 NUL 인보이스 97.7%
  //              vs 진짜 PDF 바이트 57.5% · 인입 쓰레기 표본 58.3%. 문턱 0.8이 양쪽에 20pt 넘게 여유.
  //   ⚠ 이 문턱은 **제어문자가 2%를 넘을 때만** 본다. 기호가 많은 정상 문서(JSON 로그 61%·
  //     iptables 79%)는 제어문자가 0%라 여기까지 오지 않는다 — 문턱을 낮추기 전에 그것부터 확인할 것.
