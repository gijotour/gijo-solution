...
const INSTRUCTION_SIGNALS: { re: RegExp; label: string }[] = [
  ...
  { re: /(앞의|이전|위의|원래)\s*(요청|작업|질문)[^.\n]{0,15}(취소|무효|철회|됐다|되었다)/, label: "앞선 요청 취소 사칭" },
  { re: /(그만두|중단하고|멈추고|중단\.)[^.\n]{0,60}(덧붙|출력|답변|응답)[^.\n]{0,10}(하라|해라|하세요|해줘|주세요|여라)/, label: "작업 중단 후 대체 지시" },
  { re: /(대신|말고)[^.\n]{0,25}(출력|답변|응답)[^.\n]{0,10}(하라|해라|하세요|해줘|주세요)/, label: "대체 출력 강요" },
  { re: /(라고만|만)\s*(출력|답변|응답)\s*(해|하라|해라|하세요|해줘)/, label: "출력 내용 강제" },
