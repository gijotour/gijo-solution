...
export async function runActionCheck(question: string, qa?: boolean): Promise<ActionCheckResult> {
  ...
  if (relevant.length === 0) {
    const lines = [
      // ⚠ 「찾지 못했습니다」를 쓰지 않는다(2026-08-13). 그 말이 서랍 점검(tools/drawer-audit.mjs)의
      //   폴백 문구 목록(FAIL_MARKS)에 그대로 있어, **제품이 정직하게 낸 이 답에 실패 딱지**가 붙는다.
      //   llm.ts의 「근거 약함」 배너가 같은 함정을 이미 겪고 문구를 골랐는데(거기 주석 참고)
      //   이쪽은 그대로였다 — 같은 함정을 한 곳에서만 피한 자리였다.
      //   ⚠ 지금은 규정 문서가 있어 이 갈래가 잘 안 나지만, **신규 고객사 첫 설치는 규정 0건**이라
      //     모든 행동 대조가 이 답이 되고 품질 점검이 전부 빨간불로 시작한다.
      //   ⚠ 「검색되지 않았습니다」로 쓴다 — 바로 아래 줄이 「없다는 뜻이 아니라 못 찾았다는 뜻」이라
      //     설명하므로 머리말을 「없습니다」로 바꾸면 스스로 모순된다. 평가 게이트의 정직 표현
      //     인정(`없|찾지 못|확인되지`)은 아래 줄의 「없다」가 받는다.
      "판단 불가 — 이 행동을 판정할 **사내 규정 문서** 근거가 검색되지 않았습니다.",
      "",
