...
export const ACTION_CHECK_RE =
  ...
  /[가-힣]{1,8}\s?(아도|어도|여도|해도|줘도|봐도|와도|가도|내도|써도|켜도|둬도|놔도|돼도|져도|쳐도|워도|꿔도|눠도|펴도)\s*(돼|되나|될까|되는지|괜찮|문제\s*없)|[가-힣]+(려는데|려고\s*하는데)\s*(돼|되나|괜찮|문제)|허용\s*(되나|되는지|돼|될까)|위반(이야|인가|인지|되나|아닌가)|규정(에|상)\s*(맞|어긋|위배|문제)/;
  ...
  if (relevant.length === 0) {
    ...
    if (references.length) {
      lines.push("", "참고 자료(규정 문서가 아니라 **판정 근거로 쓰지 않았습니다**):",
        ...references.slice(0, 3).map((c) => `  · ${c.documentId ?? "문서"} — "${c.text.replace(/\s+/g, " ").slice(0, 90)}…"`));
    }
    if (lawHits.length) {
