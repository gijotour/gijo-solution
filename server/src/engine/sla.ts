...
//   server/test/sla.test.ts 소스 감시가 이 파일의 `^import` 줄 수가 0임을 못 박는다 —
//   새로 import를 추가하면 그 시험이 먼저 빨개진다.
//
 ...
 *
 * ⚠ 조치대상수가 0이면 이 함수를 부르지 않는다 — 표본 0 갈래(「집계 전」 문장)는 호출부
 *   (report.ts)가 별도로 처리한다. slaclue.test.ts:98-103이 그 갈래가 두 곳(Word·HTML)
 *   모두에 있는지 센다.
