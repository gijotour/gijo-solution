...
export const 상세최대 = 3; // 요약·접점을 붙이는 편수 — **소식이 있는** 앞 3편, 나머지는 이름 줄만
export const 답글자상한 = 1800; // 예산 — 넘으면 보이는 편수를 줄인다(머리·「(외 N건)」·표기·갈 곳은 안 깎는다)
 ...
 *
 *  상세(요약·접점)는 **소식이 있는 앞 `상세최대`편에만** 붙인다 — 나머지는 이름 줄만 나가고,
 *  보인 편수를 넘는 편은 「(외 N건)」으로 밝힌다. 머리의 건수·갈래 집계는 **표시 여부와
 *  무관하게 전체(rows)를 센다** — 숫자 잣대는 한 글자도 안 바꾼다.
 *
 *  글자 예산은 **본문(목록)에서만** 깎는다(검토관 2026-09-12 [중]) — 머리·「(외 N건)」·
 *  「자체 요약」 표기·갈 곳 한 줄은 잘림 밖이다. 예산에 걸리면 보이는 편수를 줄이고,
 *  그만큼 「(외 N건)」이 커진다 — 숫자는 계속 참이고 잘렸다는 사실도 그 숫자가 말한다.
 *  (답 끝을 통째로 자르면 「67건」이라 말해 놓고 갈 곳이 사라진다 — incidentcases.ts:527-529
 *   가 적어 둔 바로 그 사고다.)
 */
export function recentDocumentsText(days = 7): string {
  ...
  // 상세를 붙일 편 — 「맨 앞 3편」이 아니라 **「소식이 있는 앞 3편」**이다(검토관 2026-09-12 [중]).
  // 왜: 정렬은 ingestedAt DESC뿐이라 맨 앞은 「중요한 편」이 아니라 **가장 최근 편**이고,
  // 요약은 반입 뒤 백그라운드로 만들어진다(memory.ts → makeDigest). 4100 실측에서도 앞 두
  // 편(용어사전·제품소개)이 요약도 접점도 없는 builtin이라, 자리로만 고르면 요약 8편 중
  // 1편만 살아남았다 — 「요약·접점까지 즉답」 계약(tools/ops-sim.mjs ⑨ 물음 주석)이 무너진다.
  // 실패 사유도 소식으로 친다 — 실패를 숨기지 않는다(이 파일 머리말 원칙 ②).
  const 상세대상 = new Set<string>();
  for (const r of rows.slice(0, 목록최대)) {
    if (상세대상.size >= 상세최대) break;
    if (r.summary || r.matches || r.failedReason) 상세대상.add(r.documentId);
  }

  const 한줄 = (r: RecentDoc): string => {
    const when = r.ingestedAt.slice(5, 10).replace("-", "/");
    const who = r.uploadedBy ? ` · ${r.uploadedBy}` : "";
    let 소식 = "";
    if (상세대상.has(r.documentId)) {
      // 요약 줄 최대 2줄(원래 저장은 최대 3줄) · 접점 최대 1건(원래 최대 2건) — 상세 편수를
      // 좁힌 대신 편당 상세도 줄여 예산을 지킨다. 「자체 요약」 표기는 편마다 안 붙이고
  ...
  // ⚠ 갈 곳은 **한 곳만** 말한다(검토관 중4 — 처음엔 tail이 「지식 화면」, 이 줄이 「내 문서」로
  //   한 답에서 두 화면을 가리켜 실제로 어긋났었다). ★ 2026-09-12 재검토(B8) — 그 「내 문서
  //   화면의 🩹 반입」은 업로드 영수증(/api/upload/receipts)이 원천이라 이 목록의 원천
  //   (memory_documents)과 다르다: builtin 문서·2026-08-22 이전 반입은 영수증이 없어 「전체
  //   목록」 약속이 거짓이 된다(screenguide.ts mydocs.html이 스스로 그렇게 설명한다).
  // ★ 약속은 **그 화면에 실제로 있는 것만** 한다(검토관 2026-09-12 [상]) — 「요약 전문」은
  //   어디에도 없다: 요약이 사는 doc_digests를 읽는 코드는 이 파일 하나뿐이고(전수 grep),
  //   「AI 지식」(memory.html)은 문서 이름·조각 수·등급·연결·원본 열기만 그린다. 「원문」도
  //   약속하지 않는다 — 원본 열기 단추는 hasSource인 문서에만 뜬다(memory.html). 그 화면이
  //   모든 문서에 대해 확실히 주는 것은 **전체 목록** 하나다.
  // ⚠ 「"요약해줘"」 안내도 안 붙인다(검토관 중5 — 그 지시를 받는 결정 경로가 없어 회차마다
  //   흔들린다. 안 되는 안내는 없느니만 못하다).
  const 갈곳 = `\n\n전체 목록은 「AI 지식」 화면에서 봅니다.`; // agentloop.ts 이어서표 ontology_query와 같은 표기

  /** 앞에서 N편만 보여 주는 답을 조립한다 — 본문(깎는 쪽)과 꼬리(지키는 쪽)를 갈라 돌려준다. */
  const 조립 = (보일수: number): { 본문: string; 꼬리: string } => {
    const 보일것 = rows.slice(0, 보일수);
    const tail = rows.length > 보일수 ? `\n(외 ${rows.length - 보일수}건)` : "";
    // 요약은 "자체 요약" 표기와 함께 보인다 — 7B 숫자 오독 전례(2026-08-03), 이 파일 머리말 원칙 ③.
    const 표기 = 보일것.some((r) => 상세대상.has(r.documentId) && r.summary)
      ? `\n(로컬 모델 자체 요약 — 원문 대체가 아닙니다)`
      : "";
    return { 본문: `${머리}\n${보일것.map(한줄).join("\n")}`, 꼬리: `${tail}${표기}${갈곳}` };
  };

  let 보일수 = Math.min(목록최대, rows.length);
  let { 본문, 꼬리 } = 조립(보일수);
  while (본문.length + 꼬리.length > 답글자상한 && 보일수 > 1) {
    보일수 -= 1;
    ({ 본문, 꼬리 } = 조립(보일수));
  }
  // 마지막 안전망 — 한 편만 남기고도 넘치면 머리(갈래 수)가 비정상이라는 뜻이다. 그때도 꼬리는
  // 지키고 본문만 자르며, 잘린 자리에 말줄임을 남긴다(조용히 자르지 않는다).
  if (본문.length + 꼬리.length > 답글자상한) 본문 = `${본문.slice(0, Math.max(0, 답글자상한 - 꼬리.length - 1))}…`;
  return `${본문}${꼬리}`;
}
