...
/**
 * 후보함에 **담기 전에** 쓰는 판별. 데이터셋을 만들 때 거르는 것만으로는 늦다 —
 * 후보함이 시점 데이터로 채워지면 담당자가 그걸 하나씩 들여다보며 시간을 쓰고,
 * 주제별 진척(300건)도 가짜로 부푼다.
 *
 * 실측(2026-08-09): 후보함 대기 4건이 **전부** 못 쓰는 것이었다 —
 *   "오늘 할 일" · "이번 주 보안 현황을 요약해줘" · "기한 지난 일 보여줘"(셋 다 그날의 숫자)
 *   + 위협보고서 본문이 통째로 들어온 것.
 * 데이터셋 만들 때는 걸러졌지만, 후보함에는 그대로 쌓여 있었다.
 */
export function 학습재료가못되나(질문: string, 답: string): string | null {
  const q = String(질문 ?? "").trim();
  const a = String(답 ?? "").trim();
  if (시점데이터(a)) return "그날의 숫자·날짜(시점 데이터)";
  if (시험문항인가(q)) return "시험 문항(회귀·게이트)";
  // 문서 본문이 통째로 답에 들어온 것 — 질문이 물음이 아니라 문장이고 답이 길다.
  if (a.length > 1200 && !/[?？]|알려|뭐|어떻게|무엇|왜|해줘|인가요|입니까/.test(q)) return "문서 본문 복사";
  return null;
}
