...
const RESTORE_STEPS = [
  ...
  "⑤ 짝 폴더 gijo-as-<같은 시각>.docs 를 data/docs 로 **폴더째** 복사한다 (빠뜨리면 「원본 보관됨」 배지는 남는데 원본 열기가 404가 되고, 사람이 고친 추출본(.md)이 사라진다. 2026-08-22부터 그 안의 attach/ 에 **문서 편집기 첨부 그림**도 들어 있다 — 하위 폴더를 빠뜨리면 문서 안 그림이 전부 깨진다)",
  "⑥ 서버를 다시 켜고 자가 진단(설정 > 자가 진단)에서 지식베이스·DB 항목이 정상인지 확인한다",
