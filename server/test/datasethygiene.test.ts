...
describe("운영 확인용 질문 — 내 시험 흔적", () => {
  ...
  it("점검용 문구를 뺀다", () => {
    const r = cleanForTraining([
      ...
      ex("현재 시스템 정상 여부 한 문장으로", "정상 동작 중입니다."),
      ex("현재 기본 LLM으로 짧게 인사해줘.", "안녕하세요, GIJO AS입니다."),
      ex("오늘 서울 날씨 어때", "날씨 정보는 제공하지 않습니다."),
    ]);
    expect(r.kept).toHaveLength(0);
    expect(r.dropped["운영 확인용 질문(내 시험 흔적)"]).toBe(4);
  });
  ...
  it("업무 질문은 남긴다", () => {
    ...
    expect(r.kept, "업무 질문이 잘못 걸렸다").toHaveLength(2);
  });
});
