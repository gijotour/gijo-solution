...
describe("★★ 실전 답 기록으로 잰 오탐 — 넓힌 잣대가 정상 답을 지우지 않는가", () => {
  ...
  //
  //   (a) **도구를 안 쓴 답**(챗 경로)에 `sources`가 있으면 건너뛴다 — 그 답은 라이브에서 가드가
  //       **조각을 갖고** 돌았는데 재생은 `[]`를 준다. 기록에 근거 **본문**이 없어(sources는 문서
  //       이름 목록뿐: 실측 ["GIJO_AS_사용자_매뉴얼.md", …]) 되먹일 길이 지금은 없다.
  //       ⚠ 기록이 근거 본문을 담는 날 **이 규칙을 지운다** — 그러면 이 답들이 다시 잣대에 든다.
  ...
  const 모집단밖 = (r: Record<string, unknown>): string | null => {
    ...
    // (a)는 **챗 경로에만** 건다 — 도구 답의 sources는 RAG 조각이 아니다(위 (a') 참고).
    if (도구.length === 0) {
      const s = r.sources;
      if (Array.isArray(s) ? s.length > 0 : !!s) return "챗 경로 + 근거 조각 있었음(재생에 되먹일 수 없다)";
    }
    return null;
  };
  ...
  it.skipIf(!!건너뛸사유)("실전 답을 재생하면 가드가 한 글자도 안 바꾼다", () => {
    ...
    for (const r of rows) {
      ...
      const 무엇 = 건드린바(t, guardCitations(t, [], [glossaryGroundingFor(String(r.q ?? ""))]));
      if (!무엇) continue;
    ...
    console.log(`📊 실전 답 오탐 시험 — 기록 ${총}건 · **잰 것 ${잰것}건**(챗 경로 ${챗} = 라이브와 같은 조건 · 도구 경로 ${도구} = 라이브에선 가드 무동작인 예방 그물) · 건너뜀 ${총 - 잰것}건(${[...건너뜀].map(([k, v]) => `${k} ${v}`).join(" · ") || "없음"})`);
