...
describe("★ F7 표지 자체닫힘 — 안쪽 번호 대괄호의 `]`에 속지 않는다", () => {
  ...
  it("★ 정직한 한계 — 「[출처]: \"제목\"」은 **아직 안 본다**(미탐, 이 라운드가 안 넓혔다)", () => {
    // 표식 낱말과 도입 어구 사이에 `]`가 끼면 주장_RE가 그 자리를 표지로 안 읽는다.
    //   옛 판도 새 판도 똑같이 안 뗀다 — 회귀가 아니라 처음부터 없던 갈래다.
    //   여기서 넓히지 않는 이유: 넓히면 「[비고]: …」류 정상 주석의 오탐을 새로 재야 하는데,
    //   이 라운드는 **6차가 만든 오탐을 되짚는** 라운드다(넓히기와 좁히기를 한 커밋에 섞지 않는다).
    const 답 = '앞 문장입니다. [출처]: "없는 제목입니다 길게"';
    expect(guardCitations(답, 조각).text, "안 보던 갈래를 조용히 넓혔다(그러면 오탐을 안 잰 것이다)").toBe(답);
  });
});
