...
describe("★★ 실전 답 기록으로 잰 오탐 — 넓힌 잣대가 정상 답을 지우지 않는가", () => {
  ...
  //   여기서는 **가장 사나운 조건(chunks=0)**으로 실전 답을 훑어 0건을 못박는다.
  //
  // ★★★ **이 시험이 재는 것 / 못 재는 것**(2026-09-08 — 모집단을 좁히며 정직하게 적는다)
  //   ① 잰다: 라이브에서 **조각 없이 나간 답**(sources 빈 칸)을 chunks=0으로 재생해 한 글자도
  //      안 바뀌는가. 그 답들은 라이브에서도 가드가 같은 조건으로 지나갔으므로 재생이 곧 실전이다.
  //   ② **못 잰다**: 라이브에 근거 조각이 있었던 답(sources 있음)의 오탐은 이 시험이 **원리상**
  //      못 본다 — 기록(ops-sim.json)이 근거 **본문**을 안 담아 재생에 조각을 되먹일 길이 없고,
  //      조각을 안 주면 재생이 늘 라이브보다 사납다(citeguard.ts:922 — chunks가 null이면 무동작).
  //   ③ 되살릴 조건: 기록이 근거 본문을 담는 날 아래 (a) 규칙을 지우면 그 답들이 다시 잣대에 든다.
  //   ⚠ ②가 **얼마나 큰지**는 재서 적는다(2026-09-08 격리 사본 반증): 똑같이 지어낸 인용
  //     「원문: "이 절차는 사내 규정 제12조에 명시되어 있습니다."」를 모집단 **안** 답에 넣으면
  //     1 failed로 잡히고, sources 있는 답에 넣으면 **226/226 초록**이다. 숨기지 않고 적어 둔다.
  ...
  //   이 시험의 오라클은 「실전 답은 정상이다」라는 가정인데, 그 가정이 성립하는 답은
  //   **라이브에서도 조각 없이 가드를 지난 답**뿐이다. 나머지 둘은 재생이 라이브보다 사나워서
  //   떨어지는 것이라 결함의 증거가 아니다 — 그래서 물음 이름이 아니라 **기록의 칸으로** 가른다.
  //
  //   (a) `sources`가 비어 있지 않으면 건너뛴다 — 라이브에는 조각이 있었는데 재생은 `[]`를 준다.
  //       기록에 근거 **본문**이 없어(sources는 파일 이름 목록뿐: 실측 ["GIJO_AS_사용자_매뉴얼.md", …])
  //       조각을 되먹일 길이 지금은 없다. ⚠ **그 한계가 사라지는 날(기록이 근거 본문을 담는 날)
  //       이 규칙을 지운다** — 그러면 이 답들이 다시 잣대에 든다.
  //   (b) 답 본문에 **제품이 외부 출처 링크를 직접 찍는 도구**가 쓰였으면 건너뛴다. 그 글은 모델이
  //       지은 인용이 아니라 코드가 쓴 글이고, 도구 답은 라이브에서 **가드를 아예 안 지난다**
  //       (2026-09-08 grep 확인: guardCitations 호출은 **모델 답 출구 둘**뿐 — llm.ts:1154 chat ·
  //       cloudllm.ts:381. 강제 도구는 agentloop.ts:2703에서 `사람에게내보낸다`로 곧장 돌아가는데
  //       그 함수(561행)에는 가드가 없다).
  //   (c) 나머지는 종전대로 **한 글자도** 안 바뀌어야 한다.
  //
  // ★ 옛 손 예외 2건(「보안 교육 참여율 높이려면?」·「개인정보 교육 연 2회 맞지?」)은 **지웠다** —
  //   둘 다 sources 2건짜리 답이라 규칙 (a)에 그대로 흡수된다(실측 2026-09-08). 함께 있던
  ...
  const 모집단밖 = (r: Record<string, unknown>): string | null => {
    const s = r.sources;
    if (Array.isArray(s) ? s.length > 0 : !!s) return "sources있음(라이브엔 조각이 있었다)";
    const 도구 = Array.isArray(r.도구) ? r.도구.map(String) : [];
    const 걸린 = 도구.filter((t) => 출처를제품이찍는도구.has(t));
    return 걸린.length ? `출처링크 도구(${걸린.join(",")})` : null;
  };

  it.skipIf(!!건너뛸사유)("라이브에서 조각 없이 나간 답은 한 글자도 안 바꾼다", () => {
    ...
    console.log(`📊 실전 답 오탐 시험 — 기록 ${총}건 · **잰 것 ${잰것}건** · 건너뜀 ${총 - 잰것}건(${[...건너뜀].map(([k, v]) => `${k} ${v}`).join(" · ") || "없음"})`);
    // ★ 문턱은 **잰 것**에 건다(기록 전체가 아니다 — 모집단이 쪼그라들면 알아야 한다).
    //   실측 2026-09-08(177문항 완주본): 기록 177 · sources있음 52 · 출처링크 도구 1 → **잰 것 124**.
    //   여유 24건이라 종전 문턱 100을 **그대로 둔다**(낮출 근거가 없다). 이 아래로 떨어지면
    //   재료가 바뀐 것이니 숫자를 낮추기 전에 **왜 줄었는지**부터 본다.
