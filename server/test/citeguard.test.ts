...
describe("★★ 실전 답 기록으로 잰 오탐 — 넓힌 잣대가 정상 답을 지우지 않는가", () => {
  ...
  /**
   * 기록에 **가드가 본 조각**이 실려 있나(2026-09-08 신설 칸 — tools/ops-sim.mjs가 적는다).
   * ⚠ `근거조각: null`도 **있는 기록**이다 — 라이브에서 가드가 판정을 보류했다(RAG 미실행·검색
   *   실패)는 사실 그 자체라, 재생도 null로 돌려야 라이브와 같다. `[]`(0건)과 뭉개지 않는다.
   * 칸이 아예 없는 행은 옛 재료(또는 옛 서버)라 종전 갈래(근사)로 돈다.
   */
  const 조각기록있나 = (r: Record<string, unknown>): boolean => "근거조각" in r;
  ...
  it.skipIf(!!건너뛸사유)("실전 답을 재생하면 가드가 한 글자도 안 바꾼다", () => {
    ...
    console.log(`📊 실전 답 오탐 시험 — 기록 ${총}건 · **잰 것 ${잰것}건**(챗 경로 ${챗} = 라이브와 같은 조건 · 도구 경로 ${도구} = 라이브에선 가드 무동작인 예방 그물 · 그중 조각 되먹임 ${조각재생}건 = 라이브가 준 원천 그대로) · 건너뜀 ${총 - 잰것}건(${[...건너뜀].map(([k, v]) => `${k} ${v}`).join(" · ") || "없음"})`);
