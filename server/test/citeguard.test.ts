...
describe("★★ 실전 답 기록으로 잰 오탐 — 넓힌 잣대가 정상 답을 지우지 않는가", () => {
  ...
  // ★★ **재생본에서만 떨어지는 실전 답** — 물음·**뗀 자국**·사유를 함께 적어 두고 지나간다.
  ...
  //   그래서 넓히는 대신 **사례를 못박는다.** 목록에 없는 새 사례는 반드시 빨강으로 온다.
  //
  // ⚠⚠ **물음 이름만으로 봐주지 않는다**(2026-09-07 재수리 · 검토관이 돌연변이로 증명했다).
  //   첫 판은 `x.물음 === q` 하나로 통과시켰다 — 그래서 **같은 물음의 답이 다음 회차에 완전히
  //   달라져 진짜 지어낸 인용이 나와도** 이 목록이 통째로 삼켰다. 실행으로 확인한 자리다:
  //   봐주기 물음의 답을 「원문: "…사내 규정 제12조에 명시되어 있습니다."」로 바꿔도 224/224 초록,
  //   **똑같은 글자**를 목록 밖 물음에 넣으면 1 failed. 차이가 물음 이름뿐이면 그건 오라클이 아니다.
  //   → 이제 **뗀 자국(지문)까지 맞아야** 봐준다. 답이 달라지면 자국이 달라지고, 그러면 빨강이다.
  //
  // ⚠ 여기 적을 수 있는 조건 셋: ① 가장 사나운 조건(chunks=0)이라 **재생본에서만** 떨어지고
  //   ② 라이브 답에는 그 자국이 없으며 ③ 사람이 답 전문을 읽고 판정했다.
  // ⚠ 「오탐이라서 봐준다」는 여기 적지 않는다 — 그건 가드를 고칠 일이다(같은 날 ⑭ 「[1] 또는
  //   [2]」가 그랬고, citeguard.ts 떼면형제가홀로남나로 고쳤다).
  const 검증된제거: { 물음: string; 지문: RegExp; 사유: string }[] = [
    {
      물음: "보안 교육 참여율 높이려면?",
      지문: /^블록없음 "The authoring agencies recommend end-user organiza/,
      // ★★ 2026-09-07 **사실 정정**(검토관 적발). 첫 판은 이 영문을 「RAG에 없는 재료」라고
      //   적었는데 **틀렸다.** knowledge/ 폴더만 보고 내린 판단이었고, 운영 RAG는 그 폴더가
      //   아니라 memory.lancedb다 — 거기에 이 영문이 **글자 그대로** 들어 있다(읽기전용 확인:
      //   memory.lancedb/documents.lance 및 운영 sqlite의 문서 「승인문답:dtmtlgtnkpb2340x」
      //   2026-09-03 · 본문 끝이 이 141자 · 원천으로 cisa-aa-01.md를 함께 적고 있다).
      //   즉 모델은 **제 근거에서 그대로 따왔고**, 라이브 답에도 그 문장이 남아 있다(안 떨어졌다).
      //   여기서 떨어지는 이유는 하나뿐이다 — **재생본이 조각을 안 준다**(guardCitations(t, [])).
      //   그 조각을 주면 removed 0건이다(실측). 가드가 옳았다는 말이 아니라 **재생의 한계**다.
      // ⚠ 기록(ops-sim.json)에 sources 본문이 없어 재생에 조각을 되먹일 방법이 지금은 없다.
      //   그 한계가 사라지는 날(기록이 근거 본문을 담는 날) 이 예외는 지워야 한다.
      사유: "재생본이 조각을 안 줘서 떨어진다 — 실제 근거(승인문답:dtmtlgtnkpb2340x)에 그대로 있다",
    },
    {
      물음: "개인정보 교육 연 2회 맞지?",
      지문: /^블록없음 번호 표지 \[1\] .*\(\[1\]에서\)/,
      ...
      사유: "재생본이 조각을 안 줘서 [1]이 가리킬 곳이 없다 — 번호 표지만 뗌",
    },
  ];
  ...
  it.skipIf(!!건너뛸사유)("실전 답 전체에서 한 글자도 안 바꾼다", () => {
    ...
    for (const r of rows) {
      ...
      if (!무엇) continue;
      건드린물음.add(q);
      // ★ 물음 **그리고** 자국이 함께 맞아야 봐준다(위 ⚠⚠).
      if (검증된제거.some((x) => x.물음 === q && x.지문.test(무엇))) continue;
    ...
    //   봐줄 일도 없으므로, **재료에 있는데 아예 안 걸린** 것만 나무란다(자국이 달라진 경우는
    //   위 `건드림`이 실제 자국과 함께 이미 빨강으로 알린다 — 여기서 두 번 말하지 않는다).
    const 물음들 = new Set(rows.map((r) => String(r.q ?? "")));
    const 낡은예외 = 검증된제거.filter((x) => 물음들.has(x.물음) && !건드린물음.has(x.물음)).map((x) => x.물음);
