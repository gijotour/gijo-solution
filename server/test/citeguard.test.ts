...
// ⚠ 실전 답 재생에 **라이브가 함께 견주는 추가원천**을 되먹이려고 부른다(llm.ts:1154·881과 같은 함수).
//   시험이 추가원천을 흉내 내 적으면 라이브와 갈라지므로 제품 함수를 그대로 쓴다.
import { glossaryGroundingFor } from "../src/engine/glossary";
...
describe("★ 배선 감시 — 출구 한 곳에서 실제로 불린다", () => {
  ...
  it("가드가 **복창 방어선 뒤 · 배너 앞**에서 불린다(설계관이 정한 자리)", () => {
    expect(llm, "guardCitations 호출이 없다").toContain("guardCitations(reply, 가드조각, 가드추가원천)");
    // ★★ 원천은 **한 곳에서만 만든다**(2026-09-08). 가드가 견주는 값과 qa 응답에 실어 보내는
    //   값이 갈리면, 재생 시험이 「라이브와 같은 조건」이라 말하면서 **다른 재료**로 재게 된다 —
    //   가장 나쁜 초록이다. 그래서 두 변수의 정의와 **보고가 그 둘을 그대로 쓰는 것**까지 문다.
    expect(llm, "조각을 한 곳에서 안 만든다").toContain("const 가드조각 = ragResult?.chunks ?? null;");
    expect(llm, "번호 없는 원천을 한 곳에서 안 만든다")
      .toContain("const 가드추가원천 = [...(ragResult?.추가원천 ?? []), grounding, 첨부].filter((c): c is string => !!c);");
    expect(llm, "기록이 가드와 **다른 값**을 본다 — 재생이 라이브와 어긋난다")
      .toContain("인용원천보고(가드조각, 가드추가원천);");
    // 숫자 접지도 같은 묶음을 쓴다 — 전엔 같은 조립식이 두 벌이라 한쪽만 고쳐지곤 했다.
    expect(llm, "숫자 접지가 원천 묶음을 또 조립한다")
      .toContain("숫자가원천에있나(reply, 가드조각, [...가드추가원천, ...본것])");
...
describe("★★ 실전 답 기록으로 잰 오탐 — 넓힌 잣대가 정상 답을 지우지 않는가", () => {
  ...
  //   여기서는 **조각 0건**으로 실전 답을 훑어 0건을 못박는다(추가원천은 라이브만큼 준다 — 아래 ⓓ).
  //
  // ★★★ **이 시험이 재는 것 / 못 재는 것**(2026-09-08 · 검토관 적발로 **고쳐** 적는다)
  //   ⚠ 이 자리의 첫 판은 「잰 답은 라이브에서도 가드가 같은 조건으로 지나갔으니 **재생이 곧
  //     실전**」이라고 적었는데 **사실과 다르다.** 잰 것 135건 중 94건(70%)이 도구 경로인데, 그 답은
  //     라이브에서 가드가 **아예 안 돈다**. 안심시키는 주석은 틀리면 없느니만 못하다 — 갈래를
  //     갈라 실측 숫자와 함께 적는다(아래 숫자는 2026-09-08 177문항 완주본에서 잰 것이다).
  //
  //   ⓐ **챗 경로 · 조각 없음**(41건) — 라이브에서 guardCitations가 chunks=[]로 **실제로 돌았고**
  //      답이 한 글자도 안 바뀌어 나갔다. 「재생 ≒ 실전」이 성립하는 유일한 자리다(단 ⓓ).
  //   ⓑ **도구 경로**(94건) — 라이브에서는 가드가 **무동작**이었다. 길을 눈으로 따라간 것:
  //      agentloop.ts:903 composeFinalAnswer가 chat()을 **`remember` 없이** 부르고(929행 주석이
  //      「끄고」라고 밝힌다) → llm.ts:863 `ragResult = args.remember ? … : null` → llm.ts:1154가
  //      `ragResult?.chunks ?? null`을 넘겨 → citeguard.ts:922 `chunks === null`이면 즉시 되돌린다.
  //      directAnswer 도구(agentloop.ts:481)는 모델조차 안 탄다.
  //      → **그래서 여기 나는 빨강은 「라이브 답이 훼손됐다」가 아니라 「가드를 이 글에 씌우면
  //        훼손된다」는 예방 신호다.** 그래도 재는 이유 둘: ① `remember: true` **한 낱말**이면
  //        94건이 통째로 진짜 가드 대상이 되는데, 그 호출 인자는 실제로 손질됐다 되돌려진 전례가
  //        있다(agentloop.ts:897) ② 오늘 값이 0건이라 그물 값이 공짜다.
  //      ⚠ **「실전을 쟀다」고는 말하지 않는다.** 이 시험의 초록은 「지금 라이브가 안전하다」가
  //        아니라 「가드를 씌워도 안전할 글이다」까지만 뜻한다.
  //   ⓒ **못 잰다 ①** — 챗 경로에 근거 조각이 있었던 답(41건). 기록이 근거 **본문**을 안 담아
  //      조각을 되먹일 길이 없고, 안 주면 재생이 라이브보다 사나워 거짓 빨강이 난다. 그 크기도
  //      재서 적는다(격리 사본 실측): 그 41건 중 **2건**이 실제로 빨강이다 — 「협력사에 자산
  //      목록 줘도 되나?」·「AI가 뭘 근거로 답했는지 볼 수 있어?」. 되살릴 조건은 하네스
  //      (tools/ops-sim.mjs)가 조각 본문을 기록하는 것이다. 그날 규칙 (a)를 지운다.
  //   ⓓ **못 잰다 ②** — 라이브가 함께 견주는 **추가원천**(llm.ts:1154) 셋 중 온톨로지
  //      (ragResult.추가원천)는 기록에 없어 못 되먹인다. **용어 그라운딩은 질문만으로 되살아나므로
  //      아래에서 실제로 되먹인다**(안 주면 재생이 라이브보다 사납다 — 검토관 적발). 📎 첨부는
  //      하네스가 아예 안 쓴다(tools/ops-sim.mjs에 attach가 한 줄도 없다 — 라이브에서도 null이었다).
  ...
  const 재료상태 = (): string => {
    // ⚠ 「재료 없음」의 뜻이 사본마다 다르다 — **어느 쪽인지 사유에 함께 적는다.**
    //   gb10 사본에서는 하네스를 안 돌려서가 아니라 **일부러 안 옮겨서** 없다(2026-09-08):
    //   이 기록에는 가드가 본 **근거 조각 본문**이 실리는데, 하네스 계정(claude-deploy·admin)
    //   탓에 등급 C 사내 문서 조각까지 담긴다. 그것을 원격 기계로 복제하지 않으려고
    //   tools/gb10-test.sh에서 복사를 뺐다(wsl-test.sh는 같은 기계라 그대로 옮긴다).
    //   → 그래서 gb10에서 이 시험이 건너뛰는 것은 **고장이 아니라 설계**다. 다시 재려면 WSL로.
    if (!fs.existsSync(f)) return "재료 없음(하네스를 아직 안 돌렸다 · gb10 사본은 일부러 재료를 안 옮긴다 — 등급 C 조각이 원격으로 나가지 않게)";
    try {
  ...
  // ★★★ **모집단 규칙 — 무엇을 재고 무엇을 안 재는가**(2026-09-08 · 여기 한 곳에만 적는다)
  //   빼는 답은 「재생이 라이브보다 사나워서 떨어지는 것」뿐이다 — 그건 결함의 증거가 아니다.
  //   판정은 물음 이름이 아니라 **기록의 칸으로** 한다.
  //
  //   (a) **도구를 안 쓴 답**(챗 경로)에 `sources`가 있는데 **기록에 조각 본문이 없으면** 건너뛴다 —
  //       그 답은 라이브에서 가드가 **조각을 갖고** 돌았는데 재생은 `[]`를 준다. sources는 문서
  //       이름 목록뿐이라(실측 ["GIJO_AS_사용자_매뉴얼.md", …]) 되먹일 길이 없다.
  //   (a'') **기록에 조각이 있으면 건너뛰지 않는다 — 그 조각으로 재생한다**(2026-09-08 · 이 라운드).
  //       서버가 qa 응답에 「가드가 본 원천」을 실어 주고(citesource.ts) 하네스가 그것을 행에
  //       적으면서(근거조각·추가원천·가드횟수), (a)가 대던 이유(「되먹일 길이 없다」)가 사라졌다.
  //       ⚠ **되먹이는 것은 근사가 아니라 그 값 자체다** — llm.ts가 guardCitations에 넘긴 두 배열을
  //         그대로 적는다(원천을 두 번 계산하지 않는다 — 그 감시는 위 「배선 감시」에 있다).
  //       ⚠ 그 대신 **가드가 한 턴에 여러 번 돈 답**(가드횟수>1)은 뺀다 — 기록은 첫 호출 것이라
  //         어느 호출의 조각인지 못 가린다. 모른 채 재는 초록이 가장 나쁘다.
  //       ⚠ 오늘 재료(2026-09-08 회차)에는 이 칸이 **한 행도 없다** — 새 재료는 운영 배포 뒤
  //         야간 회귀가 한 번 돌아야 생긴다. 그때까지 이 갈래는 격리 사본 모의 재료로만 실증됐다.
  //       ★★ **「조각이 있다」는 `근거조각`이 배열일 때만이다**(2026-09-08 검토관 [높음]·[중간] 수리).
  //         `"근거조각" in r`로 세면 **값이 null이어도 참**이라, 라이브에서 가드가 판정을 보류한
  //         행이 잰것·챗 문턱에 그대로 더해지면서 guardCitations(t, null, …)은 citeguard.ts:922에서
  //         **즉시 되돌려 한 글자도 안 잰다.** 문턱 100/20이 「잴 수 없는 행」으로 채워져도 초록이
  //         나고, 종전엔 사유가 찍히던 건너뜀이 숫자 안에 묻힌다(이 저장소가 이름 붙인 「삼켜진 건너뜀」).
  //         그래서 판정을 **배열인가**로 바꾸고, 배열이 아닌 기록은 아래에서 **사유를 찍어** 뺀다.
  //         ↳ 생산자 쪽도 함께 고쳤다: citesource.ts가 `조각 === null`인 보고를 **담지도 세지도 않는다**
  //           (한 요청의 첫 chat은 답이 아니라 분류기 routeIntent라, 안 그러면 챗 행의 조각이
  //           **언제나 분류기의 null**이 되고 가드횟수가 2가 돼 41건이 통째로 다시 빠졌다).
  //           그러니 새 재료에는 null 기록이 원리상 안 생긴다 — 여기 규칙은 **옛 재료·옛 서버**를 막는
  //           두 번째 자물쇠다(한쪽만 두면 재료 출처가 바뀔 때 조용히 뚫린다).
  //   (a') **도구를 쓴 답은 `sources`가 있어도 잰다**(2026-09-08 검토관 적발로 좁혔다 · +11건).
  //       그 sources는 RAG 조각이 아니다 — 도구가 읽은 근거 수거물이고(agentloop.ts:2662
  //       `근거를수거하며` → toolevidence), 그 경로의 가드는 애초에 무동작이었다(머리글 ⓑ).
  //       그러니 (a)의 이유가 하나도 안 걸린다. 옛 판은 이 11건을 「라이브엔 조각이 있었다」는
  //       **틀린 사유**로 건너뛰었다.
  //       ⚠ `sources`는 어차피 **근사 잣대**다 — dispatcher.ts:203이 스스로 「답이 실제로 인용한
  //         자료가 아니라 따로 재검색해서 나온 후보」라 적었고, 그 값은 chat이 끝난 **뒤**
  //         queryMemoryGraded로 다시 뽑는다(dispatcher.ts:1093 — llm.ts:1154가 쓴 조각과 다른 호출).
  //         **반대 방향의 틀림은 못 가린다**: sources가 비었는데 라이브엔 조각이 있었을 수 있고,
  //         그 답은 재생에서 거짓 빨강이 된다. 기록에 「가드가 본 조각 수」 칸이 생기면 그리 바꾼다.
  //   (b) 답 본문에 **제품이 외부 출처 링크를 직접 찍는 도구**가 쓰였으면 건너뛴다. 이유는
  //       **그 글을 모델이 아니라 코드가 썼다**는 것 하나다 — 「지어낸 인용인가」라는 이 시험의
  //       물음 자체가 성립하지 않는다.
  //       ⚠ 「도구 답은 가드를 안 지난다」를 (b)의 이유로 적지 않는다(옛 판의 잘못) — 그건 도구 답
  //         **94건 전부**에 해당해서 아래 도구 **둘만** 빼는 근거가 못 된다(오늘 재료로는 1건이다).
  //         그 사실은 머리글 ⓑ에 적었다.
  //       ⚠ 남는 물음(제품 몫): 배선이 바뀌어 이 글이 가드를 지나게 되면 제품이 **자기가 쓴 출처
  //         줄을 스스로 지운다**(실측 — incident_cases 답이 「블록없음: 참고 자료가 0건인데 인용
  //         꼬리표를 썼다(출처:)」로 걸린다). 그건 가드를 고칠 일이라 이 시험 파일에서 안 닫는다.
  //   (c) 나머지는 종전대로 **한 글자도** 안 바뀌어야 한다.
  //
  // ★ 옛 손 예외 2건(「보안 교육 참여율 높이려면?」·「개인정보 교육 연 2회 맞지?」)은 **지웠다** —
  //   둘 다 sources 2건짜리 챗 답이라 규칙 (a)에 그대로 흡수된다(실측 2026-09-08). 함께 있던
  //   「낡은예외」 검사도 지운다(봐줄 목록 자체가 없어졌다). 물음 이름으로 봐주는 자리는 이제 없다.
  // ⚠ 「오탐이라서 봐준다」식 물음 이름 예외는 **다시 만들지 않는다** — 그건 가드를 고칠 일이다.
   ...
   */
  const 출처를제품이찍는도구 = new Set(["incident_cases", "incident_sources"]);

  const 도구목록 = (r: Record<string, unknown>): string[] => (Array.isArray(r.도구) ? r.도구.map(String) : []);

  /**
   * 기록에 **가드가 실제로 견준 조각**이 실려 있나(2026-09-08 신설 칸 — tools/ops-sim.mjs가 적는다).
   *
   * ★ 판정은 **배열인가** 하나다 — 있고 없고(`in`)가 아니다(2026-09-08 검토관 수리, 위 (a'')).
   *   `근거조각: null`은 라이브에서 가드가 **판정을 보류**한 것이라 되먹여도 재는 것이 0이다
   *   (citeguard.ts:922가 즉시 되돌린다). 그걸 「있는 기록」으로 세면 잴 수 없는 행이 문턱을
   *   채우고 건너뜀 사유가 숫자에 묻힌다.
   * ⚠ `[]`(0건)은 **배열이라 잰다** — 가드가 실제로 돌았고 대조할 조각이 없었다는 판정이다.
   * 칸이 아예 없거나 배열이 아닌 행은 옛 재료(또는 옛 서버)라 종전 갈래(근사)로 돌거나 아래에서 빠진다.
   * ⚠ **두 함수가 같은 잣대를 본다** — 있나 판정을 값 읽기로부터 따로 적으면 둘이 어긋난다.
   */
  const 조각기록 = (r: Record<string, unknown>): string[] | null =>
    Array.isArray(r.근거조각) ? r.근거조각.map(String) : null;
  const 조각기록있나 = (r: Record<string, unknown>): boolean => 조각기록(r) !== null;
  const 추가원천기록 = (r: Record<string, unknown>): string[] =>
    Array.isArray(r.추가원천) ? r.추가원천.map(String) : [];

  /** 모집단 밖이면 사유, 안이면 null. 판정은 **기록의 칸**만 본다(물음 이름을 안 본다). */
  const 모집단밖 = (r: Record<string, unknown>): string | null => {
    const 도구 = 도구목록(r);
    const 걸린 = 도구.filter((t) => 출처를제품이찍는도구.has(t));
    if (걸린.length) return `출처를 코드가 찍는 도구(${걸린.join(",")})`;
    // ⚠ 한 턴에 가드가 여러 번 돈 답 — 기록은 **첫 호출** 것이라 어느 호출의 조각인지 못 가린다.
    const 횟수 = r.가드횟수;
    if (typeof 횟수 === "number" && 횟수 > 1) return `한 턴에 가드가 ${횟수}번(어느 호출의 조각인지 못 가린다)`;
    // (a)는 **챗 경로 + 조각 기록이 없을 때만** 건다(2026-09-08 좁힘 · 위 (a'')).
    //   도구 답의 sources는 RAG 조각이 아니고(위 (a')), 조각 기록이 있으면 그것으로 재생한다.
    if (도구.length === 0 && !조각기록있나(r)) {
      // 칸은 있는데 배열이 아니다 = 라이브에서 가드가 **판정 보류**였다(옛 재료·옛 서버).
      //   재생이 []로 재면 라이브보다 사나워 없는 오탐이 난다 — **사유를 찍고** 뺀다.
      //   ⚠ 조용히 숫자에 섞지 않는다(삼켜진 건너뜀은 없는 시험과 같다).
      //   ⚠ 도구 경로에는 안 건다 — 그 갈래는 라이브에서 어차피 가드 무동작이라 종전대로
      //     []로 재는 **예방 그물**이다(머리글 ⓑ). 여기까지 빼면 그물이 통째로 사라진다.
      if ("근거조각" in r) return "챗 경로 + 가드 판정 보류(근거조각이 배열이 아니다 — 되먹일 것이 없다)";
      const s = r.sources;
      if (Array.isArray(s) ? s.length > 0 : !!s) return "챗 경로 + 근거 조각 있었음(기록에 조각 본문이 없다)";
    }
    return null;
  };

  // ★★ **왜 재생에 용어 그라운딩을 주는가 — 말이 아니라 시험으로 남긴다**(2026-09-08 검토관 적발).
  //   아래 재생 시험의 `guardCitations(t, [], [glossaryGroundingFor(q)])`에서 3번째 인자를 지우면
  //   무슨 일이 나는지를 여기서 **실행해** 보여 준다. 주석만 있으면 다음 사람이 「필요 없어 보인다」며
  //   지운다 — 그러면 용어사전 정의를 그대로 옮겨 적은 정상 답이 **거짓 빨강**이 된다.
  //   ⚠ 이 시험은 재료(ops-sim.json)가 없어도 돈다 — 재료가 없는 날에도 이유가 살아 있어야 한다.
  it("★ 용어사전에서 그대로 옮긴 인용 — 추가원천을 안 주면 거짓 빨강이 난다(라이브는 준다)", () => {
    const 질문 = "KEV 걸린 거 있어?";
    const 정의 = "Known Exploited Vulnerabilities — 실제 공격에 악용된 것으로 확인된 취약점 목록(미국 CISA가 관리).";
    const 답 = `미조치 취약점을 정리했습니다. 아래를 확인하세요.\n[1]에 따르면 "${정의}"`;
    const 그라운딩 = glossaryGroundingFor(질문);
    expect(그라운딩, "용어사전이 이 물음에 정의를 안 깐다 — 예문이 낡았다").toContain(정의);
    // 라이브(llm.ts:1154)는 이 정의를 추가원천으로 함께 견준다 → 안 뗀다.
    const 주고 = guardCitations(답, [], [그라운딩]);
    expect(주고.removed, "라이브와 같은 조건인데 정상 인용을 뗐다").toHaveLength(0);
    expect(주고.text, "라이브와 같은 조건인데 글자가 바뀌었다").toBe(답);
    // 안 주면 「참고 자료가 0건」으로 떨어진다 — 재생이 라이브보다 사나워지는 자리다.
    expect(guardCitations(답, []).removed, "추가원천 없이도 살아남았다 — 이 시험의 전제가 바뀌었다").toHaveLength(1);
  });

  it.skipIf(!!건너뛸사유)("실전 답을 재생하면 가드가 한 글자도 안 바꾼다", () => {
    const rows = JSON.parse(fs.readFileSync(f, "utf8")) as Record<string, unknown>[];
    const 건드림: string[] = [];
    const 건너뜀 = new Map<string, number>();
    let 총 = 0, 잰것 = 0, 챗 = 0, 도구 = 0, 챗되먹임 = 0, 도구되먹임 = 0;
    for (const r of rows) {
      const t = String(r.out ?? ""); if (!t) continue;
      총++;
      const 밖 = 모집단밖(r);
      if (밖) { 건너뜀.set(밖, (건너뜀.get(밖) ?? 0) + 1); continue; }
      잰것++;
      const 도구답 = 도구목록(r).length > 0;
      도구답 ? 도구++ : 챗++;
      // ★ **추가원천을 라이브만큼 준다**(2026-09-08 검토관 적발 — 안 주면 재생이 라이브보다 사납다).
      //   라이브: llm.ts:1154 `guardCitations(reply, chunks, [...추가원천, grounding, 첨부])`
      //   여기: 셋 중 **되살릴 수 있는 것 하나**(용어 그라운딩)만 준다. 질문만 있으면 되는 순수
      //   함수라 재생이 된다(glossary.ts:142 — DB·env 접근 없음). 안 주면 용어사전 정의를 그대로
      //   옮겨 적은 답이 「블록없음」으로 떨어져 **없는 오탐**을 만든다.
      //   ⚠ 라이브는 buildRagQuery(맥락+지시)로 뽑는데 기록엔 지시(q)뿐이라 여기 그라운딩은
      //     라이브의 **부분집합**이다 — 즉 잣대가 라이브보다 느슨해지지 않는다(안전한 방향).
      //   ⚠ 오늘 재료에서 이 인자는 판정을 **한 건도** 안 바꿨다(실측: 그라운딩이 붙는 행 14건,
      //     초록/빨강 변화 0). 값을 낸 적이 없다고 지우지 말 것 — 이건 **보험**이지 수리가 아니다.
      // ★★ **조각 기록이 있으면 그것으로 재생한다**(2026-09-08) — 라이브가 가드에게 넘긴 바로
      //   그 두 배열이라, 이 갈래는 근사가 아니라 **같은 조건**이다(위 (a'')).
      //   ⚠ 여기에 그라운딩을 또 얹지 않는다 — 기록의 추가원천에 **이미 들어 있다**(llm.ts가 넘긴
      //     값 그대로다). 겹쳐 주면 라이브에 없던 원천이 붙어 잣대가 느슨해진다(안 되는 방향).
      // ⚠ 되먹임은 **갈래별로 따로 센다** — 한 숫자로 찍으면 로그에서 「도구 N 중 M」으로 읽힌다
      //   (2026-09-08 검토관 [낮음]: 실제로 되먹임 41은 전부 챗 행인데 도구 94의 부분집합처럼 보였다).
      const 조각있음 = 조각기록있나(r);
      if (조각있음) (도구답 ? 도구되먹임++ : 챗되먹임++);
      const 재생 = 조각있음
        ? guardCitations(t, 조각기록(r), 추가원천기록(r))
        : guardCitations(t, [], [glossaryGroundingFor(String(r.q ?? ""))]);
      const 무엇 = 건드린바(t, 재생);
      if (!무엇) continue;
      건드림.push(`${String(r.q ?? "").slice(0, 30)} :: ${무엇}`);
    }
    // ⚠ **건너뜀을 삼키지 않는다** — 사유별로 몇 건인지 회차마다 찍는다(삼켜진 건너뜀은 없는 시험과 같다).
    // ⚠ 갈래도 함께 찍는다 — 「도구 경로」는 라이브에서 가드가 **무동작**인 예방 그물이라
    //   그 몫이 얼마인지 안 보이면 이 초록을 「실전을 쟀다」로 잘못 읽는다(머리글 ⓑ).
    // ⚠ 되먹임을 도구 경로 괄호 **안에** 적지 않는다 — 「도구 94 중 41」로 읽힌다(검토관 [낮음]).
    //   갈래를 찍는 줄에서 갈래가 뭉개지면 이 초록을 「실전을 쟀다」로 오독한다.
    console.log(`📊 실전 답 오탐 시험 — 기록 ${총}건 · **잰 것 ${잰것}건**(챗 경로 ${챗} = 라이브와 같은 조건 · 도구 경로 ${도구} = 라이브에선 가드 무동작인 예방 그물) · 그중 **조각 되먹임 ${챗되먹임 + 도구되먹임}건**(챗 ${챗되먹임} · 도구 ${도구되먹임} — 라이브가 가드에게 준 원천 그대로 잰 것) · 건너뜀 ${총 - 잰것}건(${[...건너뜀].map(([k, v]) => `${k} ${v}`).join(" · ") || "없음"})`);
    // ★ 문턱은 **잰 것**에 건다(기록 전체가 아니다 — 모집단이 쪼그라들면 알아야 한다).
    //   실측 2026-09-08(177문항 완주본): 기록 177 · 챗+조각 41 · 출처를 코드가 찍는 도구 1
    //   → **잰 것 135**(챗 41 · 도구 94). 여유 35건이라 문턱 100을 **그대로 둔다**(낮출 근거가 없다).
    //   이 아래로 떨어지면 재료가 바뀐 것이니 숫자를 낮추기 전에 **왜 줄었는지**부터 본다.
    // ★ 2026-09-08 (a'') 좁힘 뒤 **오늘 재료로는 숫자가 그대로다** — 이 회차 기록에는 조각 칸이
    //   한 행도 없어(서버 배포 전) 규칙이 한 건도 안 갈렸다(실측: 잰 것 135 · 챗 41 · 조각 되먹임 0).
    //   운영 배포 + 야간 회귀가 한 번 돌면 챗+조각 41건이 **모집단에 들어와** 잰 것이 176 근처로
    //   오르고 조각 되먹임이 41 근처가 된다. 그때 이 숫자들을 다시 적는다.
    //   ⚠ 이 예상은 citesource.ts가 **분류기의 null 보고를 안 세는 것**에 걸려 있다(2026-09-08 수리).
    //     그 한 줄이 없으면 챗 행이 전부 「한 턴에 가드가 2번」으로 빠져 잰 것이 94로 **떨어진다**
    //     (모의 실측). 배포 뒤 첫 회차에 그 꼴이 보이면 문턱이 아니라 **거기**를 먼저 본다.
    //   ⚠ 그 회차에 빨강이 나면 **모집단을 되돌리지 말 것** — 그건 가드를 고칠 일이다
    //     (미리 알려진 후보 2건: 「협력사에 자산 목록 줘도 되나?」·「AI가 뭘 근거로 답했는지 볼 수 있어?」).
    expect(잰것, `모집단이 너무 작다 — 잰 것 ${잰것}건(기록 ${총}건). 재료나 모집단 규칙이 바뀌었다`).toBeGreaterThanOrEqual(100);
    // ⚠ 챗 경로(라이브와 같은 조건인 유일한 갈래)가 통째로 빠지면 이 시험은 예방 그물만 남는다.
    //   그때도 위 문턱은 통과하므로 **따로** 못박는다 — 실측 41건 기준 하한 20.
    expect(챗, `챗 경로가 ${챗}건뿐이다 — 라이브와 같은 조건으로 재는 갈래가 사라졌다(재료·규칙을 보라)`).toBeGreaterThanOrEqual(20);
...
describe("★★ H4-2 맨 [n] — 참고문헌 목록이 답 전체 판정을 끄던 자리", () => {
  ...
  it("★ 이음말이 무엇이든 같다 — 쉼표·가운뎃점·「과」도 형제가 붙어 있으면 못 뗀다", () => {
    for (const 답 of [
      "번호는 [1], [2]에 붙습니다.",
      "번호는 [1]·[2]처럼 적습니다.",
      "근거 번호 [1] 과 [2]를 보세요.",
  ...
  // ★★ 2026-09-07 **재수리**(검토관 적발) — 첫 판은 「나열 안이면 무조건 건너뛴다」였고,
  //   그것이 ⑤ 블록이 적어 둔 **실사고를 통째로 되돌려 놓았다.** 이음말에 `,`·`~`·`·`가 들어
  //   있어서, 조각 수와 무관하게 **꼬리 인용목록이 통째로 범위검사 밖으로** 빠졌다.
  //   아래 넷은 그때 실행으로 재현한 그대로다 — 형제가 **안 붙어 있으므로 함께 떨어진다.**
  it("★★ 형제가 안 붙어 있으면 종전대로 뗀다 — ⑤ 실사고를 되돌리지 않는다", () => {
    const 사례: [string, string[], number[]][] = [
      ["권고 사항은 [1]~[8] 에 따릅니다.", ["a", "b", "c", "d"], [8]],   // ⑤가 고친 그 실물
      ["근거는 [1], [2] 입니다.", ["a"], [2]],                            // 흔한 꼬리 인용목록
      ["조치는 [1] 및 [5] 를 따르세요.", ["a", "b"], [5]],
      ["근거는 [1] 또는 [8] 입니다.", [], [1, 8]],                        // 클라우드 출구(chunks=[])
      ["이 취약점은 [7], [8] 문서에서 확인됩니다", ["a"], [7, 8]],
    ];
    for (const [답, 조각, 기대] of 사례) {
      const r = guardCitations(답, 조각);
      expect(r.removed.map((x) => x.n), `${답} — 나열이라는 이유로 범위검사를 빠져나갔다`).toEqual(기대);
    }
  });

  // ★★ 형제가 **낱말에 붙어 있어도** 앞선 훑기(주장_RE)가 이미 떼기로 했으면 살아남는 것이 아니다.
  //   「[7], [8]에서」는 [8]이 「…에서」라는 도입 어구를 달아 주장_RE가 먼저 판정한다 —
  //   그때 [7]만 살려 두면 **떼는 쪽과 안 떼는 쪽이 다시 어긋난다.** 실행으로 확인한 자리다.
  it("★★ 붙어 있어도 그 형제가 이미 떨어질 자리면 이쪽도 뗀다", () => {
    const r = guardCitations("이 취약점은 [7], [8]에서 확인됩니다", ["조각 하나"]);
    expect(r.removed.map((x) => x.n), "형제가 떨어지는데 이쪽만 남겼다").toEqual([7, 8]);
  });
