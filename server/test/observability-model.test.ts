...
const 항목 = async (id: string) => (await systemHealth()).checks.find((c) => c.id === id);
...
describe("자가 진단 — AI 모델", () => {
  it("정상이면 ok", async () => {
    expect((await 항목("model"))!.level).toBe("ok");
  });

  it("★ 떠 있는데 응답 준비가 안 되면 fail — GPU 유휴 정지가 이 모양이다", async () => {
    상태.v.loaded = [{ modelId: "m1", port: 8080, ready: false }];
    const c = (await 항목("model"))!;
  ...
  it("상주 모델이 없으면 warn — 고장이 아니라 필요할 때 올라온다", async () => {
    상태.v.loaded = [];
    const c = (await 항목("model"))!;
  ...
  it("상태를 못 재면 unknown — 정상으로 세지 않는다", async () => {
    상태.v = null as unknown as Record<string, unknown>;
    expect((await 항목("model"))!.level).toBe("unknown");
  });
});

describe("자가 진단 — 임베딩", () => {
  it("정상이면 ok", async () => {
    expect((await 항목("embedding"))!.level).toBe("ok");
  });

  it("안 떠 있으면 fail — 문서 인입·검색이 조용히 실패한다", async () => {
    상태.v.embedding = { running: false, port: 8081, modelId: null };
    const c = (await 항목("embedding"))!;
...
describe("전체 판정", () => {
  it("모델이 fail이면 전체도 fail — 정상 항목에 묻히지 않는다", async () => {
    상태.v.loaded = [{ modelId: "m1", port: 8080, ready: false }];
