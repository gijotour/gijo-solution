...
describe("★ 뜻 없는 조건이 전건을 건드리지 않는다", () => {
  ...
  it("★ 구어체 지시대명사도 알아본다 — 「이거 전부」", () => {
    // ⚠ 검토관 지적(2026-08-18): 「이것」은 막았는데 구어체 **「이거」**가 빠져 있었다.
    //   담당자는 글이 아니라 말로 친다 — "이거 전부 배정해줘"가 실제 말버릇이다.
    for (const 말 of ["이거", "그거", "저거", "이거 전부", "그거들", "얘네", "요거", "모두들"]) {
      expect(조건이좁히나(말), `"${말}"이 조건으로 인정됐다`).toBe(false);
    }
  });

  it("★★ 두 함수의 낱말표가 어긋나 전건이 새지 않는다 — 「다들」", () => {
    // ⚠ 이것이 이 수리의 뿌리다(검토관 지적). 예전엔 조건이좁히나와 matchFindingsByFilter가
    //   **서로 다른 낱말표**를 봤다. 그래서 「다들」은
    //     · 조건이좁히나 → "다들"이 남아 2글자 → **좁힌다(true)**
    //     · matchFindingsByFilter → "다"를 털어 "들" 1글자 → 키워드 거르기 건너뜀 → **전건**
    //   이 되어, 막으려던 바로 그 사고(뜻 없는 말로 전건 처리)가 그대로 났다.
    //   이제 같은 표(집합어_RE)를 공유하므로 「여기서 통과한 말은 저기서도 좁힌다」가 보장된다.
    const 전체건수 = matchFindingsByFilter("").length;
    const 새는말 = [];
    for (const 말 of ["다들", "모두들", "이거 전부", "그것들 다", "취약점들", "이것들전부", "얘네 다"]) {
      // 조건으로 인정됐는데(true) 실제로는 전건이 걸리면 그게 누수다.
      if (조건이좁히나(말) && matchFindingsByFilter(말).length === 전체건수) 새는말.push(말);
    }
    expect(새는말.join(", "), `이 말들이 전건을 잡는다: ${새는말.join(", ")}`).toBe("");
  });
