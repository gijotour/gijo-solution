...
describe("라이트 도구 목록", () => {
  ...
  //   라이트에서 새로 만들 것이 아니다.
  // 자체 게이트 = 허용목록에 있어도 **다른 조건**이 더 걸려 기본 카탈로그에 안 나오는 도구.
  //   · law_lookup — 기본 꺼짐(사용자가 계정을 넣어야 켜짐, 2026-08-12 사장님 결정)
  //   · watch_folder_add/remove — requiredRole:"admin"(서버 파일시스템 접근이라 프로와 같은 급).
  //     라이트 단독 사용자는 첫 계정이 admin이라 실제로 쓸 수 있고, 원격 로그인 라이트의
  //     일반 계정에서는 조용히 비켜간다(fail-closed). 조회(watch_folder_list)는 게이트 없음.
  const 자체게이트가있는것 = ["law_lookup", "watch_folder_add", "watch_folder_remove"];
