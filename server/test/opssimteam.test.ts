...
describe("⑲ 두뇌기록 — 그 팀원의 항목으로 견준다", () => {
  ...
  it("★ 배정과 다른 두뇌·되돌림을 기록으로 잡는다 — 상세에 **무엇으로 쟀는지**가 적힌다", () => {
    ...
    // ★★ 폴백은 **한 사실**이다 — 「원격 배정인데 로컬에서 돌았다」와 「원격이 안 닿아 되돌렸다」가
    //   동시에 참이라, 종전에는 불편이 두 줄로 나갔다(보고서의 「불편 종류별」이 항목 단위로 세므로
    //   10건이 20건으로 찍힌다). toContain은 두 건이 나와도 통과해서 못 잡던 자리라 **건수를 문다**.
    const 폴백 = 팀원판정(리포트행, { 팀원: "report", ms: 9000, 두뇌기록: [항목({ location: "local", fallback: true })] }, 배정);
    expect(
      폴백.불편.map((x: { 종류: string }) => x.종류),
      "폴백 한 번이 불편 두 건으로 세어진다 — 이 파일이 지연 판정에서 스스로 금지한 「같은 사실 두 번 세기」다",
    ).toEqual(["원격이 안 닿아 이 PC로"]);
    expect(두뇌말(폴백.기록)).toBe("폴백(local·기록)");
  });

  it("★ 전역 원격이 꺼진 회차는 상세가 **까닭까지** 적는다 — 제품 결함과 꺼진 스위치를 안 뭉갠다", () => {
    // 제품은 팀원이 remote여도 전역이 꺼져 있으면 로컬로 보낸다(llm.ts resolveRemoteTarget ②).
    const 꺼짐 = 팀원판정(리포트행, { 팀원: "report", ms: 9000, 두뇌기록: [항목({ location: "local" })] }, 배정, false, false);
    expect(꺼짐.불편[0].상세, "매일 밤 원인 추적을 사람이 하게 된다").toContain("전역 원격 꺼짐");

    // 켜져 있거나(제품 결함) 못 읽었으면(모름) 없는 까닭을 지어내지 않는다.
    const 켜짐 = 팀원판정(리포트행, { 팀원: "report", ms: 9000, 두뇌기록: [항목({ location: "local" })] }, 배정, false, true);
    expect(켜짐.불편[0].상세).not.toContain("전역 원격 꺼짐");
    const 모름 = 팀원판정(리포트행, { 팀원: "report", ms: 9000, 두뇌기록: [항목({ location: "local" })] }, 배정, false, null);
    expect(모름.불편[0].상세, "못 읽은 상태를 「꺼짐」으로 단언했다").not.toContain("전역 원격 꺼짐");
  });

  it("★ 기록이 왔는데 그 팀원 항목이 없으면 **「안 돎」**이다 — 「돎」이라 우기지 않는다", () => {
    // 원격이 예산·시간 초과로 그 chat을 아예 못 보낸 밤 — 표가 「돎」이라 적으면 정의 A가 과대보고된다.
    const 남의것뿐 = 팀원판정(리포트행, { 팀원: "report", ms: 21000, 두뇌기록: [항목({ agentId: "orchestrator", location: "local" })] }, 배정);
    expect(남의것뿐.기록.기록받음, "응답에 기록 칸이 왔다는 사실을 안 적었다 — 옛 서버와 구별이 안 된다").toBe(true);
    expect(남의것뿐.기록.기록으로잼).toBe(false);
    expect(두뇌말(남의것뿐.기록), "새로 얻은 증거가 반박하는 주장을 표가 계속 우긴다").toBe("안 돎(기록에 없음)");

    // 서버 상한에 걸려 **잘린** 회차는 「안 돌았다」가 아니라 「못 잰다」다.
    const 잘림 = 팀원판정(
      리포트행,
      { 팀원: "report", ms: 21000, 두뇌기록: [항목({ agentId: "orchestrator", location: "local", 생략: 7 })] },
      배정,
    );
    expect(잘림.기록.기록생략, "항목을 못 찾은 회차에 생략을 안 셌다 — 그때가 바로 필요한 순간이다").toBe(7);
    expect(두뇌말(잘림.기록), "「자리가 없어 못 담았다」를 「안 돌았다」로 단언했다").toBe("못 잼(기록이 잘림)");

    // 옛 서버(칸 자체가 없음)는 종전 그대로 — 없다고 빨강이 아니다.
    const 옛서버 = 팀원판정(리포트행, { 팀원: "report", ms: 21000 }, 배정);
    expect(옛서버.기록.기록받음).toBe(false);
    expect(두뇌말(옛서버.기록)).toBe("돎(표식 안 남는 갈래)");
  });

  it("★ 한 줄 안에서 잣대를 섞지 않는다 — 기록이 있으면 **모델도** 기록만 본다", () => {
    // llm.ts는 시간 초과·요청 실패 갈래에서 model=null로 보고한다. 그때 남의 chat(표식)의 모델
    // 이름으로 떨어지면 위치·폴백은 이 팀원 것, 모델은 남의 것이 되어 한 행이 두 잣대로 채워진다.
    const 판 = 팀원판정(
      리포트행,
      { 팀원: "report", ms: 21000, 두뇌: { location: "local", model: "남의것-14b" }, 두뇌기록: [항목({ model: null })] },
      배정,
    );
    expect(판.기록.모델, "다른 호출의 모델 이름을 이 팀원 줄에 적었다").toBeNull();
    expect(판.기록.실제위치, "위치는 기록에서 왔는데 모델만 남의 것이면 한 행에 두 잣대가 섞인다").toBe("remote");
  });
  ...
  it("★ 반증 — 기록이 없거나 **남의 항목뿐**이면 종전 갈래 그대로다", () => {
    ...
    // ⚠ 판정은 안 걸지만 **표의 말은 달라진다** — 기록이 온 회차라 「옛 서버」가 아니라 「안 돎」이다.
    expect(두뇌말(남의것.기록)).toBe("안 돎(기록에 없음)");
  ...
  it("★ 고르는 규칙 — 「담당자가 읽는 답」이 먼저, 개수와 잘림도 함께 적는다", () => {
    ...
    // ★ 두 가지 「없음」을 가른다 — 칸이 **아예 없는** 옛 서버만 null이다.
    //   칸은 왔는데 이 팀원 항목이 없으면 항목만 null이고, 그때도 **생략은 세어 돌려준다**
    //   (그래야 「자리가 없어 못 담았다」와 「안 돌았다」가 표에서 안 뒤섞인다).
    expect(두뇌기록고르기([], "report").항목, "빈 배열에서 무언가를 골랐다").toBeNull();
    expect(두뇌기록고르기([항목({ agentId: "ti", 생략: 5 })], "report"), "항목이 없다고 생략까지 버렸다")
      .toEqual({ 항목: null, 개수: 0, 사람답: false, 생략: 5 });
  ...
  it("★★ 표가 「원격을 못 봤다」를 더는 거짓으로 적지 않는다 — 본 회차는 본 대로 적는다", () => {
    ...
    //   ⚠ **몇 회 중 하나를 잰 값인지**까지 적는다 — 안 적으면 「이 팀원 두뇌의 제 시간」으로 읽혀,
    //     한 답에 여러 번 도는 팀원의 시간이 실제보다 작게 보인다(무엇을 쟀는지 모르는 숫자).
    expect(md).toContain("(두뇌 19.0s·1/1)");
    expect(md, "새 숫자를 뜻풀이 없이 표에 실었다").toContain("「(두뇌 N)」은 **그 팀원 호출 한 번의 왕복**입니다");
    expect(md).toContain("**평균에 넣은 줄 수 a**와 **그 팀원이 실제로 돈 호출 수 b**");
    // 이 회차의 둘은 다 봤으므로 「못 재는 팀원」 줄 자체가 없다 — 없는 한계를 지어내지 않는다.
    expect(md, "다 본 회차에 「못 재는 팀원」이 남았다").not.toContain("못 재는 팀원:");
  });

  it("★ 표가 「잘렸다」·「안 돎」·「문만인데 돌았다」를 각각 말한다 — 뒤섞이면 판독이 못 갈린다", () => {
    const 회차 = [
      // ① 기록은 왔는데 이 팀원 항목이 없고, 서버가 잘라 버렸다 → 「못 잼」
      { q: 리포트행.q, 불편: [], 팀원기록: 팀원판정(리포트행, { 팀원: "report", ms: 21000, 두뇌기록: [항목({ agentId: "ti", 생략: 4 })] }, 배정).기록 },
      // ② 「문만(반입 때 돎)」이라 적어 둔 팀원인데 그 요청에서 두뇌가 돌았다 → 경고만
      { q: 사서행.q, 불편: [], 팀원기록: 팀원판정(사서행, { 도구: ["knowledge_status"], ms: 1200, 두뇌기록: [항목({ agentId: "curator" })] }, 배정).기록 },
    ];
    const md = 팀원표(회차, 배정).join("\n");
    expect(md, "잘려서 못 담은 회차를 조용히 넘겼다").toContain("잘린 문항 1건");
    expect(md, "판정은 그대로 두되 반증 증거를 버리지 않는다").toContain("그 팀원 호출이 **기록에 있었습니다**");
    expect(md).toContain("curator");
    // 「문만」 판정 자체는 안 바꾼다 — 문항표가 재는 것은 「문으로 갔나」뿐이다.
    expect(md).toContain("문만(반입 때 돎)");
  });

  it("★ 기록이 온 회차에는 「M2 미반영」이 안 찍힌다 — 항목이 없어도 그렇다", () => {
    // 잣대가 `기록으로잼`이면 「새 서버인데 이 요청에 그 팀원 chat이 없던 줄」이 옛 서버로 찍힌다.
    const 총괄기록없음 = 팀원판정(총괄행, { 도구: ["explain"], ms: 9000, 두뇌기록: [항목({ agentId: "ti" })] }, 배정).기록;
    expect(총괄기록없음.기록받음).toBe(true);
    expect(두뇌말(총괄기록없음), "옛 서버가 아닌데 「표식 없음(옛 서버)」이라 적었다").toBe("안 돎(기록에 없음)");
    const md = 팀원표([{ q: 총괄행.q, 불편: [], 팀원기록: 총괄기록없음 }], 배정).join("\n");
    expect(md, "서버가 기록을 실었는데 「옛 서버」라는 거짓 진단이 찍혔다").not.toContain("M2 미반영");
    // ⚠ 「표식 없음(옛 서버)」 글자는 표 머리말의 **뜻풀이**에도 나온다 — 표 본문이 아니라
    //   판정 결과(위 두뇌말)로 문다. 글자 검색으로 물면 뜻풀이를 늘릴 때마다 헛빨강이 난다.
    expect(md, "표 본문 줄이 이 팀원을 「옛 서버」로 적었다").not.toContain("| 표식 없음(옛 서버) 1 |");
  });

  it("하네스가 그 칸을 받아 적는다 — 서버가 실어 줘도 기록에 안 남으면 없는 것과 같다", () => {
    const 하네스 = 읽기(join("tools", "ops-sim.mjs"));
    expect(하네스, "두뇌기록을 안 받아 적는다").toContain("두뇌기록: j.두뇌기록,");
    // ★ 전역 원격 스위치도 **회차마다** 읽는다 — 배정이 remote여도 전역이 꺼지면 로컬로 돈다.
    expect(하네스, "전역 원격 상태를 안 읽는다 — 「배정과 다른 두뇌」의 까닭을 매일 밤 사람이 뒤진다")
      .toContain('BASE + "/api/llm/remote/where"');
    expect(하네스, "판정에 전역 원격 상태를 안 넘긴다").toContain("팀원판정(팀원행, r, 배정표, 목.some((x) => x.종류 === \"너무 오래 걸림\"), 전역원격)");
    expect(하네스, "그날의 전역 상태를 meta에 안 남긴다 — 나중에 원인을 못 가린다").toContain("전역원격,");
  });
});
...
describe("⑲ 보고서 팀원별 표", () => {
  ...
  // ★★ 2026-09-12 검토관 적발 수리 — **건너뛴 ⑲ 줄**(프로필·설정 때문에 이 회차엔 안 잰 문항)은
  //   `팀원기록`이 안 붙어 표에서 통째로 빠진다. 그러면 분모가 줄고, 위 경고가 「--only·중단」
  //   탓으로 잘못 읽힌다 — 건너뜀은 **그와 다른 일**이라 사유까지 적어야 한다.
  it("★ 건너뛴 ⑲ 줄은 「--only·중단」과 구별해 적는다(사유까지)", () => {
    const 건너뜀줄 = { q: "최근 올라온 문서 뭐야?", 건너뜀: "데이터:씨앗문서", out: "", 불편: [] };
    expect(팀원문항.some((r) => r.q === 건너뜀줄.q), "⑲에 그 물음이 없다 — 시험 재료가 낡았다").toBe(true);
    const md = 팀원표([...회차, 건너뜀줄], 배정).join("\n");
    expect(md).toContain("27문항 중 **4개만**");
    expect(md, "건너뜀이 --only·중단과 뭉쳐 적혔다").toContain("그중 건너뜀 1");
    expect(md, "건너뜀 사유가 표에 안 적혔다").toContain("데이터:씨앗문서");
  });

  it("건너뜀이 없으면 문구는 종전 그대로(--only·--limit 또는 중단)", () => {
    const md = 팀원표(회차, 배정).join("\n");
    expect(md).toContain("(--only·--limit 또는 중단)");
    expect(md).not.toContain("그중 건너뜀");
  });
});
