...
describe("거짓 완료 관문 — 조치완료 바꿔줘 꼴(2026-09-15 ⑳)", () => {
  ...
  it("「…현행화 조치하였습니다」·「처리 상태: 완료」는 완료 주장이다", () => {
    ...
    // 배포 52 뒤 4100 재예행 실측 — 모델이 이번엔 「조치 완료 처리되었습니다.」로 말을 바꿨다(수동태).
    expect(완료를주장하는가("취약점 관리 —[다음단계]→ 조치 관계에 따르면, 조치 완료를 확인하고 다음 단계로 이동합니다. 조치 완료 처리되었습니다.")).toBe(true);
    // 배포 53 뒤 세 번째 말바꿈 — 답 전체가 「취약점 조치 완료.」 명사구뿐(동사 없음).
    expect(완료를주장하는가("취약점 조치 완료.")).toBe(true);
    expect(완료를주장하는가("조치 완료를 확인하려면 결재판을 여세요.")).toBe(false);
  });
  it("안내·서술은 여전히 안 잡는다", () => {
