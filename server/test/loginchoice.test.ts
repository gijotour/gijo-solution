...
//   ① **그 값을 누가 넣는가·누가 읽는가**: 실제로 `gijo:recoveryRemaining`은 login.html이 쓰는데
//      읽는 곳이 없다(settings는 서버 값을 읽는다). 소비자 없는 값은 조용히 죽는다.
...
describe("로그인 화면의 모드·위치 고르기", () => {
  ...
  it("★ 라이트·클라우드는 **다른 무게**로 그린다 — 나란한 버튼이면 거짓말이 된다", () => {
    ...
    expect(로그인, "라이트·클라우드가 다른 테두리로 안 그려진다").toMatch(
      /\.mp-btn\[data-mode="lite"\][\s\S]{0,80}border-style:\s*dashed/
    );
  ...
  it("★★ 적용 실패를 **읽는 곳이 있다** — 생산자와 소비자를 묶는다", () => {
    ...
    expect(셸, "실패를 사람에게 안 알린다").toMatch(/로그인선택_못한것알리기[\s\S]{0,900}gijoTell\(/);
