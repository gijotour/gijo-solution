...
describe("로그인 화면의 모드·위치 고르기", () => {
  ...
  it("★ 못 한 일을 「했다」고 말하지 않는다 — 원격 주소가 없으면 그렇게 적는다", () => {
    ...
    // ⚠ 2026-09-02(F8-10)에 이름표를 `위치이름` 표 한 곳으로 모았다 — 문장이 조립식이 되어
    //   「클라우드」와 콜론이 더 이상 소스에서 붙어 있지 않다. 지키려는 것은 표기가 아니라
    //   **「로그인 창에서 조용히 켜지 않고 설정으로 보낸다」**이므로 두 표기를 다 받는다.
    expect(로그인, "클라우드를 로그인 창에서 조용히 켠다").toMatch(
      /위치이름\.cloud \+ ": 로그인 뒤 설정|클라우드\)?: 로그인 뒤 설정/
    );
  });
