...
// ═══════════════════════════════════════════════════════════════════════════
// ★★ 검토관 2026-09-06 적발 수리 — 범위가 **틀린 자리를 가리키던** 다섯 자리
//
// 앞 라운드는 「범위를 실어 보낸다」까지 했는데, 그 범위를 **만드는 쪽**과 **읽는 쪽**에
// 서로 어긋나는 자리가 남아 있었다. 다섯 다 결과가 같다 — 지어낸 숫자가 진하게 나가거나,
// 문서에 그대로 있는 숫자가 회색이 된다. 둘 다 이 기능이 막으려는 바로 그 해악이다.
import { 숫자가원천에있나 } from "../src/engine/citeguard";
import { 숫자무근거배너붙이기 } from "../src/engine/noevidence";

describe("★★ 범위 수리 — 배너는 맞는데 범위가 틀리던 자리들", () => {
  it("④ 배너가 맨 앞인데 본문에 【N.】이 있으면 — 꼬리를 통째로 버리던 자리", () => {
    // 근거범위()가 단계나누기 성공 시 **머리표 앞 도입부를 안 봤다.** 배너는 도입부에 있으므로
    // 범위가 undefined가 되고, 화면은 답 전체를 옅게 그린다(참인 82.3%까지 회색).
    const 답 = `${숫자무근거배너}${없는수치꼬리(["12.5%"])}\n\n【1. 개요】 전년 대비 12.5% 늘어 이수율은 82.3%입니다.`;
    expect(근거범위(답), "머리표 앞 배너를 못 읽어 범위가 사라졌다").toEqual({ 수치: ["12.5%"] });
    for (const 그리기 of [지휘소로, 위젯으로]) {
      const el = 그리기(답);
      P.dimEstimates(el, "숫자무근거", 근거범위(답));
      expect(옅힌것(el), "문서에 그대로 적힌 82.3%가 회색이 됐다").toEqual(["12.5%"]);
    }
  });

  it("④' 머리표 앞 배너에 **꼬리가 없으면** 답 전체가 범위다(undefined = 오늘 동작)", () => {
    const 답 = `${자료요청배너}\n\n【1. 개요】 만족도는 75점입니다.`;
    expect(근거없음종류판정(답)).toBe("자료요청");
    expect(근거범위(답), "꼬리가 없으면 좁힐 근거가 없다 — 전체가 범위여야 한다").toBeUndefined();
  });

  it("⑤ 두 단계가 각각 배너인데 **한쪽만 꼬리** — 꼬리 없는 단계가 통째로 옅어져야 한다", () => {
    // 옛 코드는 수치를 한 배열에 합치고 두 단계를 AND로 걸어, **꼬리 없는 단계(=전부 지어낸 단계)의
    // 숫자가 하나도 안 옅어졌다.** 배너는 「모델 추정치」라 말하는데 화면엔 표시가 없었다.
    const 답 = [
      `【2. 이수율】 ${숫자무근거배너}${없는수치꼬리(["12.5%"])}`, "",
      "전년 대비 12.5% 늘었고 이수율은 82.3%입니다.", "",
      `【3. 전망】 ${숫자무근거배너}`, "",
      "내년 이수율은 91.4%로 오르고 미조치는 7.2%가 됩니다.",
    ].join("\n");
    expect(근거범위(답)).toEqual({ 단계: [3], 수치: ["12.5%"] });
    for (const 그리기 of [지휘소로, 위젯으로]) {
      const el = 그리기(답);
      P.dimEstimates(el, "숫자무근거", 근거범위(답));
      expect(옅힌것(el), "전부 지어낸 3단계가 진하게 남았다").toEqual(["12.5%", "91.4%", "7.2%"]);
    }
  });

  it("⑥ 서버가 콕 집은 **한 자리 수**를 화면도 옅게 한다 — 배너와 회색이 딴말을 하면 안 된다", () => {
    // 서버 비율맨수_RE는 한 자리도 싣는데 화면은 「단위 없으면 두 자리 이상」이라 0개를 옅게 했다.
    // 꼬리에 「(사내 자료에 없는 수치: 5)」라 적어 놓고 아무것도 안 옅게 하는 자리다.
    const 본문 = "이수율은 82.3%이고 조치율은 5 수준입니다.";
    const 접지 = 숫자가원천에있나(본문, ["2분기 전사 이수율 82.3%로 집계"]);
    expect(접지.없는것표면, "서버 잣대가 바뀌었으면 화면 쪽도 다시 볼 것").toEqual(["5"]);
    const 답 = `${숫자무근거배너}${없는수치꼬리(접지.없는것표면)}\n\n${본문}`;
    for (const 그리기 of [지휘소로, 위젯으로]) {
      const el = 그리기(답);
      P.dimEstimates(el, "숫자무근거", 근거범위(답));
      expect(옅힌것(el), "배너는 「없는 수치: 5」라 적었는데 화면은 아무것도 안 옅게 했다").toEqual(["5"]);
    }
  });

  it("⑥' 허용목록에 없는 한 자리 수는 그대로 진하다 — 「3가지 방법」의 3", () => {
    const el = 지휘소로(`${숫자무근거배너}${없는수치꼬리(["12.5%"])}\n\n3가지 방법으로 12.5% 줄었습니다.`);
    P.dimEstimates(el, "숫자무근거", { 수치: ["12.5%"] });
    expect(옅힌것(el)).toEqual(["12.5%"]);
  });

  it("⑧ 범위 **안**의 머리표 번호도 안 옅어진다 — 머리끝 가드가 실제로 걸리는 자리", () => {
    // 옛 시험은 배너 덩어리(어차피 건너뛰는 자리)의 머리표를 재서, 가드를 지워도 초록이었다.
    const 답 = `【9. 스캔】 ${자료요청배너}\n\n【10. 리포트】 총평 점수는 75점입니다.`;
    for (const 그리기 of [지휘소로, 위젯으로]) {
      const el = 그리기(답);
      P.dimEstimates(el, "자료요청", { 단계: [9, 10] });
      expect(옅힌것(el), "머리표의 10이 답의 값으로 새어 나왔다").toEqual(["75점"]);
    }
  });

  it("⑨ 이어보기(worksessions GET)도 범위를 싣는다 — 되살린 답만 전체가 옅어지면 안 된다", () => {
    const ws = fs.readFileSync(path.join(__dirname, "../src/engine/worksessions.ts"), "utf8");
    expect(ws, "이어보기가 표식만 싣고 범위를 안 싣는다 — 「자리마다 달라 보인다」의 재발 경로")
      .toMatch(/근거범위\(t\.content\)/);
    expect(ws, "범위 칸 이름이 화면과 다르다").toMatch(/근거범위: 범위/);
  });
});

describe("★★ ① 배너를 붙이는 규칙 — **시험이 닿는 자리**로 꺼냈다", () => {
  // 2026-09-06 검토관(상): 이 라운드의 핵심 변경(조건 「전부 없을 때만」→「하나라도 없으면」,
  //   그리고 꼬리)을 잡는 시험이 **하나도 없었다.** 두 줄을 옛 코드로 되돌려도 전체가 초록이었다.
  //   원인은 규칙이 chat() 안에 박혀 있던 것 — 그 함수는 시험 76개가 통째로 흉내 내서 못 닿는다.
  const 부분접지 = { 판정: "있음", 없는것: ["12.5"], 없는것표면: ["12.5%"] };

  it("부분 접지에 배너 + 꼬리가 붙는다 — 옛 조건(판정==='없음')으로 되돌리면 여기가 빨개진다", () => {
    expect(숫자무근거배너붙이기("본문", 부분접지))
      .toBe(`${숫자무근거배너}${없는수치꼬리(["12.5%"])}\n\n본문`);
  });

  it("전부 없으면 **꼬리 없이** 배너만 — 답 전체가 범위다", () => {
    expect(숫자무근거배너붙이기("본문", { 판정: "없음", 없는것: ["82.3"], 없는것표면: ["82.3%"] }))
      .toBe(`${숫자무근거배너}\n\n본문`);
  });

  it("없는 수치가 하나도 없으면 답을 **한 글자도** 안 건드린다", () => {
    expect(숫자무근거배너붙이기("본문", { 판정: "있음", 없는것: [], 없는것표면: [] })).toBe("본문");
    expect(숫자무근거배너붙이기("본문", { 판정: "해당없음", 없는것: [], 없는것표면: [] })).toBe("본문");
  });

  it("상한(6)을 넘으면 꼬리를 안 붙인다 = 답 전체가 범위", () => {
    const 일곱 = ["1%", "2%", "3%", "4%", "5%", "6%", "7%"];
    const r = 숫자무근거배너붙이기("본문", { 판정: "있음", 없는것: 일곱.map((x) => x.slice(0, -1)), 없는것표면: 일곱 });
    expect(r).toBe(`${숫자무근거배너}\n\n본문`);
    expect(근거범위(r), "꼬리가 없으면 범위도 없다").toBeUndefined();
  });

  it("붙인 답이 그대로 **범위로 되읽힌다** — 붙이는 쪽과 읽는 쪽이 한 짝이다", () => {
    const 답 = 숫자무근거배너붙이기("전년 대비 12.5% 늘어 이수율은 82.3%입니다.", 부분접지);
    expect(근거없음종류판정(답)).toBe("숫자무근거");
    expect(근거범위(답)).toEqual({ 수치: ["12.5%"] });
    const el = 지휘소로(답);
    P.dimEstimates(el, "숫자무근거", 근거범위(답));
    expect(옅힌것(el)).toEqual(["12.5%"]);
  });

  it("llm.ts는 그 규칙을 **다시 적지 않는다** — 두 벌이 되면 어긋난다", () => {
    const llm = fs.readFileSync(path.join(__dirname, "../src/engine/llm.ts"), "utf8");
    expect(llm, "붙이는 자리가 noevidence를 안 부른다").toContain("숫자무근거배너붙이기(reply, 접지)");
    expect(llm.includes("없는수치꼬리("), "꼬리 규칙이 llm.ts에도 박혀 두 벌이 됐다").toBe(false);
    expect(llm.includes("접지.없는것.length"), "조건이 llm.ts에 다시 박혔다").toBe(false);
  });
});
