...
describe("collectBasis — 상태에 따라 필요한 근거만 찾는다", () => {
  it("아직 취약(FAIL)이면 보상통제 근거를 찾는다", async () => {
    queryMemoryScored.mockResolvedValue([chunk("해당 CVE는 경계 IPS 시그니처로 차단 중이며 내부망에서만 접근 가능하다.")]);
  ...
  it("자동 판정 불가(NA)면 장비 확인 방법을 붙인다", async () => {
    queryMemoryScored.mockResolvedValue([chunk("펌웨어 버전 확인: System > Firmware & Registration 메뉴에서 확인한다.")]);
    const b = await collectBasis({ findingKey: "k1", title: "FortiOS 취약점", status: "NA", expectedKind: "version" });
  ...
  it("출처(documentId)를 함께 준다 — 담당자가 원문을 찾아갈 수 있어야 한다", async () => {
    queryMemoryScored.mockResolvedValue([chunk("WAF 룰로 차단 중", 0.2, "보안운영지침.md")]);
    const b = await collectBasis({ findingKey: "k1", title: "SQL Injection", status: "FAIL", expectedKind: "manual" });
...
describe("attachBasis — 결과 목록에 근거 붙이기", () => {
  ...
  it("판정(status)은 근거 때문에 바뀌지 않는다 — 근거는 제안일 뿐", async () => {
    queryMemoryScored.mockResolvedValue([chunk("IPS 시그니처로 차단 중")]);
    const out = await attachBasis([R({ status: "FAIL" })]);
