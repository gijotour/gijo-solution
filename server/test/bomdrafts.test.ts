...
describe("등록부 — 부품표(bom)는 8번째 팀원이고 전원에게 맡은 메뉴가 있다", () => {
  ...
  it("맡은 메뉴는 screenguide가 아는 화면 파일명이고 제목이 따라온다 — 빈 팀원은 이유가 적혀 있다", () => {
    // 메뉴 칩이 없는 팀원은 이유와 함께 예외 — 빈 배열을 조용히 허용하지 않는다. 사유는 agents.ts의 menus 주석과 짝(뜻이 갈리면 한쪽을 고친다).
    const 예외: Record<string, string> = { normaltic: "전용 메뉴가 없다 — 📚 침해사고 히스토리 판(incidentcases.html)은 메뉴 항목이 아니라 카드·칩으로 연다(팀원 카드 「📚 히스토리」 단추·취약점 카드 「비슷한 사례」 칩·AI 허브 판 카드). 칩까지 달면 같은 곳이 두 번 보인다(2026-09-03, agents.ts 사유와 짝)" };
