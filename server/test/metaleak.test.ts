...
describe("★ 배선 감시 — 호출 한 줄이 사라지면 조용히 다시 샌다", () => {
  ...
  it("ⓒ llm.chat 출구가 인용 가드 **뒤에서** 한 번 더 뗀다 — 계수는 같은 kind=cite 신호", () => {
    ...
    // ⚠ 이름표만 본다 — 2026-09-06에 `type 메타걷기결과`가 같은 줄에 붙었다(가드사유의 인자 타입).
    //   글자를 통째로 박아 두면 **정당한 추가에도 빨개져** 감시가 「고치라는 신호」가 아니라 소음이 된다.
    expect(src, "메타줄걷기를 안 물고 있다 — 출구 방어가 통째로 빠졌다")
      .toMatch(/import \{ 메타줄걷기[^}]*\} from "\.\/metaleak"/);
