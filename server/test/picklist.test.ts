...
describe("★ 목록 질문만 규칙으로 가로챈다", () => {
  ...
  const 목록질문 = [
    ...
    "남은 취약점 뭐 있지",
    "취약점 현황 보여줘",
    // ★ 야간 회귀 ⑪마당 「사람은 이렇게 친다 — 모호·오타·짧게」 실위반(2026-09-04).
    //   「알려」가 목록 동사에 없어 **오타가 아닌 「취약점 알려줘」조차** 규칙을 비켰고,
    //   도구 없이 모델로 가서 "특정 취약점이 명시되어 있지 않습니다"라고 답했다(202건 있는데).
    //   어미 오타(주세여·줘여·주세용)는 scopeguard의 요청어미 한 곳에서 받는다.
    "취약점 알려주세여",
    "취약점 알려줘여",
    "취약점 알려줘",
    "취약점 좀 알려주세요",
  ...
  const 건드리면안됨 = [
    ...
    // ★ 위 오타·모호 허용이 **넓어지지 않았음**을 지킨다(2026-09-04).
    "취약점이란 뭐야",                             // 개념 질문 — 해설의 영토
    "취약점이 뭔지 알려줘",                        // 동사 앞에 다른 말이 낀다 = 개념 질문
    "안전대부 웹서버 취약점 알려줘",               // 대상 지목 — FORCED_INTENTS[1](search)의 영토
    "SSH 취약점 알려줘",                           // 같은 이유(대상이 앞에 있다)
    "미조치 취약점 알려줘",                        // 상태어 — 상태어취약점(finding_status 필터)의 영토
    "고위험 취약점 알려주세여",                    // 같은 이유(조건이 사라지면 안 된다)
  ];
