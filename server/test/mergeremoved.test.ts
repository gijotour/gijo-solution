...
describe("★ merge(LLM 합성) 삭제 — 반쪽만 지워지지 않았다", () => {
  ...
  it("★ 지운 화면 이름이 셸의 「옛탭」 표에 살아 있다 — 빈 탭 대신 닫고 알린다", () => {
    ...
    // ③′ **선언만 보면 안 된다**(2026-09-08 반증에서 드러남).
    //   예전엔 위 한 줄이 전부였다. 그래서 호출부 5곳을 **전부 지워도** 이 시험이 초록이었다 —
    //   선언만 남은 죽은 함수를 「알린다」의 증거로 읽은 셈이다. 배선을 직접 센다.
    //   ⚠ 알림 사슬은 갈래가 여럿이라(로그인선택 실패·복구코드 없음·읽기 실패·정상 종료 …)
    //     한 갈래라도 빠지면 「어떤 날은 뜨고 어떤 날은 안 뜨는」 알림이 된다.
    const 호출수 = (셸.match(/폐지된탭알리기\(\)/g) || []).length
      - (셸.match(/function 폐지된탭알리기\(\)/g) || []).length;
    expect(호출수, "폐지된탭알리기를 부르는 자리가 거의 없다 — 함수만 있고 아무도 안 부르면 닫기만 하고 조용하다")
      .toBeGreaterThanOrEqual(4);
    // ③″ 못 띄우는 갈래에서 **목록을 비우지 않는다**(형제 규약과 같게 맞춘 것 — 2026-09-08).
    //   비운 뒤에 gijoTell 유무를 보면, 대화상자가 아직 없는 회차에서 그 회차만이 아니라
    //   **영영** 안 알린다(복원부 save()가 저장값도 이미 지운 뒤라 되살릴 근거가 없다).
    const 알리기본문 = /function 폐지된탭알리기\(\)\s*\{([\s\S]*?)\n  \}/.exec(셸);
    expect(알리기본문, "폐지된탭알리기 본문을 못 읽었다 — 시험이 헛돈다").toBeTruthy();
    const 비우는자리 = 알리기본문![1].indexOf("버려진옛탭.length = 0");
    const 가드자리 = 알리기본문![1].indexOf('typeof window.gijoTell !== "function"');
    expect(비우는자리, "목록을 비우는 줄이 없다").toBeGreaterThan(-1);
    expect(가드자리, "gijoTell 방어가 없다").toBeGreaterThan(-1);
    expect(비우는자리, "gijoTell 방어보다 **먼저** 목록을 비운다 — 못 띄운 회차에서 알림이 영영 사라진다(형제 함수는 「없을 때는 키를 지우지 않는다」로 정반대다)")
      .toBeGreaterThan(가드자리);
    ...
    // ⑤ **널을 거른다** — 바로잡기가 폐지 화면에 null을 주므로 그대로 open(null)을 부르면
    //   탭 이름이 `null`이 되고 iframe 주소가 문서 자신이 되어 **탭 안에 셸이 통째로 실린다**.
    //   ②가 「null을 준다」까지만 보고 「그 null을 거른다」를 안 봐서, 거르기를 지워도 초록이었다.
    const 복원자리 = 셸.indexOf("st.open.map(옛탭바로잡기)");
    expect(복원자리, "복원부에서 옛탭바로잡기를 안 쓴다 — 시험이 헛돈다").toBeGreaterThan(0);
    expect(/st\.open\.map\(옛탭바로잡기\)\.forEach\(function \(o\) \{\s*\n\s*if \(!o\) return;/.test(셸),
      "바로잡기가 준 null을 안 거른다 — open(null)이 탭 안에 셸을 통째로 싣는다(healFrames 주석이 적어 둔 그 사고다)").toBe(true);
  });
