...
// ── 표 인지 — **이 시험이 쓰는 잣대**(제품 잣대는 engine/tabletext.ts에 한 벌로 있다) ──────
//   ⚠ 일부러 제품 함수를 import하지 않는다. 제품이 자기 잣대로 「표가 안 갈렸다」고 답하면
//     잣대가 틀렸을 때 시험도 함께 틀린다 — 여기서는 마크다운 규격을 그대로 다시 적어 잰다.
//   ⚠ 「독립」은 **제품 함수를 안 부른다**는 뜻이지 **딴 규격을 쓴다**는 뜻이 아니다. 규격(GFM)은
//     칸마다 대시 하나면 구분선이라, 2026-09-10에 여기도 그 꼴을 받도록 고쳤다.
//     실측: 지식 24편·매니페스트 35편에 홑대시 구분선은 0줄이라 **아래 셈은 한 건도 안 바뀐다**
//     (안 고치면 그런 문서가 들어오는 날 이 하네스만 조용히 딴 숫자를 센다).
...
describe("청커 판 — 규칙을 고치면 CHUNKER_VERSION을 올린다 (소스 감시)", () => {
  // ★ 2026-09-10: 표 술어가 engine/tabletext.ts(잎)로 옮겨 갔다. **지문 범위도 함께 옮긴다** —
  //   안 옮기면 앞으로 표 술어를 고쳐도 이 감시가 안 울어, 리팩터가 스스로 구멍을 뚫는 꼴이 된다
  //   (2026-07-31 「코드는 새것인데 지식은 옛 조각」의 재발 경로). 그래서 지문은 **두 조각**이다:
  //     ① memory.ts — 청킹 본체(chunkText·cleanExtractedText·머리글만남나)
  //     ② tabletext.ts — 「읽는 쪽」 술어 구간(표줄·구분선·열수·칸가르기·표머리글들·표본문행자리)
  //   ②의 「내는 쪽」(칸글·파이프표)은 **일부러 뺀다** — 그쪽은 업로드본 추출 규격이라 매니페스트
  //   내장 문서(손으로 쓴 .md)의 조각을 안 바꾼다. 넣으면 헛올림(재임베딩 1,500조각)만 늘어난다.
  const 구간들: [파일: string, 시작표식: string, 끝표식: string][] = [
    ["src/engine/memory.ts", "// ── 표(마크다운 테이블) 인지", "/** 글자로 그냥 읽으면 안 되는(추출이 필요한) 형식"],
    ["src/engine/tabletext.ts", "// ── 표(마크다운 테이블) 인지", "// ── 여기까지가 「읽는 쪽」"],
  ];
  /** 판 ↔ 그 판이 자르던 규칙의 지문. **둘은 언제나 함께 바뀐다.** */
  const 청커지문: Record<string, string> = {
    // ⚠ 아래 두 줄은 **옮기기 전**(술어가 memory.ts 한 파일에 있던 배치)에서 잰 값이다.
    //   그 판으로 되돌리려면 소스 배치까지 함께 되돌려야 하므로, 지금 배치에서는 안 맞는다.
    //   지우지 않고 남기는 이유: 「그때 무엇이 잣대였나」의 기록이 사라지면 되돌림을 못 읽는다.
    "2026-09-08-table-2": "4d7dfbeb4d40", // 표 구분선·머리글 예외까지 (옛 배치)
    "2026-09-08-table-3": "60ee63f200a7", // + 표 본문 행 예외를 **자리로** (검토관 [상] · 옛 배치)
    // ★ table-4의 지문은 2026-09-10에 **두 번 갱신했다 — 판은 안 올렸다.**
    //   ① tabletext.ts 이사: 술어를 한 글자도 안 고치고 파일만 옮겼다(자를 결과 불변).
    //   ② 구분선을 GFM 꽉 꼴(`|-|-|`)까지 넓혔다(검토관 적발 ①③).
    //   근거는 둘 다 memory.ts의 「자를 결과가 안 바뀌는 수정에는 올리지 않는다」이고, ②는
    //   **말이 아니라 두 가지로 잰다**: ⑴ 홑대시 구분선이 매니페스트 35편·지식 24편에 **0줄**
    //   (그 꼴이 없으니 예외 판정이 달라질 줄 자체가 없다) ⑵ 위 코퍼스 시험이 옮기기 앞뒤로
    //   같은 값을 낸다(지식 24편 · 조각 1,226 · 고아 0 · 머리글만 든 표 0).
    //   ⚠ 다음 사람에게: 코퍼스에 `|-|-|`이 들어오는 문서가 생기면 이 근거 ⑴이 무너진다.
    //     그때는 조각 수가 바뀌는지 다시 재고, 바뀌면 판을 올려라(재임베딩이 필요해진다).
    "2026-09-08-table-4": "1613802d6cc0", // + 열 수로 좁힘 (검토관 2차) · 2026-09-10 이사 + 구분선 GFM 꽉 꼴
  };
  ...
  it("★★ chunkText·cleanExtractedText·표 술어의 지문이 지금 판과 짝이다", async () => {
    ...
    const 알맹이들: string[] = [];
    for (const [파일, 시작표식, 끝표식] of 구간들) {
      const src = fs.readFileSync(path.resolve(파일), "utf8");
      const 시작 = src.indexOf(시작표식), 끝 = src.indexOf(끝표식);
      expect(시작, `${파일}: 표식이 사라졌다 — 감시가 헛돈다(지문을 빈 글자로 세고 있었다)`).toBeGreaterThan(0);
      expect(끝, `${파일}: 끝 표식이 사라졌다 — 감시 범위가 파일 끝까지 늘어난다`).toBeGreaterThan(시작);
      // 주석·이름·들여쓰기는 뺀다 — 「자를 결과가 안 바뀌는 수정에는 판을 올리지 않는다」는
      // 코드 주석과 어긋나지 않게, **알맹이**만 센다.
      알맹이들.push(src.slice(시작, 끝)
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/(^|[^:/])\/\/[^\n]*/g, "$1")
        .replace(/\s+/g, " ").trim());
    }
    const 지문 = createHash("sha256").update(알맹이들.join(" "), "utf8").digest("hex").slice(0, 12);
