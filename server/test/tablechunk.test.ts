...
describe("청커 판 — 규칙을 고치면 CHUNKER_VERSION을 올린다 (소스 감시)", () => {
  ...
  const 청커지문: Record<string, string> = {
    ...
    // ★ table-4의 지문은 2026-09-10에 **갱신했다 — 판은 안 올렸다.**
    //   근거: 술어를 한 글자도 안 고치고 파일만 옮겼다(자를 결과 불변). memory.ts:「자를 결과가
    //   안 바뀌는 수정에는 올리지 않는다」가 이 경우다. 불변임을 말이 아니라 **위 코퍼스 시험**이
    //   잰다(지식 24편 · 조각 1,226 · 고아 0 · 머리글만 든 표 0 — 옮기기 앞뒤로 같아야 한다).
    "2026-09-08-table-4": "0b915fd9c2a8", // + 열 수로 좁힘 (검토관 2차) · 2026-09-10 tabletext 이사 반영
  };
