...
describe("★ 목록 끝에 다음 걸음 한 줄 (2026-08-01 하루 실전)", () => {
  ...
  it("★ 아무 답에나 붙이지 않는다 — 네 곳뿐이다", () => {
    ...
    //   approval_status(결재·승인 대기 현황, 2026-09-11 — **조건부 한 자리**).
    //   앞 셋은 늘 붙고, approval_status는 **대기가 하나라도 있을 때만** 붙는다:
    //   그때는 각각이 가는 화면이 결정적이지만(취약점 결재=③ 조치 › ✅ 조치·승인 판,
    //   점검 승인=③ 조치 › 🛠 정기 점검 판), 0건이면 갈 화면이 아예 없어 「다음 걸음」에
    //   되풀이만 적히게 된다 — 그게 아래 「안 붙인 곳」이 말하는 추측이다(2026-09-11 검토관 [하]).
    //   0건 갈래는 마침말을 낸다(짝 시험: approvalstatustool.test.ts).
    ...
    // 2026-08-03: exposed_assets(인터넷 노출 자산)를 새로 만들며 여기 붙였다가 이 시험이 잡았다.
    //   현황 조회인데 붙였고, 게다가 「다음 걸음」에 챗봇 질문을 추천하는 옛 함정까지 되풀이했다.
    //   등록부가 비었다는 안내는 그 답의 ⚠ 정직 문구 안으로 합쳤다 — 자리가 늘지 않았다.
