...
// ─────────────────────────────────────────────────────────────────────────────
// ★★ 반대 방향 — **사내 문답이 정답인 물음**에서 근거가 줄지 않는가 (2026-09-07 검토관 적발)
//
// 지적: 가족 몫은 ⑰ 물음만이 아니라 **전 물음**에 걸린다. 사내 규정·절차를 되묻는 자리
//   (승인 문답이 곧 정답인 자리)에서 top-4의 관련 근거가 4→1로 줄고, RRF가 더 낮게 매긴
//   비가족 조각이 그 자리를 메운다. 「사내 문답이 법을 이긴다」를 고치면서
//   「일반 지식이 사내 문답을 이긴다」를 만든 것 아니냐 — 그리고 **섞인 후보를 무는 시험이
//   하나도 없었다**(위 ④는 후보가 전부 문답인 경우만 문다).
//
// 답: 몫 자체는 유지한다(승인 문답은 문답 한 쌍이 통째로 한 조각이라 **한 자리에 답이 다
...
describe("★★ 버려질 조각으로는 가족을 못 밀어낸다 — 고치려다 근거를 줄이지 않는다", () => {
  const 구성 = (비가족거리: number): 점수조각[] => [
    ...Array.from({ length: 5 }, (_, i) => ({ documentId: approvedQaDocId(`rel${i}`), text: `승인 답 ${i}`, distance: 0.42 })),
    ...Array.from({ length: 20 }, (_, i) => ({ documentId: `잡음${i}.md`, text: `잡음 ${i}`, distance: 비가족거리 })),
  ];

  it("★ 무관 조각(0.99 > 게이트 0.95)은 가족 자리를 못 뺏는다 — 뺏으면 근거가 **줄기만** 한다", () => {
    const 결과 = 문서를섞어자르기(구성(0.99), 4);
    expect(결과.length, "개수 계약").toBe(4);
    expect(
      가족수(결과),
      "관련성 게이트가 뒤에서 버릴 조각이 살아 있을 근거를 밀어냈다",
    ).toBe(4);
  });

  it("★★ 관련 있는 비가족은 **여전히** 자리를 가져간다 — 감수하는 대목을 숨기지 않는다", () => {
    // 이것이 이 수리가 치르는 값이다: 사내 문답이 정답인 물음에서도 top-4의 문답은 1건이다.
    // 대신 그 1건에 승인된 문답 **한 쌍이 통째로** 들어 있다(문서당 조각 1.006).
    const 결과 = 문서를섞어자르기(구성(0.5), 4);
    expect(결과.length).toBe(4);
    expect(가족수(결과)).toBe(1);
  });

  it("어휘로 걸린 조각(거리 무한·lexicalHit)은 무관이 아니다 — CVE·U-01 코드 매치를 뒤로 밀지 않는다", () => {
    const 후보: 점수조각[] = [
      ...Array.from({ length: 5 }, (_, i) => ({ documentId: approvedQaDocId(`lx${i}`), text: `답 ${i}`, distance: 0.42 })),
      ...Array.from({ length: 5 }, (_, i) => ({ documentId: `코드${i}.md`, text: `CVE-2026-000${i}`, distance: Number.POSITIVE_INFINITY, lexicalHit: true })),
    ];
    const 결과 = 문서를섞어자르기(후보, 4);
    expect(가족수(결과), "글자 그대로 걸린 조각이 뒤로 밀렸다").toBe(1);
  });

  it("★ **가족 경쟁이 없으면 아무 일도 안 일어난다** — 실측 없이 전 질의의 자리 배분을 흔들지 않는다", () => {
    // 승인 문답이 후보에 하나도 없으면 이 줄은 원리상 발화하지 않는다(미룸가족이 빈 채로 남는다).
    const 후보: 점수조각[] = Array.from({ length: 10 }, (_, i) => ({
      documentId: `문서${i}.md`, text: `조각 ${i}`, distance: i < 4 ? 0.99 : 0.4,
    }));
    const 결과 = 문서를섞어자르기(후보, 4);
    expect(결과.map((c) => c.documentId), "가족이 없는데 순서가 바뀌었다").toEqual(
      ["문서0.md", "문서1.md", "문서2.md", "문서3.md"],
    );
  });
