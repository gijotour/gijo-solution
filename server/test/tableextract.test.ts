...
//      ⚠ 2026-09-10에 **pptx만 이 계약에서 빠졌다**(갈래 P: 슬라이드 경계·도해/차트·노트 표시).
//        지우지 않고 아래 「pptx 새 계약」 절로 옮겨, **낱말 다중집합이 앞뒤로 같다**로 다시 묶었다 —
//        계약을 갈 때 「무엇이 달라졌는가」를 시험에 남기지 않으면 다음 사람이 그 변경을 회귀로 읽는다.
...
describe("pptx 표 — DrawingML <a:tbl>을 파이프 표로 낸다", () => {
  it("표가 슬라이드 자리에 파이프 표로 나오고, 표 없는 슬라이드는 종전 그대로다", async () => {
    ...
    // ★ 2026-09-10 — 앞에 「[슬라이드 N]」 한 줄이 붙었다(갈래 P). **표 갈래는 그대로다** —
    //   아래 표 줄·열 수·병합 뒤칸 시험이 종전 값 그대로인 것이 그 증거다.
    expect(t).toBe([
      "[슬라이드 1]",
      "보안 점검 요약",
      "",
...
describe("회귀 — 표가 없는 오피스 문서는 종전 출력 그대로다", () => {
  ...
  // ⚠ **pptx는 이 골든에서 빠졌다** — 2026-09-10에 계약이 바뀌었기 때문이다(바로 아래 절).
  //   빼면서 그냥 지우지 않고 **무엇이 달라졌는지를 시험으로 남긴다.**
  it("pptx가 이 절에 남아 있지 않다 — 계약이 갈린 것을 알고 뺐다는 표시", () => {
    expect(Object.keys(골든), "pptx 골든이 두 곳에 생겼다 — 어긋나기 시작한다").not.toContain("tiny.pptx");
  });
});

// ★ pptx **새 계약** (2026-09-10, 갈래 P) — 「글자 하나까지 같다」를 갈아 끼운 자리.
//
// ■ 왜 갈았나: 2026-09-08에 파이썬 pptx 갈래를 지우며 못 옮긴 약속 3종(「[슬라이드 N]」 경계 ·
//   SmartArt/차트 회수 · 「(노트)」 표시)을 옮기려면 pptx 출력이 반드시 바뀐다. 그 셋은 진짜
//   공백이었다 — 특히 노트가 **전부 문서 끝에** 몰려 어느 장의 메모인지 알 수 없었다.
// ■ 갈면서 무엇을 지켰나: **글자를 지어내지도, 잃지도 않는다.** 아래 두 시험이 그것을 잰다 —
//   ① 새 골든(자리까지) ② 표시를 걷어내면 **낱말 다중집합이 옛 골든과 똑같다**.
//   즉 달라진 것은 「표시 2종 + 노트가 제 슬라이드로 옮겨간 자리」뿐임을 시험이 증명한다.
describe("pptx 새 계약 — 표시가 붙되 낱말은 하나도 안 늘고 안 준다", () => {
  const 옛_골든 = "보안 교육 자료 1장 개요\n\n2장 대응 절차\n\n발표자 메모입니다";
  const 새_골든 = "[슬라이드 1]\n보안 교육 자료 1장 개요\n\n(노트) 발표자 메모입니다\n\n[슬라이드 2]\n2장 대응 절차";

  it("tiny.pptx — 새 골든과 글자 단위로 같다", async () => {
    expect(await extractDocumentText("tiny.pptx", b64("tiny.pptx"))).toBe(새_골든);
  });

  it("표시를 걷어내면 옛 출력과 낱말이 똑같다 — 손실 0·창작 0", async () => {
    const t = await extractDocumentText("tiny.pptx", b64("tiny.pptx"));
    // 이번에 **더한 것은 이 둘뿐**이다: 줄 하나짜리 「[슬라이드 N]」 표시와 「(노트) 」 접두.
    const 걷기 = (s: string) => s.split("\n").filter((l) => !/^\[슬라이드 \d+\]$/.test(l))
      .map((l) => l.replace(/^\(노트\) /, "")).join("\n");
    const 낱말 = (s: string) => s.split(/\s+/).filter(Boolean).sort();
    expect(낱말(걷기(t)), "표시 말고 다른 것이 바뀌었다 — 글자가 늘었거나 사라졌다").toEqual(낱말(옛_골든));
    // 자리는 **일부러** 바뀌었다: 노트가 제 슬라이드(1장) 뒤로 옮겨 갔다. 그 사실을 못박는다.
    expect(걷기(t)).not.toBe(옛_골든);
    expect(t.indexOf("발표자 메모입니다"), "노트가 아직 문서 끝에 몰려 있다").toBeLessThan(t.indexOf("2장 대응 절차"));
  });
});
