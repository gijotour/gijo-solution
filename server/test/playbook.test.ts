...
describe("★ 출처를 묻는 말에는 표를 내밀지 않는다 (2026-08-13)", () => {
  const 비켜야 = [
    ...
    // ★ 2026-08-13 QA 회귀(kev-deadline)에서 잡힘 — 의문사가 명사 **앞**에 오는 꼴.
    //   「근거 지침이 뭐야」는 잡는데 「어떤 지침에 근거하고」는 뒤-의문사 자로 못 쟀다.
    "KEV에 등재된 취약점 조치 기한은 어떤 지침에 근거하고 며칠이야?",  // ← 회귀 문항 그대로
    "이거 어떤 규정에 근거해서 막는 거야?",
  ];
  for (const 말 of 비켜야) {
