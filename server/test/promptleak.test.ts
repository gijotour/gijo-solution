...
// ── 청크 번호 나열 누출 (2026-07-30) ─────────────────────────────────────────
// 실측(QA 로그): ASA 로그 질문의 답변 끝에 "참고 자료: #1, #2, … #241"이 붙어 나갔다.
// 본문은 정확했는데 뒤가 번호로 가득 찼다 — 우리가 청크에 매긴 내부 번호를 모델이
// "출처 표기"인 줄 알고 옮겨 적은 것이다. 담당자에게 그건 답이 아니라 잡음이다.
describe("청크 번호 나열 제거", () => {
  it("답변 뒤에 붙은 '참고 자료: #1, #2, #3…'을 잘라낸다", () => {
    const out = stripScaffoldEcho(
      "ASA 106023은 ACL에 의해 패킷이 차단된 것을 의미합니다.\n\n참고 자료: #1, #2, #3, #6, #15, #16"
    );
    expect(out).toContain("ACL에 의해");
    expect(out).not.toContain("#1");
  });

  it("본문은 그대로 남는다(답을 통째로 버리지 않는다)", () => {
    const body = "ASA 106023은 ACL 차단입니다. 반복되면 스캔 시도를 의심합니다.";
    expect(stripScaffoldEcho(body + "\n참조: #4, #5, #9, #12")).toBe(body);
  });

  // ⚠ 정상 문장을 자르면 안 된다 — 번호가 하나둘 있는 것은 목록·표기일 수 있다.
  it("번호가 두 개 이하면 건드리지 않는다", () => {
    for (const s of [
      "관련 항목은 #1 과 #2 입니다.",
      "참고 자료: 장비 공통 7항목 중 방화벽 정기 점검 절차가 있습니다.",
      "1. 정책 백업 2. 로그 확인 3. 담당자 승인",
    ]) {
      expect(stripScaffoldEcho(s), s).toBe(s);
    }
  });
});
