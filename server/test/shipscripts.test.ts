...
// ─────────────────────────────────────────────────────────────────────────────
// ★★ 실전 답 기록은 **원격 기계로 나가지 않는다** (2026-09-08 신설)
//
// ■ 무엇을 지키나
//   `.tmp-reports/ops-sim.json`에는 2026-09-08부터 **가드가 본 근거 조각 본문**이 실린다.
//   야간 하네스가 도는 계정 claude-deploy는 admin이라, 등급 C 사내 문서(운영 1,598건)의
//   조각까지 그대로 담긴다. 즉 이 파일은 그날부터 **시험 재료가 아니라 사내 문서 묶음**이다.
//   그래서 tools/gb10-test.sh(사본을 **원격 기계 gb10**에 만든다)에서 이 파일 복사를 뺐다.
//   tools/wsl-test.sh(같은 기계의 WSL)는 경계를 안 넘으므로 **그대로 둔다.**
//
// ■ 왜 시험으로 매다나 — 지운 것은 **되돌아온다**
//   이 저장소가 반복해 겪은 꼴이다: 누군가 「gb10에서 말투 감시가 자꾸 건너뛴다」를 보고
//   두 줄을 되살리면, 그날부터 사내 문서 본문이 다시 원격으로 나간다. 그 되살림은
//   **diff 두 줄**이라 검토관도 놓치기 쉽다. 그래서 코드가 스스로 막게 한다.
//
// ■ 어떻게 보나 — **주석은 빼고 실행되는 줄만** 본다
//   막은 이유를 적은 머리말에는 "ops-sim.json"이 여러 번 나온다. 파일 전체에서 찾으면
//   **이유를 적었다는 이유로 빨강**이 난다(설명을 지우게 만드는 시험은 없느니만 못하다).
describe("★ 실전 답 기록 — gb10(원격)으로는 복사하지 않는다", () => {
  const 도구 = path.join(서버루트, "..", "tools");

  /** 전체 줄에서 **주석만인 줄**을 뺀다 — 실제로 실행되는 줄만 남긴다. */
  const 실행되는줄 = (경로: string): string => {
    expect(fs.existsSync(경로), `${경로}를 못 찾았다 — 감시가 헛돈다`).toBe(true);
    return fs
      .readFileSync(경로, "utf8")
      .split("\n")
      .filter((l) => !/^\s*#/.test(l))
      .join("\n");
  };
  ...
  it("★ 짝: wsl-test.sh는 **그대로 옮긴다** — 둘 다 지우면 그 시험들이 영영 안 돈다", () => {
    // 위 시험만 있으면 다음 사람이 「그럼 WSL도 위험한가」 하며 함께 지울 수 있다.
    // 그러면 말투 감시·프롬프트 누출 감시가 **어디서도 안 돌고** 조용히 초록이 된다.
    // WSL은 같은 기계다 — 경계를 넘지 않으므로 여기서 재는 것이 정답이다.
    const 코드 = 실행되는줄(path.join(도구, "wsl-test.sh"));
    expect(코드, "wsl-test.sh가 ops-sim.json을 안 옮긴다 — 실전 답을 재는 시험이 어디서도 안 돈다")
      .toMatch(/cp\s+"\$SRC_ROOT\/\.tmp-reports\/ops-sim\.json"/);
    expect(코드, "wsl-test.sh가 짝인 메타를 안 옮긴다 — 「완주본인가」 잣대가 하한선으로 헐거워진다")
      .toMatch(/ops-sim\.meta\.json/);
  });

  it("왜 뺐는지가 gb10-test.sh에 적혀 있다 — 이유 없는 빈자리는 곧 메워진다", () => {
    const 전체 = fs.readFileSync(path.join(도구, "gb10-test.sh"), "utf8");
    expect(전체, "ops-sim을 왜 안 옮기는지가 안 적혀 있다 — 다음 사람은 그냥 빠뜨린 줄 알고 되살린다")
      .toContain("ops-sim.json");
    expect(전체, "「조각 본문이 실린다」는 사실이 안 적혀 있다 — 그게 이 결정의 전부다")
      .toMatch(/조각 본문/);
  });

  it("★ gb10에서 건너뛰는 것이 **초록으로 안 읽히게** 사유가 찍힌다", () => {
    // 재료가 없으면 citeguard의 재생 시험은 건너뛴다. 그 건너뜀이 조용하면
    // 「gb10 전체 초록」이 실제로는 안 잰 것을 잰 것처럼 보이게 한다(이 저장소의 「헛도는 시험」).
    const 시험 = fs.readFileSync(path.join(서버루트, "test", "citeguard.test.ts"), "utf8");
    expect(시험, "citeguard가 건너뛴 사유를 안 찍는다 — 삼켜진 건너뜀은 없는 시험과 같다")
      .toMatch(/건너뜁니다/);
    expect(시험, "건너뜀 사유에 「gb10은 일부러 안 옮긴다」가 없다 — 사람이 고장으로 오해한다")
