...
describe("스마트 통합 업로드 — 파일 유형 자동 판별·라우팅", () => {
  ...
  describe("원본 보관 토글", () => {
    ...
    it("토글을 껐다면 예전에 남긴 원본도 치운다 (cleanupOldOriginal 계약)", async () => {
      // 검토관 2026-08-22 — 같은 이름으로 다시 올릴 때 토글을 꺼도 옛 원본이 남아,
      // 화면은 「원본은 서버에 남지 않았습니다」라고 말했다(프라이버시 기본값과 정반대).
      // ⚠ 여기서 **함수를 직접 부른다**. 라우트로 재면 시험 환경에 임베딩이 없어 수집이 실패하고,
      //   그러면 정리를 **일부러 안 한다**(아래 시험이 그 계약을 지킨다) — 라우트로는 이 동작을
      //   못 재는데 그걸 모르고 시험을 짜면 「고쳤다」의 근거가 헛것이 된다.
      const { cleanupOldOriginal } = await import("../src/engine/memory");
      const 이름 = "원본끄기_되돌림_시험.txt";
      const p = 원본(이름);
      fs.mkdirSync(path.dirname(p), { recursive: true });
      fs.writeFileSync(p, "예전에 보관해 둔 원본");
      expect(fs.existsSync(p), "먼저 원본이 있어야 이 시험이 뜻이 있다").toBe(true);
      await cleanupOldOriginal(이름);
      expect(fs.existsSync(p), "토글을 껐는데 옛 원본이 그대로 남았다 — 화면 문구가 거짓이 된다").toBe(false);
    });

    it("수집이 실패하면 옛 원본을 건드리지 않는다 — 살아 있는 문서의 원본을 잃지 않게", async () => {
      // 재검토관 2026-08-22 — 정리를 저장과 함께 하면, 수집이 실패했을 때 **옛 문서는 화면·검색에
      // 그대로 살아 있는데 그 원본만 사라진다**. 그래서 정리를 수집 성공 뒤로 옮겼고, 이 시험이
      // 그 순서를 지킨다(시험 환경엔 임베딩이 없어 수집 실패 경로가 그대로 재현된다).
      const 이름 = "수집실패_옛원본_보호_시험.txt";
      const p = 원본(이름);
      지우기(추출본(이름));
      fs.mkdirSync(path.dirname(p), { recursive: true });
      fs.writeFileSync(p, "살아 있는 옛 문서의 원본");
      const res = await request(app).post("/api/upload/auto").set("Authorization", `Bearer ${token}`)
        .send({ filename: 이름, content: b64("보관 끄고 다시 올림 — 그러나 수집은 실패한다"), forceType: "document" });
      expect(res.status).toBe(200);
      expect(res.body.ingested, "이 시험은 수집 실패 상태를 전제로 한다").toBe(false);
      expect(fs.existsSync(p), "수집이 실패했는데 옛 원본을 지웠다 — 살아 있는 문서가 원본을 잃는다").toBe(true);
      지우기(p);
    });
