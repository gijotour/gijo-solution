...
describe("★ 신고율 — 뜻이 깔리고 방향이 함께 실린다", () => {
  ...
  it("★★ 뜻이 지식 문서의 낱말과 **겹친다**(방향 주장은 아래 4절 표 시험이 문다)", () => {
...
// ─────────────────────────────────────────────────────────────────────────────
// ★★ 뜻 줄의 **방향 주장**을 지식 문서 4절 표에 못 박는다 (2026-09-07 검토관 적발)
//
// ■ 무엇이 새어 나갔나
//   바로 위 시험은 조각 두 개(「훈련 메일을 신고 창구로 알린」·「높을수록」)의 **존재**만 봤다.
//   그래서 그 조각들 사이에 문서가 **명시로 부정하는 문장**이 끼어도 초록이었다 — 실제로
//   「모의훈련 지표 중 유일하게 높을수록 좋다」가 실렸는데, 문서는 「**피싱** 모의훈련 지표
//   중에서는 유일하게 … 교육 쪽 지표(응답률·이수율·참여율)도 높을수록 좋으니 「높을수록 좋은
//   지표는 신고율뿐」이 아니다」라 못 박는다. 시험 이름이 「글자로 일치」라 다음 사람은 대조가
//   끝났다고 읽는다 — **거짓 안심**이 결함보다 오래 간다.
//
// ■ 더 나쁜 자리 — **같은 프롬프트 안에서 부딪친다**
//   DOMAIN_SENSE_MAX=2라 「신고율과 참여율 중 뭘 봐야 해?」에는 두 줄이 함께 깔리는데,
//   참여율 줄은 「보안 교육·**모의훈련** 대상자 중 실제로 참여한 비율」이다. 모델은 한 블록에서
//   「모의훈련 지표 중 신고율만 높을수록 좋다」와 「참여율도 모의훈련 지표다」를 동시에 받는다.
//
// ■ 그래서 무엇을 무는가 — **문서 4절 표가 정본**이고 뜻 줄은 그것을 어길 수 없다
//   ⓐ 방향 낱말(높을수록·낮을수록)이 표의 화살표와 같다.
//   ⓑ 어느 줄도 **배타 주장**(유일·…뿐)을 못 한다 — 표에 ↑가 여럿이라 원리상 거짓이고,
//      두 줄이 함께 깔리는 구조에서는 이웃 줄과 정면으로 부딪친다.
//   ⓒ 어느 줄도 **남의 지표의 방향**을 규정하지 못한다(자기 열쇠의 방향만 말한다).
//   셋 다 표를 **문서에서 떼어 읽어** 잰다 — 여기 베껴 적으면 「제 사본이 제 사본과 같다」가 된다.
describe("★★ 뜻 줄의 방향 주장이 지식 문서 4절 표를 어기지 않는다", () => {
  const 문서 = fs.readFileSync(
    path.join(__dirname, "../../knowledge/GIJO_지식_보안인식교육.md"), "utf8");
  /** 4절 표 → {지표: "↑"|"↓"} */
  const 표 = new Map<string, string>();
  for (const m of 문서.matchAll(/^\|\s*\*\*([^*|]+)\*\*[^|]*\|[^|]*\|[^|]*\|\s*(↑|↓)/gm)) {
    표.set(m[1].trim(), m[2]);
  }
  const 오름 = [...표.entries()].filter(([, v]) => v === "↑").map(([k]) => k);

  it("표를 실제로 읽어 왔다 — 못 읽으면 아래 셋이 통째로 헛돈다", () => {
    console.log(`[방향표] 4절 표 ${표.size}개 · ↑ ${오름.length}개(${오름.join("·")})`);
    expect(표.size, "지식 문서 4절 표를 못 읽었다 — 표 모양이 바뀌었으면 이 시험도 함께 본다")
      .toBeGreaterThanOrEqual(7);
    expect(표.get("신고율"), "표에서 신고율의 방향을 못 찾았다").toBe("↑");
    expect(표.get("클릭률"), "표에서 클릭률의 방향을 못 찾았다").toBe("↓");
  });

  it("ⓐ 방향 낱말이 표의 화살표와 같다 — 뜻 줄이 문서를 뒤집지 못한다", () => {
    for (const [낱말, 뜻] of Object.entries(DOMAIN_SENSE)) {
      const 방향 = 표.get(낱말);
      if (!방향) continue;                       // 표에 없는 낱말은 잴 잣대가 없다
      const 어긋난말 = 방향 === "↑" ? "낮을수록" : "높을수록";
      expect(뜻, `「${낱말}」 뜻 줄에 「${어긋난말}」이 있는데 문서 표는 ${방향}다 ` +
        "(남의 지표를 가리킨 말이라면 아래 ⓒ가 그 자리를 짚어 준다 — 어느 쪽이든 줄을 고친다)")
        .not.toContain(어긋난말);
    }
  });

  it("ⓑ 어느 뜻 줄도 배타 주장(유일·…뿐)을 하지 않는다 — 표의 ↑가 여럿이다", () => {
    expect(오름.length, "표에 ↑가 하나뿐이면 이 시험의 전제가 바뀐다 — 문서를 다시 볼 것")
      .toBeGreaterThan(1);
    for (const [낱말, 뜻] of Object.entries(DOMAIN_SENSE)) {
      expect(뜻, `「${낱말}」 뜻 줄이 배타 주장을 한다 — 문서 표의 ↑는 ${오름.join("·")} ${오름.length}개이고, ` +
        "이 표는 한 번에 두 줄이 함께 깔린다(이웃 줄과 부딪친다)")
        .not.toMatch(/유일|좋은\s*지표는[^.]{0,20}뿐/);
    }
  });

  it("ⓒ 어느 뜻 줄도 **남의 지표**의 방향을 규정하지 않는다", () => {
    // 자기 열쇠의 방향만 말한다. 「클릭률과 반대 방향이라 함께 본다」처럼 **방향을 안 박는**
    // 짝 언급은 그대로 둔다 — 무는 것은 남의 이름 **바로 뒤**에 붙는 방향 낱말이다.
    for (const [낱말, 뜻] of Object.entries(DOMAIN_SENSE)) {
      for (const 남 of 표.keys()) {
        if (남 === 낱말) continue;
        for (let i = 뜻.indexOf(남); i >= 0; i = 뜻.indexOf(남, i + 1)) {
          expect(뜻.slice(i + 남.length, i + 남.length + 16),
            `「${낱말}」 뜻 줄이 남의 지표 「${남}」의 방향까지 적는다 — 문서 표가 정본이다`)
            .not.toMatch(/높을수록|낮을수록/);
        }
      }
    }
  });
