...
describe("이렇게 쓰면 됩니다 (문서함 매뉴얼)", () => {
  ...
  it("화면 안내(screenguide)가 새 자리(내 문서 📘 GIJO AS 안내)로 안내한다 — 없는 카드를 누르라고 하지 않는다", () => {
    ...
    // 2026-08-21 v4에서 탭줄이 왼쪽 칸이 됐다 — 잣대는 「새 자리를 가리키는가」이지
    // 「탭이라는 낱말이 있는가」가 아니다. 낱말이 아니라 **자리**를 지킨다.
    // ⚠ 2026-08-22 「제품 안내」→「GIJO AS 안내」로 이름이 바뀌었다(같은 화면의 📦 보안제품 자료와 가르려고).
    expect(본문).toContain("내 문서 → 📘 GIJO AS 안내");
