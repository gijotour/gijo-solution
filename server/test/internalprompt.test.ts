...
describe("내부 프롬프트는 게이트를 다시 지나지 않는다", () => {
  ...
  it("LLM에 직접 붙는 fetch는 검토된 곳만 있다", () => {
    const 검토됨: Record<string, string> = {
      ...
      // 2026-08-14 검토(원격 GPU 내주기 신설). ⚠ 여기는 **다른 부류**라 숨기지 않고 적는다:
      //   이 창구는 **남의 요청을 중계**한다 — 사용자 입력이 실제로 흐른다. 그런데도 관문을
      //   다시 태우지 않는 이유:
      //    ① 오는 것은 OpenAI 호환 요청이라 **시스템 프롬프트·RAG가 이미 합쳐진 완성본**이다.
      //       여기에 gateUserInput을 걸면 정상 요청이 걸린다(RAG 조각·페르소나 문구가 그대로 실려 온다).
      //    ② 그 입력은 **붙는 쪽 GIJO의 관문을 이미 지났다** — 저쪽 chat()이 보낸 것이다.
      //   ⚠ 다만 우리는 상대가 GIJO인지 **확인하지 않는다.** 그 자리를 대신하는 것이 경계 셋이다:
      //     기본 꺼짐 · VPN·사설 대역에서 온 요청만 · 에어갭이면 차단(llmserve.test.ts가 못박는다).
      //   자격증명이 붙는 날, 여기가 그것을 검사할 자리다.
      "llmserve.ts": "원격 GPU 내주기 — 중계 창구(입력 흐름). 관문 대신 VPN 대역·기본 꺼짐·에어갭 차단으로 막는다",
    };
    const 발견: string[] = [];
    for (const f of fs.readdirSync(engineDir).filter((x) => x.endsWith(".ts"))) {
      const src = fs.readFileSync(path.join(engineDir, f), "utf8");
