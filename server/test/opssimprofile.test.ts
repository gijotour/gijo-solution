...
describe("★ 건너뜀사유 — 프로필별 건너뜀 수가 기대와 같다", () => {
  ...
  it("qa4100 + 법령 켜짐 — 가리는 업무 문항만 건너뛴다(씨앗문서 여덟·가리지않음 둘은 계속 잰다 — B8·exec_brief 같은 결함을 안 가린다)", () => {
    for (const q of 가리는업무) expect(건너뜀사유(q, qa4100, 켜짐)).toBe("데이터:업무");
    for (const q of [...안가리는업무, ...씨앗목록, ...법령목록]) {
      expect(건너뜀사유(q, qa4100, 켜짐), `"${q}"가 부당하게 건너뛰어졌다`).toBeNull();
    }
    expect(가리는업무.length, "업무 표식 셋 중 가리는 것은 「10.0.0.100…」 하나다").toBe(1);
  });

  it("qa4100 + 법령 꺼짐 — 가리는 업무 1 + 법령 5 = 6건 건너뜀, 씨앗문서 8·가리지않음 2는 그대로 잰다", () => {
    const 건너뜀개수 = [...업무목록, ...씨앗목록, ...법령목록].filter((q) => 건너뜀사유(q, qa4100, 꺼짐)).length;
    expect(건너뜀개수).toBe(6);
    for (const q of [...씨앗목록, ...안가리는업무]) expect(건너뜀사유(q, qa4100, 꺼짐)).toBeNull();
  });
