...
describe("★ 건너뜀사유 — 프로필별 건너뜀 수가 기대와 같다", () => {
  ...
  // ★★ 2026-09-13 검토관 [중] 수리 — ⑤ 이후 남은 업무 표식 둘이 **둘 다 `가리지않음`**이라
  //   건너뜀사유()의 `if (의존 === "데이터:업무" && !프로필.업무데이터) return "데이터:업무";`
  //   줄이 **어떤 문항에서도 도달 불가**가 됐고, 그 갈래를 재던 유일한 양성 단언도 함께 사라졌다
  //   (`expect(가리는업무.length).toBe(0)`로 바뀌었다). 프로필의 `업무데이터` 칸이 선언만 남고
  //   효과가 0인데, 그 갈래가 깨져도(예: 조건 오타) 빨간불이 안 나는 상태였다 — 「건너뜀」 기제
  //   전체가 설정:법령 하나로만 검증되고 있었다. 아래 세 줄로 그 갈래를 **가짜 표식 주입**으로
  //   되살린다(같은 파일의 설정 의존 시험과 같은 형식).
  it("★ 업무데이터 칸은 여전히 효력이 있다 — 가리는 업무 표식이 생기면 qa4100에서 건너뛴다(가짜 표식)", () => {
    const 표적 = "우리 자산 몇 대야?"; // 표식이 없는 평범한 문항을 잠깐 빌린다
    const 가짜표 = { 의존: ["데이터:업무"], 왜: "시험용 — 업무데이터 칸의 효력을 재는 자리", };
    const 원래 = (표식표 as any)[표적];
    (표식표 as any)[표적] = 가짜표;
    try {
      expect(건너뜀사유(표적, qa4100, 켜짐), "업무데이터 없는 프로필에서는 건너뛴다").toBe("데이터:업무");
      expect(건너뜀사유(표적, { 이름: "업무있음", 업무데이터: true, 씨앗문서: true } as any, 켜짐),
        "업무데이터가 있는 프로필에서는 안 건너뛴다").toBeNull();
      expect(건너뜀사유(표적, null, 켜짐), "프로필이 없으면(기본 4000) 절대 안 거른다").toBeNull();
    } finally {
      if (원래 === undefined) delete (표식표 as any)[표적];
      else (표식표 as any)[표적] = 원래;
    }
  });
