...
describe("★ 배선 — 창구가 실제로 열려 있고, 열린 중계기가 아니다 (소스 감시)", () => {
  ...
  it("★ 관문은 **대역 판정을 맨 앞에서** 한다 — 밖에서 훑는 쪽에 제품 정보를 주지 않는다", () => {
    ...
    // 프록시 안내는 **꺼짐 검사 뒤**다(L-2) — 기본값(꺼짐)에서 XFF 한 줄로 안내 문구를 얻지 못하게.
    // 이 순서는 이미 세 번 회귀한 자리라(A-5·A-6·H-4) 시험으로 못박는다(관문 검토 지적).
    const 프록시 = 본문.indexOf("proxyDetected");
    expect(프록시, "관문에 프록시 검사가 없다").toBeGreaterThan(-1);
    expect(꺼짐, "프록시 안내가 꺼짐 검사보다 먼저다 — 꺼진 기본값에서 헤더 한 줄로 안내를 얻는다").toBeLessThan(프록시);
  });
