...
function makeBackup(ageHours: number) {
  const dir = path.join(tmp, "backups");
  fs.mkdirSync(dir, { recursive: true });
  const f = path.join(dir, `snap-${ageHours}.sqlite`);
...
describe("자가 진단", () => {
  ...
  it("백업이 이틀 넘게 안 돌면 경고, 사흘 넘으면 실패 — 자동 백업이 멈춘 것을 잡는다", () => {
    makeBackup(60);
  ...
  it("전체 판정은 가장 나쁜 항목을 따른다 — 문제 하나가 정상 넷에 묻히지 않는다", () => {
    makeBackup(2); // 백업은 정상
    const h = systemHealth();
  ...
  it("24시간 내 오류가 있으면 드러낸다 — 차단은 오류가 아니다", () => {
    makeBackup(2);
