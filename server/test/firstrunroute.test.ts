...
describe("F6-11 — 실시간 연결이 끊긴 것을 알린다", () => {
  it("★ 끊긴 시각이 재시도마다 밀리지 않는다 — 밀리면 문턱을 영영 못 센다", () => {
    ...
    expect(ws, "끊긴 시각을 재시도마다 새로 찍는다 — 문턱을 못 센다").toMatch(
      /끊긴시각:\s*상태\.붙은적있나\s*\?\s*\(상태\.끊긴시각\s*\?\?\s*Date\.now\(\)\)/
    );
  });

  it("★★ 「한 번도 안 붙음」을 끊김으로 세지 않는다 — 상시 거짓 경보 재발 방지", () => {
    // 2026-09-03 게시 전 검토가 잡았다. 이 모듈은 **프레임마다 따로** 뜨는데 최상위 셸은
    // 소켓을 여는 자리가 아니라 늘 「연결 안 됨」이다. 그것을 끊김으로 알리면 서버가 멀쩡한데도
    // 붉은 경보가 15초 뒤 떠서 **영영 안 사라진다**(open이 그 프레임에 안 오므로).
    // 그러면 진짜 끊겼을 때 아무도 그 표시를 안 믿는다 — 이 저장소가 금지한 가짜 UI다.
    expect(ws, "붙은 적이 있는지를 안 가린다").toContain("붙은적있나");
    expect(상단바, "칩이 「한 번도 안 붙음」에도 뜬다").toMatch(/!s\.붙은적있나/);
  });
