...
describe("팝업 릴레이 — 목록이 어긋나면 팝업에서만 조용히 죽는다", () => {
  ...
  it("★★ 화면이 셸로 보내는 상호작용 타입은 릴레이 목록에도 있다", () => {
    ...
    //   gijo:explain·gijo:ask — 안내·대필은 화면 ⓘ가 셸 대화창을 부르는 것으로,
    //     팝업에는 자기 chatwidget이 있어 릴레이가 필요 없다
    //   (gijo:prefill은 2026-08-30 제외에서 뺐다 — chatwidget에 prefill 수신이 0건이라
    //    그 사유가 거짓이었고, 팝업 대시보드 「적어 넣기」가 조용히 죽고 있었다. 이제 릴레이 목록에 있다.)
    //   gijo:scope:set — 방향이 반대다(셸→화면 배포용)
    const 제외 = new Set(["gijo:view", "gijo:explain", "gijo:ask", "gijo:scope:set",
      ...
      "gijo:pickdone",
      // hubStage — 허브가 판을 갈아 끼울 때 「지금 무대에 올린 실화면」을 셸에 알린다(F4-02, 2026-09-02).
      //   셸 수신부(app.html)는 **보낸 프레임의 주인 탭을 사슬로 찾아** 그 탭의 무대 맥락에만 도장을 찍는다.
      //   팝업은 그 탭 자체가 없어(창으로 빠져나온 것이다) 중계해도 주인을 못 찾고 그대로 버려진다 —
      //   죽는 경로가 아니라 **받을 자리가 없는 것**이다. 보내는 쪽도 try/catch로 「셸 밖이면 없던 일로」라 적었다.
      //   ⚠ 남은 틈(알고 둔다): 팝업 안 ⓘ는 자기 chatwidget을 부르는데, 그 chatwidget이 허브의 지금 판을
      //     아는 길은 아직 없다 — 팝업에서는 ⓘ가 허브 공통 안내를 낼 수 있다. 팝업 ⓘ 맥락은 별도 건이다.
      "gijo:hubStage",
