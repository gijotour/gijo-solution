...
describe("⑤ 「그거 어떻게 해」가 25초 걸려 23자 — 모델이 결국 똑같이 되물었다", () => {
  ...
  it("★★ 어느 쪽이든 모델을 타지 않는다 — 대상이 있으면 짚고, 없으면 되묻는다", () => {
    // 2026-08-07 검토관 발견으로 관문이 넓어졌다: 대상 안 밝힌 자산 질문(가리킨자산이없나)도
    // 같은 병(대상 불명)이라 같은 관문에서 잡는다. **호출 인자까지** 못 박는다 —
    // 함수를 만들어 놓고 배선 안 한 채 시험만 초록이던 사고가 바로 이 자리에서 났다.
    // 2026-08-09 2단계: 화면에서 고른 항목(선택)이 있으면 대상이 있으므로 되묻지 않는다 —
    // 관문 자체는 그대로 배선돼 있고, !선택 조건이 앞에 붙었을 뿐이다.
    // 꼬리 조건 허용 — 설명칩 가드(2026-08-21 B중2), 원형(두 판정 호출)은 그대로 지킨다.
    expect(disp).toMatch(/if \(!선택 && \(대명사뿐인가\(instructionText\) \|\| 가리킬것없는대명사\(instructionText, 대화열쇠\)( \|\| 설명칩인데대상없음)?\)\)/);
    // 되묻기 문구가 갈래졌다(2026-08-07): 자산을 가리킨 말에는 자산용 되묻기 — 「그거」라
    // 답하면 어색해서다. 즉답(확인 ?? …) 구조 자체가 살아 있는지를 본다.
    expect(disp, "대상이 있을 때의 즉답 경로").toMatch(/대명사확인\(대화열쇠\)[\s\S]{0,400}확인 \?\? 물음/);
  });
});
