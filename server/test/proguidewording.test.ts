...
describe("★ 자리를 못박은 다른 문장들도 프로에서 참이다", () => {
  it("문서함·문서 작성이 「사이드바」로 못박혀 있지 않다", () => {
    // 그 둘은 프로에서 **왼쪽 아래(레일)와 ☰ 전체 메뉴**에 있다 — 자리 이름이 다르다.
    // 실측(2026-08-19): `gijoScreenList()`에 문서함·문서 작성·팀 사무실이 다 들어 있어
    // ☰로 열 수 있다 — 기능이 사라진 것이 아니라 **자리만 다르다.**
    const 문서함 = 안내.slice(안내.indexOf('"docbox.html"'), 안내.indexOf('"docbox.html"') + 900);
    expect(문서함, "문서함 안내가 사이드바를 못박는다").not.toMatch(/사이드바 '📚 문서함/);
