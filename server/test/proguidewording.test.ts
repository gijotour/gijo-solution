...
describe("길찾기 안내 — 셸에 맞는 자리를 말한다", () => {
  ...
  // 0-5(2026-08-28): 프로에 **상설 왼쪽 메뉴 판**이 생겼다(셸 재구축 0-3) — 안내도 그리로.
  //   「☰ 전체 메뉴」는 이제 검색 길일 뿐 첫 안내가 아니다. 「사이드바」라는 낱말은 여전히
  //   금지 — 표준 셸 고유 명칭이라 프로 담당자에게는 다른 물건을 가리키는 말이 된다.
  it("★ 프로는 「왼쪽 메뉴」라고 말한다 — 상설 판이 생겼으니 그것이 첫 안내다", () => {
    const s = 화면위치안내("triage.html", "우선순위", true);
    expect(s, "프로인데 사이드바를 가리킨다 — 이름이 다르다").not.toContain("사이드바");
    expect(s, "프로에서 왼쪽 메뉴를 안 가리킨다").toContain("왼쪽 메뉴");
    expect(s, "접힘 안내(‖)가 없다 — 접어 둔 사람은 갈 수 없다").toContain("‖");
  });
  ...
  it("★★ **말이 되는 문장**이어야 한다 — 낱말만 갈아 끼우면 겹친다", () => {
    ...
    // 프로의 비-절차 안내는 **검색 길**까지 말한다(0-5: 레일은 상단바로 가서 「왼쪽 레일」은 거짓).
    expect(화면위치안내("settings.html", "설정", true)).toMatch(/검색은 ☰ 또는 Ctrl\+K/);
    expect(화면위치안내("settings.html", "설정", true), "「왼쪽 레일」이 되살아났다 — 0-2에서 상단바로 갔다")
      .not.toContain("왼쪽 레일");
  });
