...
describe("★ 메뉴에 있는 화면은 챗봇 안내도 있어야 한다", () => {
  ...
  it("nav.js 메뉴와 screenguide 목록이 어긋나지 않는다", () => {
    ...
    expect(메뉴.length, "nav에서 화면을 못 찾았다 — 시험이 헛돌고 있다").toBeGreaterThan(9);
...
/* 감독 화면 별명 — **낱말 하나**가 도구·다른 구역 질문을 가로채지 않는다.
 * 같은 파일(screenguide.ts) 84~89줄 주석이 2026-08-05 검토 지적으로 이미 경고한 형태다:
 *   「별명이 낱말 하나면 도구 질문을 가로챈다 — 그림을 가리키는 말과 **함께 올 때만** 걸리게 좁힌다」
 * 2026-09-06 ✂ 사유 펼침을 넣으면서 홑낱말 「사유」를 넣어, 이 화면 위에서 「이 오류 사유가
 * 뭐야?」·「반려 사유 알려줘」가 전부 ✂ 인용 제거 안내로 끌려갔다(검토관 wiring 갈래 실측).
 */
describe("screenguide — supervision 별명이 홑낱말로 가로채지 않는다 (검토관 2026-09-06)", () => {
  const 인용안내 = "인용 제거(✂)";
  // 구역에 걸리면 첫 줄이 「화면이름 › 구역이름」이다. 화면 전체 안내로 떨어지면 그 줄은
  // 「… — 이 화면 사용 안내」이고 구역 이름은 본문의 목차에만 나온다 — 그래서 **첫 줄**로 잰다.
  const 걸린구역 = (q: string) => formatScreenGuide("supervision.html", q).split("\n")[0];
  it("「사유」 홑낱말 질문이 ✂ 안내로 끌려가지 않는다", () => {
    for (const q of ["이 오류 사유가 뭐야?", "차단 사유 알려줘", "위험수용 사유 알려줘", "사유서 어떻게 써?"]) {
      expect(걸린구역(q), q).not.toContain(인용안내);
    }
  });
  it("✂를 가리키는 말과 함께 오면 그대로 걸린다 — 좁히느라 없애지 않았다", () => {
    for (const q of ["인용 사유가 뭐야?", "제거 사유 알려줘", "겹침없음이 뭐야?", "인용 제거 뭐야?"]) {
      expect(걸린구역(q), q).toContain(인용안내);
    }
  });
});
