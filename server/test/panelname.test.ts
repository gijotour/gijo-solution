...
describe("★ 구역 이름 전수 — 강제 도구의 말을 채 가는 이름은 대장에 적힌 것뿐이다", () => {
  ...
  it("★★ 대화창 전역 구역은 **어느 화면에서도** 같은 답을 낸다", () => {
    ...
    //   VM 격리를 묻는 **지식 물음** 8문장을 전부 이 구역으로 끌어갔다). 남은 넷은 mydocs.html의
    //   ☑ 근거 지정·📎 첨부 계열이다 — 전부 제품 고유 이름이다.
    expect(전역.length, "전역표를 못 읽었다").toBeGreaterThanOrEqual(4);
  ...
  it("★ 「알려줘 ↔ 보여줘」 대칭이 **대화 홈 전역 구역**에서도 깨지지 않는다", () => {
    // ⚠⚠ 2026-09-12 검토관 [중] 수리 — 첫 판은 여기에 `if (!fa || !fb) continue;`(강제 도구가
    //   둘 다 있을 때만 잰다)를 두었는데, 전역표 이름들은 같은 파일 위쪽이 「강제 도구가 0이어야
    //   한다」고 **전제로 못 박은** 것들이라 이 반복문이 **0회** 돌았다. 이름은 계약을 지킨다고
    //   적혀 있고 단언은 한 번도 안 하는 **빈 초록**이었다(주석은 「비어 돈다」고 스스로 적었다 —
    //   정직했지만, 지켜지는 줄 아는 다음 사람에게는 거짓 안심이다).
    //   제품은 실제로 갈려 있었다: 「근거 지정 알려줘」→[22] / 「근거 지정 보여줘」→⑨(실측).
    //   같은 날 뿌리를 고쳤다 — **전역 히트는 사유(정체물음·안내낱말)가 있을 때만 이긴다**
    //   (screenguide.isHelpIntent). 값요구인 「알려줘」도, 게이트에 없는 「보여줘」도 함께 ⑨로
    //   가므로 대칭이 성립한다. 그래서 이제 강제 도구 조건을 **빼고** 그냥 잰다.
    // ⚠ 「알려줘 ↔ 뭐야?」는 **일부러** 갈린다(값 vs 뜻) — 그쪽은 재지 않는다(위 시험들이 그 쪽).
    // ⚠ **자기 화면 위는 모집단이 아니다.** 거기서는 셋째 훑기가 아니라 첫째 훑기(화면)가 걸리고,
    //   강제 도구가 없는 이름이 「알려줘」에서만 이기는 것은 구역 165개 전부에 걸친 **기존 계약**
    //   (panelNameHit의 게이트 홑물음_RE에 「보여」가 없다)이지 이 라운드가 만든 비대칭이 아니다.
    //   그 계약을 바꿀지는 별도 결정으로 올린다(메인·사장님) — 여기서 몰래 바꾸지 않는다.
    //   그래서 **전역 히트가 실제로 도는 자리**(대화 홈 + 그 구역의 화면이 아닌 모든 화면)만 잰다.
    const 갈림: string[] = [];
    for (const item of 대화창구역들()) {
      const 화면들 = [...안내화면열쇠들().filter((s) => s !== item.screen), undefined] as (string | undefined)[];
      for (const sc of 화면들) {
  ...
  it("★ 이 대칭 시험이 실제로 재고 있다 — 반복 횟수가 0이 아니다(빈 초록 감시)", () => {
    // ⚠ 위 시험이 **0회 돌아 초록**이던 것이 이 라운드의 적발이다. 같은 일이 다시 나지 않게
    //   모집단 × 화면 수를 직접 센다. 전역표가 비거나 화면 목록을 못 읽으면 여기서 빨개진다.
    const 잰횟수 = 대화창구역들().length * 안내화면열쇠들().length; // (N-1 화면 + 대화 홈) = N
    expect(잰횟수, "대칭 시험이 아무것도 안 재고 있다").toBeGreaterThan(50);
  });
});
