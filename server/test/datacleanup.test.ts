...
describe("★ 정리 대장이 새 표를 빠뜨리지 않는다", () => {
  ...
  it("★★ DB에 있는 표 중 **정리 대장이 모르는 업무 표**가 없다 — 새 표를 만들면 여기서 걸린다", () => {
    ...
    const 일부러제외: Record<string, string> = {
      ...
      watch_folders: "📂 지켜보는 폴더 — 업무 데이터가 아니라 **구성**이다(alert_schedules와 같은 갈래). 실사용 전환에서 안 지운다 — 지우면 감시가 말없이 멎는다(2026-08-31 설계관)",
      incident_cases: "📚 침해사고 히스토리(내장 씨앗 + 담당자 등록) — 업무 데이터가 아니라 **제품 지식**이다(결정 ① 2026-09-03, 사례 1건 = 지식 문서 1건). 실사용 전환에서 지우지 않는다 — 표만 지우면 memory_documents(origin=incident-case) 문서가 고아가 되고, 내장 사례는 재기동 때 되살아나 「지웠다」가 거짓이 된다",
