...
import { 등급판정, 등급요약, 라이선스모름, 상용사용금지, 면책문구 } from "../src/engine/licenserisk.js";
...
describe("★★ 상용으로 아예 못 쓰는 조건 — 등급 5단계와 다른 축", () => {
  // 우리는 이날 CC BY-NC-ND 문서 7종을 상용 제품에 실어 배포하다가 **사람 눈으로** 잡았다.
  // 그것이 부품이었다면 판정기는 「아는 라이선스가 아닙니다」라고만 답했을 것이다 —
  // 맞는 말이지만 담당자가 할 일을 못 알려 준다. NC는 소스를 공개해도 안 풀린다.
  it("NC·ND는 「상용 못 씀」이라고 이름을 붙여 말한다", () => {
    for (const s of ["CC-BY-NC-ND-2.0-KR", "CC BY-NC-ND 2.0", "CC-BY-NC-4.0", "공공누리 제4유형"]) {
      const r = 등급판정(s);
      expect(상용사용금지(s), `${s} — 상용 금지로 잡혀야 한다`).toBe(true);
      expect(r.근거, `${s} — 「모르는 라이선스」로 끝내면 안 된다`).toContain("상용 제품에 쓸 수 없는");
      expect(r.확인필요).toBe(true);
    }
  });
  ...
  it("★ 반례 — 상업적으로 **쓸 수 있는** 것을 막지 않는다", () => {
    ...
    for (const s of ["CC-BY-4.0", "CC-BY-3.0", "공공누리 제1유형", "MIT", "Apache-2.0", "OFL-1.1"]) {
