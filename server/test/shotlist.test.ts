...
describe("★★ 자료에 들어갈 화면이 빠지지 않는다", () => {
  ...
  it("★★ 소개덱 생성기도 은퇴한 화면 사진을 쓰지 않는다", () => {
    ...
    // 18-LLM합성: 2026-08-19 추가 — 화면이 08-08에 메뉴에서 내려갔는데(고아 확정) 덱·매뉴얼이
    //   계속 그 사진과 「AI 운영 › 모델 합치기」 경로를 가리키고 있었다.
    const 은퇴사진 = ["04-운영가이드-점검거버넌스", "07-추천LLM가이드", "16-지식모델-온톨로지", "19-사용량-요금", "18-LLM합성-모델머지"];
  ...
  it("★★ 고객에게 주는 자료가 **없는 화면**을 가리키지 않는다", () => {
    ...
    const 은퇴 = ["04-운영가이드", "07-추천LLM가이드", "16-지식모델-온톨로지", "19-사용량-요금", "26-MCP연동", "18-LLM합성-모델머지"];
