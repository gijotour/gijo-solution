...
it("★ 「등록되지 않았습니다」 꼴 — 세 번째 실측 변주", () => {
  expect(없다는답인가("해외 지사 보안 담당자 연락처는 사내 지식 베이스에 등록되지 않았습니다.")).toBe(true);
  // 존재-부재가 아닌 「적용되지 않습니다」류 규정 답은 동사 목록 밖 — 강등하지 않는다.
  expect(없다는답인가("이 규정은 개인 이용자에게는 적용되지 않습니다. 기업은 …")).toBe(false);
});

// 검토관 오탐 지적(2026-08-13) — 범위·상태를 정확히 말한 좋은 답은 강등하면 안 된다.
it("★ 범위 서술은 강등 안 한다 — 「에」 조사 없는 되지-않 꼴", () => {
  expect(없다는답인가("이 지침에 클라우드 환경은 포함되지 않습니다. 온프레미스만 다룹니다.")).toBe(false);
  expect(없다는답인가("로그는 90일 이상 저장되지 않습니다 — 보관 정책에 따라 순환됩니다.")).toBe(false);
  // 부재 선언(에+동사)은 그대로 잡힌다
  expect(없다는답인가("해당 값은 시스템에 저장되지 않았습니다.")).toBe(true);
});
