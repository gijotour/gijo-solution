...
describe("프로 확정 계약 — 메뉴 클릭 = 화면+카드 나란히(2026-08-31 사장님 개정 — 실화면 캡처 3장 「안티그래비티처럼, 3번 그림처럼」; 종전 「메뉴는 카드가 전부」 2026-08-20 ×3을 대체)", () => {
  ...
  it("📓 내 문서 — 메뉴 이름 짝이 한 글자까지 같다(nav.js가 정본 · 「(옛 …)」 꼬리 제외, 2026-09-14 승인 시안 write-hub-unify §5)", () => {
    ...
    // ⚠ 셸의 **옛탭 표**도 같은 이름을 말한다(2026-09-14 검토관 [상] 수리). app.html:585-587이
    //   「이름은 도착해서 실제로 보이는 것과 같게 둔다」고 스스로 규칙을 적어 뒀는데, 이름 통일이
    //   그 줄을 빠뜨려 옛 intro.html 탭을 켜 둔 담당자가 탭엔 옛 이름·화면엔 새 이름을 동시에
    //   봤다. 위 짝 대조는 nav.js↔mydocs.html만 보므로 이 표는 모집단 밖이었다 — 여기서 넣는다.
    const ap = 코드만(join(PAGES, "app.html"));
    expect(ap, "옛탭 표(app.html)의 intro.html 이름이 📦 갈래 이름과 다르다 — 그 탭을 켜 둔 담당자는 잘못 눌렀다고 생각한다(app.html:585-587이 적어 둔 규칙)")
      .toContain('"intro.html": { page: "mydocs.html?tab=vendor", label: "보안제품 비교·소개" }');
  });
  it("📝 문서작성 판 — 문 4개 · 줄 하나=행동 하나 · 비관리자는 숨기지 말고 흐리게(2026-09-14 결정)", () => {
    ...
    // ⚠ 위 alternation은 **오른쪽 가지만으로 통과한다**(2026-09-14 검토관 [하] 수리). 헬퍼가
    //   속성을 동적 조립하니 왼쪽 `data-w="new"`는 원리상 안 걸리고, 걸리는 것은 호출부
    //   `줄("new", …)`뿐이다 — 즉 헬퍼에서 `' data-w="' + 표식 + '"' ` 한 도막을 지워도 위 for가
    //   전부 초록이다(실측). 그런데 게시 관문(publish-gate-ui.mjs:1710~)은 그 **속성**으로 줄을
    //   찾으므로, 속성이 죽으면 서버 시험 55초는 통과하고 9227 실화면에서 ⓐ~ⓓ가 한꺼번에
    //   빨개진다 — 가장 비싼 자리에서 가장 늦게. 그래서 **속성 방출 자체**를 여기서 따로 잰다.
    const 방출 = /data-w="'\s*\+\s*표식\s*\+\s*'"/.test(md)
      || ["new", "fix", "doc", "aibom"].every((w) => md.includes(`data-w="${w}"`));
    expect(방출, '줄 헬퍼가 data-w 속성을 실제로 안 뱉는다 — 호출부만 남으면 이 시험은 초록인데 게시 관문 ⓐ~ⓓ(#list .wline[data-w="…"])가 전부 죽는다').toBe(true);
