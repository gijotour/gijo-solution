...
describe("장애·침해 초동 절차가 플레이북에 먹히지 않는다 (전-4)", () => {
  ...
  //   의무 프레이밍(신고/통지 의무·법률상·법적 의무)만으로 「법을 묻는 말」을 가른다.
  it("법적 신고 의무 질문은 침해가 아니다 — 「신고해야」(사고 중)는 침해로 보존", () => {
    for (const q of [
    ...
    // ⚠ 「신고해야」(사고 중 신고)·「사내 보고 의무」(내부보고)는 여전히 침해 — 「의무」 배제를 좁게 잡았다
    for (const q of ["랜섬웨어 걸렸는데 신고해야 하나?", "해킹당했는데 사내 보고 의무 있어 어떻게 해?"]) {
      expect(침해사고질문인가(q), `${q}는 사고 중이라 여전히 침해다`).toBe(true);
    }
  });

  // ★ 비켜 준 법령 질문이 FORCED law_lookup에 결정적으로 닿는가 — 약칭 「망법·신용정보법」 추가(설계관 뿌리).
  //   약칭이 침해아님·FORCED·법령질문RE 세 곳에서 투명했던 것이 뿌리였다. 여기선 FORCED 등록만 지킨다.
  it("FORCED law_lookup 정규식이 약칭 법이름(정보통신망법·신용정보법)을 등록했다", () => {
    const agentloopSrc = fs.readFileSync(
      path.join(__dirname, "..", "src", "engine", "agentloop.ts"),
      "utf8",
    );
    expect(agentloopSrc, "FORCED law_lookup에 약칭 망법·신용정보법이 있어야 한다").toMatch(
      /보안법\|망법\|신용정보법/,
    );
  });
