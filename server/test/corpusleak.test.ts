...
describe("RAG 코퍼스 — 개발 문서가 섞이지 않는다", () => {
  ...
  it("이 감시가 헛돌고 있지 않다", () => {
    ...
    //   ★ 2026-09-07 32→36 · knowledge 20→24 — 같은 날 **네 편을 한 번에** 넣었다(사장님 승인
    //     「추천 첫 묶음 4편 배포 끝나면 진행」): 금융 취약점 평가기준 · 금융 AI 보안 점검표 ·
    //     중소기업 정보보호 점검표 · 개인정보 자율점검표. 넷 다 **우리가 직접 쓴 글**이고
    //     머리말이 「어느 기관의 공식 점검표도 아니다」를 못 박는다 — 그 줄을 지우면 제품이
    //     「기관 점검표를 받았다」로 답하므로, 문서를 손볼 때 함께 지키는 계약이다(docs-manifest why).
    //     ⚠ 숫자는 눈으로 센 것이 아니라 **매니페스트를 실제로 훑어** 얻은 값이다(36/24 확인).
    //   ★ 2026-09-07 36→35 — 「GIJO_AS_LLM_합성_안내.md」를 뺐다(knowledge는 24 그대로).
    //     안내하던 기능(LLM 합성)을 이날 제품에서 화면·창구까지 통째로 내려서, 문서만
    //     남기면 챗봇이 없는 기능을 계속 안내한다. files[]에서 빼는 것으로 끝내지 않고 _제외에
    //     등재했다 — 그래야 이미 인입된 조각이 다음 기동에 지워진다(2026-08-03 실측).
    //   ⚠ 여기가 빨개지면 **먼저 매니페스트를 의심**한다. 문서를 일부러 늘리거나 뺐다면
    //     아래 두 숫자를 그때 고친다(고칠 때 무엇을 왜 늘렸는지 커밋 메시지에 적을 것).
    expect(문서.length, "코퍼스 문서 수가 달라졌다 — 매니페스트를 고쳤으면 이 숫자도 함께 고친다").toBe(35);
    expect(
