...
describe("★★ [상] 쓰기 흐름을 한 수로 삼키지 않는다 — 잣대(writeflow.쓰기흐름인가) 한 곳이 막는다", () => {
  ...
  const 조회로못박지않을것흐름: [string, string][] = [
    ...
    // ★ 2026-09-13 검토관 수리(적발 ⑦) — Set에 더한 5개 중 **knowledge_status만 짝이 빠져 있었다.**
    //   커밋이 근거로 든 N09 문장(「문서 목록 보여주고 김보안한테 배정해줘」 → knowledge_status,
    ...
    // ★★ 2026-09-13 검토관 [중] 수리 — 위 한 줄은 **등록 대상이 있을 때만** 재는 시험이었다
    //   (아래 beforeEach가 FW-01을 등록한다). 첫 판의 Set 원소가 파생지였던 탓에, 등록 대상이
    //   없으면 같은 문장이 run_hardening_scan(self 결재판)으로 못 박혀 배정이 그대로 안 갔다 —
    //   등록 대상 0건인 설치본(4100·새 고객)에서는 수리가 원리상 한 번도 발화하지 않았다.
    //   그래서 **미등록 장비**와 **장비 코드가 아예 없는 문장** 둘을 짝으로 더한다.
    ["FW-99 하드닝 점검 돌려주고 김보안한테 배정해줘", "run_hardening_scan"],
    ["하드닝 점검 돌려주고 김보안한테 배정해줘", "run_hardening_scan"],
  ];
