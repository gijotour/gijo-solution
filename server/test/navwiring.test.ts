...
// ⚠ 「내 업무 — 고르면 직전 가이드가 남지 않는다」 시험은 걷어냈다(2026-08-01, 화면 폐지).
//   그 시험이 지키던 실사고(2026-07-31, 4.10.0 실측: 다른 업무를 골랐는데 직전 업무의 단계가
//   그대로 떠 있었다)는 **화면이 한 자리를 다시 그려서** 생긴 것이다. 대화창은 답이 새 말풍선
//   으로 쌓여 직전 것이 남을 자리가 없으므로, 같은 버그가 생길 구조가 아니다.
//   절차 카드 자체의 계약(순서를 지어내지 않는다·단계가 저장된다)은 worksteps.test.ts가 본다.
