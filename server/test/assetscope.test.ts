...
describe("★ 판을 뺐으면 빈칸이 남지 않는다", () => {
  it("발견·수집에서 자산 판이 빠졌다", () => {
    expect(판들, "자산 판이 아직 있다").not.toMatch(/\{ id: "assets", title: "🖥 자산"/);
    // 「보안 태세」는 사장님 확인 전이라 남겨 둔다 — 승인 없이 화면을 지우지 않는다.
    expect(판들, "확인 안 받은 판까지 지웠다").toMatch(/id: "posture"/);
  });
...
describe("★ 범위 배관 5고리 — 하나만 끊겨도 조용히 죽는다", () => {
  ...
  it("⑤ 대화창이 그리고, 지시에 싣는다", () => {
    expect(대화창, "알약을 안 그린다").toContain('id="csScope"');
  ...
  it("★ 🎯(고른 한 건)과 🗂(범위)를 **다른 색**으로 둔다", () => {
    ...
    expect(대화창, "범위 알약 CSS가 없다").toMatch(/\.cs-scope\{[^}]*rgba\(59,130,246/);
    expect(대화창, "고른 것 알약이 주황이 아니다 — 두 알약 색이 같아졌다").toMatch(/\.cs-sel\{[^}]*#ffd9a8/);
  });

  it("★★ 되살린 범위를 **그린다** — 값만 살고 안 보이면 함정이 된다", () => {
    // 2026-08-18 실화면: 새로고침 뒤 값은 살아 지시에 실리는데 알약이 안 떴다.
    // 담당자는 범위가 풀린 줄 알고 묻고, 답이 왜 적은지 모른다. 보이지 않는 범위는
    // 범위가 아니라 함정이다. 상자를 만든 직후 반드시 다시 그려야 한다.
    expect(대화창, "되살린 범위를 안 그린다").toMatch(
      /csScope"\)\.addEventListener[\s\S]{0,600}?\n\s*renderScope\(\);/
    );
  });
