...
describe("배선 (소스 계약)", () => {
  ...
  it("★ 보강·딱지는 그 함수 **안에서 한 번씩만** 불린다 — 갈래로 흩어지면 또 샌다", () => {
    ...
    // ⚠ 호출 **꼴**이 아니라 **자리**를 본다(2026-09-01). 예전엔 붙어 있는 모양
    //   (`법령한계를밝힌다(guardAgainstDenial(`)을 못 박았는데, 그 사이에 다른 출구 관문
    //   (조문근거를보탠다)을 끼우자 **뜻은 그대로인데 시험이 깨졌다.** 지키려는 것은
    //   「출구 한 곳에서만 불린다」이지 「누구를 바로 감싸느냐」가 아니다.
    const 출구시작 = src.indexOf("return 다음단계붙이기(");
    expect(출구시작, "출구를 못 찾았다 — 이 시험이 낡았다").toBeGreaterThan(-1);
    const 출구 = src.slice(출구시작, 출구시작 + 300);
    expect(출구, "법령 한계 딱지가 출구 사슬 밖에 있다").toContain("법령한계를밝힌다");
    expect(출구, "조문 관문이 출구 사슬 밖에 있다").toContain("조문근거를보탠다");
    // 출구가 하나여야 한다 — 여러 곳이면 한쪽이 조용히 안 걸린다.
    expect((src.match(/return 다음단계붙이기\(/g) ?? []).length, "출구가 여러 곳이다").toBe(1);
  });
