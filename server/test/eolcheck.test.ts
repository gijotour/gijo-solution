...
describe("★★ 「없다」를 「괜찮다」로 말하지 않는다", () => {
  ...
  it("표 전체 상태를 함께 말한다 — 얼마나 믿을 수 있는 표인지", async () => {
    const 답 = await runEolCheck({});
    // ⚠ 「확인 전」을 못박아 두었다가 2026-09-02 대조 완료로 낡았다. 둘 중 하나면 된다.
    expect(답, "믿을 수 있는 정도를 안 밝힌다").toMatch(/확인 전|대조 완료/);
  });
});

describe("표 자체의 정직성 — 확인 전 값을 확인된 것처럼 보이게 하지 않는다", () => {
  // ⚠ **씨앗 표에 확인 전 줄이 있든 없든** 이 기계는 돌아야 한다. 예전엔 표에 확인 전 줄이
  //   있다는 것을 전제로 삼았다가, 대조를 끝내자 시험이 통째로 낡았다.
  it("★ 확인 전 줄의 한 줄 답에는 **대조 안 했다는 말**이 붙는다", () => {
    const 만든것 = { 제품: "가상제품", 버전: "1.0", 종료일: "2020-01-01", 출처: "시험용", 확인필요: true };
    expect(eol한줄(만든것), "확인 안 했다는 말이 없다").toContain("대조하지 않은 값");
    // 씨앗에 확인 전 줄이 남아 있다면 그것들도 같은 말을 해야 한다.
    for (const r of EOL_SEED.filter((x) => x.확인필요)) {
      expect(eol한줄(r), `${r.제품} ${r.버전 ?? ""}`).toContain("대조하지 않은 값");
    }
  });

  it("대조를 마친 줄은 **대조한 날**을 말한다 — 확인 전과 구분된다", () => {
    for (const r of EOL_SEED.filter((x) => !x.확인필요)) {
      const 글 = eol한줄(r);
      expect(글, `${r.제품} ${r.버전 ?? ""}: 대조일이 없다`).toContain("대조함");
      expect(글, `${r.제품}: 확인 전 문구가 남아 있다`).not.toContain("대조하지 않은 값");
    }
  });

  it("★★ 유상 연장이 살아 있는 줄은 「패치가 없다」고 **단정하지 않는다**", () => {
    // 2026-09-02 벤더 대조에서 드러난 것: 9줄 중 6줄이 오늘도 유상 패치를 받는 중이다.
    // 그 고객에게 「고칠 패치가 없습니다」는 **거짓**이고, 멀쩡한 서버가 교체 대상에 오른다.
    const 오늘 = new Date("2026-09-02T00:00:00");
    for (const r of EOL_SEED) {
      const 연장 = r.유상연장;
      if (!r.종료일 || !연장) continue;
      if (new Date(r.종료일 + "T00:00:00").getTime() >= 오늘.getTime()) continue; // 아직 무상 지원 중
      const 끝 = 연장.종료일;
      const 살아있나 = !끝 || new Date(끝.length === 7 ? `${끝}-28` : 끝).getTime() >= 오늘.getTime();
      const 글 = eol한줄(r, 오늘);
      if (살아있나) {
        expect(글, `${r.제품} ${r.버전}: 연장이 살아 있는데 「패치가 없다」로 단정했다`).toContain("무상 지원이 끝난");
        expect(글, `${r.제품} ${r.버전}: 연장 이름을 안 말한다`).toContain(연장.이름);
        expect(글, `${r.제품} ${r.버전}: 확인을 요청하지 않는다 — 「괜찮다」로 읽힌다`).toMatch(/확인해 주세요/);
      } else {
        expect(글, `${r.제품} ${r.버전}: 연장도 끝났으면 그렇다고 말해야 한다`).toContain("끝났습니다");
      }
    }
  });

  it("★ 유상 연장을 **「괜찮다」로 말하지 않는다** — 계약 여부를 우리는 모른다", () => {
    for (const r of EOL_SEED.filter((x) => x.유상연장)) {
      const 글 = eol한줄(r, new Date("2026-09-02T00:00:00"));
      for (const 금지 of [/괜찮습니다/, /안전합니다/, /문제\s*없습니다/]) {
        expect(글, `${r.제품} ${r.버전}: ${금지}`).not.toMatch(금지);
      }
    }
  });
