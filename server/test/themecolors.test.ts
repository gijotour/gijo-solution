 ...
 *
 * ⚠ 2026-09-03: 열쇠를 **줄 번호에서 코드 내용으로** 바꿨다.
 *   줄 번호로 묶어 뒀더니 파일 윗부분을 고칠 때마다 밀려서, 오늘까지 **네 번**을 따라 옮겼다
 *   (697 → 665 → 688 → 689 → 692). 예외 대상 자체는 한 번도 안 바뀌었는데 열쇠만 계속 틀렸다 —
 *   손대는 사람마다 「내가 뭘 깨뜨렸나」를 확인하는 데 시간을 쓰고, 그때마다 **줄만 고치고 넘어가서**
 *   예외가 정말 아직 옳은지는 아무도 다시 안 봤다. 「같은 것을 여러 곳에 적으면 어긋난다」의 전형이다.
 *   내용으로 묶으면 그 코드가 사라질 때 예외도 **저절로 안 맞아** 다시 보게 된다.
 */
const JS예외 = new Map([
  [
    'nav.js|wBadge.style.color = "#fff"',
    "기한 지난 업무 배지 — 바로 위 같은 요소에 background=var(--red,#e2483d)를 깔아 흰 글자가 옳다(채움)",
  ],
]);

/** 이 줄이 예외인가 — 파일과 **코드 내용**으로 찾는다(줄 번호로 묶지 않는다). */
function 예외인가(파일: string, 줄: string): boolean {
  for (const 열쇠 of JS예외.keys()) {
    const [f2, 조각] = 열쇠.split("|");
    if (파일 === f2 && 줄.includes(조각)) return true;
  }
  return false;
}
