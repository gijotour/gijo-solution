...
describe("★ 붙이는 방식이 정직한가 — 소스 감시", () => {
  it("답을 버리지 않고 **앞에 한 줄만** 붙인다", () => {
    ...
    // ⚠ 2026-09-05: 문구가 붙이는 자리에서 **상수(근거약함배너)로** 빠졌다(근거 없음 표식이
    //   같은 문장을 다시 적지 않고 비교해야 해서). 보는 것은 그대로다 — 「배너 + \n\n + 원래 답」.
    expect(llm소스, "답을 버리고 있다").toMatch(/근거약함배너\}\\n\\n\$\{reply\}/);
  });

  it("이미 「못 찾았다」고 말한 답에는 두 번 안 붙인다", () => {
    // 2026-09-05: 억제 정규식이 **noevidence.ts로 옮겨졌다**(모델자기거절_RE). 여기 리터럴로
    //   남겨 두면 dispatcher 출구가 「배너를 왜 안 붙였는지」를 같은 잣대로 못 읽는다 —
...
describe("★ 재는 쪽과 재이는 쪽이 같은 말을 쓰지 않는다", () => {
  ...
  function 실제문구(): string {
    ...
    // ⚠ 2026-09-05: 문구가 붙이는 자리(reply = ...)에서 **상수 선언**(noevidence.ts)으로 옮겨졌다.
    //   뽑는 자리만 따라간다 — 이 시험이 지키는 것은 「문구가 폴백 목록과 안 겹친다」이지
    //   문구가 어느 파일에 적혔나가 아니다.
    const 배너소스 = fs.readFileSync(path.join(__dirname, "../src/engine/noevidence.ts"), "utf8");
    const 줄 = 배너소스.split("\n").find((l) => l.includes("export const 근거약함배너 = `${표식.") && l.includes("**"));
    if (!줄) return "";
