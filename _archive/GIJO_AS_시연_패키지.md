...
- 작성: 2026-07-29 · **화면 갱신: 2026-08-19(클라 5.26.0 — 아래 「⚠ 화면 갱신」 셋) · 대사 재실측: 2026-08-13** — 지시 10문장을 운영에 다시 던져 대조했다. ①~④ 대사는 08-13 실측 그대로 유효(그 뒤 라우팅 변경 없음), ①★·⑥은 08-19 QA 실측
...
- **⑥ 프로 셸(대화가 주인공)도 보너스**(2026-08-19 신설, 아래) — "화면이 너무 많다"는 반응이 나올 때 꺼낸다.

### ⚠ 2026-08-19 화면 갱신(클라 5.26.0) — 대본 시작 전에 알아둘 셋
1. **로그인 창이 두 칸이다** — 왼쪽에서 제품(🏢표준/🚀프로/🪶라이트)과 AI 위치를 고른 뒤 로그인한다. 시연은 **표준**을 고른 채 진행하면 기존 대본 그대로다(안 고르면 쓰던 값 유지).
2. **메뉴 맨 위에 「⓪ 자산」이 생겼다** — 자산 현황과 **🗂 지금 범위**(자산 하나로 화면·AI 답을 걸러 보는 것)의 입구. 시연 ①에 써먹을 수 있다(아래 ★).
3. ~~캡처 일부가 이 갱신 전이다~~ → **2026-08-19 전면 재생성 완료** — `screenshots/deck/` 30장이 5.26.0 실화면·운영 실데이터·현행 메뉴(⓪자산 포함)로 다시 찍혔고, 소개덱(intro-inserts) 18장도 재생성됐다. 예외: 지도 3장(32~34)·참고 2장(30~31)은 별도 도구 몫이라 08-04판 그대로(내용 변화 적음), 로그인 화면은 캡처 대상이 아니다.
...
> ★ **🗂 지금 범위를 곁들일 수 있다**(5.26.0 · 2026-08-19 실측). ⓪ 자산에서 서버 하나를 「지금 범위」로 걸면
>   취약점 화면이 **54줄 → 1줄**로 줄고 「53건 숨김 · 전체 보기」 띠가 붙는다. 콘솔 답도 같은 범위로 좁혀진다
>   (답 머리에 「🗂 … 범위」 표시). 시연 멘트: "혼자 남은 날은 다 못 봅니다 — **오늘은 이 서버만** 하고
>   선언하면, 화면도 AI도 그 서버만 봅니다." 못 거르는 화면은 「전체를 보여 줍니다」라고 스스로 말한다.
...
> ★ **작업 내역 화면이 「사람이 시킨 일/시스템이 돈 일」을 가른다**(5.26.0). "이번 주 한 일"의 건수를 팀장이 의심하면 이 화면을 열어 보인다 — "담당자가 시킨 것과 자동으로 돈 것이 갈라져 보입니다." 아낀 시간 셈 자체도 시험·QA 실행은 애초에 안 센다(작업 원장 설계 — 적게 세는 쪽으로).
...
## 시연 ⑥ (보너스) "화면이 아니라 대화가 주인공" — 프로 셸 (2026-08-19 신설·실측)

**언제 꺼내나:** 시연 중 "화면이 너무 많다 / 우리는 어차피 물어보고 시킬 건데"라는 반응이 나올 때. 또는 **세로 모니터** 사용 고객(실측: 세로에서 대화 홈이 전폭 1024px로 가장 잘 맞는다).

**상황 설정(내레이션 30초):** 같은 제품, 같은 데이터 — 화면 배치만 바꿉니다. 로그인에서 🚀프로를 고르면 앱이 **대화창 하나로** 시작하고, 화면은 필요할 때 **별도 창(팝업)**으로 뜹니다.

| 분 | 행동 | 핵심 대사 |
|---|---|---|
| 0:30 | 로그인에서 🚀프로 선택 → 대화 홈 | "메뉴 대신 대화가 첫 화면입니다. 왼쪽 좁은 띠 ☰ 하나로 전체 메뉴, 💬로 언제든 대화 홈" |
| 1:30 | 콘솔에 지시 → 화면이 필요한 답이면 **별도 창**으로 뜬다 | "화면은 대화를 밀어내지 않습니다 — 옆 모니터로 던져 놓고 대화를 계속합니다" |
| 2:30 | 팝업(취약점 화면)에서 자산 하나를 고른다 → **본창 🗂 범위에 바로 잡힌다** | "창이 나뉘어도 한 몸입니다 — 팝업에서 고른 것을 본창 AI가 압니다"(2026-08-19 실측) |
| 3:30 | 결재가 필요한 지시 → 본창이 가려져 있으면 **작업표시줄이 깜빡인다** | "승인은 본창 한 곳에서만 — 놓치지 않게 깜빡이되, 하던 일을 뺏지는 않습니다" |

**실측(2026-08-19, 5.26.0 QA):** 셸 동선 7/7 통과(전환·복귀·계정 생존·☰=Ctrl+K·💬 홈), 팝업에서 고른 자산이 본창 🗂에 IPC로 닿음, 표준↔프로 전환에 **재시작 불필요**.

**시연자 주의(정직):** 프로 셸은 배치이지 기능이 아니다 — "표준에 없는 기능이 생기는 게 아니라, 같은 기능을 대화 중심으로 씁니다"라고 말한다(없는 차이를 만들어 말하면 안 된다). 화면 자체는 표준과 같아 수리·개선도 함께 받는다.
