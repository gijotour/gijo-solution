> **⚠ 2026-08-10 — 에디션 축이 생겼습니다(정합 주의).** 제품은 이제 **라이트/표준 두 에디션**으로
> 나뉩니다(기능·두뇌가 다름 — `GIJO_AS_제품_라인업_판매가이드.md`가 기준). **이 문서는 「표준
> 에디션」의 VRAM 티어**를 다룹니다. **라이트 에디션**(gijo-main 7.6B·핵심 8도구, 실측 6.5GB)은
> **전용 GPU 8GB 또는 통합메모리(Apple Silicon Mac) 16GB**에서 돌며 상세는 라인업 문서를 따릅니다.
> 아래 「Lite/Standard/Pro」는 표준 에디션 안의 **하드웨어 등급**이지 라이트 에디션이 아닙니다
> (이름이 겹쳐 혼동 주의 — 다음 개정에서 등급명을 바꿉니다). 라이트 CUDA 실측: 7.6B@8192 5.58GB +
> bge@2048 0.90GB = 6.48GB(Metal 6.45와 거의 동일 · `tools/light-cuda-measure.sh`).
>
> **2026-08-09 확인** — 지금 운영 구성은 **Qwen3-14B 한 개 + 전문가 어댑터**입니다. 아래에서 「7B 2개」로 적힌 대목은 2026-07-22 시절 구성이며, 산정 방법만 유효합니다.
