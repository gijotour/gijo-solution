    ...
    <div class="header-inner">
      ...
      <div class="header-tools">
        <button class="btn btn-sm" style="background:#f3e8ff; color:#7c3aed; border-color:#d8b4fe; font-weight:700;" onclick="printBoardroomAnnualReport()" title="경영진 및 이사회 제출용 연간 정보보호 성과평가 보고서 A4 출력">
          <i data-lucide="presentation" style="width:13px; height:13px;"></i> 📊 CISO 이사회 보고서 A4 인쇄
        </button>
        ...
        <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:16px;">
          ...
          <div style="background:#fff; border:1px solid var(--border); border-left:5px solid #dc2626; border-radius:10px; padding:20px; box-shadow:var(--shadow); display:flex; flex-direction:column; justify-content:space-between;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding-top:12px; border-top:1px solid var(--border);">
              ...
              <button class="btn btn-sm btn-primary" style="background:#dc2626; border-color:#dc2626; font-weight:700;" onclick="openIncidentQuarantineModal(&apos;ASSET-05&apos;)" title="호스트 네트워크 긴급 차단 및 KISA 5단계 런북 가동">
                <i data-lucide="power" style="width:12px; height:12px;"></i> 🚨 긴급 격리 & 런북
              </button>
    ...
    <section id="view-bom" class="view-page">
      <div class="white-panel" style="padding:1.25rem; height:100%; overflow-y:auto;">
        ...
        <div style="display:flex; gap:0.4rem; border-bottom:2px solid var(--border); margin-bottom:1.25rem; padding-bottom:0.4rem;">
          ...
          <button id="bomSubBtn-dlp" class="btn btn-sm" style="background:#f8fafc; color:var(--text-sub);" onclick="switchBomSubView('dlp')">
            <i data-lucide="lock" style="width:12px; height:12px;"></i> 🛡️ 생성형 AI DLP 보안 감사
          </button>
        </div>
        ...
        <!-- Sub View 4: GenAI Security Prompt Audit & Realtime DLP Gate -->
        <div id="bomSubView-dlp" style="display:none; flex-direction:column; gap:1rem; margin-top:1rem;">
          
          <div class="white-panel" style="padding:1.1rem; border:1px solid var(--border); border-radius:8px;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.75rem;">
              <div>
                <span class="x-badge danger" style="font-weight:800;">실시간 DLP 기밀 유출 차단 엔진 가동 중</span>
                <h3 style="font-size:1.05rem; font-weight:800; color:var(--text-main); margin-top:0.25rem; display:flex; align-items:center; gap:0.4rem;">
                  <i data-lucide="lock" style="width:16px; height:16px; color:var(--primary);"></i>
                  생성형 AI 프롬프트 보안 감사 & 실시간 개인정보/소스코드 DLP 게이트
                </h3>
                <p style="font-size:0.75rem; color:var(--text-sub);">
                  사내 임직원이 상용 LLM(OpenAI GPT-4o, Claude 3.5, Gemini) 사용 시 주민등록번호, AWS API Key, DB 접속정보 유출을 사전 차단하고 마스킹합니다.
                </p>
              </div>
              <div style="display:flex; gap:0.4rem;">
                <button class="btn btn-sm" onclick="renderDlpAuditTable()">
                  <i data-lucide="refresh-cw" style="width:12px; height:12px;"></i> 감사 대장 새로고침
                </button>
              </div>
            </div>

            <!-- Interactive DLP Prompt Test Simulator -->
            <div style="background:#f8fafc; border:1px solid var(--border); border-radius:6px; padding:0.8rem; display:flex; flex-direction:column; gap:0.5rem;">
              <div style="font-size:0.75rem; font-weight:700; color:var(--text-main);">🧪 실시간 프롬프트 DLP 검사 시뮬레이터 (실시간 테스트)</div>
              <div style="display:flex; gap:0.5rem;">
                <input type="text" id="dlpSimInput" class="search-input" style="flex:1; background:#fff;" value="홍길동 고객 주민번호 900101-1234567 및 클라우드 KEY: AKIAIOSFODNN7EXAMPLE 유출 검사" placeholder="테스트 프롬프트 입력...">
                <button class="btn btn-sm btn-primary" onclick="runInteractiveDlpTest()">DLP 실시간 검사</button>
              </div>
              <div id="dlpSimResultBox" style="font-size:0.73rem; padding:0.4rem 0.6rem; border-radius:4px; background:#eff6ff; border:1px solid #bfdbfe; color:#1d4ed8;">
                💡 위 입력창에 주민번호, 계좌번호, API Key를 입력하고 [DLP 실시간 검사]를 누르면 자동 차단 및 마스킹 결과를 확인할 수 있습니다.
              </div>
            </div>
          </div>
          ...
          <div class="white-panel" style="padding:0; border:1px solid var(--border); border-radius:8px; overflow:hidden;">
            <div style="padding:0.75rem 1rem; background:#f8fafc; border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
              <span style="font-weight:800; font-size:0.85rem; color:var(--text-main);">📋 생성형 AI 프롬프트 보안 위반 및 차단 감사 대장 (Excel Grid)</span>
              <span style="font-size:0.72rem; color:var(--text-dim);">사내 모든 LLM 전송 트래픽 실시간 감사 기록</span>
            </div>
            <div class="excel-wrapper" style="max-height:360px; overflow-y:auto;">
              <table class="excel-table">
                <thead>
                  <tr>
                    <th class="center" style="width:40px;">No</th>
                    <th style="width:120px;">감사 일시</th>
                    <th style="width:110px;">사용 부서</th>
                    <th style="width:160px;">대상 AI 모델</th>
                    <th>프롬프트 입력 내용 (마스킹 적용)</th>
                    <th class="center" style="width:130px;">탐지 위반 유형</th>
                    <th class="center" style="width:85px;">조치 결과</th>
                  </tr>
                </thead>
                <tbody id="dlpAuditTableBody">
                  <!-- Rendered dynamically -->
                </tbody>
              </table>
            </div>
    ...
    <section id="view-sbom" class="view-page">
      <div style="display:flex; flex-direction:column; gap:1rem;">
        ...
        <div class="portal-filter-bar">
          ...
          <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
            <button class="btn btn-sm" style="background:#fef3c7; color:#b45309; border-color:#fde68a; font-weight:700;" onclick="openCvePatchModal()" title="사내 자산 및 SBOM 부품과 CVE 취약점 1:1 상관분석">
              <i data-lucide="scan-search" style="width:13px; height:13px;"></i> 🔍 CVE 상관분석 & 벤더 패치
            </button>
...
>
  ...
  <!-- INCIDENT QUARANTINE & 5-STEP PLAYBOOK MODAL -->
  <div id="incidentQuarantineModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.7); z-index:99999; justify-content:center; align-items:center; padding:20px;">
    <div style="background:#fff; border-radius:10px; width:820px; max-width:95vw; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 12px 36px rgba(0,0,0,0.3); overflow:hidden;">
      
      <!-- Modal Header -->
      <div style="padding:1.1rem 1.4rem; background:#fee2e2; border-bottom:1.5px solid #fca5a5; display:flex; justify-content:space-between; align-items:center;">
        <div style="display:flex; align-items:center; gap:0.5rem;">
          <div style="width:32px; height:32px; border-radius:50%; background:#dc2626; color:#fff; display:flex; justify-content:center; align-items:center; font-weight:800; font-size:1.1rem;">🚨</div>
          <div>
            <h3 style="font-size:1.1rem; font-weight:800; color:#991b1b;">침해사고 긴급 격리 프로토콜 & KISA 5단계 대응 런북</h3>
            <p style="font-size:0.75rem; color:#b91c1c; margin-top:2px;">금융 코어 WAS (JEUS 8.5) 및 주요 시스템 침해위협 즉각 통제</p>
          </div>
        </div>
        <button class="btn btn-sm" onclick="closeIncidentQuarantineModal()" style="font-weight:700;">✕ 닫기</button>
      </div>

      <!-- Modal Body -->
      <div style="padding:1.2rem; overflow-y:auto; display:flex; flex-direction:column; gap:1rem;">
        
        <!-- Target Host Status Box -->
        <div style="background:#fff; border:1.5px solid #fca5a5; border-radius:8px; padding:1rem; display:flex; justify-content:space-between; align-items:center; box-shadow:var(--shadow-sm);">
          <div>
            <div style="font-size:0.75rem; color:var(--text-dim); font-weight:700;">타겟 정보시스템 (ASSET-05)</div>
            <div style="font-size:1.05rem; font-weight:800; color:var(--text-main); margin-top:2px;">금융 코어 WAS (TmaxSoft JEUS 8.5)</div>
            <div style="font-size:0.76rem; color:var(--text-sub); margin-top:4px;">
              <b>IP:</b> 10.10.40.20 | <b>망분리 구역:</b> TRUST (내부망) | <b>식별 CVE:</b> <span style="color:#dc2626; font-weight:800;">CVE-2016-1000027</span>
            </div>
          </div>
          <div>
            <button id="btnToggleQuarantine" class="btn btn-danger" style="font-size:0.88rem; padding:0.5rem 1.1rem; font-weight:800;" onclick="toggleHostQuarantine(&apos;ASSET-05&apos;)">
              <i data-lucide="power" style="width:14px; height:14px;"></i> 원클릭 네트워크 긴급 격리 실행
            </button>
          </div>
        </div>

        <div id="quarantineStatusNotice" style="display:none; font-size:0.78rem; background:#fef2f2; border:1px solid #fca5a5; color:#991b1b; padding:0.6rem 0.8rem; border-radius:6px; font-weight:700;">
          🚨 [격리 프로토콜 가동 완료] FOCS 경계 방화벽 및 WAAP에서 10.10.40.20 호스트의 인바운드/아웃바운드 전체 트래픽이 차단되었습니다!
        </div>

        <!-- 5-Step Playbook Progress -->
        <div>
          <div style="font-size:0.82rem; font-weight:800; color:var(--text-main); margin-bottom:0.5rem;">📋 KISA 표준 5단계 침해사고 대응 런북 이행표</div>
          <table class="excel-table">
            <thead>
              <tr>
                <th style="width:55px;">단계</th>
                <th style="width:160px;">표준 절차명</th>
                <th>세부 조치 및 시스템 통제 가이드</th>
                <th class="center" style="width:90px;">이행 상태</th>
                <th class="center" style="width:75px;">액션</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td class="center"><b>1단계</b></td>
                <td><b>이상 징후 탐지 & 초동보고</b></td>
                <td>SIEM 이상 트래픽 탐지 및 CISO 직속 침해대응반 비상소집</td>
                <td class="center"><span class="x-badge success">완료</span></td>
                <td class="center"><span style="font-size:0.7rem; color:var(--text-dim);">-</span></td>
              </tr>
              <tr>
                <td class="center"><b>2단계</b></td>
                <td><b>호스트 긴급 격리 (차단)</b></td>
                <td>방화벽 차단 정책 배포 및 피해 시스템 망분리 격리</td>
                <td class="center" id="step2Status"><span class="x-badge warning">대기중</span></td>
                <td class="center"><button class="x-btn" onclick="toggleHostQuarantine(&apos;ASSET-05&apos;)">실행</button></td>
              </tr>
              <tr>
                <td class="center"><b>3단계</b></td>
                <td><b>메모리 & 로그 포렌식</b></td>
                <td>웹서버 Access Log 및 JVM 힙 덤프 추출, 침투 공격자 IP 추적</td>
                <td class="center" id="step3Status"><span class="x-badge warning">조치중</span></td>
                <td class="center"><button class="x-btn" onclick="completePlaybookStep(3)">완료</button></td>
              </tr>
              <tr>
                <td class="center"><b>4단계</b></td>
                <td><b>WAAP 가상패치 가동</b></td>
                <td>CVE-2016-1000027 Spring 직렬화 취약점 차단 룰셋 배포</td>
                <td class="center" id="step4Status"><span class="x-badge success">적용완료</span></td>
                <td class="center"><button class="x-btn" onclick="completePlaybookStep(4)">완료</button></td>
              </tr>
              <tr>
                <td class="center"><b>5단계</b></td>
                <td><b>사고조사 보고 & 복구</b></td>
                <td>침해사고 조사 결과 보고서 CISO 결재 기안 및 정상 서비스 복구</td>
                <td class="center" id="step5Status"><span class="x-badge warning">기안대기</span></td>
                <td class="center"><button class="x-btn" style="color:var(--primary); font-weight:700;" onclick="createIncidentApprovalDraft()">기안주입</button></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <!-- Modal Footer -->
      <div style="padding:0.9rem 1.4rem; background:#f8fafc; border-top:1px solid var(--border); display:flex; justify-content:space-between; align-items:center;">
        <span style="font-size:0.75rem; color:var(--text-dim);">기안 주입 시 스마트 전자결재 기안판으로 100% 자동 이동합니다.</span>
        <div style="display:flex; gap:0.4rem;">
          <button class="btn btn-sm btn-primary" onclick="createIncidentApprovalDraft()">
            <i data-lucide="send" style="width:13px; height:13px;"></i> 사고조사 보고서 전자결재 기안문 자동 생성
          </button>
          <button class="btn btn-sm" onclick="closeIncidentQuarantineModal()">닫기</button>
        </div>
      </div>
  ...
  <!-- CVE SCOPE & VENDOR PATCH MODAL -->
  <div id="cvePatchModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15,23,42,0.7); z-index:99999; justify-content:center; align-items:center; padding:20px;">
    <div style="background:#fff; border-radius:10px; width:780px; max-width:95vw; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 12px 36px rgba(0,0,0,0.3); overflow:hidden;">
      <div style="padding:1.1rem 1.4rem; background:#fef3c7; border-bottom:1.5px solid #fde68a; display:flex; justify-content:space-between; align-items:center;">
        <div style="display:flex; align-items:center; gap:0.5rem;">
          <i data-lucide="scan-search" style="width:20px; height:20px; color:#b45309;"></i>
          <div>
            <h3 style="font-size:1.1rem; font-weight:800; color:#92400e;">CVE 취약점 자동 상관분석 & 벤더 공식 패치 권고문</h3>
            <p style="font-size:0.75rem; color:#b45309;">사내 IT 자산 5대 및 SBOM 부품 7종과 NVD/KISA 취약점 실시간 1:1 대조</p>
          </div>
        </div>
        <button class="btn btn-sm" onclick="closeCvePatchModal()">✕</button>
      </div>

      <div style="padding:1.2rem; overflow-y:auto;">
        <div id="cvePatchContentBox"></div>
      </div>

      <div style="padding:0.9rem 1.4rem; background:#f8fafc; border-top:1px solid var(--border); display:flex; justify-content:flex-end;">
        <button class="btn btn-sm" onclick="closeCvePatchModal()">닫기</button>
      </div>
    </div>
  </div>
  ...
  NGFW -.->|Syslog| SIEM\`
    ...
    // [ENGINE 1] INCIDENT QUARANTINE & 5-STEP PLAYBOOK ENGINE
    let hostQuarantineState = { 'ASSET-05': false };

    function openIncidentQuarantineModal(assetId) {
      const modal = document.getElementById('incidentQuarantineModal');
      if (modal) {
        modal.style.display = 'flex';
        updateQuarantineUi(assetId || 'ASSET-05');
      }
    }

    function closeIncidentQuarantineModal() {
      const modal = document.getElementById('incidentQuarantineModal');
      if (modal) modal.style.display = 'none';
    }

    function toggleHostQuarantine(assetId) {
      hostQuarantineState[assetId] = !hostQuarantineState[assetId];
      const isQ = hostQuarantineState[assetId];
      updateQuarantineUi(assetId);

      const step2 = document.getElementById('step2Status');
      if (step2) {
        step2.innerHTML = isQ ? '<span class="x-badge success">격리완료</span>' : '<span class="x-badge warning">대기중</span>';
      }

      if (isQ) {
        alert('🚨 [FOCS 경계방화벽 연동] 금융 코어 WAS (10.10.40.20) 네트워크 긴급 격리 프로토콜이 성공적으로 발동되었습니다!\\n- 전체 인바운드/아웃바운드 포트가 차단되었습니다.\\n- 관리자 콘솔 포렌식 접속(SSH mTLS)만 허용됩니다.');
      } else {
        alert('✅ 10.10.40.20 호스트의 네트워크 격리가 해제되고 정상 통신 정책으로 복원되었습니다.');
      }
    }

    function updateQuarantineUi(assetId) {
      const isQ = hostQuarantineState[assetId];
      const btn = document.getElementById('btnToggleQuarantine');
      const notice = document.getElementById('quarantineStatusNotice');

      if (btn) {
        btn.className = isQ ? 'btn btn-primary' : 'btn btn-danger';
        btn.innerHTML = isQ 
          ? '<i data-lucide="refresh-cw" style="width:14px; height:14px;"></i> 네트워크 격리 해제 (정상화)'
          : '<i data-lucide="power" style="width:14px; height:14px;"></i> 원클릭 네트워크 긴급 격리 실행';
      }
      if (notice) {
        notice.style.display = isQ ? 'block' : 'none';
      }
      lucide.createIcons();
    }

    function completePlaybookStep(stepNum) {
      const el = document.getElementById('step' + stepNum + 'Status');
      if (el) {
        el.innerHTML = '<span class="x-badge success">완료</span>';
      }
      alert('✅ ' + stepNum + '단계 조치가 정상적으로 완료 처리되었습니다.');
    }

    function createIncidentApprovalDraft() {
      closeIncidentQuarantineModal();
      openSmartApprovalModal();
      loadApprovalTemplate('INCIDENT_RESPONSE');

      // Set custom title and injected details
      const titleEl = document.getElementById('approvalDocTitleInput');
      if (titleEl) titleEl.value = '[긴급 침해사고] 금융 코어 WAS (JEUS 8.5) 취약점 격리 및 WAAP 가상패치 조치 결과 보고서';

      alert('📋 스마트 전자결재 기안판에 [침해사고 긴급 격리 및 대응 결과 보고서] 서식이 100% 자동 주입되었습니다! CISO 결재 관인을 날인해 주세요.');
    }


    // [ENGINE 2] CVE ZERO-DAY CORRELATION & VENDOR PATCH DISPATCHER ENGINE
    const realCveCorrelations = [
      {
        cve: 'CVE-2016-1000027',
        component: 'spring-framework 5.3.39',
        targetAsset: '금융 코어 WAS (TmaxSoft JEUS 8.5) [ASSET-05]',
        cvss: '9.8 (Critical)',
        severity: 'danger',
        vector: 'Remote Code Execution via Java Deserialization in HttpInvokerServiceExporter',
        status: 'WAAP 가상패치 룰셋 가동 완료 (조치율 100%)',
        mitigation: 'Imperva WAAP RCE 룰셋 가동 및 Spring 5.3.39 최신 가상패치 적용',
        vendorNotice: 'Pivotal Spring 보안 권고: HttpInvoker 비활성화 및 신규 REST/mTLS 엔드포인트 교체 권고'
      },
      {
        cve: 'CVE-2025-24813',
        component: 'apache-tomcat 5.5.36',
        targetAsset: '대외 연계 웹서버 (Apache WAS) [ASSET-01]',
        cvss: '9.8 (Critical)',
        severity: 'danger',
        vector: 'AJP Connector Ghostcat 계열 임의 파일 읽기/실행 취약점',
        status: 'AJP 8009 포트 비활성화 완료 (조치율 100%)',
        mitigation: 'server.xml 내 AJP Connector 포트 주석 처리 및 SecretRequired 강제',
        vendorNotice: 'Apache Security Team: Apache Tomcat 5.5 EOL 안내 및 9.0/10.1 최신 LTS 버전으로 마이그레이션 권고'
      },
      {
        cve: 'CVE-2026-54512',
        component: 'jackson-databind 2.17.1',
        targetAsset: '금융 코어 WAS (TmaxSoft JEUS 8.5) [ASSET-05]',
        cvss: '7.5 (High)',
        severity: 'warning',
        vector: 'Polymorphic Type Handling 역직렬화 가젯 취약점',
        status: '차기 정기 배포 시 2.18 패치 예정 (가상패치 방어중)',
        mitigation: 'ObjectMapper DefaultTyping 비활성화 및 화이트리스트 검증 로직 적용',
        vendorNotice: 'FasterXML Jackson 권고: jackson-databind 2.18.0 정식 릴리즈 업그레이드 권고'
      }
    ];

    function openCvePatchModal() {
      const modal = document.getElementById('cvePatchModal');
      if (!modal) return;
      modal.style.display = 'flex';

      const box = document.getElementById('cvePatchContentBox');
      if (!box) return;

      let html = '';
      realCveCorrelations.forEach((item, idx) => {
        html += 
          '<div style="background:#fff; border:1px solid var(--border); border-radius:8px; padding:1rem; margin-bottom:0.8rem; box-shadow:var(--shadow-sm);">' +
            '<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.4rem;">' +
              '<div>' +
                '<span class="x-badge ' + item.severity + '" style="font-size:0.75rem; font-weight:800;">' + item.cve + '</span> ' +
                '<span style="font-weight:800; font-size:0.92rem; margin-left:0.4rem;">' + sanitizeHtml(item.component) + '</span>' +
              '</div>' +
              '<span class="x-badge ' + item.severity + '">CVSS ' + item.cvss + '</span>' +
            '</div>' +
            '<div style="font-size:0.76rem; color:var(--text-sub); margin-bottom:0.4rem;">' +
              '<b>사내 영향 자산:</b> ' + sanitizeHtml(item.targetAsset) + '<br/>' +
              '<b>공격 벡터 (Vector):</b> ' + sanitizeHtml(item.vector) +
            '</div>' +
            '<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:6px; padding:0.6rem; font-size:0.73rem; color:#334155; margin-bottom:0.5rem;">' +
              '<div style="font-weight:700; color:#1e40af; margin-bottom:2px;">🛡️ 사내 조치 및 가상패치 상태:</div>' +
              '<div>' + sanitizeHtml(item.status) + ' (' + sanitizeHtml(item.mitigation) + ')</div>' +
            '</div>' +
            '<div style="font-size:0.72rem; color:#64748b; display:flex; justify-content:space-between; align-items:center;">' +
              '<span>📢 ' + sanitizeHtml(item.vendorNotice) + '</span>' +
              '<button class="x-btn" style="color:var(--primary); font-weight:700;" onclick="alert(&apos;[' + item.cve + '] 벤더 공식 보안 권고문 전문이 확인되었습니다.\\n\\n' + item.vendorNotice.replace(/'/g, '') + '&apos;)">권고문 전문</button>' +
            '</div>' +
          '</div>';
      });
    ...
    function closeCvePatchModal() {
      const modal = document.getElementById('cvePatchModal');
      if (modal) modal.style.display = 'none';
    }


    // [ENGINE 3] CISO BOARDROOM ANNUAL GOVERNANCE REPORT ENGINE
    function printBoardroomAnnualReport() {
      ...
      const org = customerOrgName || '한국수력원자력 정보보호본부';

      printWin.document.write(
        '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>CISO 경영진 및 이사회 연간 정보보호 성과평가 보고서</title>' +
        '<style>' +
          '@page { size: A4 portrait; margin: 12mm 15mm; }' +
          'body { font-family: "Pretendard", -apple-system, sans-serif; color: #0f172a; line-height: 1.5; font-size: 0.82rem; }' +
          '.cover { height: 92vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; }' +
          '.cover h1 { font-size: 2.3rem; font-weight: 800; margin: 15px 0 10px 0; }' +
          '.page-break { page-break-after: always; }' +
          'table { width: 100%; border-collapse: collapse; margin-top: 12px; font-size: 0.78rem; }' +
          'th, td { border: 1px solid #cbd5e1; padding: 6px 10px; }' +
          'th { background: #f1f5f9; font-weight: 700; }' +
          '.sec-title { font-size: 1.15rem; font-weight: 800; border-bottom: 2px solid #2563eb; padding-bottom: 5px; margin: 25px 0 10px 0; color: #1e3a8a; }' +
          '.kpi-box { background: #f8fafc; border: 1px solid #cbd5e1; border-radius: 6px; padding: 12px; margin-bottom: 12px; }' +
        '</style></head><body>' +

        // COVER
        '<div class="cover">' +
          '<div style="font-size:1.15rem; font-weight:800; color:#2563eb;">2026年度 經營陣 및 理事會 報告 案件</div>' +
          '<h1>전사 정보보호 연간 거버넌스 및 AI FinOps 투자 성과평가 보고서</h1>' +
          '<div style="font-size:1rem; color:#64748b; margin-top:10px;">Annual Information Security Governance & AI FinOps Executive Report</div>' +
          '<div style="width:280px; border-top:2px solid #0f172a; margin:30px 0;"></div>' +
          '<table style="width:380px; margin:0 auto; font-size:0.85rem;">' +
            '<tr><th>보고 기관</th><td>' + org + '</td></tr>' +
            '<tr><th>보고 일자</th><td>' + todayStr + '</td></tr>' +
            '<tr><th>보고 책임자</th><td>정보보호최고책임자 (CISO)</td></tr>' +
            '<tr><th>보고 안건</th><td>법정 거버넌스 100% 달성 및 온프레미스 AI 투자 ROI 보고</td></tr>' +
          '</table>' +
        '</div>' +
        '<div class="page-break"></div>' +

        // SECTION 1: EXECUTIVE SUMMARY
        '<div class="sec-title">제 1 장. 연간 정보보호 핵심 성과 요약 (Executive Summary)</div>' +
        '<div class="kpi-box">' +
          '<b>1. KISA ISMS-P 80개 법정 통제항목 준수율: 100% 이행 달성</b> (관리체계 16개, 보호대책 64개 완비)<br/>' +
          '<b>2. 코어 정보시스템(JEUS 8.5 등) 식별 고위험 취약점 조치율: 100% 통제완결</b> (WAAP 가상패치 가동)<br/>' +
          '<b>3. 법정 14대 일일 보안점검 이행율: 100% 완결</b> (20종 솔루션 무장애 가동)<br/>' +
          '<b>4. 온프레미스 GB10 (177B MoE) 도입에 따른 연간 클라우드 API 예산 절감: ₩3,888,000 (90% 절감 달성)</b>' +
        '</div>' +

        // SECTION 2: AI FINOPS & CLOUD COST SAVING ROI
        '<div class="sec-title">제 2 장. 생성형 AI 인프라 투자 회수(ROI) 및 FinOps 성과</div>' +
        '<p>외부 상용 LLM(OpenAI GPT-4o, Claude 3.5) 전송 대비 사내 온프레미스 GB10 클러스터를 경유함으로써 기밀 유출 위험 0% 통제 및 예산 절감을 입증하였습니다.</p>' +
        '<table>' +
          '<thead><tr><th>구분</th><th>대상 모델 / API</th><th>계약형태</th><th>연간 비용</th><th>온프렘 GB10 전환 시 연간 절감액</th></tr></thead>' +
          '<tbody>' +
            '<tr><td>사내인프라</td><td><b>온프레미스 GB10 (177B MoE)</b></td><td>자체구축</td><td>₩0 (자체운용)</td><td><b>연간 ₩3,888,000 절감 (ROI 달성)</b></td></tr>' +
            '<tr><td>사내인프라</td><td><b>사내 bge-m3 임베딩 & OCR API</b></td><td>자체운용</td><td>₩0 (자체운용)</td><td>외부 클라우드 OCR 대비 연간 ₩1,200,000 절감</td></tr>' +
            '<tr><td>외부상용</td><td>OpenAI GPT-4o Enterprise</td><td>종량제</td><td>₩4,320,000</td><td>외부 필수 업무에 한정 경유</td></tr>' +
          '</tbody>' +
        '</table>' +

        // SECTION 3: CORE INCIDENT & VULNERABILITY STATUS
        '<div class="sec-title">제 3 장. 정보시스템 취약점 및 침해사고 긴급 격리 대응 현황</div>' +
        '<table>' +
          '<thead><tr><th>자산명</th><th>운영 구역</th><th>식별 CVE</th><th>위험도</th><th>조치 결과</th></tr></thead>' +
          '<tbody>' +
            '<tr><td>금융 코어 WAS (JEUS 8.5)</td><td>TRUST (내부망)</td><td>CVE-2016-1000027</td><td><span style="color:#dc2626; font-weight:700;">Critical 9.8</span></td><td>WAAP 가상패치 적용 및 긴급 격리 프로토콜 완비</td></tr>' +
            '<tr><td>대외 연계 웹서버</td><td>DMZ 구간</td><td>CVE-2025-24813</td><td><span style="color:#dc2626; font-weight:700;">Critical 9.8</span></td><td>AJP Connector 비활성화 완료</td></tr>' +
            '<tr><td>금융 코어 WAS (JEUS 8.5)</td><td>TRUST (내부망)</td><td>CVE-2026-54512</td><td><span style="color:#d97706; font-weight:700;">High 7.5</span></td><td>2.18 패치 예정 (WAAP 방어중)</td></tr>' +
          '</tbody>' +
        '</table>' +

        '<div style="text-align:right; margin-top:40px; font-size:0.8rem; color:#64748b;">위 보고서의 기재 내용은 실제 인프라 지표 및 회계 내역과 100% 일치하며 이사회 보고 안건으로 승인합니다.<br/><br/><b>정보보호최고책임자 (CISO): ____________________ (인)</b></div>' +
    ...
    // [ENGINE 4] GENAI PROMPT SECURITY AUDIT & REALTIME DLP GATE ENGINE
    let dlpAuditLogs = [
      { id: 1, time: '2026-09-16 15:20', dept: '경영기획팀', model: 'OpenAI GPT-4o Enterprise', prompt: 'VIP 고객 홍길동 (주민번호: 900101-*******) 여신 평가안 검토 요청', violation: '주민등록번호 (개인정보)', result: '차단/마스킹' },
      { id: 2, time: '2026-09-16 14:10', dept: 'AI엔진개발팀', model: 'Anthropic Claude 3.5 Sonnet', prompt: '사내 DB 접속: jdbc:oracle:thin:@10.10.40.30:1521/ORCL (PWD: [차단됨])', violation: 'DB 접속 패스워드', result: '전송차단' },
      { id: 3, time: '2026-09-16 11:05', dept: '보안운영팀', model: '온프레미스 GB10 (177B MoE)', prompt: '금융 코어 WAS (JEUS 8.5) 취약점 CVE-2016-1000027 가상패치 룰셋 생성', violation: '없음 (사내망 통과)', result: '정상통과' }
    ];

    function inspectDlpPrompt(text) {
      if (!text) return { isBlocked: false, cleanText: text, violations: [] };

      const violations = [];
      let cleanText = text;

      // 1. Korean Resident Number: 6 digits - 7 digits
      const rrnRegex = /\b(\d{6})[- ]?([1-4]\d{6})\b/g;
      if (rrnRegex.test(text)) {
        violations.push('주민등록번호 (개인정보)');
        cleanText = cleanText.replace(rrnRegex, '$1-*******');
      }

      // 2. AWS / Cloud API Key: AKIA[0-9A-Z]{16}
      const awsKeyRegex = /\b(AKIA[0-9A-Z]{16})\b/g;
      if (awsKeyRegex.test(text)) {
        violations.push('Cloud API Key (인증정보)');
        cleanText = cleanText.replace(awsKeyRegex, '[CLOUD_API_KEY_BLOCKED]');
      }

      // 3. Database Password Keyword Pattern
      const pwdRegex = /(?:pwd|password|passwd|비밀번호)\s*[:=]\s*([^\s,;]+)/gi;
      if (pwdRegex.test(text)) {
        violations.push('DB/시스템 패스워드');
        cleanText = cleanText.replace(pwdRegex, 'password: [PASSWORD_MASKED]');
      }
    ...
    function runInteractiveDlpTest() {
      const input = document.getElementById('dlpSimInput');
      const box = document.getElementById('dlpSimResultBox');
      if (!input || !box) return;

      const text = input.value.trim();
      if (!text) return;

      const res = inspectDlpPrompt(text);
      ...
      if (res.isBlocked) {
        ...
        box.innerHTML = 
          '<b>🚨 [DLP 차단 발동] 프롬프트 전송이 차단되었습니다!</b><br/>' +
          '- 탐지된 위반 유형: <b>' + res.violations.join(', ') + '</b><br/>' +
          '- 마스킹 안전 처리본: <span style="font-family:monospace; background:#fee2e2; padding:2px 4px; border-radius:3px;">' + sanitizeHtml(res.cleanText) + '</span>';

        // Add to audit logs
        dlpAuditLogs.unshift({
          id: dlpAuditLogs.length + 1,
          time: new Date().toISOString().slice(0, 16).replace('T', ' '),
          dept: '실시간 테스트 사용자',
          model: 'OpenAI GPT-4o Enterprise',
          prompt: res.cleanText,
          violation: res.violations.join(', '),
          result: '차단/마스킹'
        });
        renderDlpAuditTable();
      } else {
        ...
        box.innerHTML = '<b>✅ [DLP 통과] 개인정보 및 기밀 인증정보가 탐지되지 않았습니다. (외부 LLM 전송 안전)</b>';
      }
    }

    function renderDlpAuditTable() {
      const tbody = document.getElementById('dlpAuditTableBody');
      if (!tbody) return;
      tbody.innerHTML = '';

      dlpAuditLogs.forEach((log, idx) => {
        const tr = document.createElement('tr');
        const badge = log.result.includes('차단') 
          ? '<span class="x-badge danger">' + sanitizeHtml(log.result) + '</span>'
          : '<span class="x-badge success">' + sanitizeHtml(log.result) + '</span>';

        tr.innerHTML = 
          '<td class="center">' + (idx + 1) + '</td>' +
          '<td>' + sanitizeHtml(log.time) + '</td>' +
          '<td>' + sanitizeHtml(log.dept) + '</td>' +
          '<td><b>' + sanitizeHtml(log.model) + '</b></td>' +
          '<td style="font-size:0.73rem;">' + sanitizeHtml(log.prompt) + '</td>' +
          '<td class="center"><span style="font-size:0.72rem; color:' + (log.violation.includes('없음') ? '#059669' : '#dc2626') + '; font-weight:700;">' + sanitizeHtml(log.violation) + '</span></td>' +
    ...
    function switchBomSubView(subView) {
      ...
      const subViews = ['sol', 'api', 'lifecycle', 'dlp'];
      subViews.forEach(v => {
