...
- **스캔 문서·이미지(OCR)만 파이썬이 필요합니다.** 없으면 화면이 *"스캔된 문서·이미지는 글자를 알아보는 도구(OCR)가 있어야 읽을 수 있습니다"*라고 정직하게 안내합니다(조용히 실패하지 않습니다).
  ...
  - **mac 설치본에는 아직 없습니다.** 업로드 화면이 서버에 물어보고 「아직 못 읽습니다」라고 정직하게 표시합니다.
    - ⚠ mac **단독 모드**(앱 안의 서버)는 앱 번들 안에 파이썬 환경을 만들 자리가 없어, 지금은 켤 방법이 없습니다. 스캔 문서가 필요하면 **분산 모드**(사내 서버에 붙이기)를 쓰십시오.
    - **분산 모드·운영 서버**라면 그 서버에서 깝니다: `cd server && venv/bin/pip install -r requirements-ocr.txt`
      (⚠ 이 명령은 **서버 소스가 있는 기계**에서 도는 것입니다 — 고객 설치본 안에서 도는 것이 아닙니다.)
...
|---|---|---|
| `GIJO_KPI_SUBJECT_RULE=1` | 「우리·사내·자사·당사 + **영토 낱말**(보안·취약점·자산·계정·패치·백업·침해…) + …율/률/비율/평균/건수」를 **사내 지표 물음**으로 못 박아 `kpi_status`로 보냅니다(강제도구 [83] 「사내 지표율」의 주체어 갈래). 낱말 목록에 없는 지표(예산 집행률·평균 조치 기간·등록 건수)가 모델 재량으로 새는 것을 막습니다. ⚠ 영토 낱말이 없는 사내 물음(연차 사용률·평균 연봉·문서 등록 건수)은 **안 채 갑니다** — 첫 판이 그것까지 채 가던 것을 2026-09-06에 좁혔습니다. | 넓은 규칙은 **남의 답을 삼킵니다.** 같은 규칙을 낱말로 넓혔다가 「업계 평균 클릭률」·「국내 백신 설치율 통계」까지 채 가서 되돌린 전례가 있습니다(2026-09-06). 그래서 켜서 **재 본 뒤** 승격합니다. |
