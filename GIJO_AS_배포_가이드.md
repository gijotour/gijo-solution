...
- **단독 모드**는 설치본에 **추출기 스크립트와 `requirements.txt`가 함께 나갑니다**(2026-08-22부터). 다만 **파이썬 런타임 자체는 아직 동봉하지 않습니다** — 고객 기계에 파이썬이 없으면 위 ⚠ 항목이 안 됩니다.
  - 그때 화면은 *"이 설치본에 문서 추출 도구가 없습니다"*라고 정직하게 안내합니다(조용히 실패하지 않습니다).
  - 파이썬이 있으면 `GIJO_PYTHON`으로 가리키거나, 서버 폴더에 `venv`를 만들어 `requirements.txt`를 설치하면 됩니다.
