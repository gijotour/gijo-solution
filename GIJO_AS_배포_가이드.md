...
| 형식 | 무엇으로 읽나 | 분산 모드 | 단독 모드 |
|---|---|---|---|
| `.md` `.txt` `.csv` `.log` `.json` `.yaml` | 서버가 직접 읽음(파이썬 불필요) | ✅ | ✅ |
| **HWPX · DOCX · XLSX · PPTX** | 동봉 추출기(파이썬, 추가 라이브러리 **불필요**) | ✅ | ⚠ **파이썬 필요** |
| **PDF** | 동봉 추출기 + `pypdf` | ✅ | ⚠ 파이썬 + `pypdf` 필요 |
| 이미지 · 스캔 PDF(OCR) | `rapidocr`·`pymupdf`(옵션, 약 480MB) | 설치했다면 ✅ | ⚠ 별도 설치 |
| `.doc` `.hwp`(구형 바이너리) | 지원 안 함 — 변환 안내 | ❌ | ❌ |

- **분산 모드**는 GPU 서버에 venv를 만들어 `requirements.txt`를 설치합니다(§ 아래 준비 단계). 그래서 전부 됩니다.
- **단독 모드(Windows)**는 **파이썬 런타임을 앱과 함께 동봉합니다**(2026-08-22부터, 약 25MB). 고객 기계에 파이썬이 없어도 **HWPX·DOCX·XLSX·PPTX와 텍스트 PDF가 그대로 됩니다** — 폐쇄망이라 `pip install`을 못 해도 상관없습니다.
  - 동봉본에는 **PDF용 `pypdf`만** 들어 있습니다. OCR(스캔 문서·이미지)은 아직 옵션입니다.
  - 운영자가 `GIJO_PYTHON`을 지정했거나 서버 폴더에 `venv`가 있으면 **그쪽을 먼저 씁니다** — 동봉본이 그 환경을 가리지 않습니다(장비 접속·모델 검사·OCR까지 갖춘 환경을 쓰던 곳이 안 깨지게).
- ⚠ **단독 모드(mac)는 아직 동봉 전입니다** — mac은 앱 서명 절차가 따로 붙어 별도 작업으로 남겼습니다. mac 설치본에서 PDF·한글을 읽으려면 파이썬을 깔고 `GIJO_PYTHON`으로 가리키거나 서버 폴더에 `venv`를 만들어야 합니다. 안 되면 화면이 *"이 설치본에 문서 추출 도구가 없습니다"*라고 정직하게 안내합니다(조용히 실패하지 않습니다).
- ⚠ **2026-08-22 이전 설치본은 단독 모드에서 위 ⚠ 항목이 전부 실패했습니다** — 추출기 스크립트 자체가 안 실려 있었습니다(파이썬을 깔아도 안 됐습니다).
